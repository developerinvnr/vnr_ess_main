...
function ClickEdit(Sno,EI,v)
{
  ...
  var AllInc = parseFloat(document.getElementById("AllInc"+Sno).value);
  if(AllInc>0){document.getElementById("CtcProGSPM_"+Sno).readOnly=false; }
  
  document.getElementById("CtcProGSPM_"+Sno).disabled=false;
  document.getElementById("CtcProGSPM_"+Sno).readOnly=false;
...
{ //alert(No+","+PmsId+","+EmpId+","+C+","+Y+","+HE);
  ...
  var MTPreG = document.getElementById("MyTTPreGross").value; 
  
  var OnePerPre = document.getElementById("OnePerPre").value; 
 ...
 <td style="width:100%;">
  <table border="0" style="width:100%;" cellspacing="0">
   <tr>
	...
	<td class="tdr" style="width:500px;vertical-align:middle;"><?php /*PGSPM - <font color="#FFFFFF">Proposed Gross Salary PM</font>&nbsp;&nbsp;PIS - <font color="#FFFFFF">Proposed Increment Salary</font>&nbsp;&nbsp;SC - <font color="#FFFFFF">Salary Correction</font>*/ ?></td>	
   </tr>
  </table>
 </td>
</tr>
...
if($_REQUEST['FilD']>0){ $qSub='p.HR_Curr_DepartmentId='.$_REQUEST['FilD']; }else{ $qSub='1=1'; }

$SqlCount=mysql_query("select SUM(EmpCurrCtc) as Old_GROSS, SUM(Hod_ProIncCTC) as H_IS, SUM(Hod_ProCorrCTC) as H_SC, SUM(Hod_IncNetCTC) as Tinc from hrm_employee_pms p INNER JOIN hrm_employee e ON p.EmployeeID=e.EmployeeID where e.EmpStatus='A' AND e.CompanyId=".$CompanyId." AND p.AssessmentYear=".$_SESSION['PmsYId']." AND ".$qSub." AND p.HOD_EmployeeID=".$EmployeeId, $con);
 ...
 <td style="width:100%;background-color:#FFFF91;">
  ...
  <table border="0" style="width:100%;">
   <tr>
	...
	<?php /*&nbsp;&nbsp;<u>Total Previous CTC</u>&nbsp;:&nbsp;<input class="rinput" style="width:120px;" id="MyTTPreGross" value="<?php echo floatval($TotOldGrossPM); ?>" />&nbsp;&nbsp; */ ?>
	<input type="hidden" id="MyTTPreGross" value="<?php echo floatval($TotOldGrossPM); ?>" />
	[<?php if($_REQUEST['FilD']>0){echo 'Department Wise:';}else{echo 'All Employee:';}?>]
&nbsp;&nbsp;
	&nbsp;&nbsp;<u>Total Proposed CTC</u>&nbsp;:&nbsp;<input class="rinput" style="width:120px;" id="MyTTProGross" value="<?php echo floatval($TotNewGrossPM); ?>" />&nbsp;&nbsp;&nbsp;&nbsp;<u>Total Increment</u>&nbsp;:&nbsp;<input class="rinput" style="width:100px;" id="MyTTIncGross" value="<?php echo floatval($Increment); ?>"/>&nbsp;<input class="rinput" style="width:60px;" id="MyTTIncGrossPercent" value="<?php echo $PerInc; ?>"/>&nbsp;%&nbsp;&nbsp;&nbsp;&nbsp;<u>SC</u>&nbsp;:&nbsp;<input class="rinput" style="width:60px;" id="MyTTSCPercent" value="<?php echo $TotalTPerPSC; ?>" />&nbsp;% 
    ...
    <?php /*<u>Total Previous CTC</u>&nbsp;:&nbsp;<input class="rinput" style="width:120px;" id="MyTTPreGross_22" value="<?php echo $TotOldGrossPM; ?>" />*/?>
    <input type="hidden" id="MyTTPreGross_22" value="<?php echo $TotOldGrossPM; ?>" />
    [<?php if($_REQUEST['FilD']>0){echo 'Department Wise:';}else{echo 'All Employee:';}?>]
&nbsp;&nbsp;
    &nbsp;&nbsp;&nbsp;&nbsp;<u>Total Proposed CTC</u>&nbsp;:&nbsp;<input class="rinput" style="width:120px;" id="MyTTProGross_22" value="<?php echo $TotNewGrossPM; ?>" />&nbsp;&nbsp;&nbsp;&nbsp;<u>Total Increment</u>&nbsp;:&nbsp;<input class="rinput" style="width:100px;" id="MyTTIncGross_22" value="<?php echo $Increment; ?>"/>&nbsp;<input class="rinput" style="width:60px;" id="MyTTIncGrossPercent_22" value="<?php echo $PerInc; ?>"/>&nbsp;%&nbsp;&nbsp;&nbsp;&nbsp;<u>SC</u>&nbsp;:&nbsp;<input class="rinput" style="width:60px;" id="MyTTSCPercent_22" value="<?php echo $TotalTPerPSC; ?>" />&nbsp;%
	</div>
...
{ $WagesBonus=$resB['PerMonthOct']; }else{ $WagesBonus=$resB['PerMonthApr']; }
...
//if($res['EmpCode']==75){echo '.'.$res['EmpVertical'];}
...
<?php /************** 222222222222222222 Open Open *****************/ ?>			   
	    ...
	    <?php $sqlMax = mysql_query("SELECT MAX(SalaryChange_Date) as SalaryChDate FROM hrm_pms_appraisal_history where EmpCode=".$res['EmpCode']." AND SalaryChange_Date!='".$_SESSION['EffDate']."' AND CompanyId=".$CompanyId, $con); 
			  ...
			  if($TPreGross==0){$TPreGross=$res['EmpCurrGrossPM'];}
...
/*------------------------------------------------------------*/
/*------------------------------------------------------------*/
$suqry="Rating=".$RatingHod." AND MgmtId=".$EmployeeId." AND YearId=".$_SESSION['PmsYId']." AND IncDistriFrom>0 AND CompanyId=".$CompanyId;

$app4 = $suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=".$res['Rev2_EmployeeID']." AND VertId=".$res['EmpVertical']." AND RevId=".$res['Reviewer_EmployeeID']." AND AppId=".$res['Appraiser_EmployeeID'];
$app3 = $suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=0 AND VertId=".$res['EmpVertical']." AND RevId=".$res['Reviewer_EmployeeID']." AND AppId=".$res['Appraiser_EmployeeID'];
$app2 = $suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=".$res['Rev2_EmployeeID']." AND VertId=0 AND RevId=".$res['Reviewer_EmployeeID']." AND AppId=".$res['Appraiser_EmployeeID'];
$app1 = $suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=0 AND VertId=0 AND RevId=".$res['Reviewer_EmployeeID']." AND AppId=".$res['Appraiser_EmployeeID'];
$app0 = $suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=0 AND VertId=0 AND RevId=0 AND AppId=".$res['Appraiser_EmployeeID'];
 ...
 { 
  ...
  { 
   ...
   { 
    ...
    { 
      /*Reviewer*/ 
	  $rev3=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=".$res['Rev2_EmployeeID']." AND VertId=".$res['EmpVertical']." AND RevId=".$res['Reviewer_EmployeeID'];
      $rev2=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=0 AND VertId=".$res['EmpVertical']." AND RevId=".$res['Reviewer_EmployeeID'];
      $rev1=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=".$res['Rev2_EmployeeID']." AND VertId=0 AND RevId=".$res['Reviewer_EmployeeID'];
      $rev0=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=0 AND VertId=0 AND RevId=".$res['Reviewer_EmployeeID'];
	  $s_r3= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_dislevel WHERE ".$rev3,$con); 
      ...
      { 
       ...
       { 
        ...
        { 
         ...
         { 
           /*Vertical*/
		   $ver1=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=".$res['Rev2_EmployeeID']." AND VertId=".$res['EmpVertical'];
           $ver0=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=0 AND VertId=".$res['EmpVertical'];
           ...
           else
           {
            ...
            {
			 /*Hod*/
			 $hod0=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId']." AND HodId=".$res['Rev2_EmployeeID'];
             ...
             {
			  /*Department*/
			  $dep0=$suqry." AND DeptId=".$res['HR_Curr_DepartmentId'];
...
/**(3)*************************If Prorata wise New Open Open Open Open Open **************************/
  ...
  $cY=$PrvY; $pY=$PrvY-1;                                              //$cY=date("Y"); $pY=date("Y")-1; $cY=2020, $pY=2019
  ...
  if($res['DateJoining']<=$pY.'-'.$PrvMD AND $RatingHod>0)  //if($res['DateJoining']<=$pY.'-03-31' AND $RatingHod>0)
  {
   ...
   {
    $oneC=($res['EmpCurrAnnualBasic']*$resMaxMin['IncDistriFrom'])/100;
  ...
  elseif($res['DateJoining']>=$pY.'-'.$ExtraOneMD AND $res['DateJoining']<=$cY.'-'.$PrvMD AND $RatingHod>0)
  {
   ...
   {
     $TotIncAmt=($TPreCtc*$resMaxMin['IncDistriFrom'])/100;
     //$NewGrosst=10*(ceil(($TPreCtc+$TotIncAmt)/10));
     $NewGrossAmt=10*(ceil(($TPreCtc+$TotIncAmt)/10));
     $NewGrossAmt2=$TPreCtc+$TotIncAmt;
     ...
     {
	  $oneC=($res['EmpCurrAnnualBasic']*$resMaxMin['IncDistriFrom'])/100;
      $oneCM=($res['EmpCurrAnnualBasic']*$Month_Per2)/100;
	  $oneCD=($res['EmpCurrAnnualBasic']*$Day_Per2)/100;
	 ...
	 /*
	 if($res['EmpCode']==772 OR $res['EmpCode']==773)
	 {
	   echo 'MainInc For100='.$NewGrossAmt.' ->For10='.$NewGrosst.'<br>';  
	   echo 'Inc%='.$resMaxMin['IncDistriFrom'].'<br>';    
	   echo 'D='.$DD.' M='.$MM.' Y='.$YY.' IMnt%='.$PerM2.' - IDate%='.$PerD2.'<br>';
	   echo 'Month_Per='.$Month_Per2.' Day_Per='.$Day_Per2.'<br>';
	   echo 'MSal='.$MSal2.' DSal='.$DSal2.'<br>';
	   echo 'T='.$TotInc2;
	 }
	 */
   ...
   elseif($res['DateJoining']>=$cY.'-'.$EffDM AND $res['DateJoining']<=$cY.'-'.$PrvMD AND $RatingHod>0)
   {
    ...
    if($MaxVCtc>0 && $TPreCtc>=$MaxVCtc)
    {
     $oneCM=($res['EmpCurrAnnualBasic']*$Month_Per)/100;
	 $oneCD=($res['EmpCurrAnnualBasic']*$Day_Per)/100;
	...
	if($res['EmpCode']==772 OR $res['EmpCode']==773)
	 {
	   echo 'D='.$D.' M='.$M.' Y='.$Y.' IMnt%='.$PerM.' - IDate%='.$PerD.'<br>';
	   echo 'Month_Per='.$Month_Per.' Day_Per='.$Day_Per.'<br>';
	   echo 'MSal='.$MSal.' DSal='.$DSal;
	 }
	*/
...
/*********************************/
/*********************************/
$oy=date("Y")-1; //if($res['Hod_TotalFinalRating']==3.7){$CRat=1;}else{$CRat=0;}
if($res['DepartmentId']==2 AND ($res['EmpVertical']==17 OR $res['EmpVertical']==18) AND $_SESSION['PmsYId']==10)
{ 
  $yExp10=date('Y-m-d', strtotime("-10 years", strtotime(date($oy."-12-31"))));
  $yExp18=date('Y-m-d', strtotime("-18 years", strtotime(date($oy."-12-31"))));
  
  $sqRe=mysql_query("select count(*) as TotRat from hrm_pms_appraisal_history p INNER join hrm_employee e on p.EmpCode=e.EmpCode INNER JOIN hrm_employee_general g on e.EmployeeID=g.EmployeeID where p.EmpCode=".$res['EmpCode']." and e.EmpStatus='A' and g.DepartmentId=2 and g.GradeId>=67 and p.Rating>=3.7 and p.CompanyId=".$CompanyId." and (g.EmpVertical=17 OR g.EmpVertical=18)",$con); $reRe=mysql_fetch_assoc($sqRe); $TotCountRat=$reRe['TotRat'];
  $HalfAmt=round($TPreCtc/2);
  if($res['DateJoining']<=$yExp10 && $TotCountRat>=7)
  {	
   $ActualNewGS=$ActualNewGS+$HalfAmt; $ExtRnDMsg='Eligible for Utkrisht Reward';
  }
  else { $ExtRnDMsg=''; }
}  
...
<input type="hidden" id="ActualInc<?=$sno?>" value="<?=$MainOne?>"/>
<input type="hidden" id="ProRataInc<?=$sno?>" value="<?=$MainTwo?>"/>
<input type="hidden" id="ActualIncAmt<?=$sno?>" value="<?=$NewGrossAmt2?>"/>
<input type="hidden" id="ProRataIncAmt<?=$sno?>" value="<?=$TotInc2?>"/>
...
<?php if($MainTwo!=''){?>Annual (%): <font color="#FFFFA8"><?=$MainOne?></font><?php } ?>
<?php /* if($MainTwo!=''){?>, &nbsp;&nbsp;Prorata(%): <font color="#FFFFA8"><?=$MainTwo?></font><?php } */ ?>
&nbsp;&nbsp;
<font color="#FFFFFF"><span class="blink_me"><?php echo $OverMsg; if($ExtRnDMsg!=''){ echo $ExtRnDMsg;} ?></span></font>
&nbsp;&nbsp;
<?php if($MaxVCtc>0){?>
...
<?php 
//if($res['EmployeeID']==140){$ActualNewGS=7072640;}
//elseif($res['EmployeeID']==392){$ActualNewGS=2413302;}
//elseif($res['EmployeeID']==661){$ActualNewGS=337440;}
//elseif($res['EmployeeID']==531){$ActualNewGS=6013039;}
//else{$ActualNewGS=$ActualNewGS;} ?>

<font color="#DDDD00">Proposed Increment : <!--Gross Salary Per Month--></font>&nbsp;<font color="#FFFFFF"><?php echo $ActualNewGS; ?></font>&nbsp;&nbsp; 			  
...
<?php /************** 222222222222222222 Close Close *****************/ ?>
		  ...
		  <tr bgcolor="#7a6189">
           ...
           <?php } ?>	
		     ...
		     <td class="th" style="width:6%;"><b>Proposed<br>Increment</b></td>
		     <td class="th" style="width:4%;"><b>Increment<br>(%)</b></td>
		     <td class="th" style="width:5%;"><b>CTC<br>Batch Mark.</b></b></td>
           ...
           <?php } } ?>	
            ...
            <?php $sqlHistory=mysql_query("select * from hrm_pms_appraisal_history where EmpCode=".$res['EmpCode']." AND CompanyId=".$CompanyId." AND SalaryChange_Date>='".date("2012-01-01")."' AND SalaryChange_Date!='".$_SESSION['EffDate']."' order by SalaryChange_Date ASC", $con); $rowHis=mysql_num_rows($sqlHistory); 
			while($resHistory=mysql_fetch_array($sqlHistory))
			{  
			  $cpid=0;
			  if($resHistory['SalaryChange_Date']>='2020-01-01' && $resHistory['EmpPmsId']>0)
			  {
			   $cpid=1;
			   $qryct=mysql_query("select EmpCurrCtc from hrm_employee_pms where EmpPmsId=".$resHistory['EmpPmsId'],$con);
			...
			<tr bgcolor="#FFFFFF" height="20px;">
		     ...
		     <td class="tdc"><?php echo date("d-M-y",strtotime($resHistory['SalaryChange_Date'])); ?></td>
		     <!--<td class="tdc">&nbsp;</td>-->
		     <td class="tdr" align="right"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo floatval($resHistory['Previous_GrossSalaryPM']); } ?>&nbsp;</td>			
		     <td class="tdr" align="right"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo floatval($resHistory['Proposed_GrossSalaryPM']); } ?>&nbsp;</td>
		     <?php /*?><td class="tdc"><?php echo $resHistory['Prop_PeInc_GSPM']; ?></td><?php */?>
		     <td class="tdr" align="right"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo floatval($resHistory['PropSalary_Correction']); } ?>&nbsp;</td>
		     <td class="tdr" align="right"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo floatval($resHistory['TotalProp_GSPM']); } ?>&nbsp;</td>
		     <td class="tdc"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo $resHistory['TotalProp_PerInc_GSPM']; } ?></td>
           <?php if($TotInEM>0) { ?>
             <td class="tdr"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo floatval($resHistory['Incentive']); } ?>&nbsp;</td>
             <?php $TotalPay=$resHistory['TotalProp_GSPM']+$resHistory['Incentive']; ?>
             <td class="tdr"><?php if($cpid==0 && $resHistory['SalaryChange_Date']<'2020-01-01'){ echo number_format($TotalPay, 2, '.', ''); } ?>&nbsp;</td>
		   <?php } ?>	
		   
		     <?php /************************ CTC Open ***********/?>
			 
			 <td class="tdr"><?php if($cpid==1){ echo floatval($rqryct['EmpCurrCtc']); }elseif($resHistory['SalaryChange_Date']>='2020-01-01' OR $resHistory['SalaryChange_Date']<'2020-01-01'){ echo floatval($rctc['Tot_CTC']); } ?>&nbsp;</td>
		     <td class="tdr"><?php if($cpid==1){ echo floatval($resHistory['ProIncCTC']); } ?>&nbsp;</td>
		     <td class="tdc"><?php if($cpid==1){ echo $resHistory['Percent_ProIncCTC']; } ?></td>
			 <td class="tdr"><?php if($cpid==1){ echo floatval($resHistory['ProCorrCTC']); } ?>&nbsp;</td>
		     <td class="tdr"><?php if($cpid==1){ echo floatval($resHistory['Proposed_ActualCTC']); }elseif($resHistory['SalaryChange_Date']>='2020-01-01'){ echo floatval($rctc['Tot_CTC']); } ?>&nbsp;</td>
		     <td class="tdc"><?php if($cpid==1){ echo $resHistory['Percent_IncNetCTC']; } ?></td>
			 <?php /************************ CTC Close ***********/?>
...
<?php //$_SESSION['EffDate'] -->Salary Change Date
			 ...
			 */ ?>
		     ...
		     <td class="tdc" bgcolor="#B9FFB9" id="TDD1_<?php echo $sno; ?>"><input type="hidden" id="ProGSPM_<?php echo $sno; ?>" class="tdrr" style="border-style:hidden;border:0;background-color:#B9FFB9;width:100%;text-align:right; padding-right:2px;" value="<?php echo floatval($res['Hod_ProIncSalary']); ?>" maxlength="15" <?php if($rowChh>0){ echo 'readonly'; }else{ echo 'readOnly'; }?> onKeyUp="CalProInc(<?php echo $sno; ?>)" onClick="CalProInc(<?php echo $sno; ?>)" onChange="CalProInc(<?php echo $sno; ?>)" onKeyPress="return isNumberKey(event)"/></td>
		     ...
		     <td class="tdc" bgcolor="#B9FFB9" id="TDD2_<?php echo $sno; ?>"><input type="hidden" id="ProSC_<?php echo $sno; ?>" class="tdrr" style="border-style:hidden;border:0;background-color:#B9FFB9;width:100%;text-align:right;padding-right:2px;" value="<?php echo floatval($res['Hod_ProCorrSalary']); ?>" <?php if($rowChh>0){ echo 'readonly'; }else{ echo 'readonly'; }?>  maxlength="15" onKeyUp="CalProInc(<?php echo $sno; ?>)" onClick="CalProInc(<?php echo $sno; ?>)" onChange="CalProInc(<?php echo $sno; ?>)" onKeyPress="return isNumberKey(event)"/></td>
		     <td class="tdc" bgcolor="#B9FFB9"><input type="hidden" id="TotalProGSPM_<?php echo $sno; ?>" class="tdrr" style="border-style:hidden; border:0; background-color:#B9FFB9;width:100%;text-align:right; padding-right:2px;" value="<?php echo floatval($res['Hod_GrossMonthlySalary']); ?>" readOnly maxlength="15"/></td>
  ...
  <input type="hidden" value="<?php echo $rowChh;?>" id="AllInc<?php echo $sno;?>"/>
