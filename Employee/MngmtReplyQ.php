...
{ var QS=document.getElementById("QSubject_"+SN).value; var agree=confirm("Are you sure you want to close this query...?"); 
  if (agree) {window.location="MngmtReplyQ.php?ac=CloseQuery&QCid="+i+"&page="+P+"&QEI="+QEI+"&DI="+DI+"&QS="+QS+"&HYN="+HYN;}}
</script>
</head>
<body class="body">
<table class="table">
...
<?php //*******************************************Status********** ?>       
		...
		<td align="center" style="font-family:Times New Roman; color:#004A00;width:82px; font-size:15px;"><a href="javascript:window.location='MngmtReplyQ.php?page=1'"><b>Refresh</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>	  
...
$sql_statement = mysql_query("select * from hrm_employee_queryemp where QStatus=2 AND Mngmt_ID=".$EmployeeId." AND (Mngmt_QStatus=2 OR Mngmt_QStatus=3) order by QueryDT DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp where QStatus=2 AND Mngmt_ID=".$EmployeeId." AND (Mngmt_QStatus=2 OR Mngmt_QStatus=3) order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
...
<?PHP doPages($offset, 'MngmtReplyQ.php', '', $total_records); ?>
</td>
</tr>					   
			 </table>
		   </td>
