...
$SqlCount=mysql_query("select SUM(EmpCurrGrossPM) as TEGSPM, SUM(EmpCurrIncentivePM) as TEAMINC, SUM(Hod_ProIncSalary) as H_IS, SUM(Hod_ProCorrSalary) as H_SC, SUM(Hod_IncNetMonthalySalary) as TINMS, SUM(Hod_GrossMonthlySalary) as TEPGS, SUM(HodPer_PIS_From_PreMyTGrossPM) as PIS, SUM(HodPer_SC_From_PreMyTGrossPM) as PSC, SUM(Hod_Incentive) as HINCentv from hrm_employee_pms INNER JOIN hrm_employee ON hrm_employee_pms.EmployeeID=hrm_employee.EmployeeID where hrm_employee.EmpStatus='A' AND hrm_employee.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$YearId." AND hrm_employee_pms.HOD_EmployeeID=".$EmployeeId, $con); $sno=1; $ResCount=mysql_fetch_array($SqlCount);

$TPreGrossSalary=$ResCount['TEGSPM']+$ResCount['TEAMINC']; $TotalTeamPreGrossPM=number_format($TPreGrossSalary, 2, '.', '');
$TProGrossSalary=$ResCount['TEPGS']+$ResCount['HINCentv']; $TotalTeamProposedGrossPM=number_format($TProGrossSalary, 2, '.', '');
$Inc=$TProGrossSalary-$TPreGrossSalary;
$One=($TotalTeamPreGrossPM*1)/100; $OnePerPre=number_format($One, 2, '.', ''); 
$Increment=number_format($Inc, 2, '.', '');
$IncPer=$Increment/$OnePerPre; $PerInc=number_format($IncPer, 4, '.', '');

//$PISDiff=$ResCount['H_IS']-$TPreGrossSalary;
$PISDiff=($ResCount['H_IS']+$ResCount['HINCentv'])-$TPreGrossSalary;

$TotalPerPIS=$PISDiff/$OnePerPre; $TotalTPerPIS=number_format($TotalPerPIS, 2, '.', '');
...
/***************************If Prorata wise New Open **************************************************************/
$cY=date("Y"); $pY=date("Y")-1; 
if($resDJ['DateJoining']<=$pY.'-03-31' AND $RatingHod>0)
{
...
elseif($resDJ['DateJoining']>=$pY.'-04-01' AND $resDJ['DateJoining']<=$cY.'-03-31' AND $RatingHod>0)
{
  ...
  $sqlCk1 = mysql_query("SELECT Previous_GrossSalaryPM,TotalProp_GSPM FROM hrm_pms_appraisal_history INNER JOIN hrm_employee ON hrm_pms_appraisal_history.EmpCode=hrm_employee.EmpCode where SalaryChange_Date='".$resDJ['DateJoining']."' AND hrm_employee.EmployeeID=".$res['EmployeeID']." AND hrm_pms_appraisal_history.CompanyId=".$CompanyId, $con); $resCk1=mysql_fetch_assoc($sqlCk1);
  $sqlCk2 = mysql_query("SELECT Previous_GrossSalaryPM,TotalProp_GSPM FROM hrm_pms_appraisal_history INNER JOIN hrm_employee ON hrm_pms_appraisal_history.EmpCode=hrm_employee.EmpCode where SalaryChange_Date='".$resCorr['SalaryChDate']."' AND hrm_employee.EmployeeID=".$res['EmployeeID']." AND hrm_pms_appraisal_history.CompanyId=".$CompanyId, $con); $resCk2=mysql_fetch_assoc($sqlCk2);

  if($resDJ['DateJoining']>=$pY.'-04-01' AND $resDJ['DateJoining']<=$pY.'-09-30' AND $RatingHod>0)
  { 
	...
	else
	{
	 ...
	 $date11 = new DateTime($resDJ['DateJoining']);  $date22 = new DateTime($pY."-09-30");
  ...
  elseif($resDJ['DateJoining']>=$pY.'-10-01' AND $resDJ['DateJoining']<=$cY.'-03-31' AND $RatingHod>0)
  { 
    if($resCorr>0 AND $resCk2['TotalProp_GSPM']>$resCk1['TotalProp_GSPM']){ $date1 = new DateTime($resCorr['SalaryChDate']); }
	else{ $date1 = new DateTime($resDJ['DateJoining']); }
    $date2 = new DateTime($cY."-09-30");
    $interval = date_diff($date2, $date1);
    $Y=$interval->format('%y');  $M=$interval->format('%m');  $D=$interval->format('%d')+1;
    $PerM=$resMaxMin['IncDistriFrom']/12;  $PerD=$PerM/30;
    $Month_Per=round($PerM*$M, 2); $Day_Per=round($PerD*$D, 2);
...
/***************************If Prorata wise New Close **************************************************************/

/***************************If Prorata  wise**************************************************************/	 
/*** Opennnnnnnnnnnnnnnn  Opennnnnnnnnnnnnnnn ******/
/*
...
*/	  
/*** Closeeeeeeeeeeeeeeeeee  Closeeeeeeeeeeeeeeeeee ******/
?> 
...
?>
            ...
            <?php if($CompanyId==1 OR $CompanyId==2){ /*Check Open*/ //AND $rowHi2==0 ?>
            <tr bgcolor="#FFFFFF" height="20px;">					
		      ...
		      <td bgcolor="#B9FFB9" align="center" style="font:Georgia; font-size:11px; width:80px;" valign="top">
			  <input id="ProSC_<?php echo $sno; ?>" style="border-style:hidden; border:0; background-color:#B9FFB9; width:78px; height:20px;text-align:right;" height="20" value="<?php echo $res['Hod_ProCorrSalary']; ?>" <?php if($rowChh>0){ echo ''; }else{ echo 'disabled'; }?>  maxlength="15" onKeyDown="CalProInc(<?php echo $sno; ?>)" onClick="CalProInc(<?php echo $sno; ?>)" onChange="CalProInc(<?php echo $sno; ?>)"/>
			  </td>
		      <td bgcolor="#B9FFB9" align="center" style="font:Georgia; font-size:11px; width:80px;" valign="top">
			  <input id="TotalProGSPM_<?php echo $sno; ?>" style="border-style:hidden; border:0; background-color:#B9FFB9; width:78px; height:20px;text-align:right;" height="20" value="<?php echo $res['Hod_GrossMonthlySalary']; ?>" readOnly maxlength="15"/>
			  </td>
		      <td bgcolor="#B9FFB9" align="center" style="font:Georgia; font-size:11px; width:80px;" valign="top">
			  <input id="Per_TotalProGSPM_<?php echo $sno; ?>" style="border-style:hidden; border:0; background-color:#B9FFB9; width:78px; height:20px;text-align:center;" height="20" value="<?php echo $res['Hod_Percent_IncNetMonthalySalary']; ?>" readOnly maxlength="15"/>
...
<input style="border-style:hidden; border:0; background-color:#B9FFB9; width:78px; height:20px;text-align:center;" id="EmpInctv<?php echo $sno; ?>" value="<?php echo $res['Hod_Incentive']; ?>" maxlength="15" onKeyDown="CalProInc(<?php echo $sno; ?>)" onClick="CalProInc(<?php echo $sno; ?>)" onChange="CalProInc(<?php echo $sno; ?>)" disabled/>
<?php } ?></td>
...
<input style="border-style:hidden; border:0; background-color:#B9FFB9; width:78px; height:20px;text-align:center;" id="TotalPay<?php echo $sno; ?>" value="<?php echo number_format($TotpayEmp, 2, '.', ''); ?>" maxlength="15" readonly/>
<?php } ?></td><?php } ?> 
