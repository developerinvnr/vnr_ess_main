...
$sqlSY=mysql_query("select OldY,CurrY,NewY from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='KRA'", $con);  
$sqlSYP=mysql_query("select OldY,CurrY,NewY from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='PMS'", $con); $resSY=mysql_fetch_array($sqlSY); $resSYP=mysql_fetch_array($sqlSYP); 
...
$SD=mysql_query("select DepartmentId,DateJoining,DesigId from hrm_employee_general where EmployeeID=".$EmployeeId, $con); 
$RD=mysql_fetch_assoc($SD); $RDe=mysql_fetch_assoc($SD); $resDoJ=mysql_fetch_assoc($SD);
$Before31Day=date("Y-m-d",strtotime('-30 day'));  $After31DayDoJ=date("Y-m-d",strtotime($resDoJ['DateJoining'].'+31 day'));
...
$search = str_split($search);
 for($i=1; $i<=$_POST['EditTNRow']; $i++)
 {
  if($_POST['KRA_'.$i]!='')
  { $str_KRA = $_POST['KRA_'.$i]; $str_KRADes = $_POST['KRADes_'.$i]; $KRA=str_replace($search, "", $str_KRA); $KRADes=str_replace($search, "", $str_KRADes);
    $sqlSaveK=mysql_query("update hrm_pms_kra set KRA='".$KRA."', KRA_Description='".$KRADes."', Measure='".$_POST['Measure_'.$i]."', Unit='".$_POST['Unit_'.$i]."', Weightage=".$_POST['Weightage_'.$i].", Logic='".$_POST['Logic_'.$i]."', Period='".$_POST['Period_'.$i]."', Target=".$_POST['Target_'.$i].", EmpStatus='P', CreatedBy=".$EmployeeId.", CreatedDate='".date("Y-m-d")."' where KRAId=".$_POST['KId_'.$i], $con); 
  }
  ...
  $TotalWeight=$_POST['WWeightage_1']+$_POST['WWeightage_2']+$_POST['WWeightage_3']+$_POST['WWeightage_4']+$_POST['WWeightage_5']+$_POST['WWeightage_6']+$_POST['WWeightage_7']+$_POST['WWeightage_8']+$_POST['WWeightage_9']+$_POST['WWeightage_10']+$_POST['WWeightage_11']+$_POST['WWeightage_12']+$_POST['WWeightage_13']+$_POST['WWeightage_14']+$_POST['WWeightage_15'];
      ...
      if($sqlSaveK)
	  {
	   ...
	   if($TotalWeight==100)
       {
	    ...
	    { 
		  $sqlU=mysql_query("update hrm_pms_kra set KRAStatus='R',UseKRA='A',EmpStatus='A' where YearId=".$resSY['CurrY']." AND EmployeeID=".$EmployeeId, $con); 
...
if($_REQUEST['actsubkra']=='OkSubKraEdit' && $_REQUEST['MWei']!='' && $_REQUEST['KId']!='')
{
 ...
 { 
  ...
  if($sU)
  { 
   if($TotalWei==$_REQUEST['MWei']){echo '<script>alert("Sub-KRAs have been saved successfully."); window.location="EmpAddNewKRA.php?e='.$_REQUEST['e'].'&DI='.$_REQUEST['DI'].'&ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=0&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1"; </script>';}
   ...
   echo '<script>alert("Error..Please Check!, Sub-KRAs weightage is not equal to main KRA weightage !");
    window.location="EmpAddNewKRA.php?e='.$_REQUEST['e'].'&DI='.$_REQUEST['DI'].'&ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=0&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1"; </script>';}
  } 	  
}
...
$sqlEK2=mysql_query("select KRA,KRA_Description,Measure,Unit,hrm_employee_pms_kraforma.Weightage,hrm_employee_pms_kraforma.Target from hrm_employee_pms_kraforma INNER JOIN hrm_pms_kra ON hrm_employee_pms_kraforma.KRAId=hrm_pms_kra.KRAId where hrm_employee_pms_kraforma.EmpPmsid=".$resEK['EmpPmsId']." AND hrm_employee_pms_kraforma.KRAFormAStatus='A' AND hrm_pms_kra.CompanyId=".$CompanyId, $con); $rowEK2=mysql_num_rows($sqlEK2); if($rowEK2>0) {$no=1; while($resEK2=mysql_fetch_array($sqlEK2)){ ?>
  ...
  <td valign="top" style="width:80px;font-family:Georgia;font-size:12px;">
...
<script>function FunLog(){ window.open("viewlogic.php", "F", "menubar=no,scrollbars=yes,resizable=no,directories=no,width=1000,height=500");}</script>	  
	  ...
	  <input type="button" name="Refresh" id="Refresh" style="width:90px;" value="Refresh" onClick="javascript:window.location='EmpAddNewKRA.php?ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=0&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1'"/>
<?php if($RD['DepartmentId']!=6 AND $RD['DepartmentId']!=4){ ?>
...
$sqlKy=mysql_query("select FromDate,ToDate from hrm_year where YearId=".$resSY['CurrY'], $con); 
$sqlPy=mysql_query("select FromDate,ToDate from hrm_year where YearId=".$resSYP['CurrY'], $con);  
...
$resDoJ=mysql_fetch_assoc($sqlDoJ); $Before31Day=date("Y-m-d",strtotime('-30 day'));  $After31DayDoJ=date("Y-m-d",strtotime($resDoJ['DateJoining'].'+31 day'));
...
$sql=mysql_query("select * from hrm_pms_kra where YearId=".$resSY['CurrY']." AND CompanyId=".$CompanyId." AND EmployeeID=".$EmployeeId." AND (KRAStatus='R' OR KRAStatus='A') order by KRAId ASC", $con);  $row=mysql_num_rows($sql); if($row>0){$resRe=mysql_fetch_assoc($sql);}
...
<input type="hidden" id="DateJoining" value="<?php echo $resDoJ['DateJoining']; ?>" />
<input type="hidden" id="After31DayDoJ" value="<?php echo $After31DayDoJ; ?>" />
<input type="hidden" id="CuDate" value="<?php echo $CuDate; ?>" />
<input type="hidden" id="EmpFromDate" value="<?php echo $resSch['EmpFromDate']; ?>" />
<input type="hidden" id="EmpToDate" value="<?php echo $resSch['EmpToDate']; ?>" />
<input type="hidden" id="ExtraAllowKRA" value="<?php echo $resPm['ExtraAllowKRA']; ?>" />
<input type="hidden" id="AppFromDate" value="<?php echo $resSch['AppFromDate']; ?>" />
<input type="hidden" id="AppToDate" value="<?php echo $resSch['AppToDate']; ?>" />
<input type="hidden" id="RevFromDate" value="<?php echo $resSch['AppFromDate']; ?>" />
<input type="hidden" id="RevToDate" value="<?php echo $resSch['RevToDate']; ?>" />
<input type="hidden" id="EmpDateStatus" value="<?php echo $resSch['EmpDateStatus']; ?>" />
<input type="hidden" id="AppDateStatus" value="<?php echo $resSch['AppDateStatus']; ?>" />
<input type="hidden" id="RevDateStatus" value="<?php echo $resSch['RevDateStatus']; ?>" />
<input type="hidden" id="HodDateStatus" value="<?php echo $resSch['HodDateStatus']; ?>" />
...
<?php if($row>0) { $sql=mysql_query("select * from hrm_pms_kra where YearId=".$resSY['CurrY']." AND CompanyId=".$CompanyId." AND EmployeeID=".$EmployeeId." AND (KRAStatus='R' OR KRAStatus='A') order by KRAId ASC", $con); $k=1; while($res=mysql_fetch_assoc($sql)) {?>
<tr style="height:22px;" id="TR_<?php echo $k; ?>">	
  ...
  <option value="Process">Process</option><option value="Maintenance">Maintenance</option><option value="Time">Time</option><option value="Yield">Yield</option><option value="None">None</option></select>
  <?php } ?>
  </td>
...
<tr style="height:22px;" id="TR_<?php echo $i; ?>">	
  ...
  <input name="Target_<?php echo $i; ?>" id="Target_<?php echo $i; ?>" class="Input" style="width:68px; text-align:center;" maxlength="8" onChange="ChangeTarget(<?php echo $i; ?>)" value="<?php if($RD['DepartmentId']!=6) { echo '100'; } elseif($RD['DepartmentId']==6) { echo '0'; }?>" /></td>
  </tr></table></td>
</tr>
