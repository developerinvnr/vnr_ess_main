...
<?php if($rowHod>0) { ?><td align="center"><a href="AttApplhod.php?m=<?php echo $_REQUEST['m']; ?>&Y=<?php echo $_REQUEST['Y'];?>&e=4e&w=234&tt=10234&aa=f&e=4e2&e=4e&retd=ee&wwrew=t%T@sed818&d=101&a=0"><img id="Img_hod1" style="display:block;" src="images/RevHod1.png" border="0"/></a>
		     </td><?php } ?>
...
<?php 
//AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' AND 
//AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' AND 
//AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' AND 
if($_REQUEST['a']==0){$query="select * from hrm_employee_attendance_req where RId=".$EmployeeId; }elseif($_REQUEST['a']==1){$query="select * from hrm_employee_attendance_req where Status=0 AND RId=".$EmployeeId; }elseif($_REQUEST['a']==2){$query="select * from hrm_employee_attendance_req where Status=1 AND RId=".$EmployeeId;}
...
//AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' AND 
//AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' AND 
if($_REQUEST['a']==0){$query2="select * from hrm_employee_attendance_req where RId=".$EmployeeId." order by AttDate DESC LIMIT " . $from . "," . $offset; }elseif($_REQUEST['a']==1){$query2="select * from hrm_employee_attendance_req where Status=0 AND RId=".$EmployeeId." order by AttDate DESC LIMIT " . $from . "," . $offset; }elseif($_REQUEST['a']==2){$query2="select * from hrm_employee_attendance_req where Status=1 AND RId=".$EmployeeId." order by AttDate DESC LIMIT " . $from . "," . $offset; }
