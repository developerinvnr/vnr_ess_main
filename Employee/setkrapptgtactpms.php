...
{
 $up=mysql_query("update hrm_pms_kra_tgtdefin set AppAch='".$_POST['ach']."', AppCmnt='".$_POST['cmnt']."', AppLogScr='".$_POST['logscr']."', AppScor='".$_POST['scor']."' where TgtDefId=".$_POST['id'],$con);
 if($up)
 { 
  //$sqlt=mysql_query("select SUM(AppAch) as tAch from hrm_pms_kra_tgtdefin where ".$_POST['fn']."=".$_POST['kid'],$con); 
...
{ 
 $up=mysql_query("update hrm_pms_kra_tgtdefin set Applockk=1 where TgtDefId=".$_POST['id'],$con);
...
{
  $sqlt=mysql_query("select SUM(AppAch) as tAch, SUM(AppLogScr) as tLogScr, SUM(AppScor) as tScor from hrm_pms_kra_tgtdefin where KRAId=".$_POST['kid'],$con); $rest=mysql_fetch_assoc($sqlt);
  echo '<input type="hidden" id="tAch" value="'.floatval($rest['tAch']).'" />';
...
{
  $sqlt=mysql_query("select SUM(AppAch) as tAch, SUM(AppLogScr) as tLogScr, SUM(AppScor) as tScor from hrm_pms_kra_tgtdefin where KRASubId=".$_POST['kid'],$con); $rest=mysql_fetch_assoc($sqlt);
  echo '<input type="hidden" id="tAch" value="'.floatval($rest['tAch']).'" />';
  echo '<input type="hidden" id="tLogScr" value="'.floatval($rest['tLogScr']).'" />';
  echo '<input type="hidden" id="tScor" value="'.floatval($rest['tScor']).'" />';
}
