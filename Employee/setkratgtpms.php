...
{$LDate=date("Y-09-30");}
	 ...
	 ?>
	 <?php $Nxt10Day=date("Y-m-d",strtotime('+7 day', strtotime($LDate))); //date("2021-01-12") //$LDate
	       $Nxt14Day=date("Y-m-d",strtotime('+14 day', strtotime($LDate))); 
	       ?>
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ //AND $_REQUEST['prd']=='Monthly' ?>
	  <span style="cursor:pointer;">
	 <?php //echo $PerM.'-'.$res['lockk'].'-'.$res['AppRevert']; ?>
	 
	  <img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($PerM==1 AND $res['lockk']==0 AND date("Y-m-d")<=$Nxt10Day) OR ($PerM==1 AND $res['lockk']==2 AND date("Y-m-d")<=$Nxt14Day) OR ($PerM==1 AND $res['lockk']==0 AND $rpms['UnLckKRA']==1) OR ($PerM==1 AND $res['lockk']==0 AND $res['Applockk']==0 AND $res['AppRevert']=='Y' AND date("Y-m-d")<=$Nxt14Day) OR ($res['Cmnt']=='' AND ($res['Ach']=='' OR $res['Ach']==0) AND $res['Wgt']!='' AND $res['Wgt']!=0)){echo 'block';}else{echo 'none';} ?>" onClick="FunEdit(<?php echo $i; ?>)" />
