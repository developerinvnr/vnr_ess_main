   ...
   if($fileSize > 1000000)
    {
     ...
     if ((($ext == "docx")||($ext == "doc")||($ext == "xls")||($ext == "xlsx")||($ext == "ppt")||($ext == "ods")||($ext == "odt")||($ext == "pdf")||($ext == "jpg")||($ext == "jpeg")) && ($_FILES["ufile"]["size"] < 1000000))
      {$ext=".".$ext;  $newname = dirname(__FILE__).'/AppUploadFile/'.$newfilename; //Determine the path to which we want to save this file
       if((move_uploaded_file($_FILES['ufile']['tmp_name'],$newname)))
        { $sqlUp=mysql_query("insert into hrm_employee_pms_uploadfile(EmpPmsId,EmployeeID,FileName,Ext,YearId) values(".$_SESSION['P'].", ".$_SESSION['E'].",'".$newfilename."', '".$ext."', ".$_SESSION['Y'].")", $con); 
...
{$sqlD=mysql_query("delete from hrm_employee_pms_uploadfile where FileId=".$_REQUEST['I'], $con); 
...
{ var Name = document.getElementById("UpFileName").value; 
  ...
  if(test_bool==false) { alert('Please Enter Only Alpha-Numeric in file Name field');  return false; } 
}

function deleteMyfile(FN,PI,EI,YI,I)
{ document.getElementById("up").style.display='none'; 
  window.location = "UploadAppFileEmp.php?action=delete&I="+I+"&P="+PI+"&E="+EI+"&Y="+YI+"&FN="+FN;
}
...
<td>
 ...
 $DBFileName=substr($resCheck['FileName'],5); ?>
   <tr bgcolor="#FFFFFF" style="height:21px;">
	...
	<td style="width:80px; font-family:Times New Roman; font-size:12px;" align="center">
	<a href="#"><img src="images/deletefile.png" border="0" onClick="deleteMyfile('<?php echo $resCheck['FileName']; ?>',<?php echo $_SESSION['P']; ?>, <?php echo $_SESSION['E']; ?>, <?php echo $_SESSION['Y']; ?>, <?php echo $resCheck['FileId']; ?>)" /></a></td>
   </tr>
 <?php $no++; } ?>  
