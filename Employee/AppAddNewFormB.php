...
if(isset($_POST['SaveFormB']))
{
 ...
 $sqlPms=mysql_query("select EmpPmsId from hrm_employee_pms where AssessmentYear=".$_POST['KraYId']." AND EmployeeID=".$_POST['empiid'], $con); $rowPms=mysql_num_rows($sqlPms);
 if($rowPms>0){ $resPms=mysql_fetch_array($sqlPms); $PmsId=$resPms['EmpPmsId']; }else{ $PmsId=0; }

   /********************** FormB FormB Open ****************************/
   $sqlb=mysql_query("select * from hrm_employee_pms_behavioralformb where EmpId=".$_POST['empiid']." AND YearId=".$_POST['KraYId']."", $con); $rowb=mysql_num_rows($sqlb);
   if($rowb>0)
   {
    while($resb=mysql_fetch_assoc($sqlb))
	{
	 ...
	 if($test==0){ $sqlDe=mysql_query("delete from hrm_employee_pms_behavioralformb where FormBId=".$resb['FormBId']." AND EmpId=".$_POST['empiid']." AND YearId=".$_POST['KraYId']."", $con); }
	} //while
	
	for($j=1; $j<=$totRw; $j++)
	{
	 $sqlbf=mysql_query("select * from hrm_employee_pms_behavioralformb where FormBId=".$_POST['FormBIdM_'.$j]." AND EmpId=".$_POST['empiid']." AND YearId=".$_POST['KraYId']."", $con); $rowbf=mysql_num_rows($sqlbf);
	 if($rowbf==0){ $sqlIn=mysql_query("insert into hrm_employee_pms_behavioralformb(EmpPmsId, FormBId, EmpId, YearId, AppStatus) values(".$PmsId.", ".$_POST['FormBIdM_'.$j].", ".$_POST['empiid'].", ".$_POST['KraYId'].", 'P')",$con); }	
    }
   }
   else
   {
	...
	for($j=1; $j<=$totRw; $j++)
	{ 
	 $sqlIn=mysql_query("insert into hrm_employee_pms_behavioralformb(EmpPmsId, FormBId, EmpId, YearId, AppStatus) values(".$PmsId.", ".$_POST['FormBIdM_'.$j].", ".$_POST['empiid'].", ".$_POST['KraYId'].", 'P')",$con); 
...
if(isset($_POST['SubmitKRA'])) 
{
 ...
 if($rowPms>0){ $resPms=mysql_fetch_array($sqlPms); $PmsId=$resPms['EmpPmsId']; }else{ $PmsId=0; }
   ...
   if($rowb>0)
   {
	...
	{
	 $sqlbf=mysql_query("select * from hrm_employee_pms_behavioralformb where FormBId=".$_POST['FormBIdM_'.$j]." AND EmpId=".$_POST['empiid']." AND YearId=".$_POST['KraYId']."", $con); $rowbf=mysql_num_rows($sqlbf);
	 if($rowbf==0){ $sqlIn=mysql_query("insert into hrm_employee_pms_behavioralformb(EmpPmsId, FormBId, EmpId, YearId, AppStatus) values(".$PmsId.", ".$_POST['FormBIdM_'.$j].", ".$_POST['empiid'].", ".$_POST['KraYId'].", 'A')",$con); }	
    }
   }
   else
   {
	...
	for($j=1; $j<=$totRw; $j++)
	{
	 $sqlIn=mysql_query("insert into hrm_employee_pms_behavioralformb(EmpPmsId, FormBId, EmpId, YearId, AppStatus) values(".$PmsId.", ".$_POST['FormBIdM_'.$j].", ".$_POST['empiid'].", ".$_POST['KraYId'].", 'A')",$con); 
    }	
	  
   }
   $sqlIn=mysql_query("update hrm_employee_pms_behavioralformb set EmpStatus='A', AppStatus='A' where EmpId=".$_POST['empiid']." AND YearId=".$_POST['KraYId'],$con);
   /********************** FormB FormB Close ****************************/
   
 if($sqlIn){ $msg='Form-B submitted successfully'; }
...
<?php 
$sqlDoj=mysql_query("select DateJoining,GradeId,DepartmentId from hrm_employee_general where EmployeeID=".$_REQUEST['e'],$con); $resDoj=mysql_fetch_assoc($sqlDoj);
?>		
...
<input type="hidden" id="empiid" name="empiid" value="<?php echo $_REQUEST['e']; ?>"/>
...
<?php 
$sqlB=mysql_query("select * from hrm_employee_pms_behavioralformb where YearId=".$_SESSION['KraYId']." AND EmpId=".$_REQUEST['e']." AND (AppStatus='D' OR AppStatus='P')", $con); $rowB=mysql_num_rows($sqlB); 

$sqlBB=mysql_query("select * from hrm_employee_pms_behavioralformb where YearId=".$_SESSION['KraYId']." AND EmpId=".$_REQUEST['e']." AND AppStatus='A' AND AppStatus!='D' AND EmpStatus!='P'", $con); $rowBB=mysql_num_rows($sqlBB);
?>     	
	 ...
	 <td colspan="7" style="width:98%;">
	  <table border="0" style="width:100%;">
	   <tr>
	    ...
	    <td class="tdc" style="width:130px; text-align:right;">
		<input type="button" name="back" id="back" style="width:100px;" value="back" onClick="javascript:window.location='AppCheckNewFormB.php?ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&formb=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1'"/></td>
...
{ 
 $sqlBY=mysql_query("select fb.* from hrm_pms_formb fb INNER JOIN hrm_employee_pms_behavioralformb fbg ON fb.FormBId=fbg.FormBId where fb.SkillStatus='A' AND fbg.YearId=".$_SESSION['KraYId']." AND fbg.EmpId=".$_REQUEST['e']." order by FormBId", $con); 
}
...
<?php
 if($rowB>0)
 {
 $sqlBY=mysql_query("select fb.* from hrm_pms_formb fb INNER JOIN hrm_employee_pms_behavioralformb fbg ON fb.FormBId=fbg.FormBId where fb.SkillStatus='A' AND fbg.YearId=".$_SESSION['KraYId']." AND fbg.EmpId=".$_REQUEST['e']." AND fb.GroupFor!='' order by fbg.FormBId", $con);
 }
 else
 {
 ...
 <tr bgcolor="#FFFFFF" style="height:24px;">   
  ...
  <?php if($rowBB==0){ ?>
  <td class="font1" align="center"><span style="cursor:pointer;"><img src="images/close-folder.png" id="btnC<?=$j?>" onClick="FunChangeformB('O',<?=$j.','.$resBY['FormBId'].','.$resDoj['DepartmentId'].','.$resDoj['GradeId'].','.$_REQUEST['e']?>)"/><img src="images/open-folder.png" id="btnO<?=$j?>" onClick="FunChangeformB('C',<?=$j.','.$resBY['FormBId'].','.$resDoj['DepartmentId'].','.$resDoj['GradeId'].','.$_REQUEST['e']?>)" style="display:none;"/></span>
