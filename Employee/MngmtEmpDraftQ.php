...
if($_REQUEST['ac']=='CloseQuery' AND $_REQUEST['QCid']!='')
{
  ...
  $sqlDel=mysql_query("update hrm_employee_queryemp set QCloseStatus='Y', Mngmt_QStatus=3 where QueryId=".$_REQUEST['QCid'], $con);
...
{ var WindowObjectReference = window.open("ReadQuery.php?Qid="+QI,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=500,height=310");
  //var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="EmpDraftQ.php"; } }, 1000);
}


function ActionQReply(v,QI,NOfT,P,QEI,DI,SN,HYN) //openRequestedPopup()  
{ 
  if(v=='P') {var agree=confirm("Are you sure you want to change status to In-Process?"); 
  if (agree) { window.open("ReplyQueryMngmt.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350"); }
  //var timer = setInterval( function() { if(WindowObjectReference.closed) {  clearInterval(timer);  window.location="MngmtEmpDraftQ.php?page="+P; } }, 1000);
  }
  else if(v=='R') {var agree=confirm("Are you sure you want to reply to this query...?"); 
  if (agree) { window.open("ReplyQueryMngmt.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350"); }
  //var timer = setInterval( function() { if(WindowObjectReference.closed) {  clearInterval(timer);  window.location="MngmtEmpDraftQ.php?page="+P; } }, 1000);
  }
  else if(v=='U') {var agree=confirm("Are you sure you want to update this query answer...?"); 
  if (agree) { window.open("ReplyQueryMngmt.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350"); }
  //var timer = setInterval( function() { if(WindowObjectReference.closed) {  clearInterval(timer);  window.location="MngmtEmpDraftQ.php?page="+P; } }, 1000);
  }		
  else if(v=='C') { var QS=document.getElementById("QSubject_"+SN).value; var agree=confirm("Are you sure you want to close this query...?"); 
  if (agree){ window.location="MngmtEmpDraftQ.php?ac=CloseQuery&QCid="+QI+"&page="+P+"&QEI="+QEI+"&DI="+DI+"&QS="+QS+"&HYN="+HYN;} }	   
			   
  else if(v=='F') {var agree=confirm("Are you sure you want to Forward to this query...?"); 
  if (agree) { window.open("ReplyQueryMngmt.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350"); }
  //var timer = setInterval( function() { if(WindowObjectReference.closed) {  clearInterval(timer);  window.location="MngmtEmpDraftQ.php?page="+P; } }, 1000);
  }			
...
{ if(v=='P') {var agree=confirm("Are you sure you want to change status to In-Process?"); 
  if (agree) { var win = window.open("ReplyQueryMngmt.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="MngmtEmpDraftQ.php?page="+P; } }, 1000);}}
...
<?php //*******************************************Status********** ?>       
		...
		<td align="center" style="font-family:Times New Roman; color:#004A00;width:82px; font-size:15px;"><a href="javascript:window.location='MngmtEmpDraftQ.php?page=1'"><b>Refresh</b></a></td><td align="center" style="font-family:Times New Roman;width:120px;font-size:15px;"><a href="javascript:window.location='MngmtReplyQ.php?page=1'"><b style="color:#004A00;">Reply Query</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>	  
...
$sql_statement = mysql_query("select * from hrm_employee_queryemp where Mngmt_ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Mngmt_QStatus=0 OR Mngmt_QStatus=1 OR Mngmt_QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>Mngmt_QToDT order by QueryDT DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp where Mngmt_ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Mngmt_QStatus=0 OR Mngmt_QStatus=1 OR Mngmt_QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>Mngmt_QToDT order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
...
?>
		   ...
		   <td style="font-family:Times New Roman; font-size:12px; width:100px;" align="center" valign="top"><?php echo date("d-M-y", strtotime($resQuery['Mngmt_QToDT'])); ?></td>
...
<?PHP doPages($offset, 'MngmtEmpDraftQ.php', '', $total_records); ?>
</td>
</tr>					   
			 </table>
		   </td>
