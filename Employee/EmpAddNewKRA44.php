...
function SaveKraA(k,m)
{
  ...
  else if(document.getElementById("Kra_a"+k+"_2").value!='')
  {
  ...
  else if(document.getElementById("Kra_a"+k+"_3").value!='')
  {
  ...
  else if(document.getElementById("Kra_a"+k+"_4").value!='')
  {
  ...
  else if(document.getElementById("Kra_a"+k+"_5").value!='')
  {
...
<?php 
//$sqlEK=mysql_query("select * from hrm_employee_pms_kraforma INNER JOIN hrm_employee_pms ON hrm_employee_pms_kraforma.EmpPmsId=hrm_employee_pms.EmpPmsId where hrm_employee_pms.EmployeeID=".$EmployeeId." AND YearId=".$resSY['OldY']."", $con); $rowEK=mysql_num_rows($sqlEK); if($rowEK) { $resEK=mysql_fetch_assoc($sqlEK);
//$sqlEK2=mysql_query("select KRA,KRA_Description,Measure,Unit,hrm_employee_pms_kraforma.Weightage,hrm_employee_pms_kraforma.Target from hrm_employee_pms_kraforma INNER JOIN hrm_pms_kra ON hrm_employee_pms_kraforma.KRAId=hrm_pms_kra.KRAId where hrm_employee_pms_kraforma.EmpPmsid=".$resEK['EmpPmsId']." AND hrm_employee_pms_kraforma.KRAFormAStatus='A' AND hrm_pms_kra.CompanyId=".$CompanyId, $con); $rowEK2=mysql_num_rows($sqlEK2);

$sqlEK=mysql_query("select * from hrm_employee_pms_kraforma INNER JOIN hrm_employee_pms ON hrm_employee_pms_kraforma.EmpPmsId=hrm_employee_pms.EmpPmsId where hrm_employee_pms.EmployeeID=".$EmployeeId." AND YearId=".$resSY['OldY']."", $con); $rowEK=mysql_num_rows($sqlEK);
if($rowEK>0){ $resEK=mysql_fetch_assoc($sqlEK); $sqlEK2=mysql_query("select KRA,KRA_Description,Measure,Unit,hrm_employee_pms_kraforma.Weightage,hrm_employee_pms_kraforma.Target from hrm_employee_pms_kraforma INNER JOIN hrm_pms_kra ON hrm_employee_pms_kraforma.KRAId=hrm_pms_kra.KRAId where hrm_employee_pms_kraforma.EmpPmsid=".$resEK['EmpPmsId']." AND hrm_employee_pms_kraforma.KRAFormAStatus='A' AND hrm_pms_kra.CompanyId=".$CompanyId, $con); $rowEK2=mysql_num_rows($sqlEK2); }
else{ $sqlEK2=mysql_query("select * from hrm_pms_kra where EmployeeID=".$EmployeeId." AND YearId=".$resSY['OldY']."", $con); $rowEK2=mysql_num_rows($sqlEK2); }

if($rowEK2>0) {$no=1; while($resEK2=mysql_fetch_array($sqlEK2)){ ?>
...
<tr style="height:22px;" id="TR_<?php echo $k; ?>">	
  ...
  <textarea name="KRA_<?php echo $k; ?>" id="KRA_<?php echo $k; ?>" style="width:299px;" class="Inputar" rows="3" maxlength="348" readonly><?php echo $res['KRA']; ?></textarea></td>
  ...
  <td class="font" align="center" style="width:60px;">
  <?php if($rowSubK>0){ ?><input type="" style="width:68px; text-align:center; border:hidden;" value="" readonly/><input type="hidden" name="Target_<?php echo $k; ?>" id="Target_<?php echo $k; ?>" value="100" readonly/><?php } else{ ?><input name="Target_<?php echo $k; ?>" id="Target_<?php echo $k; ?>" class="Input" style="cursor:<?php if($res['Period']!='Annual' AND $res['Period']!=''){echo 'pointer';} ?>;width:68px; text-align:center;text-decoration:<?php if($res['Period']!='Annual' AND $res['Period']!=''){ echo 'underline'; }?>;color:<?php if($res['Period']!='Annual' AND $res['Period']!=''){ echo '#000099'; }?>;" maxlength="8" <?php if($res['Period']!='Annual' AND $res['Period']!='' AND $row2>0){?> onClick="FunTgt(<?php echo $res['KRAId']; ?>,'<?php echo $res['Period']; ?>',<?php echo intval($res['Target']).','.intval($res['Weightage']); ?>,'<?php echo $res['Logic']; ?>')" <?php } ?> onChange="ChangeTarget(<?php echo $k; ?>)" value="<?php echo intval($res['Target']); ?>" readonly/>
...
<?php /* While Open*/ $m=1; while($rSubK=mysql_fetch_assoc($sSubK)){ ?>
  ...
  <input id="Tar_a<?php echo $k.'_'.$m; ?>" class="Inputa" value="<?php echo $rSubK['Target']; ?>" style="cursor:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){echo 'pointer';} ?>;width:60px; text-align:center;text-decoration:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){ echo 'underline'; }?>;color:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){ echo '#000099'; }?>;" maxlength="8" <?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!='' AND $row2>0){ ?> onClick="FunSubTgt(<?php echo $rSubK['KRASubId']; ?>,'<?php echo $rSubK['Period']; ?>',<?php echo $rSubK['Target'].','.intval($rSubK['Weightage']); ?>,'<?php echo $rSubK['Logic']; ?>')" <?php } ?> onKeyUp="ChangeTargeta(this.value,<?php echo $k.', '.$m; ?>)" onChange="ChangeTargeta(this.value,<?php echo $k.', '.$m; ?>)" /></td>
