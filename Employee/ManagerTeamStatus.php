...
{ 
  $ValueId = $_REQUEST['ValueId']; $Eid = $_REQUEST['Eid']; $YId = $_REQUEST['Yid']; 
  $search =  '!"#$/=?*+\'-;:_' ; $search = str_split($search); $Rd=str_replace($search, "", $_REQUEST['Rd']);
  $sql=mysql_query("select EmpPmsId, EmployeeID, Emp_SubmitedDate, EmpFormAScore, EmpFormBScore, EmpFinallyFormA_Score, EmpFinallyFormB_Score, Emp_TotalFinalScore, Emp_TotalFinalRating, Appraiser_NoOfResend from hrm_employee_pms where AssessmentYear=".$YId." AND EmployeeID=".$Eid, $con); $res=mysql_fetch_assoc($sql);
  $NoReSend=$res['Appraiser_NoOfResend']+1;

  $sqlUp=mysql_query("update hrm_employee_pms set Emp_PmsStatus=1, Appraiser_PmsStatus=3, Appraiser_NoOfResend=".$NoReSend.", Appraiser_SubmitedDate='".date("Y-m-d")."' where AssessmentYear=".$YId." AND EmployeeID=".$Eid, $con);
  if($sqlUp)
  { 
   $sqlIns=mysql_query("insert into hrm_employee_pms_resend(EmpPmsId, CompanyId, EmployeeID, Emp_SubmitedDate, EmpFormAScore, EmpFormBScore, EmpFinallyFormA_Score, EmpFinallyFormB_Score, Emp_TotalFinalScore, Emp_TotalFinalRating, App_Reason,App_SendReasonDate) values(".$res['EmpPmsId'].", ".$CompanyId.", ".$Eid.", '".$res['Emp_SubmitedDate']."', ".$res['EmpFormAScore'].", ".$res['EmpFormBScore'].", ".$res['EmpFinallyFormA_Score'].", ".$res['EmpFinallyFormB_Score'].", ".$res['Emp_TotalFinalScore'].", ".$res['Emp_TotalFinalRating'].", '".addslashes($Rd)."', '".date("Y-m-d")."')", $con);	
  }
  if($sqlIns)
  {
   ...
   if($row>0)
   {
	...
	{
	  $sqlu=mysql_query("update hrm_pms_kra_tgtdefin set lockk=0,Applockk=0,Revlockk=0 where KRAId=".$res['KRAId'],$con);
	  ...
	  if($row2)
	  {
	   ...
	   {
	    $sql2u=mysql_query("update hrm_pms_kra_tgtdefin set lockk=0,Applockk=0,Revlockk=0 where KRASubId=".$res2['KRASubId'],$con);
	   } //while($res2=mysql_fetch_assoc($sql2))
	  } //if($row2)
   ...
   /**********************************************************/
      ...
      $msg='Employee Resend Successfully!';
...
{ //document.getElementById('MyTeamStatus').style.display='none'; 
  var url = 'AppHQStatusEmp.php'; var pars = 'HQid2='+value1+'&EmpId2='+value2+'&YId2='+value3+'&CI='+CI;	
  var myAjax = new Ajax.Request( url, { method: 'post', parameters: pars, onComplete: show_HQEmp2 });
} 
function show_HQEmp2(originalRequest)
{ document.getElementById('MyTeamStatus').innerHTML = originalRequest.responseText; }
 
function SelectStateEmp2(value1,value2,value3,CI)
{ //document.getElementById('MyTeamStatus').style.display='none'; 
  var url = 'AppStateStatusEmp.php'; var pars = 'StHQid2='+value1+'&EmpId2='+value2+'&YId2='+value3+'&CI='+CI;	
  var myAjax = new Ajax.Request( url, { method: 'post', parameters: pars, onComplete: show_HQEmp2 });
} 
...
{  if(value1==1)
   ...
   var x = 'ManagerTS.php?Id='+value1+'&Eid='+value2+'&Yid='+value3+'&action=teamS&ee=1&aa=0&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=1&fa=1&fb=1&ffeedb=1&org=1';  window.location=x; 
   }	
   if(value1==2)
   { 
    var agree=confirm("Are you sure you want to resend appraisal form.?");
	if (agree) 
	{ document.getElementById("EmpId").value=value2;
...
{ var value2=document.getElementById("EmpId").value; var value3=document.getElementById("PmsYearId").value;
  ...
  var x = 'ManagerTeamStatus.php?ValueId='+value1+'&Eid='+value2+'&Yid='+value3+'&Rd='+resend+'&action=teamS&ee=1&aa=0&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=0&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=1&fa=1&fb=1&ffeedb=1&org=1';  window.location=x;	
}
...
<?php $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, DepartmentCode, DesigName, GradeValue, HqName, StateName, EmpPmsId, Kra_filename, Kra_ext, Emp_PmsStatus, Appraiser_PmsStatus from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_department d ON g.DepartmentId=d.DepartmentId INNER JOIN hrm_designation de ON g.DesigId=de.DesigId INNER JOIN hrm_grade gr ON g.GradeId=gr.GradeId INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_state st ON hq.StateId=st.StateId where e.EmpStatus='A' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.Appraiser_EmployeeID=".$EmployeeId, $con); 

//$sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, DepartmentCode, DesigName, GradeValue, HqName, StateName, EmpPmsId, Kra_filename, Kra_ext, Emp_PmsStatus, Appraiser_PmsStatus from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_department d ON g.DepartmentId=d.DepartmentId INNER JOIN hrm_designation de ON p.HR_CurrDesigId=de.DesigId INNER JOIN hrm_grade gr ON p.HR_CurrGradeId=gr.GradeId INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_state st ON hq.StateId=st.StateId where e.EmpStatus='A' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.Appraiser_EmployeeID=".$EmployeeId, $con);
...
?>   <div id="tbody">
     ...
     <tr style="height:22px;background-color:#FFFFFF;">
	  ...
	  <td class="tdc"><?php if($res['Emp_PmsStatus']==2 AND $res['Appraiser_PmsStatus']!=2) {?><select class="tdsel" onChange="edit(this.value,<?php echo $res['EmployeeID'].','.$_SESSION['PmsYId']; ?>)"><option value="0">Select</option><option value="1">Edit</option><option value="2">Resend Form</option></select><?php } ?>
	  
	  <?php /*
	  <span class="tdsel" onClick="edit(1,<?php echo $res['EmployeeID'].','.$_SESSION['PmsYId']; ?>)">.</span> */?>
