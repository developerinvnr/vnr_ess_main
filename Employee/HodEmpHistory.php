   ...
   <tr>
    ...
    <table border="1" style="width:100%;" cellspacing="0">
     <tr bgcolor="#FFFF6C">	   
	  ...
	  <td bgcolor="#FFFF6C" class="h" style="width:22%;">Description</td>  
	  ...
	  <td bgcolor="#FFFF6C" class="h" style="width:10%;">App Remark</td>
...
$n=0; if($resK['Period']=='Monthly' OR $resK['Period']=='Quarter' OR $resK['Period']=='1/2 Annual'){ $n=1; $sqlt=mysql_query("select SUM(Ach) as tAch, SUM(LogScr) as tELogScr, SUM(AppAch) as tAppAch, SUM(AppLogScr) as tLogScr, SUM(AppScor) as tScor from hrm_pms_kra_tgtdefin where KRAId=".$resK['KRAId'],$con); $rest=mysql_fetch_assoc($sqlt); }else{$n=0;} ?>	
     <tr bgcolor="#FFFFFF" style="height:25px;"> 	   
	  ...
	  <td class="b"><?php echo floatval($resK['Weightage']);?></td>
	  <td class="b"><?php echo $resK['Logic'];?></td>
      <td class="b"><?php echo $resK['Period'];?></td>
	  <td class="b"><?php if($rowSubK==0){ echo floatval($resK['Target']); }?></td>
	  <td class="b"><?php if($n==1){ echo floatval($rest['tELogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($resK['SelfRating']); }elseif($_SESSION['eMidform']=='Y'){ echo $resK['Mid_SelfRating']; }?></td>
	  ...
	  <td class="b"><?php if($rowSubK>0){echo '';}elseif($n==1){ echo floatval($rest['tLogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($resK['AppraiserRating']);}elseif($_SESSION['eMidform']=='Y'){echo $resK['Mid_AppraiserRating']; } ?></td>
      ...
      <td class="bl" valign="top"><?php echo $resK['AppraiserRemark'];?></td>
	 </tr>
   ...
   <div id="Div<?php echo $Sno; ?>" style="display:<?php if($rowSubK>0){echo 'block';}else{echo 'none';} ?>;">
    <table style="width:97%;background-color:#71FF71;" border="1" cellpadding="0" cellspacing="0"> 
     <tr style="background-color:#71FF71;">	 
	  ...
	  <td bgcolor="#71FF71" class="h" style="width:22%;">Sub Description</td>  
	  ...
	  <td bgcolor="#71FF71" class="h" style="width:10%;">App Remark</td>
...
<?php /* While Open*/ $m=1; while($rSubK=mysql_fetch_assoc($sSubK)){
     ...
     <tr bgcolor="#FFFFFF" style="height:30px;">  
      ...
      <td class="b"><?php echo floatval($rSubK['Weightage']); ?></td>
      <td class="b"><?php echo $rSubK['Logic']; ?></td>
      <td class="b"><?php echo $rSubK['Period']; ?></td>
      <td class="b"><?php echo floatval($rSubK['Target']); ?></td>
	  <td class="b"><?php if($nn==1){ echo floatval($restt['tELogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($rSubK['SelfRating']); }elseif($_SESSION['eMidform']=='Y'){ echo $rSubK['Mid_SelfRating']; }?></td>
	  ...
	  <td class="b"><?php if($nn==1){ echo floatval($restt['tLogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($rSubK['AppraiserRating']);}elseif($_SESSION['eMidform']=='Y'){echo $rSubK['Mid_AppraiserRating']; } ?></td>
	  <td class="b"><?php if($nn==1){ echo floatval($restt['tScor']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($rSubK['AppraiserScore']);}elseif($_SESSION['eMidform']=='Y'){echo $rSubK['Mid_AppraiserScore']; } ?></td>
      <td class="bl" valign="top"><?php echo $rSubK['AppraiserRemark']; ?></td>
   </tr> 
   <?php $m++;} $KrowSub=$m-1; ?> 
  </table>
  </div> 
...
<?php $Sno++; } $Krow=$Sno-1; ?>
<tr bgcolor="#FFFFFF">
  <td colspan="12" style="font-weight:bold;" align="right">Appraiser Final KRA Score :&nbsp;</td>
  <td align="center" class="b" style="background-color:#B0D8FF;"><b><?php if($_SESSION['eAppform']=='Y'){ echo floatval($resAc['AppraiserFormAScore']);}elseif($_SESSION['eMidform']=='Y'){echo $resAc['Mid_AppraiserFormAScore']; } ?></b></td>
  <td rowspan="2">&nbsp;</td>
</tr>
<tr bgcolor="#FFFFFF">
  <td colspan="12" style="font-weight:bold;" align="right">Reviewer Score :&nbsp;</td>
  <td align="center" class="b" style="background-color:#B0D8FF;"><b><?php echo floatval($resAc['ReviewerFormAScore']); ?></b></td>
</tr>
</tr> 
...
?>
     <tr bgcolor="#FFFFFF"> 	   
	  ...
	  <td class="b"><?php echo floatval($resB2['Weightage']); ?></td>
	  <td class="b"><?php echo $resB2['Logic']; ?></td>
	  <td class="b"><?php echo $resB2['Period']; ?></td>
	  <td class="b"><?php echo floatval($resB2['Target']); ?></td>
	  <td class="b"><?php if($n==1){ echo floatval($rest['tELogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($resBY['SelfRating']);}elseif($_SESSION['eMidform']=='Y'){echo $resBY['Mid_SelfRating']; } ?></td>
	  ...
	  <td class="b"><?php if($n==1){ echo floatval($rest['tLogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($resBY['AppraiserRating']);}elseif($_SESSION['eMidform']=='Y'){echo $resBY['Mid_AppraiserRating']; } ?></td>
	  <td class="b"><?php if($n==1){ echo floatval($rest['tScor']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($resBY['AppraiserScore']);}elseif($_SESSION['eMidform']=='Y'){echo $resBY['Mid_AppraiserScore']; } ?></td>
     </tr>
...
<?php if($m==1){$n='a';}elseif($m==2){$n='b';}elseif($m==3){$n='c';}elseif($m==4){$n='d';}elseif($m==5){$n='e';} ?>  
  ...
  <td align="center" class="fbody2"><?php echo floatval($rSubB['Target']); ?></td>
  <td align="center" class="fbody2"><?php if($nn==1){ echo floatval($restt['tELogScr']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($rSubB['SelfRating']); }?></td>	  
  <td class="fbody2"><?php echo $rSubB['AchivementRemark'];?></td>
  <td align="center" class="fbody2"><?php if($nn==1){ echo floatval($restt['tLogScr']); }else{ echo floatval($rSubB['AppraiserRating']); }?></td>
 	  
  <td align="center" class="fbody2"><?php if($nn==1){ echo floatval($restt['tScor']); }elseif($_SESSION['eAppform']=='Y'){ echo floatval($rSubB['AppraiserScore']);}elseif($_SESSION['eMidform']=='Y'){echo $rSubB['Mid_AppraiserScore']; } ?></td>
...
<?php $SnoB++;} ?>
     <tr bgcolor="#FFFFFF">
      <td colspan="10" style="font-weight:bold;" align="right">Appraiser Final FormB(behavioral) Score:&nbsp;</td>
      <td align="center" class="b" style="background-color:#B0D8FF;"><b><?php if($_SESSION['eAppform']=='Y'){ echo $resAc['AppraiserFormBScore'];}elseif($_SESSION['eMidform']=='Y'){echo $resAc['Mid_AppraiserFormBScore']; } ?></b></td>
    </tr>
	<tr bgcolor="#FFFFFF">
      <td colspan="10" style="font-weight:bold;" align="right">Reviewer Score :&nbsp;</td>
...
<?php $sql=mysql_query("select Fname,Sname,Lname,DepartmentCode,f.* from hrm_feedback f inner join hrm_employee e on f.SubBy=e.EmployeeID inner join hrm_employee_general g on f.SubBy=g.EmployeeID inner join hrm_department d on g.DepartmentId=d.DepartmentId where f.EmployeeID=".$resAc['EmployeeID']." order by f.SubDate DESC",$con); 
 ...
 <tr valign="middle">
  ...
  <table style="width:95%;">
   <tr><td class="hl" style="color:#0067CE;">Appraiser:&nbsp;&nbsp;</td></tr>
