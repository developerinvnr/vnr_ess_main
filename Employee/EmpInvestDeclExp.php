...
$name='All'; 
$xls_filename = 'Employee_Declaration_'.$_REQUEST['M'].'_'.$_REQUEST['P'].'.xls';
...
$sqlE=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname,DepartmentName from hrm_employee e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID inner join hrm_department d on g.DepartmentId=d.DepartmentId where e.CompanyId=".$_REQUEST['CI']." AND e.EmpStatus='A' order by e.EmpCode ASC", $con);

 $no=1;
 while($resE=mysql_fetch_array($sqlE))
 {
     
  $sql=mysql_query("select * from hrm_employee_investment_declaration where EmployeeID=".$resE['EmployeeID']." AND Period='".$_REQUEST['P']."' AND Month=".$_REQUEST['M'], $con);  $res=mysql_fetch_array($sql); 
