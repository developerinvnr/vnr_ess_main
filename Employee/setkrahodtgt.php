...
<?php $sql=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?> 	 
	 ...
	 <td class="d" align="center"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ if($res['Tgt']!=''){echo floatval($res['Tgt']);}else{echo floatval($tgt);} }else{ echo '0%'; } ?><?php //if($res['Tgt']!=''){echo floatval($res['Tgt']);}else{echo floatval($tgt);} ?>
	...
	<tr style="height:24px;">
	 ...
	 <td class="d" align="center" bgcolor="#FFFFFF"><b><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ echo floatval($_REQUEST['tgt']); }else{ echo '0%'; } ?><?php //echo $_REQUEST['tgt']; ?></b></td>
