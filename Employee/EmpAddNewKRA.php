  ...
  $TotalWeight=$_POST['WWeightage_1']+$_POST['WWeightage_2']+$_POST['WWeightage_3']+$_POST['WWeightage_4']+$_POST['WWeightage_5']+$_POST['WWeightage_6']+$_POST['WWeightage_7']+$_POST['WWeightage_8']+$_POST['WWeightage_9']+$_POST['WWeightage_10']+$_POST['WWeightage_11']+$_POST['WWeightage_12']+$_POST['WWeightage_13']+$_POST['WWeightage_14']+$_POST['WWeightage_15'];
      ...
      if($sqlSaveK)
	  {
	   ...
	   if($TotalWeight==100)
       {
	    ...
	    { 
		  $sqlU=mysql_query("update hrm_pms_kra set KRAStatus='R',UseKRA='A',EmpStatus='A' where YearId=".$_SESSION['KraYId']." AND EmployeeID=".$EmployeeId, $con); 
...
<?php $sqlEK=mysql_query("select * from hrm_employee_pms_kraforma ek INNER JOIN hrm_employee_pms p ON ek.EmpPmsId=p.EmpPmsId where p.EmployeeID=".$EmployeeId." AND p.YearId=".$_SESSION['KraYId_Old']."", $con); $rowEK=mysql_num_rows($sqlEK);
if($rowEK>0){ $resEK=mysql_fetch_assoc($sqlEK); $sqlEK2=mysql_query("select ek.KRAId, KRA, KRA_Description, Measure, Unit, ek.Weightage,k.Target,k.Period,k.Logic from hrm_employee_pms_kraforma ek INNER JOIN hrm_pms_kra k ON ek.KRAId=k.KRAId where ek.EmpPmsid=".$resEK['EmpPmsId']." AND ek.KRAFormAStatus='A' AND k.CompanyId=".$CompanyId, $con); }else{ $sqlEK2=mysql_query("select * from hrm_pms_kra where EmployeeID=".$EmployeeId." AND YearId=".$_SESSION['KraYId_Old']."", $con); }
   ...
   <tr>
	...
	<table border="0" style="width:100%;">
	 <tr>
      ...
      <td class="tdc" style="width:6%;" valign="middle"><a href="#" onClick="UploadEmpfile(<?php echo $EmployeeId.','.$_SESSION['KraYId']; ?>)"><font color="#970097"><b>UploadFile</b></font></a></td>
	  <?php } if($_SESSION['eKraform']=='Y' AND $_REQUEST['ee']==0 AND $_REQUEST['kr']==0){ ?> 
      ...
      <td class="tdc" style="width:5%;"><a href="#" onClick="printpageKRA(<?php echo $CompanyId.', '.$_SESSION['KraYId'].', '.$EmployeeId; ?>)" style="text-decoration:none;"/><input type="button" style="width:150px;" value="View/ Print KRA"/></a></td><!--<img src="images/printSaveKRA.png" border="0" />-->
      <?php } ?>
	  ...
	  <td class="tdc" style="width:5%;" valign="middle"><input type="button" name="Refresh" id="Refresh" style="width:90px;" value="Refresh" onClick="javascript:window.location='EmpAddNewKRA.php?ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=0&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1'"/></td>
...
<?php if($rowk>0){ $sql=mysql_query("select * from hrm_pms_kra where YearId=".$_SESSION['KraYId']." AND CompanyId=".$CompanyId." AND EmployeeID=".$EmployeeId." AND (KRAStatus='R' OR KRAStatus='A') order by KRAId ASC", $con); 
