...
{ 
  if($_REQUEST['LT']!='PL' AND $_REQUEST['v']==2)
  {$sqlUp=mysql_query("update hrm_employee_applyleave set LeaveAppStatus=".$_REQUEST['v'].", LeaveAppUpDate='".date("Y-m-d")."' where ApplyLeaveId=".$_REQUEST['lid'], $con);
...
function LeaveStatus(v){ var x = "MyTeamApplyleave.php?AppLeave=yes&value="+v;  window.location=x;}
...
{ var value = document.getElementById("ReqValue").value; 
  if(v==2) {var agree=confirm("Are you sure you want to approved this apply leave?"); 
            if (agree) { var x = "MyTeamApplyleave.php?AppLeave=yes&value="+value+"&lid="+LId+"&v="+v+"&LT="+LT;  window.location=x; }}
  else {var x = "MyTeamApplyleave.php?AppLeave=yes&value="+value+"&lid="+LId+"&v="+v+"&LT="+LT;  window.location=x;}			
}
</script>
</head>
<body class="body">
...
<?php if($rowApp>0) { ?><td align="center"><a href="MyTeamApplyleave.php?AppLeave=yes&value=0"><img id="Img_manager1" style="display:none;" src="images/manager1.png" border="0"/></a>
		     <img id="Img_manager" style="display:block;" src="images/manager.png" border="0"/></td><?php } ?>
<?php if($rowHod>0) { ?><td align="center"><a href="MyTeamAppLeaveToHOD.php?AppLeave=yes&value=0"><img id="Img_hod1" style="display:block;" src="images/RevHod1.png" border="0"/></a>
...
<?php if($_REQUEST['AppLeave'] AND $_REQUEST['AppLeave']!=""){ echo '<input type="hidden" id="ReqValue" value="'.$_REQUEST['value'].'"/>';
	  ...
	  if($_REQUEST['value']==4){ $sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date ASC", $con); }
	  if($_REQUEST['value']==0 OR $_REQUEST['value']==1 OR $_REQUEST['value']==2 OR $_REQUEST['value']==3){ $sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=".$_REQUEST['value']." AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date ASC", $con); }
