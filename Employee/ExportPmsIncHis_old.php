...
{ 
 $name='HOD_Wise';
 $sqlA=mysql_query("select Fname,Sname,Lname from hrm_employee where EmployeeId=".$_REQUEST['ee'], $con); 
 $resA=mysql_fetch_assoc($sqlA); 
 $name2=$resA['Fname'].'_'.$resA['Sname'].'_'.$resA['Lname'];

$xls_filename = 'Increment_PMS_Format_'.$PRD.'-'.$name2.'.xls';
...
$sql=mysql_query("select * from hrm_pms_appraisal_history his INNER JOIN hrm_employee e ON his.EmpCode=e.EmpCode INNER JOIN hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID where his.CompanyId=".$_REQUEST['c']." AND pms.HOD_EmployeeID=".$_REQUEST['ee']." AND e.CompanyId=".$_REQUEST['c']." AND pms.AssessmentYear=".$_REQUEST['YI']." order by his.EmpCode ASC, his.SalaryChange_Date ASC", $con); 
