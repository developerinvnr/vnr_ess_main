 ...
 <tr>
  <td style="width:100%;" valign="top">
   <table align="center" border="0" cellspacing="1">
	...
	<tr style="background-color:#CC9900;height:25px;">
	 <td class="t" style="width:50px;color:#FFF;"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ ?>Self Rating<?php }elseif($_REQUEST['lgc']=='Logic6'){ ?>FC<br>(in %)<?php }elseif($_REQUEST['lgc']=='Logic7'){ ?>VC<br>(in %)<?php } ?></td>
	 ...
	 <td class="t" style="width:50px;color:#FFF;"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ ?>Rep Rating<?php }elseif($_REQUEST['lgc']=='Logic6'){ ?>FC<br>(in %)<?php }elseif($_REQUEST['lgc']=='Logic7'){ ?>VC<br>(in %)<?php } ?></td>
	 <td class="t" style="width:200px;color:#FFF;">Remark</td>
	 <!--<td class="t" style="width:50px;color:#FFF;">Allow Logic</td>-->
	 <td class="t" style="width:50px;color:#FFF;">Score</td>
	</tr>
...
<?php $sql=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?>	 
	 ...
	 <td class="d" align="center"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ if($row>0){echo 100;} }else{ echo '0%'; } //floatval($res['Tgt'])?><?php //if($row>0){echo floatval($res['Tgt']);}//else{echo floatval($tgt);} ?>
	 ...
	 <input type="hidden" id="tgt<?php echo $i; ?>" name="tgt<?php echo $i; ?>" value="<?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ if($row>0){echo floatval($res['Tgt']);}else{echo floatval($tgt);} }else{ echo '0'; } ?>"/>
	 <?php //if($res['Tgt']!=''){echo floatval($res['Tgt']);}else{echo floatval($tgt);} ?>
	 ...
	 <td class="d" align="center"><?php if($res['AppCmnt']!='' || $res['AppAch']>0){ echo floatval($res['AppAch']); }else{echo 0;} ?></td>
	 ...
	 <td align="center">
<?php $mchk=date("m",strtotime($res['Ldate'])); if($mchk==1){$LDate=date("Y-02-28");}else{$LDate=$res['Ldate'];} ?>	 
	 <?php $Nxt21Day=date("Y-m-d",strtotime('+21 day', strtotime($LDate))); //date("2018-04-23")  //$res['Ldate'] ?>
	 <?php if($res['Tital']!='' AND $res['Tgt']!=''){ ?>
	  <span style="cursor:pointer;">
	  
	  <img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($res['Wgt']>0 AND $res['Tgt']>0 AND $res['Revlockk']==0 AND date("Y-m-d")<=$Nxt21Day) OR ($res['Wgt']>0 AND $res['Tgt']>0 AND $res['Revlockk']==0 AND $rpms['UnLckKRA']==1)){echo 'block';}else{echo 'none';} ?>" onClick="FunEdit(<?php echo $i; ?>)"/>
