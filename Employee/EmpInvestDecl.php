...
function ExpInvestDelFormate(P,EI,M)
{ var YI=document.getElementById("YearId").value; var CI=document.getElementById("ComId").value;
  window.open("EmpInvestDeclExp.php?action=Print&EI="+EI+"&YI="+YI+"&CI="+CI+"&P="+P+"&M="+M,"PrintForm","location=no,scrollbars=yes,resizable=no,menubar=no,width=50,height=70");
} 
...
<?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?>
					   ...
					   <option value="All">&nbsp;All</option>
                                           ...
                                           <td align="center" style="" class="All_50"><a href="#" onClick="ExpInvestDelFormate('<?php echo $resMs['Period']; ?>',<?php echo '0,'.$resMs['Month']; ?>)"><i><b>Export</b></i></a></td>
					 </tr>
                  </table>
				</td>
			 </tr>
