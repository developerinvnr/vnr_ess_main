...
<tr>
 ...
 <table border="0" width="100%">
  ...
  <tr><td align="center" class="head">Assessment Year&nbsp;<?php if($_REQUEST['CI']==1){ echo $FromCurr; }else{ $NF=$FromCurr-1; $NT=$FromCurr; echo $NF.'-'.$NT; } ?></td></tr>
  <tr>
  <tr>
...
<?php $syer=mysql_query("select AssessmentYear from hrm_employee_pms where EmpPmsId=".$_REQUEST['b'],$con);
      $ryer=mysql_fetch_assoc($syer); $PmsYId=$ryer['AssessmentYear'];


if($rowSubK==0){$num=5;}elseif($rowSubK==1){$num=4;}elseif($rowSubK==2){$num=3;}elseif($rowSubK==3){$num=2;}elseif($rowSubK==4){$num=1;}elseif($rowSubK==5){$num=0;} ?>
...
<tr bgcolor="#FFFFFF">
  ...
  <td align="center" class="b" style="background-color:#B0D8FF;"><b><?php if($_SESSION['eAppform']=='Y'){ echo $resAc['AppraiserFormAScore'];}elseif($_SESSION['eMidform']=='Y'){echo $resAc['Mid_AppraiserFormAScore']; } ?></b></td>
</tr>
<tr bgcolor="#FFFFFF">
...
<?php $sqlBY=mysql_query("select * from hrm_employee_pms_behavioralformb where EmpPmsId=".$_REQUEST['b']." order by BehavioralFormBId ASC", $con);  $SnoB=1; while($resBY=mysql_fetch_array($sqlBY)){ $sqlB2=mysql_query("select Skill,SkillComment,Weightage,Logic,Period,Target from hrm_pms_formb where FormBId=".$resBY['FormBId'], $con); 
