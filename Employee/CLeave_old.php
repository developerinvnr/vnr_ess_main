...
{ var agree=confirm("Are you sure you want to cancel OK this apply cancelation leave?"); 
  if (agree) { var x = "CLeave.php?AppLeave=yes&lid="+LId+"&v="+v+"&LeT="+LT+"&FD="+FD+"&TD="+TD+"&TotD="+TotD+"&EI="+EI+"&LS="+LS;  window.location=x; }
}

function CloseL(i)
{window.location="CLeave.php?ac=CloseLeave&LCid="+i;}
...
<?php if($rowApp>0) { ?><td align="center"><a href="CLeave.php"><img id="Img_manager1" style="display:none;" src="images/RepMgr1.png" border="0"/></a>
		     <img id="Img_manager" style="display:block;" src="images/RepMgr.png" border="0"/></td><?php } ?>
<?php if($rowHod>0 OR $rowRev>0) { ?><td align="center"><a href="CLeaveToHOD.php"><img id="Img_hod1" style="display:block;" src="images/RevHod1.png" border="0"/></a>
		     ...
		     <?php $CFromDate='2013-04-01'; $CToDate=date("Y").'-12-31';
		           $sqlLC=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveEmpCancelStatus='Y' AND LeaveCancelStatus='Y' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date ASC", $con); $rowLC=mysql_num_rows($sqlLC);?>
		<td valign="top" style="width:250px;" align="right"><?php if($rowLC>0){?>			
             <?php if($_REQUEST['ac']=='CloseLeave') { ?>
		     <a href="#" onClick="javascript:window.location='CLeave.php'"><font color="#464600" style='font-family:Times New Roman;' size="3"><b>Refresh</b></font></a>		             <?php } else { ?>
            ...
            <tr bgcolor="#7a6189" style="height:22px;">
		     ...
		     <td style="width:80px;" class="TableHead" align="center"><b>Cancel</b></td>
		   </tr>
<?php $CFromDate='2013-04-01'; $CToDate=date("Y").'-12-31'; 
	  $sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveEmpCancelStatus='Y' AND LeaveCancelStatus='N' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date ASC", $con); 
      ...
      $resE=mysql_fetch_assoc($sqlE); $EmpName=$resE['Fname'].' '.$resE['Sname'].' '.$resE['Lname']; ?>								  					
		   <tr>
		    ...
		    <td style="width:80px;" class="TableHead1" align="center">
