...
function FunRef(m,y,s,hq){ var sts=document.getElementById("status").value; window.location="f_profile.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&sts="+sts; }
function funstate(s,m,y)
{ var sts=document.getElementById("status").value; window.location="f_profile.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq=0&dis=0&sts="+sts; }
function funhq(hq,m,y,s)
{ var sts=document.getElementById("status").value; window.location="f_profile.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&sts="+sts; }

function funSTS(sts,m,y,s,hq)
{ window.location="f_profile.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&sts="+sts; }
...
{ var sts=document.getElementById("status").value; var win = window.open("f_faexports.php?hq="+hq+"&s="+s+"&m="+m+"&y="+y+"&sts="+sts,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=50,height=50"); }
</Script>
</head>
<body class="body">
...
<?php if($_REQUEST['s']>0){ $ss=mysql_query("select StateName from hrm_state where StateId=".$_REQUEST['s'],$con); $rs=mysql_fetch_assoc($ss); ?><option value='<?php echo $_REQUEST['s'];?>' selected><?php echo ucfirst(strtolower($rs['StateName']));?></option><?php }else{?><option value="0">Select</option><?php } if($_REQUEST['s']=='All'){ ?><option value="All" selected>All</option><?php } ?><?php $sqls=mysql_query("select StateId,StateName from hrm_state group by StateName order by StateName ASC",$con); while($ress=mysql_fetch_assoc($sqls)){ ?><option value="<?php echo $ress['StateId']; ?>"><?php echo ucfirst(strtolower($ress['StateName'])); ?></option><?php } ?><option value="All">All</option></select></td>
	 ...
	 <td class="tdf" style="width:10px;" align="center">:</td>
	 <td style="width:150px;"><select style="width:120px;" class="InputSel" id="status" name="status" onChange="funSTS(this.value,<?php echo $m.','.$y.','.$_REQUEST['s'].','.$_REQUEST['hq']; ?>)">
...
if($_REQUEST['s']=='All')
{ 
 if($_REQUEST['sts']==''){$result=mysql_query("select * from fa_details where (FaStatus='A' OR FaStatus='D') order by FaId ASC",$con);}else{$result=mysql_query("select * from fa_details where FaStatus='".$_REQUEST['sts']."' order by FaId ASC",$con);}
}
elseif($_REQUEST['hq']>0)
{ 
 if($_REQUEST['sts']==''){ $result=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND (FaStatus='A' OR FaStatus='D') order by FaId ASC",$con); }else{ $result=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND FaStatus='".$_REQUEST['sts']."' order by FaId ASC",$con); } 
}
elseif($_REQUEST['s']>0)
{ 
 if($_REQUEST['sts']==''){ $result=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND (FaStatus='A' OR FaStatus='D') order by FaId ASC",$con); }else{ $result=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND FaStatus='".$_REQUEST['sts']."' order by FaId ASC",$con); }
...
if($_REQUEST['s']=='All')
{ 
if($_REQUEST['sts']==''){ $sql=mysql_query("select * from fa_details where (FaStatus='A' OR FaStatus='D') order by FaId ASC LIMIT ".$from.",".$offset,$con); }else{ $sql=mysql_query("select * from fa_details where FaStatus='".$_REQUEST['sts']."' order by FaId ASC LIMIT ".$from.",".$offset,$con); }
 
}
elseif($_REQUEST['hq']>0)
{ 
if($_REQUEST['sts']==''){ $sql=mysql_query("select * from fa_details where (FaStatus='A' OR FaStatus='D') AND HqId=".$_REQUEST['hq']." order by FaId ASC LIMIT ".$from.",".$offset,$con); }else{ $sql=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND FaStatus='".$_REQUEST['sts']."' order by FaId ASC LIMIT ".$from.",".$offset,$con); }
}
elseif($_REQUEST['s']>0)
{ 
if($_REQUEST['sts']==''){ $sql=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where (f.FaStatus='A' OR f.FaStatus='D') AND hq.StateId=".$_REQUEST['s']." order by FaId ASC LIMIT ".$from.",".$offset,$con); }else{ $sql=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND FaStatus='".$_REQUEST['sts']."' order by FaId ASC LIMIT ".$from.",".$offset,$con); }
}
