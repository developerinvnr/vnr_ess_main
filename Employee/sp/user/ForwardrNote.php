...
<option value="<?php echo $_REQUEST['c']; ?>"><?php echo ucwords(strtolower($resC['CountryName'])); ?></option><?php }else{ ?><option value="0">Select</option><?php } ?><?php $SqlCountry=mysql_query("SELECT * FROM hrm_country order by CountryName ASC", $con); while($ResCountry=mysql_fetch_array($SqlCountry)) { ?>
...
<?php if($_REQUEST['s']>0){  $sqlS=mysql_query("SELECT StateName FROM hrm_state where StateId=".$_REQUEST['s'], $con); $resS=mysql_fetch_array($sqlS); ?><option value="<?php echo $_REQUEST['s']; ?>"><?php echo ucwords(strtolower($resS['StateName'])); ?></option><?php } elseif($_REQUEST['c']>0){ ?><option value="0" selected>Select</option><?php $sqlst = mysql_query("SELECT * FROM hrm_state where CountryId=".$_REQUEST['c']." order by StateName ASC", $con); while($resst = mysql_fetch_array($sqlst)){ ?><option value="<?php echo $resst['StateId']; ?>"><?php echo ucwords(strtolower(($resst['StateName']))); ?></option><?php } } else { ?><option value="0" selected>Select</option>
<?php $sqlSt=mysql_query("SELECT * FROM hrm_state order by StateName ASC", $con); while($resSt=mysql_fetch_array($sqlSt)){ ?><option value="<?php echo $resSt['StateId']; ?>"><?php echo ucwords(strtolower($resSt['StateName'])); ?></option><?php } } ?></select></span>
	   </td>
	   <td align="center"><span id="HqSpan"><select style="font-size:12px;width:120px;height:20px;background-color:#DDFFBB;" name="Hq" id="Hq" onChange="ClickHq(this.value)"><?php if($_REQUEST['hq']>0){ $sqlhq=mysql_query("SELECT HqName FROM hrm_headquater where HqId=".$_REQUEST['hq'], $con); $reshq=mysql_fetch_array($sqlhq); ?><option value="<?php echo $_REQUEST['hq']; ?>"><?php echo ucwords(strtolower($reshq['HqName'])); ?></option>
<?php } elseif($_REQUEST['s']>0){ ?><option value="0" selected>Select</option><?php $sqlhq = mysql_query("SELECT * FROM hrm_headquater where CompanyId=".$CompanyId." AND HQStatus='A' AND StateId=".$_REQUEST['s']." order by HqName ASC", $con);	 while($reshq = mysql_fetch_array($sqlhq)){ ?><option value="<?php echo $reshq['HqId']; ?>"><?php echo ucwords(strtolower($reshq['HqName'])); ?></option><?php } } else { ?><option value="0" selected>Select</option>
<?php $sqlHq = mysql_query("SELECT * FROM hrm_headquater where CompanyId=".$CompanyId." AND HQStatus='A' order by HqName ASC", $con);  while($resHq = mysql_fetch_array($sqlHq)){ ?><option value="<?php echo $resHq['HqId']; ?>"><?php echo ucwords(strtolower($resHq['HqName'])); ?></option><?php } } ?></select></span>
	  </td>
	  <td align="center"><span id="TabResult"><select style="font-size:12px;width:200px;height:20px;background-color:#DDFFBB;" name="DealerD" id="DealerD" onChange="ClickDealer(this.value)"><?php if($_REQUEST['dil']>0){ $sqldil=mysql_query("SELECT DealerName,DealerCity FROM hrm_sales_dealer where DealerId=".$_REQUEST['dil'], $con); $resdil=mysql_fetch_array($sqldil); ?><option value="<?php echo $_REQUEST['dil']; ?>"><?php echo ucwords(strtolower($resdil['DealerName'].'-'.$resdil['DealerCity'])); ?></option><?php } elseif($_REQUEST['hq']>0){ ?><option value="0" selected>Select Dealer</option><?php $sqldil = mysql_query("SELECT * FROM hrm_sales_dealer where HqId=".$_REQUEST['hq']." order by DealerName ASC", $con); while($resdil=mysql_fetch_array($sqldil)){ ?><option value="<?php echo $resdil['DealerId']; ?>"><?php echo $resdil['DealerName'].'-'.ucwords(strtolower($resdil['DealerCity'])); ?></option><?php } } else { ?><option value="0" selected>Select Dealer</option>
<?php $sqlDe=mysql_query("SELECT * FROM hrm_sales_dealer order by DealerName ASC", $con); while($resDe = mysql_fetch_array($sqlDe)){ ?><option value="<?php echo $resDe['DealerId']; ?>"><?php echo $resDe['DealerName'].'-'.ucwords(strtolower($res['DealerCity'])); ?></option><?php } } ?></select></span>
...
<input type="hidden" name="q" id="q" value="<?php echo $_REQUEST['q']; ?>"/>
	   ...
	   <td align="center"><select style="font-size:12px;width:150px;height:20px;background-color:#DDFFBB;" name="SubNi" id="SubNi" <?php if($_REQUEST['dil']>0){echo '';}else{echo 'disabled';} ?> onChange="ClickNSub(this.value)"><?php if($_REQUEST['nsi']>0){ $sNsi=mysql_query("SELECT * FROM hrm_sales_note_subject where NoteSubId=".$_REQUEST['nsi'],$con); $rNsi=mysql_fetch_array($sNsi); ?><option value="<?php echo $_REQUEST['nsi']; ?>"><?php echo ucwords(strtolower($rNsi['SubName'])); ?></option><?php }else{ ?><option value="0">Select</option><?php } ?><?php $sqlNsi=mysql_query("select * from hrm_sales_note_subject where Status='A' order by SubName ASC",$con); while($resNsi=mysql_fetch_array($sqlNsi)){ ?><option value="<?php echo $resNsi['NoteSubId']; ?>"><?php echo ucwords(strtolower($resNsi['SubName'])); ?></option><?php } ?></select>
	   </td>
	  </tr> 
	   </table>
      </td>
...
<?php if($_REQUEST['dil']>0){ $sql_statement=mysql_query("select sn.*,SubCode,sd.HqId,DealerName,DealerCity,HqName,StateName from hrm_sales_note sn inner join hrm_sales_note_subject sns on sn.NoteSubId=sns.NoteSubId inner join hrm_sales_dealer sd on sn.DealerId=sd.DealerId inner join hrm_headquater hq on sd.HqId=hq.HqId inner join hrm_state st on hq.StateId=st.StateId where sn.DealerId=".$_REQUEST['dil']." AND sn.YearId=".$_REQUEST['y']." order by NoteDate DESC",$con); } elseif($_REQUEST['hq']>0){ $sql_statement=mysql_query("select sn.*,SubCode,sd.HqId,DealerName,DealerCity,HqName,StateName from hrm_sales_note sn inner join hrm_sales_note_subject sns on sn.NoteSubId=sns.NoteSubId inner join hrm_sales_dealer sd on sn.DealerId=sd.DealerId inner join hrm_headquater hq on sd.HqId=hq.HqId inner join hrm_state st on hq.StateId=st.StateId where sd.HqId=".$_REQUEST['hq']." AND sn.YearId=".$_REQUEST['y']." order by hq.HqName ASC, sd.DealerName ASC, NoteDate DESC",$con); } elseif($_REQUEST['s']>0){ $sql_statement=mysql_query("select sn.*,SubCode,sd.HqId,DealerName,DealerCity,HqName,StateName from hrm_sales_note sn inner join hrm_sales_note_subject sns on sn.NoteSubId=sns.NoteSubId inner join hrm_sales_dealer sd on sn.DealerId=sd.DealerId inner join hrm_headquater hq on sd.HqId=hq.HqId inner join hrm_state st on hq.StateId=st.StateId where st.StateId=".$_REQUEST['s']." AND sn.YearId=".$_REQUEST['y']." order by hq.HqName ASC, sd.DealerName ASC, NoteDate DESC",$con); } elseif($_REQUEST['c']>0){ $sql_statement=mysql_query("select sn.*,SubCode,sd.HqId,DealerName,DealerCity,HqName,StateName from hrm_sales_note sn inner join hrm_sales_note_subject sns on sn.NoteSubId=sns.NoteSubId inner join hrm_sales_dealer sd on sn.DealerId=sd.DealerId inner join hrm_headquater hq on sd.HqId=hq.HqId inner join hrm_state st on hq.StateId=st.StateId inner join hrm_country c on st.CountryId=c.CountryId where c.CountryId=".$_REQUEST['c']." AND sn.YearId=".$_REQUEST['y']." order by st.StateName ASC, hq.HqName ASC, sd.DealerName ASC, NoteDate DESC",$con); } 
...
<?php if($_REQUEST['dil']>0){ $sqlNsi2=mysql_query("select sn.*,SubCode,sd.HqId,DealerName,DealerCity,HqName,StateName from hrm_sales_note sn inner join hrm_sales_note_subject sns on sn.NoteSubId=sns.NoteSubId inner join hrm_sales_dealer sd on sn.DealerId=sd.DealerId inner join hrm_headquater hq on sd.HqId=hq.HqId inner join hrm_state st on hq.StateId=st.StateId where sn.DealerId=".$_REQUEST['dil']." AND sn.YearId=".$_REQUEST['y']." order by NoteDate DESC LIMIT ".$from.",".$offset,$con);}
	  ...
	  else if($_REQUEST['c']>0){ $sqlNsi2=mysql_query("select sn.*,SubCode,sd.HqId,DealerName,DealerCity,HqName,StateName from hrm_sales_note sn inner join hrm_sales_note_subject sns on sn.NoteSubId=sns.NoteSubId inner join hrm_sales_dealer sd on sn.DealerId=sd.DealerId inner join hrm_headquater hq on sd.HqId=hq.HqId inner join hrm_state st on hq.StateId=st.StateId inner join hrm_country c on st.CountryId=c.CountryId where c.CountryId=".$_REQUEST['c']." AND sn.YearId=".$_REQUEST['y']." order by st.StateName ASC, hq.HqName ASC, sd.DealerName ASC, NoteDate DESC LIMIT ".$from.",".$offset,$con); } $sn=1;
  ...
  <tr>  
	...
	<td class="tdf3" align="center"><span style="cursor:pointer;color:#000099;" onClick="DetailNote(<?php echo $resNsi2['NoteId']; ?>)"><u><?php echo ucwords(strtolower($resNsi2['SubCode'])); ?></u></span></td>
...
<?php $no++; } ?> 
  ...
  <?php $y=$_REQUEST['y']; $ci=$_REQUEST['ci']; $c=$_REQUEST['c']; $s=$_REQUEST['s']; $hq=$_REQUEST['hq']; 
		...
		doPages($offset, 'ForwardrNote.php', '', $total_records,$y,$ci,$c,$s,$hq,$grp,$q,$ii,$nsi); ?>
  </td>
 </tr> 
</table>	       
<?php } ?>
