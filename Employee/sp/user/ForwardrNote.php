...
<?php if($_REQUEST['s']>0){  $sqlS=mysql_query("SELECT StateName FROM hrm_state where StateId=".$_REQUEST['s'], $con); $resS=mysql_fetch_array($sqlS); ?><option value="<?php echo $_REQUEST['s']; ?>"><?php echo strtoupper($resS['StateName']); ?></option><?php } elseif($_REQUEST['c']>0){ ?><option value="0" selected>SELECT </option><?php $sqlst = mysql_query("SELECT * FROM hrm_state where CountryId=".$_REQUEST['c']." order by StateName ASC", $con); while($resst = mysql_fetch_array($sqlst)){ ?><option value="<?php echo $resst['StateId']; ?>"><?php echo strtoupper($resst['StateName']); ?></option><?php } } else { ?><option value="0" selected>SELECT</option>
<?php $sqlSt=mysql_query("SELECT * FROM hrm_state order by StateName ASC", $con); while($resSt=mysql_fetch_array($sqlSt)){ ?><option value="<?php echo $resSt['StateId']; ?>"><?php echo strtoupper($resSt['StateName']); ?></option><?php } } ?></select></span>
	   </td>
...
<?php } elseif($_REQUEST['s']>0){ ?><option value="0" selected>SELECT</option><?php $sqlhq = mysql_query("SELECT * FROM hrm_headquater where CompanyId=".$CompanyId." AND HQStatus='A' AND StateId=".$_REQUEST['s']." order by HqName ASC", $con);	 while($reshq = mysql_fetch_array($sqlhq)){ ?><option value="<?php echo $reshq['HqId']; ?>"><?php echo strtoupper($reshq['HqName']); ?></option><?php } } else { ?><option value="0" selected>SELECT</option>
<?php $sqlHq = mysql_query("SELECT * FROM hrm_headquater where CompanyId=".$CompanyId." AND HQStatus='A' order by HqName ASC", $con);  while($resHq = mysql_fetch_array($sqlHq)){ ?><option value="<?php echo $resHq['HqId']; ?>"><?php echo strtoupper($resHq['HqName']); ?></option><?php } } ?></select></span>
	  </td>
...
<?php $sqlDe=mysql_query("SELECT * FROM hrm_sales_dealer order by DealerName ASC", $con); while($resDe = mysql_fetch_array($sqlDe)){ ?><option value="<?php echo $resDe['DealerId']; ?>"><?php echo $resDe['DealerName'].'-'.strtoupper($res['DealerCity']); ?></option><?php } } ?></select></span>
