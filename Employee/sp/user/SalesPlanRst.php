...
function ChangrYear(YearV)
{
  ...
  window.location="SalesPlanRst.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&y="+YearV+"&ci="+ci+"&c="+Coutry+"&s="+State+"&grp="+CropGrp+"&ii="+ii+"&bp="+bp; 
}
...
{ var YearV=document.getElementById("YearV").value; var CropGrp=document.getElementById("CropGrp").value; //var di=document.getElementById("DealerD").value;
  ...
  var ci=document.getElementById("ComId").value; var ii=document.getElementById("ItemV").value; //var Hq=document.getElementById("Hq").value;
  var bp=document.getElementById('BPrtyCheck').value;
  window.open("SalesPlanRstPrint.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&y="+YearV+"&ci="+ci+"&c="+Coutry+"&s="+State+"&grp="+CropGrp+"&ii="+ii+"&bp="+bp,"PrintForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=100,height=100");
}

function ExportStPlan(y,g,i,s)
{ var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;   
  var bp=document.getElementById('BPrtyCheck').value;
  window.open("ReportCSVSalesPlanRSt.php?action=PlanRExport&c="+ComId+"&y="+y+"&g="+g+"&i="+i+"&s="+s+"&YId="+y+"&bp="+bp,"PrintForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20"); } 
...
<?php if($_REQUEST['grp']==1){ $sqlItem=mysql_query("select * from hrm_sales_seedsitem where GroupId=1 order by ItemName ASC", $con);}
	  ...
	  <tr>
	    <td style="margin-top:0px;width:100px;" class="heading" align="center"><u>State-Wise</u></td>
...
<?php if($_REQUEST['c']>0){$sql = mysql_query("SELECT * FROM hrm_state where CountryId=".$_REQUEST['c']." order by StateName ASC", $con); }else{$sql = mysql_query("SELECT * FROM hrm_state order by StateName ASC", $con);}while($res = mysql_fetch_array($sql)){ ?>
        <option value="<?php echo $res['StateId']; ?>"><?php echo ucwords(strtolower($res['StateName'])); ?></option><?php } ?></select>
		 ...
		 <?php if($_REQUEST['s']>0){ ?>
		 <!--<a href="#" onClick="PrintSpRst()" style="color:#FFCE9D;"><b>PRINT</b></a>-->
		 &nbsp;&nbsp;<a href="#" onClick="ExportStPlan(<?php echo $_REQUEST['y'].', '.$_REQUEST['grp'].', '.$_REQUEST['ii'].', '.$_REQUEST['s']; ?>)" style="color:#F9F900; font-size:12px;"><b>Export Excel</b></a>
		 <?php } ?>
...
<?php /* 1 */ if($i==1){
$sqlP1d=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$BeforeYId2." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$BeforeYId2." AND ".$qin." AND hq.StateId=".$_REQUEST['s'], $con); $res1=mysql_fetch_assoc($sqlP1d); 
...
<?php } /* 2 */ if($i==2){ 
$sqlP2d=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$BeforeYId." AND ".$qin." AND hq.StateId=".$_REQUEST['s'], $con); $res2=mysql_fetch_assoc($sqlP2d);
...
<?php } /* 3 */ if($i==3){ 
$sqlP3d=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$_REQUEST['y']." AND ".$qin." AND hq.StateId=".$_REQUEST['s'], $con); 
...
<?php } /* 4 */  if($i==4){ 
$sqlP4d=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$AfterYId." AND ".$qin." AND hq.StateId=".$_REQUEST['s'], $con); $res4=mysql_fetch_assoc($sqlP4d); 
...
<?php } /* 5 */  $sqlYe=mysql_query("select FromDate,ToDate from hrm_sales_year where YearId=".$_REQUEST['y'], $con); $resYe=mysql_fetch_assoc($sqlYe);
	    ...
	    if($i==5){ 
$sqlP5d=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$_REQUEST['y']." AND ".$qin." AND hq.StateId=".$_REQUEST['s'], $con); $res5=mysql_fetch_assoc($sqlP5d);  
...
<?php } /* 2 */ if($i==2){ $sqlP2d=mysql_query("select SUM(M1_Ach) as tm1,SUM(M2_Ach) as tm2,SUM(M3_Ach) as tm3,SUM(M4_Ach) as tm4,SUM(M5_Ach) as tm5,SUM(M6_Ach) as tm6,SUM(M7_Ach) as tm7,SUM(M8_Ach) as tm8,SUM(M9_Ach) as tm9,SUM(M10_Ach) as tm10,SUM(M11_Ach) as tm11,SUM(M12_Ach) as tm12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$BeforeYId." AND sd.ProductId=".$res['ProductId']." AND hq.StateId=".$_REQUEST['s'], $con); $res2=mysql_fetch_assoc($sqlP2d); 
...
<?php } /* 3 */ if($i==3){ $sqlP3d=mysql_query("select SUM(M1) as tm1,SUM(M2) as tm2,SUM(M3) as tm3,SUM(M4) as tm4,SUM(M5) as tm5,SUM(M6) as tm6,SUM(M7) as tm7,SUM(M8) as tm8,SUM(M9) as tm9,SUM(M10) as tm10,SUM(M11) as tm11,SUM(M12) as tm12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$_REQUEST['y']." AND sd.ProductId=".$res['ProductId']." AND hq.StateId=".$_REQUEST['s'], $con); $res3=mysql_fetch_assoc($sqlP3d); 
...
<?php } /* 5 */  $sqlYe=mysql_query("select FromDate,ToDate from hrm_sales_year where YearId=".$_REQUEST['y'], $con); $resYe=mysql_fetch_assoc($sqlYe);
        $yeft=date("y",strtotime($resYe['FromDate'])).'-'.date("y",strtotime($resYe['ToDate'])); $yeT='<font color="#A60053">YTD</font>'; 
	    if($i==5){ $sqlP5d=mysql_query("select SUM(M1_Ach) as tM1,SUM(M2_Ach) as tM2,SUM(M3_Ach) as tM3,SUM(M4_Ach) as tM4,SUM(M5_Ach) as tM5,SUM(M6_Ach) as tM6,SUM(M7_Ach) as tM7,SUM(M8_Ach) as tM8,SUM(M9_Ach) as tM9,SUM(M10_Ach) as tM10,SUM(M11_Ach) as tM11,SUM(M12_Ach) as tM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON d.".$HqCon."=hq.HqId where ".$subQ." AND sd.YearId=".$_REQUEST['y']." AND sd.ProductId=".$res['ProductId']." AND hq.StateId=".$_REQUEST['s'], $con); $res5=mysql_fetch_assoc($sqlP5d);  
