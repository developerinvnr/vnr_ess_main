...
<?php if($_REQUEST['hq']>0 OR $_REQUEST['s']>0 OR $_REQUEST['s']=='All'){ ?>	
	<tr>
	 <td>
	  ...
	  <td style="width:60px;" class="htf">HQ</td>
	  ...
	  <td style="width:40px;" class="htf">Present</td>
...
<td class="tdf2"><input class="InputType" style="width:60px;text-align:center;" id="Doj<?php echo $sn; ?>" value="<?php echo date("d-m-y",strtotime($res['DOJ'])); ?>" readonly/></td>
<td class="tdf2"><input class="InputType" style="width:60px;" id="Hq<?php echo $sn; ?>" value="<?php echo ucfirst(strtolower($rhq['HqName'])); ?>" readonly/></td>
...
$sal=mysql_query("select * from fa_salary where FaId=".$res['FaId']." AND Month=".$_REQUEST['m']." AND Year=".$_REQUEST['y'],$con); $rowS=mysql_num_rows($sal); $resS=mysql_fetch_assoc($sal); ?>	 
	 ...
	 <?php $PaidDays=$resS['TotP']-$resS['TotA']; ?>
