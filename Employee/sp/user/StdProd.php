...
{  var ii=0; var ci=document.getElementById("ComId").value; 
   window.location="StdProd.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&ci="+ci+"&grp="+CropGrp+"&ii="+ii; 
}
function ChangeII(ii)
{ var CropGrp=document.getElementById("CropGrp").value; var ci=document.getElementById("ComId").value; 
  window.location="StdProd.php?ac=2441&ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&ci="+ci+"&grp="+CropGrp+"&ii="+ii; 
}
function product(id)
{ var CropGrp=document.getElementById("CropGrp").value; var ci=document.getElementById("ComId").value; var ItemV=document.getElementById("ItemV").value
  window.location="StdProd.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&ci="+ci+"&grp="+CropGrp+"&ii="+ItemV+"&id="+id 
}
...
function FunSaveStdP(pi,vli)
{ var v=document.getElementById("StdProdn_"+pi+"_"+vli).value; var url = 'StdProdAct.php'; var pars = 'act=AddStdPrductV&pi='+pi+'&vli='+vli+'&v='+v;
...
{ document.getElementById('SpanCodeV').innerHTML = originalRequest.responseText; var pi=document.getElementById("pi").value; var vli=document.getElementById("vli").value;
  document.getElementById("StdProdn_"+pi+"_"+vli).style.background='#B0FB4D'; }
    ...
    <td align="left" id="type" valign="top" style="display:block;width:100%">             
	 ...
	 <tr>
	  <td>
	    <table border="0">
		 <tr>
		   <td class="heading">&nbsp;Standard Production(Per Acr)</td> 
	  <td>&nbsp;</td>
...
<table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:12px;vertical-align:top;">	
...
<?php $sqlchk=mysql_query("select Max",$con); ?>	
	
	<td colspan="40" align="center" style="width:100px;background-color:#46A3FF;font-family:Helvetica;color:#FFFFFF;"><b>AREA WITH STANDARD PRODUCTION PER ACREAGE</b></td>
  </tr>
...
<?php $sqla=mysql_query("select hrm_sales_product_area.VillageLocId,VillageLocName,AreaCode from hrm_sales_product_area INNER JOIN hrm_sales_villageloc ON hrm_sales_product_area.VillageLocId=hrm_sales_villageloc.VillageLocId INNER JOIN hrm_sales_areavillage ON hrm_sales_villageloc.AreaId=hrm_sales_areavillage.AreaId where hrm_sales_product_area.ProductId=".$res['ProductId']." group by hrm_sales_product_area.VillageLocId order by hrm_sales_villageloc.VillageLocName ASC", $con); while($resa=mysql_fetch_assoc($sqla)){ ?>	 
	 <td><input style="font-family:Times New Roman;font-size:12px;height:22px;width:120px;" value="<?php echo strtoupper($resa['AreaCode'].'/ '.$resa['VillageLocName']);?>" readonly/></td>
<?php $sqlsd=mysql_query("select StdProdPerAcr from hrm_sales_product_stdprod where ProductId=".$res['ProductId']." AND VillageLocId=".$resa['VillageLocId'], $con); 
      $ressd=mysql_fetch_assoc($sqlsd); ?>	 
	 <td><input id="StdProdn_<?php echo $res['ProductId'].'_'.$resa['VillageLocId']; ?>" style="font-family:Times New Roman;font-size:12px;height:22px;width:70px;background-color:#FFB7FF;text-align:center;" placeholder="Stnd Prod" value="<?php echo $ressd['StdProdPerAcr']; ?>" onChange="FunSaveStdP(<?php echo $res['ProductId'].', '.$resa['VillageLocId']; ?>)" maxlength="10"/></td>
<?php } ?>	  
  </tr>	
<?php $Sn++; }?>  	  
</table>	
