...
{  var ii=0; var ci=document.getElementById("ComId").value; 
   window.location="RptProdAreaDtl.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&ci="+ci+"&grp="+CropGrp+"&ii="+ii; 
}
function ChangeII(ii)
{ var CropGrp=document.getElementById("CropGrp").value; var ci=document.getElementById("ComId").value; 
  window.location="RptProdAreaDtl.php?ac=2441&ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&ci="+ci+"&grp="+CropGrp+"&ii="+ii; 
}
function product(id)
{ var CropGrp=document.getElementById("CropGrp").value; var ci=document.getElementById("ComId").value; var ItemV=document.getElementById("ItemV").value
  window.location="RptProdAreaDtl.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&ci="+ci+"&grp="+CropGrp+"&ii="+ItemV+"&id="+id 
}

function ExportProductAreaDtl(g,i)
{ window.open("ReportCSVDtl.php?action=ProdAreaDtlExport&g="+g+"&i="+i,"PrintForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20"); } 
    ...
    <td align="left" id="type" valign="top" style="display:block;width:100%">             
	 ...
	 <tr>
	  <td>
	    <table border="0">
		 <tr>
		   <td class="heading">&nbsp;Product Area Details</td> 
	  <td>&nbsp;</td>
...
<?php if($_REQUEST['grp']==0){ $sqlItem=mysql_query("select * from hrm_sales_seedsitem order by ItemName ASC", $con);}
	  ...
	  while($resItem=mysql_fetch_assoc($sqlItem)){ ?><option value="<?php echo $resItem['ItemId']; ?>"><?php echo $resItem['ItemName']; ?></option><?php } ?>
         ...
         <a href="#" onClick="ExportProductAreaDtl(<?php echo $_REQUEST['grp'].', '.$_REQUEST['ii']; ?>)" style="color:#F9F900; font-size:12px;"><b>Export Excel</b></a>
		 <?php } ?>
		 </td>
		 </tr>
		</table>
...
<table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:14px;vertical-align:top;">	
   <tr style="background-color:#D5F1D1;color:#000000;height:22px;">  
	...
	<td align="center" style="width:200px;"><b>LOCATION</b></td>
	<td align="center" style="width:200px;"><b>AREA</b></td>
	<td align="center" style="width:200px;"><b>DISTRIC</b></td>
 ...
 <table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:14px;vertical-align:top; width:1250px;">
...
<?php $sAn=mysql_query("SELECT DISTINCT hrm_sales_villageloc.VillageLocName,hrm_sales_villageloc.AreaId FROM hrm_sales_product_area INNER JOIN hrm_sales_villageloc ON hrm_sales_product_area.VillageLocId=hrm_sales_villageloc.VillageLocId INNER JOIN hrm_sales_areavillage ON hrm_sales_villageloc.AreaId=hrm_sales_areavillage.AreaId WHERE ProductId=".$res['ProductId']." AND ActiveStatus=1 ORDER BY AreaName ASC", $con); $areaN=mysql_num_rows($sAn);  ?>	
   <tr bgcolor="#FFFFFF" style="height:22px;">
     <td style="width:200px;" rowspan="<?php echo $areaN; ?>">&nbsp;<?php echo $res['ItemName']; ?></td>	 
     <td style="width:200px;" rowspan="<?php echo $areaN; ?>">&nbsp;<?php echo $res['ProductName']; ?></td>
     <td style="width:50px;" align="center" rowspan="<?php echo $areaN; ?>">&nbsp;<?php echo substr_replace($res['TypeName'],'',2);?></td>
<?php  while($resArea=mysql_fetch_assoc($sAn)){ $areaIdNeed=mysql_fetch_array(mysql_query("select AreaId from hrm_sales_areavillage where AreaId='".$resArea['AreaId']."'", $con)); 
if($areaIdNeed['AreaId']>0){ $query=mysql_query("select AreaName,DistName,StateName from hrm_sales_areavillage C INNER JOIN hrm_sales_distric D ON C.DistId=D.DistId INNER JOIN hrm_state F ON D.StateId=F.StateId where AreaId=".$areaIdNeed['AreaId']." order by AreaName asc,DistName asc,StateName asc"); while($resDiS=mysql_fetch_array($query)){
?>               
     <td bgcolor="#FFFFFF" style="width:200px;">&nbsp;<?php echo $resArea['VillageLocName']; ?></td> 
	 <td bgcolor="#FFFFFF" style="width:200px;">&nbsp;<?php echo $resDiS['AreaName']; ?></td>
	 <td bgcolor="#FFFFFF" style="width:200px;">&nbsp;<?php echo $resDiS['DistName']; ?></td> 
	 <td bgcolor="#FFFFFF" style="width:150px;">&nbsp;<?php echo $resDiS['StateName']; ?></td>
