...
<?php if($_REQUEST['hq']=='All'){echo '<option value="All">All HQ</option>';} elseif($_REQUEST['hq']>0){ $sqlHq = mysql_query("SELECT * FROM hrm_headquater where HqId=".$_REQUEST['hq'], $con); $resHq=mysql_fetch_array($sqlHq); echo '<option value='.$_REQUEST['hq'].'>'.strtoupper($resHq['HqName']).'</option>'; } 
	  if($_REQUEST['s']>0){ $sqlHq2 = mysql_query("SELECT * FROM hrm_headquater where StateId=".$_REQUEST['s']." AND CompanyId=".$CompanyId." order by HqName ASC", $con); while($resHq2=mysql_fetch_array($sqlHq2)) { echo '<option value='.$resHq2['HqId'].'>'.strtoupper($resHq2['HqName']).'</option>'; } } elseif($_REQUEST['hq']==0){ echo '<option value="">SELECT HQ</option>'; $sql = mysql_query("SELECT * FROM hrm_headquater where CompanyId=".$CompanyId." AND HQStatus='A' order by HqName ASC", $con); while($res = mysql_fetch_array($sql)){ echo '<option value='.$res['HqId'].'>'.strtoupper($res['HqName']).'</option>'; } } echo '<option value="All">All HQ</option></select>'; ?>
	   </td>
...
<tr bgcolor="#FFFFFF">
 ...
 <td bgcolor="#B0FB4D"><select name="TLRepId" class="EditInput" style="width:150px;background-color:#B0FB4D;">
<?php $sE=mysql_query("select g.EmployeeID,EmpCode,Fname,Sname,Lname,DesigCode from hrm_employee_general g INNER JOIN hrm_employee e ON g.EmployeeID=e.EmployeeID INNER JOIN hrm_designation d ON g.DesigId=d.DesigId where e.EmployeeID=".$res['TLRepId'], $con); $rE=mysql_fetch_assoc($sE); ?><option value="<?php echo $res['TLRepId']; ?>" selected><?php echo strtoupper($rE['Fname'].' '.$rE['Sname'].' '.$rE['Lname']).' - '.$rE['DesigCode']; ?></option><?php $sE=mysql_query("select g.EmployeeID,EmpCode,Fname,Sname,Lname,DesigCode from hrm_employee_general g INNER JOIN hrm_employee e ON g.EmployeeID=e.EmployeeID INNER JOIN hrm_designation d ON g.DesigId=d.DesigId where e.EmpStatus='A' AND g.DepartmentId=6 order by Fname ASC", $con); while($rE=mysql_fetch_assoc($sE)){ ?><option value="<?php echo $rE['EmployeeID']; ?>" style="color:#002953;"><?php echo strtoupper($rE['Fname'].' '.$rE['Sname'].' '.$rE['Lname']).' - '.$rE['DesigCode']; ?></option><?php } ?></select></td>
