...
if($_REQUEST['action'] && $_REQUEST['action']=='cActRemovedFA')
{ $sqlU=mysql_query("update fa_details set RemoveSts=".$_REQUEST['v'].", RemovedDate='".date("Y-m-d")."', RemovedBy=".$EmployeeId." where FaId=".$_REQUEST['fid'],$con);
 if($sqlU){echo '<script>alert("Data update successfully.."); window.location="f_removel.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m='.$_REQUEST['m'].'&y='.$_REQUEST['y'].'&filter=zero&s='.$_REQUEST['s'].'&hq='.$_REQUEST['hq'].'&dis=0&md='.$_REQUEST['md'].'&rsts='.$_REQUEST['rsts'].'";</script>';}
}
...
function ActionRemFA(v,fid,m,y,hq,s,md,sn,ei,rsts)
{ 
 var agree=confirm("Are you sure?"); if(agree){ window.location="f_removel.php?action=cActRemovedFA&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&fid="+fid+"&rsts="+rsts+"&v="+v+"&ei="+ei; } else {return false;} 
}
  ...
  <td style="width:100%;" valign="top">  
   ...
   <tr>
	<td>
	 <table border="0">
		<tr>
	    <td class="htf2" align="left"><u>FA  Removal Request</u>&nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<?php if($_REQUEST['rsts']>0 OR $_REQUEST['rsts']=='All'){ ?>	
	<tr>
	 <td>
	  ...
	  <td colspan="3" class="htf" style="background-color:#FFAEFF;">Removel Request</td>
	  <td rowspan="2" style="width:80px;" class="htf">Status</td>
	  ...
	  <td style="width:80px;" class="htf">Last Working Date</td>
	  <td style="width:210px;" class="htf">Reason/Remark</td>
	  <td style="width:60px;"class="htf" >Reporting<br>Approval</td>
	 </tr>
	
<?php if($_REQUEST['rsts']=='All'){ $result=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts>=1",$con); }
if($_REQUEST['rsts']>0){ $result=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts=".$_REQUEST['rsts'],$con); }
...
if($_REQUEST['rsts']=='All'){ $sql=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts>=1 order by FaId ASC LIMIT ".$from.",".$offset,$con); }
if($_REQUEST['rsts']>0){ $sql=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts=".$_REQUEST['rsts']."  order by FaId ASC LIMIT ".$from.",".$offset,$con); }
...
<td class="tdf2"><input class="InputType" style="width:80px;" id="Hq<?php echo $sn; ?>" value="<?php echo ucfirst(strtolower($res['OtherHq'])); ?>" readonly/></td>
	 ...
	 <td class="tdf2"><input class="InputType" style="width:80px;text-align:center;" id="LWD<?php echo $sn; ?>" value="<?php if($res['LWD']!='0000-00-00' AND $res['LWD']!='1970-01-01'){echo date("d-m-Y",strtotime($res['LWD'])); } ?>"/></td> 
	 <td class="tdf2"><input class="InputType" style="width:210px;" id="Rmk<?php echo $sn; ?>" value="<?php echo $res['RemoveRmk']; ?>"/></td>
	 
	 <td class="tdf2" style="color:<?php if($res['Remove_Reporting_AppSts']==0){echo '#000000';}elseif($res['Remove_Reporting_AppSts']==1){echo '#FF860D';}elseif($res['Remove_Reporting_AppSts']==2){echo '#008000';}elseif($res['Remove_Reporting_AppSts']==3){echo '#FF0000';} ?>;"><?php if($res['Remove_Reporting_AppSts']==0){echo 'Draft';}elseif($res['Remove_Reporting_AppSts']==1){echo 'Pending';}elseif($res['Remove_Reporting_AppSts']==2){echo 'Approved';}elseif($res['Rep_AppSts']==3){echo 'Rejected';} ?></td>
	 
	 <td class="tdf2" align="center"><?php if($res['RemoveSts']==1){echo 'Draft';}elseif($res['RemoveSts']==2){echo '<font color="#FF8040">Pending</font>';}elseif($res['RemoveSts']==3){echo '<font color="#1C951F">Approved</font>';}elseif($res['RemoveSts']==4){echo '<font color="#FF0000">Reject</font>';} ?></td>	
	 
	 <td class="tdf2" align="center"><?php if($res['RemoveSts']!=3 AND $res['RemoveSts']!=4){ ?>
	 <select class="InputSel" style="width:80px;background-color:<?php if($resS['Status']==1){ echo '#FFFFB0';}else{echo '#FFFFFF';}?>;" onChange="ActionRemFA(this.value,<?php echo $res['FaId'].','.$_REQUEST['m'].','.$_REQUEST['y'].','.$_REQUEST['hq'].','.$_REQUEST['s'].','.$_REQUEST['md'].','.$sn.','.$EmployeeId; ?>,'<?php echo $_REQUEST['rsts']; ?>')"><option value="">Select</option><option value="2">Pending</option><option value="3">Approved</option><option value="4">Reject</option></select><?php } ?>
	 </td>
