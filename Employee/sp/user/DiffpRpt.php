...
<?php if($_REQUEST['e']>0 OR $_REQUEST['dil']>0 OR $_REQUEST['hq']>0 OR $_REQUEST['z']>0 OR $_REQUEST['s']>0 OR $_REQUEST['c']>0){ ?>
    <table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:12px;width:650px;vertical-align:top;">
	 <tr style="height:22px;background-color:#D5F1D1;">
      <td colspan="4" style="width:50px;font-size:14px;text-align:center;">&nbsp;<b style="color:#E17100;"><?php if($_REQUEST['e']>0){echo $ree['Fname'].' '.$ree['Sname'].' '.$ree['Lname'].' -- '.$ree['HqName'].'&nbsp;('.$ree['StateName'].')'; }elseif($_REQUEST['dil']>0){echo strtoupper($resdil['DealerName'].'-'.$resdil['DealerCity']);}elseif($_REQUEST['hq']>0){echo strtoupper($reshq['HqName']);}elseif($_REQUEST['z']>0 OR $_REQUEST['s']>0){ if($_REQUEST['z']>0){echo strtoupper($resZ['ZoneName']);}elseif($_REQUEST['s']>0){echo strtoupper($resS['StateName']);} }elseif($_REQUEST['c']>0){echo strtoupper($resC['CountryName']);} ?></b></td>
...
<?php if($Fill<0 OR $Fill>0 OR $Proj<0 OR $Proj>0){ ?>
    <tr style="background-color:#FFFFFF;">
      ...
      <td style="font-size:12px;">&nbsp;<?php echo strtoupper($res['ItemName']); ?></td>
      <td style="font-size:12px;">&nbsp;<?php echo strtoupper($res['ProductName']); ?></td>
   <td align="center" style="font-size:12px;"><?php echo strtoupper(substr_replace($res['TypeName'],'',2));?></td>
