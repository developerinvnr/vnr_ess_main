...
$sqlHq=mysql_query("select HqName from hrm_headquater where HqId=".$_REQUEST['hq']." AND CompanyId=".$_REQUEST['c'], $con); $resHq=mysql_fetch_assoc($sqlHq);
/*************************************************************************************************************/
include("CalYCsv.php"); 
...
$sqlP3d=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where ".$subQ." AND sd.YearId=".$_REQUEST['y']." AND ".$qin." AND d.".$HqCon."=".$_REQUEST['hq'], $con); 
...
header("Content-Disposition:attachment;filename=SalesPlan_Hq_".$resHq['HqName'].".csv");
echo $csv_output;
exit;
}
?>