...
if($_REQUEST['c']>0)
{ $sql = mysql_query("SELECT * FROM hrm_sales_dealer d where `Hq_vc`='' AND `Hq_fc`='' AND `Terr_vc`='' AND `Terr_fc`='' order by d.DealerName ASC", $con); }
elseif($_REQUEST['hq']>0)
{ $sql = mysql_query("SELECT * FROM hrm_sales_dealer d where (d.Hq_vc=".$_REQUEST['hq']." OR d.Hq_fc=".$_REQUEST['hq'].") order by d.DealerName ASC", $con); }
elseif($_REQUEST['s']>0)
{ $sql = mysql_query("SELECT * FROM hrm_sales_dealer d inner join hrm_headquater hq on (d.Hq_vc=hq.HqId OR d.Hq_fc=hq.HqId) where hq.StateId=".$_REQUEST['s']." order by d.DealerName ASC", $con); }
elseif($_REQUEST['t']>0)
{ $sql = mysql_query("SELECT * FROM hrm_sales_dealer d where (d.Terr_vc=".$_REQUEST['t']." OR d.Terr_fc=".$_REQUEST['t'].") order by d.DealerName ASC", $con); }
