...
function ClickLoc(l,v)
{
  ...
  window.location="SalesDistriName.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&d=ee&c="+c+"&s="+s+"&hq="+hq+"&z="+z+"&t="+t+"&bp="+bp; 
}
  ...
  <td align="left" valign="top">
   <table border="0" style="margin-top:0px;">
     <tr>
	  ...
	  <option value="0">SELECT COUNTRY</option><?php $sqlC=mysql_query("SELECT * FROM hrm_country order by CountryName ASC",$con); while($resC=mysql_fetch_array($sqlC)){ ?><option value="<?=$resC['CountryId'];?>" <?php if($_REQUEST['c']==$resC['CountryId']){echo 'selected';}?>><?=ucwords(strtoupper($resC['CountryName'])); ?></option><?php } ?></select></td>
	  
      <!--<td class="tds" style="width:55px;">Zone:&nbsp;</td>-->
	  <td style="width:150px;"><select class="seli" name="ZZ" id="ZZ" onChange="ClickLoc('Z',this.value)">
	  <option value="0">SELECT ZONE</option><?php $sqlZ = mysql_query("SELECT * FROM hrm_sales_zone order by ZoneName ASC",$con); while($resZ=mysql_fetch_array($sqlZ)) { ?><option value="<?=$resZ['ZoneId'];?>" <?php if($_REQUEST['z']==$resZ['ZoneId']){echo 'selected';}?>><?=ucwords(strtoupper($resZ['ZoneName']));?></option><?php } ?></select>
	   </td>
...
elseif($_REQUEST['t']>0)
{ $sql = mysql_query("SELECT * FROM hrm_sales_dealer d where ".$subQ." AND (d.Terr_vc=".$_REQUEST['t']." OR d.Terr_fc=".$_REQUEST['t'].") order by d.DealerName ASC", $con); }
