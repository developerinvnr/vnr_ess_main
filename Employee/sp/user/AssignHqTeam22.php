...
<?php if($_REQUEST['c']==0){ ?> <option value="0">SELECT</option><?php } else { $SqlC=mysql_query("SELECT CountryName FROM hrm_country where CountryId=".$_REQUEST['c'], $con); $ResC=mysql_fetch_assoc($SqlC);?><option value="<?php echo $_REQUEST['c']; ?>"><?php echo strtoupper($ResC['CountryName']); ?></option><?php } ?>
<?php $SqlCty=mysql_query("SELECT * FROM hrm_country order by CountryName ASC", $con); while($ResCty=mysql_fetch_array($SqlCty)) { ?><option value="<?php echo $ResCty['CountryId']; ?>"><?php echo strtoupper($ResCty['CountryName']); ?></option><?php } ?></select>
       </td>
	   <td></td>
...
<?php if($_REQUEST['s']==0){ ?> <option value="0">SELECT</option><?php } else { $SqlS=mysql_query("SELECT StateName FROM hrm_state where StateId=".$_REQUEST['s'], $con); $ResS=mysql_fetch_assoc($SqlS);?><option value="<?php echo $_REQUEST['s']; ?>"><?php echo strtoupper($ResS['StateName']); ?></option><?php } ?>

<?php if($_REQUEST['c']==0){ $sqlStd = mysql_query("SELECT * FROM hrm_state order by StateName ASC", $con); while($resStd = mysql_fetch_array($sqlStd)){ ?><option value="<?php echo $resStd['StateId']; ?>"><?php echo strtoupper($resStd['StateName']); ?></option><?php } } elseif($_REQUEST['c']>0){ $sqlStd = mysql_query("SELECT * FROM hrm_state where CountryId=".$_REQUEST['c']." order by StateName ASC", $con); while($resStd = mysql_fetch_array($sqlStd)){ ?><option value="<?php echo $resStd['StateId']; ?>"><?php echo strtoupper($resStd['StateName']); ?></option><?php } } ?></select>
		</td>
	</tr>
   </table>
  </td>
...
<?php if($_REQUEST['c']>0 AND $_REQUEST['s']==0){ ?><td align="center" style="color:#FFFFFF;font-size:14px;width:150px;font-family:Times New Roman;"><b>State</b></td><?php } ?>
  <td align="center" style="color:#FFFFFF;font-size:14px;width:150px;font-family:Times New Roman;"><b>Head Quarter Name</b></td>
  <td align="center" style="color:#FFFFFF;font-size:14px;width:400px;font-family:Times New Roman;"><b>Assign Territory</b></td>
...
elseif($_REQUEST['c']>0 AND $_REQUEST['s']==0){$sql = mysql_query("SELECT DISTINCT hrm_sales_dealer.HqId,HqName,StateName FROM hrm_sales_dealer INNER JOIN hrm_headquater ON hrm_sales_dealer.HqId=hrm_headquater.HqId INNER JOIN hrm_state ON hrm_headquater.StateId=hrm_state.StateId where hrm_state.CountryId=".$_REQUEST['c']." AND hrm_headquater.CompanyId=".$CompanyId." AND hrm_headquater.HQStatus='A' order by hrm_state.StateName ASC,hrm_headquater.HqName ASC", $con);}
 ...
 <tr bgcolor="#FFFFF" id="<?php echo 'TR'.$res['HqId']; ?>">
  ...
  <?php if($_REQUEST['c']>0 AND $_REQUEST['s']==0){ ?><td style="font-size:14px;font-family:Times New Roman;"><?php echo strtoupper($res['StateName']); ?><?php } ?>
  <td style="font-size:14px;font-family:Times New Roman;"><?php echo strtoupper($res['HqName']); ?>
  <input type="hidden" id="Hq_<?php echo $sn; ?>" value="<?php echo $res['HqId']; ?>" /></td>
<?php $sqlTemp=mysql_query("select hrm_sales_hq_temp.EmployeeID,EmpCode,Fname,Sname,Lname from hrm_sales_hq_temp INNER JOIN hrm_employee ON hrm_sales_hq_temp.EmployeeID=hrm_employee.EmployeeID where hrm_sales_hq_temp.HqId=".$res['HqId']." AND hrm_sales_hq_temp.HqTEmpStatus='A'", $con); 
...
$Ename=$resTemp['Fname'].' '.$resTemp['Sname'].' '.$resTemp['Lname'].' - '.$rDe['DesigCode']; ?>  
