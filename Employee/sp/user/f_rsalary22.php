...
function funDetail(id,y)
{ var win = window.open("f_salyear.php?id="+id+"&y="+y,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=800,height=500");}
	...
	<td>
	 <table border="0">
		...
		<td style="width:100px;"><select style="width:100px;" class="InputSel" id="month" name="month" onChange="funmonth(this.value,<?php echo $y.','.$_REQUEST['s'].','.$_REQUEST['hq'].','.$md; ?>)"><option value="<?php echo $m; ?>" selected="selected"><?php echo $Month; ?></option><?php for($i=1;$i<=12;$i++){ if($i!=$m){ ?>
		   ...
		   <option value="13">All Month</option></select></td>
...
<?php if($_REQUEST['hq']>0 OR $_REQUEST['s']>0 OR $_REQUEST['s']=='All'){ ?>	
    <tr>
	 <td>
	  ...
	  <td style="width:50px;" class="htf">Pre- sent</td>
...
<td class="tdf2">&nbsp;<span style="cursor:pointer;color:#0079F2;" onClick="funDetail(<?php echo $res['FaId'].','.$_REQUEST['y']; ?>)"><u><?php echo $res['FaName']; ?></u></span></td>
...
$sal=mysql_query("select * from fa_salary where FaId=".$res['FaId']." AND Month=".$m." AND Year=".$y,$con); 
$rowS=mysql_num_rows($sal); $resS=mysql_fetch_assoc($sal); ?>	
	 <td class="tdf2" align="center"><?php if($resS['TotP']>0){echo floatval($resS['TotP']);}elseif($rp['P']>0){echo $rp['P'];} ?></td><td class="tdf2" align="center"><?php if($resS['TotA']>0){echo floatval($resS['TotA']);}elseif($ra['A']>0){echo $ra['A'];} ?></td>
