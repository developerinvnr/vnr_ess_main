...
if(isset($_POST['claimupdate']))
{
 ...
 if($row>0)
 { $up=mysql_query("update fa_salary set ActualSal=".$Ps.", ActualExp=".$Pe.", AppdDate='".date("Y-m-d")."', AppdBy=".$_POST['UId'].", Status=".$_POST['Sts'].", Slip='".$slipName."', Remark='".$_POST['Rmk']."' where FaId=".$_POST['fffid']." AND Month=".$_POST['ffm']." AND Year=".$_POST['ffy'],$con); }
 elseif($row==0)
 { $up=mysql_query("insert into fa_salary(FaId, Month, Year, ActualSal, ActualExp, AppdDate, AppdBy, Status, Slip, Remark) values(".$_POST['fffid'].", ".$_POST['ffm'].", ".$_POST['ffy'].", ".$Ps.", ".$Pe.", '".date("Y-m-d")."', ".$_POST['UId'].", ".$_POST['Sts'].", '".$slipName."', '".$_POST['Rmk']."')",$con); }
 ...
 <td style="width:100%;">
  <table border="0" style="width:100%;"> 
   ...
   <tr>
	...
	<td class="tdf"><select class="InputSel" style="width:100px;" id="Sts" name="Sts"><?php if($_REQUEST['v']==0){?><option value="0">Draft</option><?php }elseif($_REQUEST['v']==1){?><option value="1">Pending</option><?php }elseif($_REQUEST['v']==2){?><option value="2">Approved/Paid</option><?php }elseif($_REQUEST['v']==3){?><option value="3">Reject</option><?php } ?><?php if($_REQUEST['v']!=1){ ?><option value="1">Pending</option><?php } ?>
<?php if($_REQUEST['v']!=2){ ?><option value="2">Approved/Paid</option><?php } ?>
