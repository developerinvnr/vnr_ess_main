...
function ExportProdTarget(y,g,i)
{
window.open("ProReqNewExport.php?action=ProTargExport&y="+y+"&g="+g+"&i="+i,"Export","menubar=no,scrollbars=no,resizable=no,directories=no,width=20,height=20");
}
...
<?php if($_REQUEST['grp']==0){ $sqlItem=mysql_query("select * from hrm_sales_seedsitem order by ItemName ASC", $con);}
	  ...
	  while($resItem=mysql_fetch_assoc($sqlItem)){ ?><option value="<?php echo $resItem['ItemId']; ?>"><?php echo $resItem['ItemName']; ?></option><?php } ?></select>	  
		 ...
		 <td>&nbsp;</td>
		 <td style="font-size:11px; height:18px;width:250px;color:#E6E6E6;" align="center"><a href="#" onClick="PrintProTgt(<?php echo $_REQUEST['y'].', '.$_REQUEST['grp'].', '.$_REQUEST['ii']; ?>)" style="color:#FFCE9D;"><b>PRINT</b></a>&nbsp;&nbsp;
		 <a href="#" onClick="ExportProdTarget(<?php echo $_REQUEST['y'].', '.$_REQUEST['grp'].', '.$_REQUEST['ii']; ?>)" style="color:#FFCE9D;"><b>EXPORT</b></a>
		 </td>
	  </tr>
	   </table>
      </td>
...
<?php if($_REQUEST['ii']>0){ $sql2T = mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." where YearId=".$_REQUEST['y']." AND ItemId=".$_REQUEST['ii'], $con); 
...
$resTgtT=mysql_fetch_assoc($sqlTgtT); } 
	  if($_REQUEST['grp']==3){$sql2T = mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." where YearId=".$_REQUEST['y'], $con); 
