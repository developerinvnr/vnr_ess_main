...
else    ///1111111111111///
{
  ...
  elseif($_REQUEST['s']>0 AND $_REQUEST['md']==4){ $result=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND ((FaStatus='A' AND (f.LWD='0000-00-00' OR f.LWD='1970-01-01' OR f.LWD>='".date($y."-".$m."-01")."')) OR (FaStatus='D' AND f.LWD>='".date($y."-".$m."-01")."')) AND f.DOJ<='".date($y."-".$m."-31")."' order by FaId ASC",$con);}
...
<?php } ?>	 
	 ...
	 <td class="tdf2" align="center"><span id="spanB<?php echo $sn; ?>" style="display:<?php if($resS['ClmImag']!='') {echo 'block';}else{echo 'none';}?>;"><a href="../Employee/ExpenseImg/<?php echo $resS['ClmImag']; ?>" target="_blank">View</a></span></td>
