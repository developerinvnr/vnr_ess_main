...
{ 
  ...
  { 
    ...
    $sel=mysql_query("select * from fa_details_up where FaId=".$_POST['FaId']." AND Tdate='".$_POST['ToDate']."'",$con); 
	$rel=mysql_num_rows($sel); 
	if($rel>0)
	{
	 $u=mysql_query("update fa_details_up set Mode=".$_POST['mode2e'].", Sal_DealerId=".$_POST['sdealer2r'].", Reporting=".$_POST['rep2p'].", Salary='".$_POST['sal2l']."', OtherHq='".$_POST['ohq2q']."', SalaryMode='".$_POST['smode2e']."', JobStatus='".$_POST['jobstatus2s']."', CrBy=".$EmployeeId."  where FaId=".$_POST['FaId']." AND Tdate='".$_POST['ToDate']."'",$con);
	}
	else
	{ 
	 $u=mysql_query("insert into fa_details_up(FaId, Mode, Sal_DealerId, Reporting, Salary, OtherHq, SalaryMode, JobStatus, Fdate, Tdate, CrBy) value(".$_POST['FaId'].", ".$_POST['mode2e'].", ".$_POST['sdealer2r'].", ".$_POST['rep2p'].", '".$_POST['sal2l']."', '".$_POST['ohq2q']."', '".$_POST['smode2e']."', '".$_POST['jobstatus2s']."', '".$fD."', '".$_POST['ToDate']."', ".$EmployeeId.")",$con); 
	}
  }
  
  
  if($_POST['Re_JoiningDate']!='' && $_POST['Re_JoiningDate']!='00-00-0000')
  { 
   if($_POST['Old_JoiningDate']!='0000-00-00' && $_POST['Old_JoiningDate']!='1970-01-01' && $_POST['Old_JoiningDate']!='')
   { $subQry=", Old2_JoiningDate='".date("Y-m-d",strtotime($_POST['doj']))."'"; }
   else{ $subQry=", Old_JoiningDate='".date("Y-m-d",strtotime($_POST['doj']))."'"; }
   
   $Re_JoiningDate=", Re_JoiningDate='".date("Y-m-d",strtotime($_POST['Re_JoiningDate']))."'";
   $DateJoining=$_POST['Re_JoiningDate'];
  }
  else
  { 
   $subQry=''; $Re_JoiningDate='';
   $DateJoining=$_POST['doj'];
  }
  
  
  $Up=mysql_query("update fa_details set Mode=".$_POST['mode'].", Crop='".$_POST['Crop']."', Sal_DealerId=".$_POST['sdeal'].", HqId=".$_POST['HeadQuarter'].", OtherHq='".$_POST['ohq']."', tl=".$_POST['tl'].", Reporting=".$_POST['repI'].", FaName='".$_POST['name']."', FaStatus='".$_POST['status']."', Salary='".$_POST['sal']."', Expences='".$_POST['expences']."', DOJ='".date("Y-m-d",strtotime($DateJoining))."', JobStatus='".$_POST['jobstatus']."', SalaryMode='".$_POST['smode']."', ContactNo=".$cont1.", Contact2No=".$cont2.", AadharNo='".$_POST['aadhar']."', DrivLic='".$_POST['dl']."', PanNo='".$_POST['panno']."', VoterId='".$_POST['voterid']."', DOB='".date("Y-m-d",strtotime($_POST['dob']))."', Gender='".$_POST['gender']."', Married='".$_POST['married']."', Qualif='".$_POST['quali']."', BloodGroup='".$_POST['bg']."', EmailId='".$_POST['email']."', BankName='".$_POST['bank']."', AccountNo='".$_POST['accno']."', Location='".$_POST['loc']."', Upld_pic='".$photoName."', Upld_dl='".$dlName."', Upld_pan='".$panName."', Upld_aadhar='".$aadharName."', Upld_resume='".$resumeName."', Upld_voterId='".$voterName."', Upld_other1='".$an1Name."', Upld_other2='".$an2Name."', Address1='".$_POST['add1']."', Address2='".$_POST['add2']."', LWD='".date("Y-m-d",strtotime($_POST['lwd']))."', CreatedBy='".$EmployeeId."', CreatedDate='".date("Y-m-d")."' ".$subQry." ".$Re_JoiningDate." where FaId=".$_POST['FaId'],$con);
...
function validate(reqform) 
{
 ...
 else if(document.getElementById("Crop").value==0){alert("Please select crops."); return false;}
...
<input type="hidden" id="Old_JoiningDate" name="Old_JoiningDate" value="<?php echo $res['Old_JoiningDate'];?>" />
<input type="hidden" id="Old2_JoiningDate" name="Old2_JoiningDate" value="<?php echo $res['Old2_JoiningDate'];?>" />
...
<?php if($res['Qualif']!='Post Graduate'){?><option value="Post Graduate">Post Graduate</option><?php } ?></select></td>
    ...
    <tr>
	  <td class="tdf">&nbsp;ReJoining Date</td>
	  <td class="tdf" align="center">:</td><td><input style="width:120px;text-align:center;" class="InputType" id="Re_JoiningDate" name="Re_JoiningDate" maxlength="50" value="<?php if($res['Re_JoiningDate']!='0000-00-00' AND $res['Re_JoiningDate']!='1970-01-01')echo date("d-m-Y",strtotime($res['Re_JoiningDate'])); ?>"/><button id="f_btn8" class="CalenderButton"></button><script type="text/javascript">var cal = Calendar.setup({ onSelect: function(cal) { cal.hide()}, showTime: true }); cal.manageFields("f_btn8", "Re_JoiningDate", "%d-%m-%Y");</script></td>
	...
	<tr>
	 <td class="tdf">&nbsp;ReJoining Remark</td>
	 <td class="tdf" align="center">:</td><td><input style="width:200px;;" class="InputType" id="Re_JoiningRmk" name="Re_JoiningRmk" maxlength="20" value="<?php echo $res['Re_JoiningRmk']; ?>"/></td>
	</tr>
	<tr>
