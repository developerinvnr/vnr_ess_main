...
function FunAct(v,sn,m,y,hq,s,md,fid,ei)
{ 
  if(v==1){var stst='Pending';}else if(v==2){var stst='Approved/Paid';}else if(v==3){var stst='Reject';}
  ...
  { var win = window.open("f_upload.php?action=uploadFileslip&ee=2421&der=true2&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&fid="+fid+"&sn="+sn+"&v="+v+"&u="+ei+"&c=false&trht=FTF%%F1","uForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=500,height=500"); 
    var timer = setInterval( function(){ if(win.closed){ clearInterval(timer); window.location="f_claim.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md; } }, 1000);
  }
  else{ return false; }
}
...
function funCExport(s,hq,m,y)
{ var win = window.open("f_clmexports.php?hq="+hq+"&s="+s+"&m="+m+"&y="+y,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=50,height=50"); }
  ...
  <td style="width:100%;" valign="top">  
   ...
   <tr>
	<td>
	 <table border="0">
		<tr>
	    <td class="htf2" align="left"><u>Expenses Claim</u>&nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<?php if($s=='All'){?><option value="All">All</option><?php } elseif($s>0){ $ss=mysql_query("select StateName from hrm_state where StateId=".$s,$con); $rs=mysql_fetch_assoc($ss); ?><option value='<?php echo $s;?>' selected><?php echo ucfirst(strtolower($rs['StateName']));?></option><?php } else { ?><option value="0" selected>select</option><?php } $sqls=mysql_query("select StateId,StateName from hrm_state group by StateId order by StateName ASC",$con); while($ress=mysql_fetch_assoc($sqls)){ ?><option value="<?php echo $ress['StateId']; ?>"><?php echo ucfirst(strtolower($ress['StateName'])); ?></option><?php } ?><option value="All">All</option></select></td>
	 ...
	 <td class="tdf2" style="width:100px;" align="center"><?php if($_REQUEST['hq']>0 OR $_REQUEST['s']>0 OR $_REQUEST['s']=='All'){ ?><span style="cursor:pointer;color:#FFFFFF;" onClick="funCExport('<?php echo $_REQUEST['s']; ?>',<?php echo $_REQUEST['hq'].','.$_REQUEST['m'].','.$_REQUEST['y']; ?>)"><u>Export</u></span><?php } ?></td>
...
<?php if($_REQUEST['hq']>0 OR $_REQUEST['s']>0 OR $_REQUEST['s']=='All'){ ?>	
	<tr>
	 <td>
      ...
      <td colspan="9" class="htf">&nbsp;General Details&nbsp;&nbsp;&nbsp;&nbsp;(P-present,&nbsp;A-absent)</td>
	  <td rowspan="2" class="htf" style="width:80px;background-color:#FFAEFF;">Claim Expen.</td>
	  ...
	  <td rowspan="2" class="htf" style="width:100px;background-color:#00CC66;">Remark</td>
	</tr>
	  <tr style="height:22px;">
	  ...
	  <td style="width:30px;" class="htf">P</td>
	  <td style="width:30px;" class="htf">A</td>
	  
	  
	  </tr>
	
<?php if($_REQUEST['hq']>0 AND $_REQUEST['md']==4){ $result=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND FaStatus='A' order by FaId ASC",$con);}
elseif($_REQUEST['hq']>0 AND $_REQUEST['md']!=4){ $result=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND FaStatus='A' AND Mode=".$_REQUEST['md']." order by FaId ASC",$con);}
elseif($_REQUEST['s']=='All' AND $_REQUEST['md']==4){ $result=mysql_query("select * from fa_details where FaStatus='A' order by FaId ASC",$con);}
elseif($_REQUEST['s']=='All' AND $_REQUEST['md']!=4){ $result=mysql_query("select * from fa_details where FaStatus='A' AND Mode=".$_REQUEST['md']." order by FaId ASC",$con);}
elseif($_REQUEST['s']>0 AND $_REQUEST['md']==4){ $result=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND FaStatus='A' order by FaId ASC",$con);}
elseif($_REQUEST['s']>0 AND $_REQUEST['md']!=4){ $result=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND FaStatus='A' AND Mode=".$_REQUEST['md']." order by FaId ASC",$con);}
...
if($_REQUEST['hq']>0 AND $_REQUEST['md']==4){ $sql=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND FaStatus='A' order by FaId ASC LIMIT ".$from.",".$offset,$con);}
elseif($_REQUEST['hq']>0 AND $_REQUEST['md']!=4){ $sql=mysql_query("select * from fa_details where HqId=".$_REQUEST['hq']." AND FaStatus='A' AND Mode=".$_REQUEST['md']." order by FaId ASC LIMIT ".$from.",".$offset,$con);}
elseif($_REQUEST['s']=='All' AND $_REQUEST['md']==4){ $sql=mysql_query("select * from fa_details where FaStatus='A' order by FaId ASC LIMIT ".$from.",".$offset,$con); }
elseif($_REQUEST['s']=='All' AND $_REQUEST['md']!=4){ $sql=mysql_query("select * from fa_details where FaStatus='A' AND Mode=".$_REQUEST['md']." order by FaId ASC LIMIT ".$from.",".$offset,$con);}
elseif($_REQUEST['s']>0 AND $_REQUEST['md']==4){ $sql=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND FaStatus='A' order by FaId ASC LIMIT ".$from.",".$offset,$con);}elseif($_REQUEST['s']>0 AND $_REQUEST['md']!=4){ $sql=mysql_query("select * from fa_details f inner join hrm_headquater hq on f.HqId=hq.HqId where hq.StateId=".$_REQUEST['s']." AND FaStatus='A' AND Mode=".$_REQUEST['md']." order by FaId ASC LIMIT ".$from.",".$offset,$con);}
...
$sal=mysql_query("select * from fa_salary where FaId=".$res['FaId']." AND Month=".$_REQUEST['m']." AND Year=".$_REQUEST['y'],$con); $rowS=mysql_num_rows($sal); $resS=mysql_fetch_assoc($sal); ?>	 
	 ...
	 <td class="tdf2" align="center"><?php if($resS['TotP']>0){echo floatval($resS['TotP']);}elseif($rP['P']>0){echo $rP['P'];} ?></td>
	 <td class="tdf2" align="center"><?php if($resS['TotA']>0){echo floatval($resS['TotA']);}elseif($ra['A']>0){echo $ra['A'];} ?></td>
...
if($res['Mode']==1 OR $res['Mode']==3){ ?>	 
	 <td class="tdf2"><input class="InputType" style="width:80px;text-align:right;background-color:#FFCCFF;" id="sal<?php echo $sn; ?>" value="<?php if($resS['ClaimSal']>0 OR $resS['ClaimExp']>0){echo floatval($resS['ClaimSal']+$resS['ClaimExp']);} ?>" maxlength="10" readonly/><input type="hidden" id="exp<?php echo $sn; ?>" value="0" maxlength="10" readonly/><input type="hidden" id="tot<?php echo $sn; ?>" value="<?php if($resS['ClaimSal']>0 OR $resS['ClaimExp']>0){ echo floatval($resS['ClaimSal']+$resS['ClaimExp']); } ?>" readonly/></td>
	
<?php } elseif($res['Mode']==2){ ?>	 
	 <td class="tdf2"><input class="InputType" style="width:80px;text-align:right;" id="sal<?php echo $sn; ?>" value="" readonly/><input type="hidden" id="exp<?php echo $sn; ?>" value="" readonly/><input type="hidden" id="tot<?php echo $sn; ?>" value="" readonly/></td>
<?php } ?>	 

	 <td class="tdf2" align="center"><select class="InputSel" style="width:80px;background-color:<?php if($resS['Status']==0 OR $resS['Status']==1){ echo '#FFFFB0';}else{echo '#FFFFFF';}?>;" id="Sts<?php echo $sn; ?>" name="Sts<?php echo $sn; ?>" onChange="FunAct(this.value,<?php echo $sn.','.$_REQUEST['m'].','.$_REQUEST['y'].','.$_REQUEST['hq'].','.$_REQUEST['s'].','.$_REQUEST['md'].','.$res['FaId'].','.$EmployeeId; ?>)"><?php if($resS['Status']==0){?><option value="0">Draft</option><?php }elseif($resS['Status']==1){?><option value="1">Pending</option><?php }elseif($resS['Status']==2){?><option value="2">Approved/Paid</option><?php }elseif($resS['Status']==3){?><option value="3">Reject</option><?php } ?><?php if($resS['Status']!=1){ ?><option value="1">Pending</option><?php } ?>
<?php if($resS['Status']!=2){ ?><option value="2">Approved/Paid</option><?php } ?>
<?php if($resS['Status']!=3){ ?><option value="3">Reject</option><?php } ?></select></td>
	 ...
	 <td class="tdf2" align="center"><input class="InputType" style="width:100px;text-align:left;" id="Rmk<?php echo $sn; ?>" name="Rmk<?php echo $sn; ?>" value="<?php echo $resS['Remark']; ?>" readonly/></td>
...
<?php $sn++; } ?>
   <tr>
    ...
    <td colspan="11" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
     <?php doPages($offset, 'f_claim.php', '', $total_records,$_REQUEST['hq'],$_REQUEST['s'],$_REQUEST['m'],$_REQUEST['y'],$_REQUEST['md']); ?>
    </td>
   </tr>
<?php } ?> 
