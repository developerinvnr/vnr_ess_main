...
if($_REQUEST['HqWise']>0)
{ 
 if($_REQUEST['SelHq']>0){ $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where sd.YearId=".$yset." AND sd.ProductId=".$res['ProductId']." AND d.HqId=".$_REQUEST['SelHq'],$con); }
 if($_REQUEST['SelHq']=='All'){ $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_hq_temp hqt ON d.HqId=hqt.HqId INNER JOIN hrm_employee_general g ON hqt.EmployeeID=g.EmployeeID where HqTEmpStatus='A' AND (hqt.EmployeeID=".$_REQUEST['EmpId']." OR g.RepEmployeeID=".$_REQUEST['EmpId'].") AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$yset, $con); }
}
