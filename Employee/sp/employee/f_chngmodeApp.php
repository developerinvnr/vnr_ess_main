...
function FunRef(m,y,s,hq,md,rsts){ window.location="f_chngmodeApp.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }

function funmd(md,m,y,hq,s,rsts)
{ window.location="f_chngmodeApp.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }
...
{ var agree=confirm("Are you sure?");
  if(agree)
  { 
    var url = 'f_claimAppAct.php'; var pars = 'Act=AppModeChange&v='+v+'&sn='+sn+'&fid='+fid+'&m='+m+'&y='+y+'&eid='+eid;	var myAjax = new Ajax.Request(
...
<?php if($_REQUEST['md']>0 OR $_REQUEST['rsts']>0){ ?>	
	<tr>
	 <td>
	  ...
	  <td style="width:200px;" class="htf">Request By</td>
...
$result=mysql_query("select * from fa_details where MdChng_Reporting=".$EmployeeId." AND ".$sqrs."  AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND MdChngSts>=1 order by FaId ASC",$con); 
...
$sql=mysql_query("select * from fa_details where MdChng_Reporting=".$EmployeeId." AND ".$sqrs."  AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND MdChngSts>=1 order by FaId ASC LIMIT ".$from.",".$offset,$con);
...
<?php $sn++; } ?>
   <tr>
    ...
    <td colspan="11" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
     <?php doPages($offset, 'f_chngmodeApp.php', '', $total_records,$_REQUEST['hq'],$_REQUEST['s'],$_REQUEST['m'],$_REQUEST['y'],$_REQUEST['md'],$_REQUEST['rsts']); ?>
    </td>
   </tr>
<?php } ?> 
