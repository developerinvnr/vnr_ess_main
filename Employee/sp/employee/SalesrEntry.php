...
{ window.location="SalesrEntry.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&act="+document.getElementById("act").value+"&hq="+document.getElementById("hq").value+"&y="+document.getElementById("y").value+"&q="+v+"&ii="+document.getElementById("ii").value+"&vc="+document.getElementById("vc").value+"&fc="+document.getElementById("fc").value+"&cc="+document.getElementById("cc").value+"&di="+document.getElementById("gi").value+"&gi="+gi+"&EHq="+document.getElementById("EHq").value+"&myh="+document.getElementById("myh").value+"&myt="+document.getElementById("myt").value+'&no='+document.getElementById("no").value;
}*/
...
<tr>
 <td style="width:100%;">	      
  ...
  <?php } //if($rowHqv>0 OR $rowHqf>0) ?>
	...
	$SpEGr=mysql_query("select GradeValue from hrm_grade where GradeId=".$_SESSION['GradeId']." AND CompanyId=".$CompanyId, $con); $resSpEGr=mysql_fetch_assoc($SpEGr);
	?>
	
    <td valign="bottom" style="width:140px;"><a href="#" onClick="ChangeTeam(<?php echo $_SESSION['HqId'].', '.$_REQUEST['y']; ?>,'<?php echo $resSpEGr['GradeValue']; ?>')"><img src="images/Myteam.png" border="0" style="height:30px;width:130px;"  /></a></td>
...
*/
 //if($rowChkP>0 OR $rowChkP>0 OR $resP2['sM1']!=0 OR $resP2['sM2']!=0 OR $resP2['sM3']!=0 OR $resP2['sM4']!=0 OR $resP2['sM5']!=0 OR $resP2['sM6']!=0 OR $resP2['sM7']!=0 OR $resP2['sM8']!=0 OR $resP2['sM9']!=0 OR $resP2['sM10']!=0 OR $resP2['sM11']!=0 OR $resP2['sM12']!=0 OR $a1!=0 OR $a2!=0 OR $a3!=0 OR $a4!=0 OR $a5!=0 OR $a6!=0 OR $a7!=0 OR $a8!=0 OR $a9!=0 OR $a10!=0 OR $a11!=0 OR $a12!=0){ ?>	   
...
$sqlP2t=mysql_query("select SUM(M1) as tsM1,SUM(M2) as tsM2,SUM(M3) as tsM3,SUM(M4) as tsM4,SUM(M5) as tsM5,SUM(M6) as tsM6,SUM(M7) as tsM7,SUM(M8) as tsM8,SUM(M9) as tsM9,SUM(M10) as tsM10,SUM(M11) as tsM11,SUM(M12) as tsM12, SUM(M1_Ach) as tssM1,SUM(M2_Ach) as tssM2,SUM(M3_Ach) as tssM3,SUM(M4_Ach) as tssM4,SUM(M5_Ach) as tssM5,SUM(M6_Ach) as tssM6,SUM(M7_Ach) as tssM7,SUM(M8_Ach) as tssM8,SUM(M9_Ach) as tssM9,SUM(M10_Ach) as tssM10,SUM(M11_Ach) as tssM11,SUM(M12_Ach) as tssM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where sp.ProductSts='A' AND d.".$shq."=".$_REQUEST['hq']." AND d.DealerSts='A' AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$_REQUEST['y'], $con); $rP2t=mysql_fetch_assoc($sqlP2t);

$sqlP3t=mysql_query("select SUM(M1_Proj) as tsM1,SUM(M2_Proj) as tsM2,SUM(M3_Proj) as tsM3,SUM(M4_Proj) as tsM4,SUM(M5_Proj) as tsM5,SUM(M6_Proj) as tsM6,SUM(M7_Proj) as tsM7,SUM(M8_Proj) as tsM8,SUM(M9_Proj) as tsM9,SUM(M10_Proj) as tsM10,SUM(M11_Proj) as tsM11,SUM(M12_Proj) as tsM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where sp.ProductSts='A' AND d.".$shq."=".$_REQUEST['hq']." AND d.DealerSts='A' AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$AfterYId, $con); $rP3t=mysql_fetch_assoc($sqlP3t);
