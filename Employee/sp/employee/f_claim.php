...
{ 
 ...
 if($rowchk==0)
 {
     ...
     $sqlRep=mysql_query("select RepEmployeeID from hrm_employee_general where EmployeeID=".$EmployeeId,$con); 
	 $resRep=mysql_fetch_assoc($sqlRep);
     
     $sqlIns=mysql_query("insert into fa_salary(FaId, Month, Year, RepId, ClaimSal, ClaimExp, TotP, TotA, ClaimDate, ClaimBy, Status, LeaveRmk) values(".$_REQUEST['fid'].", ".$_REQUEST['m'].", ".$_REQUEST['y'].", ".$resRep['RepEmployeeID'].", ".$_REQUEST['s2'].", ".$_REQUEST['e2'].", '".$_REQUEST['tP']."', '".$_REQUEST['tA']."', '".date("Y-m-d")."', ".$EmployeeId.", 0, '".$_REQUEST['LeaveRmk']."')",$con);
 }
}
?>
<html>
   ...
   <tr>
	<td>
	 <table border="0">
	    ...
	    <td style="width:120px;"><select style="width:120px;" class="InputSel" id="mode" name="mode" onChange="funmd(this.value,<?php echo $m.','.$y.','.$_REQUEST['hq'].','.$_REQUEST['s']; ?>)" <?php if($m>0){echo '';}else{echo 'disabled';} ?>>
...
{$ExtD=31;}elseif(date("m")==4 OR date("m")==6 OR date("m")==9 OR date("m")==11){$ExtD=30;}
elseif(date("m")==2)
{
...
if($_REQUEST['m']!=1){ $PrvM=date('m',strtotime("-1 months",strtotime(date("Y-m-d")))); }
else{ $PrvM=0; }
	...
	<tr>
	 <td>
	  ...
	  <tr style="height:22px;">
	   ...
	   <td colspan="2" style="width:100px;" class="htf">Customer Letter</td>
...
<?php /*<td rowspan="2" style="width:40px;" class="htf">Print Calim</td>*/ ?>
	  ...
	  <tr style="height:22px;">
	   ...
	   <td style="width:50px;" class="htf">Upload</td>
	   <td style="width:50px;" class="htf">View</td>
	  </tr>
	
<?php 
if($_REQUEST['md']==4){ $qry="1=1"; }
else{ $qry="Mode=".$_REQUEST['md']; }

$result=mysql_query("select * from fa_notaccemp nf inner join hrm_sales_reporting_level rl on (nf.RepEmpId=rl.EmployeeID OR nf.EmpId=rl.EmployeeID OR nf.EmpId=rl.R1 OR nf.EmpId=rl.R2 OR nf.EmpId=rl.R3 OR nf.EmpId=rl.R4) inner join fa_details fd on rl.EmployeeID=fd.Reporting where (nf.RepEmpId=".$EmployeeId." OR rl.EmployeeID=".$EmployeeId.") AND ((FaStatus='A' AND (LWD='0000-00-00' OR LWD='1970-01-01' OR LWD>='".date($y."-".$m."-01")."')) OR (FaStatus='D' AND LWD>='".date($y."-".$m."-01")."')) AND DOJ<='".date($y."-".$m."-31")."' AND ".$qry." group by FaId order by FaId ASC",$con);
...
$sql=mysql_query("select * from fa_notaccemp nf inner join hrm_sales_reporting_level rl on (nf.RepEmpId=rl.EmployeeID OR nf.EmpId=rl.EmployeeID OR nf.EmpId=rl.R1 OR nf.EmpId=rl.R2 OR nf.EmpId=rl.R3 OR nf.EmpId=rl.R4) inner join fa_details fd on rl.EmployeeID=fd.Reporting where (nf.RepEmpId=".$EmployeeId." OR rl.EmployeeID=".$EmployeeId.") AND ((FaStatus='A' AND (LWD='0000-00-00' OR LWD='1970-01-01' OR LWD>='".date($y."-".$m."-01")."')) OR (FaStatus='D' AND LWD>='".date($y."-".$m."-01")."')) AND DOJ<='".date($y."-".$m."-31")."' AND ".$qry." group by FaId order by FaId ASC LIMIT ".$from.",".$offset,$con);
...
<?php if($res['Mode']==1 OR $res['Mode']==3){ ?>	 
	 ...
	 <?php /*?><td class="tdf2" align="center"><?php if($rowS>0){echo '<font color="#366C00">Submitted</font>';} else{echo '<span style="cursor:pointer;color:#0B51F4;" onClick="funClaim('.$res['FaId'].','.$_REQUEST['m'].','.$_REQUEST['y'].','.$_REQUEST['hq'].','.$_REQUEST['s'].','.$_REQUEST['md'].','.$sn.')"><u>claim</u></span>';} ?></td><?php */?>
	  ...
	  <td class="tdf2" align="center"><span id="spanA<?php echo $sn; ?>" style="cursor:pointer; " onClick="AttachImg(<?php echo $res['FaId'].','.$_REQUEST['m'].','.$_REQUEST['y'].','.$_REQUEST['hq'].','.$_REQUEST['s'].','.$_REQUEST['md'].','.$sn?>)" ><u>Click</u></span></td>
