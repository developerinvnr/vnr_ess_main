...
function ChngeEmpPerMi(v,e)
{
  ...
  var myh=document.getElementById("myh").value; var myt=1;
  window.location="SaleshEntryPerMi.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&act="+act+"&hq="+hq+"&y="+y+"&q="+q+"&ii="+ii+"&vc="+vc+"&fc="+fc+"&cc="+cc+"&di="+di+"&gi="+gi+'&EHq='+EHq+'&myh='+myh+'&myt='+myt+'&action=ChangePerMission&eei='+e+'&ynv='+v; 
}
...
<?php $SqlRep=mysql_query("SELECT hrm_employee.EmployeeID,DepartmentId,DesigId,GradeId,EmpCode,Fname,Sname,Lname,Resig_Permission,Married,Gender,DR,EmailId_Vnr FROM hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID WHERE hrm_employee_general.RepEmployeeID=".$EmployeeId." AND hrm_employee_general.DepartmentId=6 AND hrm_employee.EmpStatus='A' AND hrm_employee.CompanyId=".$CompanyId." order by EmpCode ASC", $con); $sn=1; while($ResRep=mysql_fetch_array($SqlRep)) { 
...
<?php $sqlMi=mysql_query("select EditPerMi from hrm_sales_reporting_level where EmployeeID=".$ResRep['EmployeeID'], $con); $resMi=mysql_fetch_assoc($sqlMi); ?>			 
			 <select style="width:60px;font-family:Georgia;height:22px;background-color:<?php if($resMi['EditPerMi']=='Y'){ echo '#BAFC61'; }else {echo '#FDEE99';} ?>;" onChange="ChngeEmpPerMi(this.value, <?php echo $ResRep['EmployeeID']; ?>)">
