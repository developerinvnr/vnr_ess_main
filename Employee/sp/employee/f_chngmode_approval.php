...
function FunRef(m,y,s,hq,md,rsts){ window.location="f_chngmode_approval.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }

function funmd(md,m,y,hq,s,rsts)
{ window.location="f_chngmode_approval.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }
...
function FucApp(v,sn,fid,m,y,eid,rep,rev,hod,by,lvl)
{ var agree=confirm("Are you sure?");
  if(agree)
  { 
    var url = 'f_claimAppAct.php'; var pars = 'Act=AppModeChange&v='+v+'&sn='+sn+'&fid='+fid+'&m='+m+'&y='+y+'&eid='+eid+"&rep="+rep+"&rev="+rev+"&hod="+hod+"&by="+by+"&lvl="+lvl;	var myAjax = new Ajax.Request(
  ...
  <td style="width:100%;" valign="top">  
   ...
   <tr>
	<td>
	 <table border="0">
		<tr>
	    <td class="htf2" align="left"><u>FA Mode Change Request</u>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	 ...
	 <?php /*?><option value="1" <?php if($_REQUEST['rsts']==1){echo 'selected'; }?>>Pending</option><?php */?>
	 ...
	 <?php /*?> <option value="4" <?php if($_REQUEST['rsts']==4){echo 'selected'; }?>>All</option><?php */?>
	 </select></td>
...
<?php if($_REQUEST['md']>0 OR $_REQUEST['rsts']>0){ ?>	
	<tr>
	 <td>
      ...
      <td colspan="3" class="htf">Mode Change Request</td>
	  ...
	  <td style="width:80px;" class="htf">Req. Date</td>
	  <td style="width:150px;" class="htf">Req. By</td>
...
//if($_REQUEST['rsts']==4){ $sqrs="1=1"; }else{ $sqrs="MdChng_Reporting_AppSts=".$_REQUEST['rsts']; }


$result=mysql_query("select * from fa_details where ((MdChng_Reporting=".$EmployeeId." AND MdChng_Reporting_AppSts=".$_REQUEST['rsts'].") OR (MdChng_Rev=".$EmployeeId." AND MdChng_Rev_AppSts=".$_REQUEST['rsts'].") OR (MdChng_Hod=".$EmployeeId." AND MdChng_Hod_AppSts=".$_REQUEST['rsts'].")) AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND MdChngSts>=1 order by FaId ASC",$con); 
...
$sql=mysql_query("select * from fa_details where ((MdChng_Reporting=".$EmployeeId." AND MdChng_Reporting_AppSts=".$_REQUEST['rsts'].") OR (MdChng_Rev=".$EmployeeId." AND MdChng_Rev_AppSts=".$_REQUEST['rsts'].") OR (MdChng_Hod=".$EmployeeId." AND MdChng_Hod_AppSts=".$_REQUEST['rsts'].")) AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND MdChngSts>=1 order by FaId ASC LIMIT ".$from.",".$offset,$con);
...
<tr style="height:22px;" bgcolor="#FFFFFF" id="TR<?php echo $sn; ?>">
 ...
 <td class="tdf2"><input class="InputType" style="width:150px;" id="name<?php echo $sn; ?>" value="<?php echo ucwords(strtolower($res['FaName'])); ?>" readonly/></td>
...
<td class="tdf2"><input class="InputType" style="width:100px;" id="Hq<?php echo $sn; ?>" value="<?php echo ucfirst(strtolower($rhq['HqName'])); ?>" readonly/></td>
	 ...
	 <td class="tdf2"><input class="InputType" style="width:99%;" id="Rmk<?php echo $sn; ?>" value="<?php echo $res['MdChngRmk']; ?>"/></td>
	 <td class="tdf2" align="center"><?php if($res['MdChngSts']>0){ echo date("d-m-Y",strtotime($res['MdChngReqDate'])); } ?></td>
	 <td class="tdf2"><input class="InputType" style="width:99%;" value="<?php $e=mysql_query("select Fname,Sname,Lname from hrm_employee where EmployeeID=".$res['MdChngReqBy'],$con); $re=mysql_fetch_assoc($e); echo ucfirst(strtolower($re['Fname'].' '.$re['Sname'].' '.$re['Lname'])); ?>" readonly/></td>	
	 
	 
	 <td class="tdf2" align="center">
	  <?php if($res['MdChng_Reporting']==$EmployeeId)
	        {
			 if($res['MdChng_Reporting_AppSts']==0){ echo '<font color="#000000">Draft</font>'; }
			 elseif($res['MdChng_Reporting_AppSts']==1){ echo '<font color="#E800E8">Pending</font>'; }
			 elseif($res['MdChng_Reporting_AppSts']==2){ echo '<font color="#FF6B24">Approved</font>'; }
			 elseif($res['MdChng_Reporting_AppSts']==3){ echo '<font color="#000000">Rejected</font>'; }
			}
			elseif($res['MdChng_Rev']==$EmployeeId)
	        {
			 if($res['MdChng_Rev_AppSts']==0){ echo '<font color="#000000">Draft</font>'; }
			 elseif($res['MdChng_Rev_AppSts']==1){ echo '<font color="#E800E8">Pending</font>'; }
			 elseif($res['MdChng_Rev_AppSts']==2){ echo '<font color="#FF6B24">Approved</font>'; }
			 elseif($res['MdChng_Rev_AppSts']==3){ echo '<font color="#000000">Rejected</font>'; }
			}
			elseif($res['MdChng_Hod']==$EmployeeId)
	        {
			 if($res['MdChng_Hod_AppSts']==0){ echo '<font color="#000000">Draft</font>'; }
			 elseif($res['MdChng_Hod_AppSts']==1){ echo '<font color="#E800E8">Pending</font>'; }
			 elseif($res['MdChng_Hod_AppSts']==2){ echo '<font color="#FF6B24">Approved</font>'; }
			 elseif($res['MdChng_Hod_AppSts']==3){ echo '<font color="#000000">Rejected</font>'; }
			}
	  ?>
	  </td>
	 ...
	 <?php /*
	 <td class="tdf2" align="center">
	  <select class="InputType" id="SelSts<?=$sn?>" style="width:99%;" <?php if($res['MdChng_Reporting_AppSts']==2 OR $res['MdChng_Reporting_AppSts']==3){echo 'disabled';} ?> onChange="FucApp(this.value,<?php echo $sn.','.$res['FaId'].', '.$_REQUEST['m'].','.$_REQUEST['y'].','.$EmployeeId;?>)" <?php if($res['MdChng_Reporting_AppSts']==2 OR $res['MdChng_Reporting_AppSts']==3){echo 'disabled';}?>>
	   <option value="0" <?php if($res['MdChng_Reporting_AppSts']==0){echo 'selected';}?>>Darft</option>
	   <option value="1" <?php if($res['MdChng_Reporting_AppSts']==1){echo 'selected';}?>>Pending</option>
	   <option value="2" <?php if($res['MdChng_Reporting_AppSts']==2){echo 'selected';}?>>Approve</option>
	   <option value="3" <?php if($res['MdChng_Reporting_AppSts']==3){echo 'selected';}?>>Reject</option>
	  </select>
	 </td> */?>
	 
	 <td class="tdf2" align="center"><?php if(($res['MdChng_Reporting']==$EmployeeId && ($res['MdChng_Reporting_AppSts']==0 OR $res['MdChng_Reporting_AppSts']==1)) OR ($res['MdChng_Rev']==$EmployeeId && ($res['MdChng_Rev_AppSts']==0 OR $res['MdChng_Rev_AppSts']==1)) OR ($res['MdChng_Hod']==$EmployeeId && ($res['MdChng_Hod_AppSts']==0 OR $res['MdChng_Hod_AppSts']==1))){ ?><select style="width:80px;" class="InputSel" onChange="FucApp(this.value,<?php echo $sn.','.$res['FaId'].', '.$_REQUEST['m'].','.$_REQUEST['y'].','.$EmployeeId.','.$res['MdChng_Reporting'].','.$res['MdChng_Rev'].','.$res['MdChng_Hod'].','.$res['MdChngReqBy'].','.$res['MdChng_lvl']; ?>)"><option value="" selected>Select</option><option value="2">Approve</option><option value="3">Reject</option></select><?php } ?></td>
...
<?php $sn++; } ?>
   <tr>
    ...
    <td colspan="11" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
     <?php doPages($offset, 'f_chngmode_approval.php', '', $total_records,$_REQUEST['hq'],$_REQUEST['s'],$_REQUEST['m'],$_REQUEST['y'],$_REQUEST['md'],$_REQUEST['rsts']); ?>
    </td>
   </tr>
<?php } ?> 
