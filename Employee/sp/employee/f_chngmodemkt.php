...
{ 
 $sqlU=mysql_query("update fa_details set MdChng3Sts=".$_REQUEST['v'].", MdChng3StsDate='".date("Y-m-d")."' where FaId=".$_REQUEST['fid'],$con);
 
 if($sqlU){echo '<script>alert("Data update successfully.."); window.location="f_chngmodemkt.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m='.$_REQUEST['m'].'&y='.$_REQUEST['y'].'&filter=zero&s='.$_REQUEST['s'].'&hq='.$_REQUEST['hq'].'&dis=0&md='.$_REQUEST['md'].'&rsts='.$_REQUEST['rsts'].'";</script>';}
}
...
function FunRef(m,y,s,hq,md,rsts){ window.location="f_chngmodemkt.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }

function funrsts(rsts,m,y,hq,s,md)
{ window.location="f_chngmodemkt.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }

function ActionModeFA(v,fid,m,y,hq,s,md,sn,ei,nm,rsts)
{ 
 var agree=confirm("Are you sure?"); if(agree){ window.location="f_chngmodemkt.php?action=cActChangeModeFA&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&fid="+fid+"&rsts="+rsts+"&v="+v+"&ei="+ei+"&ei="+ei+"&NMode="+nm; } else {return false;} 
}
...
<?php if($_REQUEST['rsts']=='All'){ $result=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND MdChng3Sts=3)) AND MdChng2Sts=3 AND MdChng3Sts>=1",$con); }
if($_REQUEST['rsts']>0){ $result=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND MdChng3Sts=3)) AND MdChng2Sts=3 AND MdChng3Sts=".$_REQUEST['rsts'],$con); }
...
if($_REQUEST['rsts']=='All'){ $sql=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND MdChng3Sts=3)) AND MdChng2Sts=3 AND MdChng3Sts>=1 order by FaId ASC LIMIT ".$from.",".$offset,$con); }
if($_REQUEST['rsts']>0){ $sql=mysql_query("select * from fa_details where (FaStatus='A' OR (FaStatus='D' AND MdChng3Sts=3)) AND MdChng2Sts=3 AND MdChng3Sts=".$_REQUEST['rsts']."  order by FaId ASC LIMIT ".$from.",".$offset,$con); }
...
<td class="tdf2"><input class="InputType" style="width:80px;" id="Hq<?php echo $sn; ?>" value="<?php echo ucfirst(strtolower($rhq['HqName'])); ?>" readonly/></td>
	 ...
	 <td class="tdf2" align="center"><?php if($res['MdChng3Sts']!=3 AND $res['MdChng3Sts']!=4){ ?>
...
<?php $sn++; } ?>
   <tr>
    ...
    <td colspan="11" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
     <?php doPages($offset, 'f_chngmodemkt.php', '', $total_records,$_REQUEST['hq'],$_REQUEST['s'],$_REQUEST['m'],$_REQUEST['y'],$_REQUEST['md'],$_REQUEST['rsts']); ?>
    </td>
   </tr>
<?php } ?> 
