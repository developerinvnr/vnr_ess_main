...
{
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON (d.Hq_vc=hq.HqId OR d.Hq_fc=Hq.HqId) where hq.StateId=".$_REQUEST['SteName']." AND sd.YearId=".$yset." AND sp.ProductSts='A' AND ".$qin."", $con); 
}
elseif($_REQUEST['NameWise']>0)
{
 if($_REQUEST['SelName']>0)
 {
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where d.DealerSts='A' AND (d.Terr_vc=".$_REQUEST['SelName']." OR d.Terr_fc=".$_REQUEST['SelName']." OR rl.R1=".$_REQUEST['SelName']." OR rl.R2=".$_REQUEST['SelName']." OR rl.R3=".$_REQUEST['SelName']." OR rl.R4=".$_REQUEST['SelName']." OR rl.R5=".$_REQUEST['SelName'].") AND sd.YearId=".$yset." AND sp.ProductSts='A' AND ".$qin."", $con); 
 }
 if($_REQUEST['SelName']=='All')
 {
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where d.DealerSts='A' AND (d.Terr_vc=".$EmployeeId." OR d.Terr_fc=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sp.ProductSts='A' AND sd.YearId=".$yset." AND ".$qin."", $con); 
 }
...
elseif($_REQUEST['HqWise']>0)
{
 if($_REQUEST['SelHq']>0)
 {
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where sd.YearId=".$yset." AND ".$qin." AND DealerSts='A' AND sp.ProductSts='A' AND (d.Hq_vc=".$_REQUEST['SelHq']." OR d.Hq_fc=".$_REQUEST['SelHq'].")", $con);
 }
 if($_REQUEST['SelHq']=='All')
 {
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_employee_general g ON (d.Terr_vc=g.EmployeeID OR d.Terr_fc=g.EmployeeID) where DealerSts='A' AND (d.Terr_vc=".$EmployeeId." OR d.Terr_fc=".$EmployeeId." OR g.RepEmployeeID=".$EmployeeId.") AND sp.ProductSts='A' AND sd.YearId=".$yset." AND ".$qin."", $con); 
 }
...
{ 
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON (d.Hq_vc=hq.HqId OR d.Hq_fc=Hq.HqId) where hq.StateId=".$_REQUEST['SteName']." AND sd.ProductId=".$res['ProductId']." AND d.DealerSts='A' AND sp.ProductSts='A' AND sd.YearId=".$yset, $con); 
}
elseif($_REQUEST['NameWise']>0)
{ 
 if($_REQUEST['SelName']>0)
 {
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where DealerSts='A' AND (d.Terr_vc=".$_REQUEST['SelName']." OR d.Terr_fc=".$_REQUEST['SelName']." OR rl.R1=".$_REQUEST['SelName']." OR rl.R2=".$_REQUEST['SelName']." OR rl.R3=".$_REQUEST['SelName']." OR rl.R4=".$_REQUEST['SelName']." OR rl.R5=".$_REQUEST['SelName'].") AND sp.ProductSts='A' AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$yset, $con); 
 }
 if($_REQUEST['SelName']=='All')
 {
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where DealerSts='A' AND (d.Terr_vc=".$_REQUEST['SelName']." OR d.Terr_fc=".$_REQUEST['SelName']." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sp.ProductSts='A' AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$yset, $con);
 }
}
elseif($_REQUEST['HqWise']>0)
{ 
 if($_REQUEST['SelHq']>0)
 {
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where sd.YearId=".$yset." AND sd.ProductId=".$res['ProductId']." AND sp.ProductSts='A' AND d.DealerSts='A' AND (d.Hq_vc=".$_REQUEST['SelHq']." OR d.Hq_fc=".$_REQUEST['SelHq'].")", $con); 
 }
 if($_REQUEST['SelHq']=='All')
 {
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_employee_general g ON (d.Terr_vc=g.EmployeeID OR d.Terr_fc=g.EmployeeID) where DealerSts='A' AND (d.Terr_vc=".$EmployeeId." OR d.Terr_fc=".$EmployeeId." OR g.RepEmployeeID=".$EmployeeId.") AND sd.ProductId=".$res['ProductId']." AND sp.ProductSts='A' AND sd.YearId=".$yset, $con);
 }
}
elseif($_REQUEST['DisWise']>0)
