...
{ 
 ...
 $sqlU=mysql_query("update fa_details set LWD='".date("Y-m-d",strtotime($_REQUEST['lwd']))."', RemoveSts=1, RemoveReqDate='".date("Y-m-d")."', RemoveRmk='".$_REQUEST['rmk']."', RemoveReqBy=".$EmployeeId.", Remove_Reporting=".$AppId.", Remove_Rev=".$RevId.", Remove_Hod=".$HodId." where FaId=".$_REQUEST['fid'],$con);
 if($sqlU)
 {
   $sqlRep=mysql_query("select Fname,Sname,Lname,EmailId_Vnr from hrm_employee_general g inner join hrm_employee e on g.EmployeeID=e.EmployeeID where g.EmployeeID=".$AppId,$con); $resRep=mysql_fetch_assoc($sqlRep);
    ...
    if($resRep['EmailId_Vnr']!='')
    {
     ...
     $emsg3 .="Your team member ".$Ename." has requested for FA removel, for any action please go to FA portal.<br>\n\n";
...
$result=mysql_query("select * from fa_details fd inner join hrm_sales_reporting_level rl on fd.Reporting=rl.EmployeeID where (fd.Reporting=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND ".$qry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) order by FaId ASC",$con);
...
$sql=mysql_query("select * from fa_details fd inner join hrm_sales_reporting_level rl on fd.Reporting=rl.EmployeeID where (fd.Reporting=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND ".$qry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) order by FaId ASC LIMIT ".$from.",".$offset,$con);
