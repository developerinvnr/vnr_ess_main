...
<?php $sql=mysql_query("select ItemId,ItemName from hrm_sales_seedsitem order by ItemName ASC", $con); while($res=mysql_fetch_assoc($sql)){ 
      if($_REQUEST['g']==72 OR $_REQUEST['g']==73)
	  {  $sql2=mysql_query("select * from hrm_sales_sal_details_region where RegionRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	     $row2=mysql_num_rows($sql2); 
	     if($row2==0)
		 {
		  $sql3=mysql_query("select * from hrm_sales_sal_details_area where AreaRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	      $row3=mysql_num_rows($sql3);
		  if($row3==0)
		  {
		   $sql4=mysql_query("select * from hrm_sales_sal_details_terr where TerrRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	       $row4=mysql_num_rows($sql4);
	       if($row4==0)
		   {
		    $sql5=mysql_query("select * from hrm_sales_sal_details_hq where HqRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	        $row5=mysql_num_rows($sql5); 
		    if($row5==0){echo $res['ItemName'].',&nbsp;&nbsp;'; }
		   }
		  } 
		 } 
	  }
      elseif($_REQUEST['g']==74 OR $_REQUEST['g']==75)
	  {
	    $sql2=mysql_query("select * from hrm_sales_sal_details_zone where ZoneRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	    $row2=mysql_num_rows($sql2); 
		if($row2==0)
		{ 
		  $sql3=mysql_query("select * from hrm_sales_sal_details_region where RegionRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	      $row3=mysql_num_rows($sql3); 
	      if($row3==0)
		  {
		   $sql4=mysql_query("select * from hrm_sales_sal_details_area where AreaRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	       $row4=mysql_num_rows($sql4); 
		   if($row4==0)
		   {
		     $sql5=mysql_query("select * from hrm_sales_sal_details_terr where TerrRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	         $row5=mysql_num_rows($sql5);
			 if($row5==0)
		     { 
		      $sql6=mysql_query("select * from hrm_sales_sal_details_hq where HqRepEmpID=".$_REQUEST['e']." AND ItemId=".$res['ItemId']." AND YearId=".$_REQUEST['y'], $con);
	          $row6=mysql_num_rows($sql6); 
		      if($row6==0){echo $res['ItemName'].',&nbsp;&nbsp;'; }
		     } 
		   }
		  } 
		 }
