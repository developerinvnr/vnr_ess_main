...
$fy11=date("Y",strtotime($ry['FromDate'])); $ty11=date("Y",strtotime($ry['ToDate']));
...
$sql = mysql_query("select hrm_sales_sal_details_".$_REQUEST['y'].".ProductId,hrm_sales_sal_details_".$_REQUEST['y'].".DealerId,M1,M2,M3,M4,M5,M6,M7,M8,M9,M10,M11,M12,M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." INNER JOIN hrm_sales_seedsproduct ON hrm_sales_sal_details_".$_REQUEST['y'].".ProductId=hrm_sales_seedsproduct.ProductId INNER JOIN hrm_sales_seedsitem ON hrm_sales_seedsproduct.ItemId=hrm_sales_seedsitem.ItemId INNER JOIN hrm_sales_dealer ON hrm_sales_sal_details_".$_REQUEST['y'].".DealerId=hrm_sales_dealer.DealerId INNER JOIN hrm_sales_hq_temp ON hrm_sales_dealer.HqId=hrm_sales_hq_temp.HqId INNER JOIN hrm_sales_reporting_level ON hrm_sales_hq_temp.EmployeeID=hrm_sales_reporting_level.EmployeeID where HqTEmpStatus='A' AND (hrm_sales_hq_temp.EmployeeID=".$_REQUEST['ei']." OR hrm_sales_reporting_level.R1=".$_REQUEST['ei']." OR hrm_sales_reporting_level.R2=".$_REQUEST['ei']." OR hrm_sales_reporting_level.R3=".$_REQUEST['ei']." OR hrm_sales_reporting_level.R4=".$_REQUEST['ei']." OR hrm_sales_reporting_level.R5=".$_REQUEST['ei'].") AND hrm_sales_sal_details_".$_REQUEST['y'].".YearId=".$_REQUEST['y']." AND (M1!=0 OR M2!=0 OR M3!=0 OR M4!=0 OR M5!=0 OR M6!=0 OR M7!=0 OR M8!=0 OR M9!=0 OR M10!=0 OR M11!=0 OR M12!=0 OR M1_Ach!=0 OR M2_Ach!=0 OR M3_Ach!=0 OR M4_Ach!=0 OR M5_Ach!=0 OR M6_Ach!=0 OR M7_Ach!=0 OR M8_Ach!=0 OR M9_Ach!=0 OR M10_Ach!=0 OR M11_Ach!=0 OR M12_Ach!=0) order by hrm_sales_seedsitem.ItemName ASC,hrm_sales_seedsproduct.ProductName ASC", $con); $Sn=1; while($res=mysql_fetch_array($sql)){ 
...
$csv_output .= '"'.str_replace('"', '""', floatval(($res['M1']*$r4t['NRV'])+($res['M2']*$r5t['NRV'])+($res['M3']*$r6t['NRV']))).'",';
...
$csv_output .= '"'.str_replace('"', '""', floatval(($res['M4']*$r7t['NRV'])+($res['M5']*$r8t['NRV'])+($res['M6']*$r9t['NRV']))).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['M7']*$r10t['NRV']).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['M8']*$r11t['NRV']).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['M9']*$r12t['NRV']).'",';
...
$csv_output .= '"'.str_replace('"', '""', floatval(($res['M7']*$r10t['NRV'])+($res['M8']*$r11t['NRV'])+($res['M9']*$r12t['NRV']))).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['M10']*$r1t['NRV']).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['M11']*$r2t['NRV']).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['M12']*$r3t['NRV']).'",';
...
$csv_output .= '"'.str_replace('"', '""', floatval(($res['M10']*$r1t['NRV'])+($res['M11']*$r2t['NRV'])+($res['M12']*$r3t['NRV']))).'",';
...
$csv_output .= '"'.str_replace('"', '""', floatval(($res['M1']*$r4t['NRV'])+($res['M2']*$r5t['NRV'])+($res['M3']*$r6t['NRV'])+($res['M4']*$r7t['NRV'])+($res['M5']*$r8t['NRV'])+($res['M6']*$r9t['NRV'])+($res['M7']*$r10t['NRV'])+($res['M8']*$r11t['NRV'])+($res['M9']*$r12t['NRV'])+($res['M10']*$r1t['NRV'])+($res['M11']*$r2t['NRV'])+($res['M12']*$r3t['NRV']))).'",';
