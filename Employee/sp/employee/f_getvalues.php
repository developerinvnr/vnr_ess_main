...
$sqlDis2=mysql_query("select DealerId,DealerName,DealerCity from hrm_sales_dealer where (Hq_vc=".$_POST['hqid2']." OR Hq_fc=".$_POST['hqid2'].") order by DealerName asc",$con); while($resDis2=mysql_fetch_assoc($sqlDis2)){
...
{ 
echo '<select style="width:160px;" class="InputSel" id="sdealer" name="sdealer" onchange="funs2deal(this.value)">
<option value="0">Select</option>';
$sqlDis2=mysql_query("select DealerId,DealerName,DealerCity from hrm_sales_dealer where (Hq_vc=".$_POST['hqid3']." OR Hq_fc=".$_POST['hqid3'].") order by DealerName asc",$con); while($resDis2=mysql_fetch_assoc($sqlDis2)){
echo '<option value='.$resDis2['DealerId'].'>'.ucfirst(strtolower($resDis2['DealerName'])).' - '.strtoupper($resDis2['DealerCity']).'</option>'; }
echo '</select>';
}


elseif($_POST['getv']=='hq4' && $_POST['hqid4']!='')
{ 
echo '<select style="width:160px;" class="InputSel" id="sdealer" name="sdealer" onchange="funs3deal(this.value)">
<option value="0">Select</option>';
$sqlDis2=mysql_query("select DealerId,DealerName,DealerCity from hrm_sales_dealer where (Hq_vc=".$_POST['hqid4']." OR Hq_fc=".$_POST['hqid4'].") order by DealerName asc",$con); while($resDis2=mysql_fetch_assoc($sqlDis2)){
echo '<option value='.$resDis2['DealerId'].'>'.ucfirst(strtolower($resDis2['DealerName'])).' - '.strtoupper($resDis2['DealerCity']).'</option>'; }
echo '</select>';
}
?>
