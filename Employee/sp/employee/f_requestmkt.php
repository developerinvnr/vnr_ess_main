...
{ if($_REQUEST['v']==0){$status=5; $statusc=2; $msg='rejected';}
  elseif($_REQUEST['v']==1){$status=3; $statusc=3; $msg='approved';}
...
function FunRef(a,b,c,d,e,f){ window.location="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f+"&s=0"; }
...
{ 
  ...
  if(agree){ var x="f_requestmkt.php?act=Action&v="+v+"&goid="+id+"&s="+st+"&a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f; window.location=x;} }


function FunCheck(v,a,b,c,d,e,f)
{ 
 if(v==0){ if(document.getElementById("v0").checked==true){var vk=1;}else{var vk=0;} 
 var x="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&s="+st+"&a="+vk+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f; }
 else if(v==1){ if(document.getElementById("v1").checked==true){var vk=1;}else{var vk=0;} 
 var x="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&s="+st+"&a="+a+"&b="+vk+"&c="+c+"&d="+d+"&e="+e+"&f="+f; }
 else if(v==2){ if(document.getElementById("v2").checked==true){var vk=1;}else{var vk=0;} 
 var x="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&s="+st+"&a="+a+"&b="+b+"&c="+vk+"&d="+d+"&e="+e+"&f="+f; }
 else if(v==3){ if(document.getElementById("v3").checked==true){var vk=1;}else{var vk=0;} 
 var x="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&s="+st+"&a="+a+"&b="+b+"&c="+c+"&d="+vk+"&e="+e+"&f="+f; }
 else if(v==4){ if(document.getElementById("v4").checked==true){var vk=1;}else{var vk=0;} 
 var x="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&s="+st+"&a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+vk+"&f="+f; }
 else if(v==5){ if(document.getElementById("v5").checked==true){var vk=1;}else{var vk=0;} 
 var x="f_requestmkt.php?ern1=r114&ern2w=234&ern3y=10234&s="+st+"&a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+vk; }
...
$stmt=mysql_query("select * from fa_request where (Status=".$bb." OR Status=".$cc." OR Status=".$dd." OR Status=".$ee." OR (Status=".$ff." AND Status_Cycle=2))", $con); $total_records=mysql_num_rows($stmt);
...
$sql=mysql_query("select * from fa_request where (Status=".$bb." OR Status=".$cc." OR Status=".$dd." OR Status=".$ee." OR (Status=".$ff." AND Status_Cycle=2)) order by Req_Date ASC LIMIT ".$from.",".$offset, $con); 	$sn=1; while($res=mysql_fetch_array($sql)){ ?>	
    <tr>
	  ...
	  <td class="tdf2" align="center"><?php if($res['Status']==2){ ?><font color="#E800E8">Pending</font>
	  <?php } elseif($res['Status']==3){ ?><font color="#FF6B24">Approved</font>
	  <?php } elseif($res['Status']==5 AND $res['Status_Cycle']==2){ ?><font color="#000000">Rejected</font><?php } ?>
	  </td>
	  <td class="tdf2" align="center"><?php if($res['Status']==2){ ?><select style="width:80px;" class="InputSel" onChange="funAction(this.value,<?php echo $res['FareqId'];?>,<?php echo $a.','.$b.','.$c.','.$d.','.$e.','.$f; ?>)"><option value="" selected>Select</option><option value="1">Approve</option><option value="0">Reject</option></select><?php } ?></td>
	</tr>
<?php $sn++; } ?>
    <tr>
    <td align="center" colspan="11" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
     <?php doPages($offset, 'f_requestmkt.php', '', $total_records,$a,$b,$c,$d,$e,$f); ?>
    </td>
 </tr> 
   </table>
  </td>
