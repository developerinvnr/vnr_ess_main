...
?>   
   ...
   <?php if($_REQUEST['vc']!=0 && $_REQUEST['ii']!=0){$sqlI=mysql_query("select ItemName from hrm_sales_seedsitem where ItemId=".$_REQUEST['ii'], $con); $resI=mysql_fetch_assoc($sqlI);?><option value="<?php echo $_REQUEST['ii']; ?>" selected><?php echo strtoupper($resI['ItemName']); ?></option><?php }elseif($_REQUEST['vc']!=0 && $_REQUEST['ii']=='All_1'){ ?><option value="All_1"> --- All ---</option><?php }else{ ?><option value="0" selected>SELECT</option><?php } ?><option value="All_1"> --- All ---</option>
   <?php $sqlItem=mysql_query("select * from hrm_sales_seedsitem where GroupId=1 order by ItemName ASC", $con); while($resItem=mysql_fetch_assoc($sqlItem)){ ?><option value="<?php echo $resItem['ItemId']; ?>"><?php echo $resItem['ItemName']; ?></option><?php } ?></select>
   </td>
   ...
   <td align="center" valign="bottom" class="det">Dealer :&nbsp;<select class="detsel" style="width:200px;background-color:#C4FFC4;" id="DealerNV" onChange="ChangeDealer(this.value,<?php echo $_REQUEST['hq'].', '.$_REQUEST['y'].', '.$_REQUEST['no']; ?>)">
   <?php if($_REQUEST['DealerV']>0){ $sql2Deal = mysql_query("select * from hrm_sales_dealer where DealerId=".$_REQUEST['DealerV'], $con); $res2Deal=mysql_fetch_array($sql2Deal); ?><option value="<?php echo $_REQUEST['DealerV']; ?>" selected="selected"><?php echo $res2Deal['DealerName']; ?></option><?php }else{?>
   <option>Select Dealer Name</option><?php } $sqlDeal = mysql_query("select * from hrm_sales_dealer where (Hq_vc=".$_REQUEST['hq']." OR Hq_fc=".$_REQUEST['hq'].") AND (Terr_vc=".$EmployeeId." OR Terr_fc=".$EmployeeId.") AND DealerSts='A' group by DealerId order by DealerId ASC", $con); while($resDeal=mysql_fetch_assoc($sqlDeal)){ ?><option value="<?php echo $resDeal['DealerId']; ?>"><?php echo $resDeal['DealerName']; ?></option><?php } ?></select>
   </td>
...
<?php if($_REQUEST['SelectV']=='DealerNo' AND $_REQUEST['DealerV']!='' AND $_REQUEST['DealerV']>0){ ?>
...
$sqlDeal = mysql_query("select * from hrm_sales_dealer where DealerId=".$_REQUEST['DealerV']." order by DealerId ASC", $con); $resDealRow=mysql_num_rows($sqlDeal);
