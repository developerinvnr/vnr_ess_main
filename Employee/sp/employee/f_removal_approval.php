...
function FunRef(m,y,s,hq,md,rsts){ window.location="f_removal_approval.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }

function funmd(md,m,y,hq,s,rsts)
{ window.location="f_removal_approval.php?ac=2441&ee=2421&der=true2&c=false&trht=FTF%%F1&tt=valuased&m="+m+"&y="+y+"&filter=zero&s="+s+"&hq="+hq+"&dis=0&md="+md+"&rsts="+rsts; }
...
{ var agree=confirm("Are you sure?");
  if(agree)
  { 
    var url = 'f_claimAppAct.php'; var pars = 'Act=AppRemoval&v='+v+'&sn='+sn+'&fid='+fid+'&m='+m+'&y='+y+'&eid='+eid+"&rep="+rep+"&rev="+rev+"&hod="+hod+"&by="+by+"&lvl="+lvl; var myAjax = new Ajax.Request(
...
$result=mysql_query("select * from fa_details where ((Remove_Reporting=".$EmployeeId." AND Remove_Reporting_AppSts=".$_REQUEST['rsts'].") OR (Remove_Rev=".$EmployeeId." AND Remove_Rev_AppSts=".$_REQUEST['rsts'].") OR (Remove_Hod=".$EmployeeId." AND Remove_Hod_AppSts=".$_REQUEST['rsts'].")) AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts>=1 order by FaId ASC",$con); 
...
$sql=mysql_query("select * from fa_details where ((Remove_Reporting=".$EmployeeId." AND Remove_Reporting_AppSts=".$_REQUEST['rsts'].") OR (Remove_Rev=".$EmployeeId." AND Remove_Rev_AppSts=".$_REQUEST['rsts'].") OR (Remove_Hod=".$EmployeeId." AND Remove_Hod_AppSts=".$_REQUEST['rsts'].")) AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts>=1 order by FaId ASC LIMIT ".$from.",".$offset,$con);
...
<td class="tdf2"><input class="InputType" style="width:100px;" id="Hq<?php echo $sn; ?>" value="<?php echo ucfirst(strtolower($rhq['HqName'])); ?>" readonly/></td>
	 ...
	 <td class="tdf2" align="center">
	  <?php if($res['Remove_Reporting']==$EmployeeId)
	        {
			 if($res['Remove_Reporting_AppSts']==0){ echo '<font color="#000000">Draft</font>'; }
			 elseif($res['Remove_Reporting_AppSts']==1){ echo '<font color="#E800E8">Pending</font>'; }
			 elseif($res['Remove_Reporting_AppSts']==2){ echo '<font color="#FF6B24">Approved</font>'; }
			 elseif($res['Remove_Reporting_AppSts']==3){ echo '<font color="#000000">Rejected</font>'; }
			}
			elseif($res['Remove_Rev']==$EmployeeId)
	        {
			 if($res['Remove_Rev_AppSts']==0){ echo '<font color="#000000">Draft</font>'; }
			 elseif($res['Remove_Rev_AppSts']==1){ echo '<font color="#E800E8">Pending</font>'; }
			 elseif($res['Remove_Rev_AppSts']==2){ echo '<font color="#FF6B24">Approved</font>'; }
			 elseif($res['Remove_Rev_AppSts']==3){ echo '<font color="#000000">Rejected</font>'; }
			}
			elseif($res['Remove_Hod']==$EmployeeId)
	        {
			 if($res['Remove_Hod_AppSts']==0){ echo '<font color="#000000">Draft</font>'; }
			 elseif($res['Remove_Hod_AppSts']==1){ echo '<font color="#E800E8">Pending</font>'; }
			 elseif($res['Remove_Hod_AppSts']==2){ echo '<font color="#FF6B24">Approved</font>'; }
			 elseif($res['Remove_Hod_AppSts']==3){ echo '<font color="#000000">Rejected</font>'; }
			}
	  ?>
	  </td>
	 ...
	 */ ?>
	 ...
	 <td class="tdf2" align="center"><?php if(($res['Remove_Reporting']==$EmployeeId && ($res['Remove_Reporting_AppSts']==0 OR $res['Remove_Reporting_AppSts']==1)) OR ($res['Remove_Rev']==$EmployeeId && ($res['Remove_Rev_AppSts']==0 OR $res['Remove_Rev_AppSts']==1)) OR ($res['Remove_Hod']==$EmployeeId && ($res['Remove_Hod_AppSts']==0 OR $res['Remove_Hod_AppSts']==1))){ ?><select style="width:80px;" class="InputSel" onChange="FucApp(this.value,<?php echo $sn.','.$res['FaId'].', '.$_REQUEST['m'].','.$_REQUEST['y'].','.$EmployeeId.','.$res['Remove_Reporting'].','.$res['Remove_Rev'].','.$res['Remove_Hod'].','.$res['RemoveReqBy'].','.$res['Remove_lvl']; ?>)"><option value="" selected>Select</option><option value="2">Approve</option><option value="3">Reject</option></select><?php } ?></td> 
...
<?php $sn++; } ?>
   <tr>
    ...
    <td colspan="11" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
     <?php doPages($offset, 'f_removal_approval.php', '', $total_records,$_REQUEST['hq'],$_REQUEST['s'],$_REQUEST['m'],$_REQUEST['y'],$_REQUEST['md'],$_REQUEST['rsts']); ?>
    </td>
   </tr>
<?php } ?> 
