...
echo "Sn\tDealer\tHeadQuarter\tState\tSalesPlan_Value(VC)\tPendingBill(VC)\tCollection Agains Salesplan(VC)\tCollection Against Outstanding(VC)\tCollection For Abs(VC)\tAbs Name(VC)\tTotal Target(VC)\tActual Collection Sp+Oust(VC)\tActual Collection Abs(VC)\tSalesPlan_Value(FC)\tPendingBill(FC)\tCollection Agains Salesplan(FC)\tCollection Against Outstanding(FC)\tCollection For Abs(FC)\tAbs Name(FC)\tTotal Target(FC)\tActual Collection Sp+Oust(FC)\tActual Collection Abs(FC)";
 ...
 if($hq>0){ $sql=mysql_query("select d.DealerId,DealerName,DealerCity,HqName,StateCode from hrm_sales_dealer d inner join hrm_headquater hq on d.HqId=hq.HqId inner join hrm_state s on hq.StateId=s.StateId where hq.HqId=".$hq." order by hq.HqName ASC, d.DealerName ASC ",$con); }elseif($s>0){$sql=mysql_query("select d.DealerId,DealerName,DealerCity,HqName,StateCode from hrm_sales_dealer d inner join hrm_headquater hq on d.HqId=hq.HqId inner join hrm_state s on hq.StateId=s.StateId inner join hrm_sales_hq_temp hqtmp on hq.HqId=hqtmp.HqId inner join hrm_sales_reporting_level rl on hqtmp.EmployeeID=rl.EmployeeID where HqTEmpStatus='A' AND (hqtmp.EmployeeID=".$ei." OR rl.R1=".$ei." OR rl.R2=".$ei." OR rl.R3=".$ei." OR rl.R4=".$ei." OR rl.R5=".$ei.") AND hq.StateId=".$s." order by hq.HqName ASC, d.DealerName ASC ",$con);} 
 ...
 while($res = mysql_fetch_array($sql))
 {
  ...
  $spb=mysql_query("select * from erp_rcp_ageing where DealerId=".$res['DealerId']." AND AgeingDate='".date($yy.'-'.$m.'-'.$Vw)."' AND AgeingMonth=".$m." AND AgeingYear=".$yy,$con2); $rpb=mysql_fetch_assoc($spb);
  ...
  $sTv=mysql_query("select SUM(CollAmount) as vamt from erp_dealer_collection where DealerId=".$res['DealerId']." AND Type='vc' AND Abs='N' AND Updt between '".date($yy.'-'.$m.'-01')."' AND '".date($yy.'-'.$m.'-31')."' ",$con2); $sTva=mysql_query("select SUM(CollAmount) as v2amt from erp_dealer_collection where DealerId=".$res['DealerId']." AND Type='vc' AND Abs='Y' AND Updt between '".date($yy.'-'.$m.'-01')."' AND '".date($yy.'-'.$m.'-31')."' ",$con2);
 ...
 if($s>0 AND $hq==0){$sql=mysql_query("select d.DealerId,DealerName,DealerCity,HqName,StateCode from hrm_sales_dealer d inner join hrm_headquater hq on d.HqId=hq.HqId inner join hrm_sales_tlemp tl on hq.HqId=tl.TLHq inner join hrm_state s on hq.StateId=s.StateId where TLStatus='A' AND tl.TLRepId=".$ei." AND hq.StateId=".$s." order by hq.HqName ASC, d.DealerName ASC",$con);} 
 ...
 while($res = mysql_fetch_array($sql))
 {
  ...
  $s2Tv=mysql_query("select SUM(CollAmount) as vamt from erp_dealer_collection where DealerId=".$res['DealerId']." AND Type='fc' AND Abs='N' AND Updt between '".date($yy.'-'.$m.'-01')."' AND '".date($yy.'-'.$m.'-31')."' ",$con2); $s2Tva=mysql_query("select SUM(CollAmount) as v2amt from erp_dealer_collection where DealerId=".$res['DealerId']." AND Type='fc' AND Abs='Y' AND Updt between '".date($yy.'-'.$m.'-01')."' AND '".date($yy.'-'.$m.'-31')."' ",$con2);
  $r2Tv=mysql_fetch_assoc($s2Tv); $r2Tva=mysql_fetch_assoc($s2Tva);
  $schema_insert .= $r2Tv['vamt'].$sep;
  $schema_insert .= $r2Tva['v2amt'].$sep;
...
$ss=mysql_query("select ms.StateId,StateCode from erp_mrp_empstate ms inner join erp_state s on ms.StateId=s.StateId where ms.EmpId=".$ei." AND ms.Sts='A' order by StateCode ASC",$con2);
