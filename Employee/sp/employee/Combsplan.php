...
function ExportCollAll(y,ei)
{ window.open("CombsplanExport.php?action=SalCollExport&y="+y+"&ei="+ei,"ExpForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20"); }
...
<table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:12px;width:550px;vertical-align:top;">
 <tr style="background-color:#D5F1D1;color:#000000;"> 
  <td colspan="4" align="center" style="width:250px;font-size:14px;"><b>Total Records Export</b></td> 
 </tr>	
 <tr style="background-color:#D5F1D1;color:#000000;"> 
  ...
  <td align="center" style="width:150px;font-size:14px;"><b>Collection (Dealer Wise)</b></td>
 </tr>   
  <?php  
  //if($_SESSION['Vertical']==14){$suq='si.GroupId=1';}
  //elseif($_SESSION['Vertical']==15){$suq='si.GroupId=2';}
  //elseif($_SESSION['Vertical']==16){$suq='1=1';}
  //else{$suq='1=1';}
  
  if($_SESSION['Vertical']==16 || ($_SESSION['Hqv']>0 && $_SESSION['Hqf']>0)){$suq='1=1';}
  elseif($_SESSION['Vertical']==14 || $_SESSION['Hqv']>0){$suq='si.GroupId=1';}
  elseif($_SESSION['Vertical']==15 || $_SESSION['Hqf']>0){$suq='si.GroupId=2';}
  else{$suq='1=1';}
  
  $sql1 = mysql_query("select sd.ProductId,sd.DealerId,M1,M2,M3,M4,M5,M6,M7,M8,M9,M10,M11,M12,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where d.DealerSts='A' AND (Terr_vc=".$EmployeeId." OR Terr_fc=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." AND (M1!=0 OR M2!=0 OR M3!=0 OR M4!=0 OR M5!=0 OR M6!=0 OR M7!=0 OR M8!=0 OR M9!=0 OR M10!=0 OR M11!=0 OR M12!=0) AND ".$suq." order by si.ItemName ASC,sp.ProductName ASC", $con); $rows1=mysql_num_rows($sql1); 
  
  //TL
  $sql1a = mysql_query("select sd.ProductId,sd.DealerId,M1,M2,M3,M4,M5,M6,M7,M8,M9,M10,M11,M12,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tle ON d.HqId=tle.TLHq INNER JOIN hrm_sales_reporting_level rl ON tle.TLRepId=rl.EmployeeID where tle.TLStatus='A' AND (tle.TLRepId=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." AND (M1!=0 OR M2!=0 OR M3!=0 OR M4!=0 OR M5!=0 OR M6!=0 OR M7!=0 OR M8!=0 OR M9!=0 OR M10!=0 OR M11!=0 OR M12!=0) AND ".$suq." order by si.ItemName ASC,sp.ProductName ASC", $con); $rows1a=mysql_num_rows($sql1a);
  //TL
  
  $sql2 = mysql_query("select sd.ProductId,sd.DealerId,M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where d.DealerSts='A' AND (Terr_vc=".$EmployeeId." OR Terr_fc=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." AND (M1_Ach!=0 OR M2_Ach!=0 OR M3_Ach!=0 OR M4_Ach!=0 OR M5_Ach!=0 OR M6_Ach!=0 OR M7_Ach!=0 OR M8_Ach!=0 OR M9_Ach!=0 OR M10_Ach!=0 OR M11_Ach!=0 OR M12_Ach!=0) AND ".$suq." order by si.ItemName ASC,sp.ProductName ASC", $con); $rows2=mysql_num_rows($sql2);
  
  //TL
  $sql2a = mysql_query("select sd.ProductId,sd.DealerId,M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tle ON d.HqId=tle.TLHq INNER JOIN hrm_sales_reporting_level rl ON tle.TLRepId=rl.EmployeeID where tle.TLStatus='A' AND (tle.TLRepId=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." AND (M1_Ach!=0 OR M2_Ach!=0 OR M3_Ach!=0 OR M4_Ach!=0 OR M5_Ach!=0 OR M6_Ach!=0 OR M7_Ach!=0 OR M8_Ach!=0 OR M9_Ach!=0 OR M10_Ach!=0 OR M11_Ach!=0 OR M12_Ach!=0) AND ".$suq." order by si.ItemName ASC,sp.ProductName ASC", $con); $rows2a=mysql_num_rows($sql2a);
  //TL
  
  $sql3 = mysql_query("select sd.ProductId,sd.DealerId,M1,M2,M3,M4,M5,M6,M7,M8,M9,M10,M11,M12,M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where d.DealerSts='A' AND (Terr_vc=".$EmployeeId." OR Terr_fc=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." AND (M1!=0 OR M2!=0 OR M3!=0 OR M4!=0 OR M5!=0 OR M6!=0 OR M7!=0 OR M8!=0 OR M9!=0 OR M10!=0 OR M11!=0 OR M12!=0 OR M1_Ach!=0 OR M2_Ach!=0 OR M3_Ach!=0 OR M4_Ach!=0 OR M5_Ach!=0 OR M6_Ach!=0 OR M7_Ach!=0 OR M8_Ach!=0 OR M9_Ach!=0 OR M10_Ach!=0 OR M11_Ach!=0 OR M12_Ach!=0) AND ".$suq." order by si.ItemName ASC,sp.ProductName ASC", $con); $rows3=mysql_num_rows($sql3);
  
  //TL
  $sql3a = mysql_query("select sd.ProductId,sd.DealerId,M1,M2,M3,M4,M5,M6,M7,M8,M9,M10,M11,M12,M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach,ProductName,ItemName,ItemCode from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tle ON d.HqId=tle.TLHq INNER JOIN hrm_sales_reporting_level rl ON tle.TLRepId=rl.EmployeeID where tle.TLStatus='A' AND (tle.TLRepId=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." AND (M1!=0 OR M2!=0 OR M3!=0 OR M4!=0 OR M5!=0 OR M6!=0 OR M7!=0 OR M8!=0 OR M9!=0 OR M10!=0 OR M11!=0 OR M12!=0 OR M1_Ach!=0 OR M2_Ach!=0 OR M3_Ach!=0 OR M4_Ach!=0 OR M5_Ach!=0 OR M6_Ach!=0 OR M7_Ach!=0 OR M8_Ach!=0 OR M9_Ach!=0 OR M10_Ach!=0 OR M11_Ach!=0 OR M12_Ach!=0) AND ".$suq." order by si.ItemName ASC,sp.ProductName ASC", $con); $rows3a=mysql_num_rows($sql3a);
  //TL
  ?>
 <tr bgcolor="#EEEEEE" style="height:22px;background-color:<?php echo '#E2F3D8'; ?>;"> 
  ...
  <?php $sql4 = mysql_query("select sd.* from hrm_sales_rcp_collection sd inner join hrm_sales_dealer d on sd.DealerId=d.DealerId inner join hrm_sales_reporting_level rl on (d.Terr_vc=rl.EmployeeID OR d.Terr_fc=rl.EmployeeID) where d.DealerSts='A' AND (Terr_vc=".$EmployeeId." OR Terr_fc=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND sd.YearId=".$_REQUEST['y']." group by sd.DealerId ASC", $con); $rows4=mysql_num_rows($sql4); ?>
  
  <td style="font-size:14px;" align="center"><?php //echo $rows4; ?><?php if($rows4>0){ ?><a href="#" onClick="ExportCollAll(<?php echo $_REQUEST['y'].','.$EmployeeId; ?>)"><b>Click</b></a><?php } ?></td>
