...
.Trh50{text-align:center;width:40px;font-size:12px;font-weight:bold;}
...
<?php //****************************** Start ************************************************ ?>		
<?php $SpP=mysql_query("select GradeId,DepartmentId,HqId from hrm_employee_general where EmployeeID=".$EmployeeId, $con); $resSpP=mysql_fetch_assoc($SpP); $SpEG=mysql_query("select GradeValue from hrm_grade where CompanyId=".$CompanyId." AND GradeId=".$resSpP['GradeId'], $con); $resSpEG=mysql_fetch_assoc($SpEG); ?>		
...
?>		 
  <td>
    <select style="font-family:Georgia;font-size:14px;width:px;background-color:#E4E0FC;height:23px;width:150px;" id="YearV" onChange="ChangeYear(this.value)"><?php $sqly=mysql_query("select FromDate,ToDate from hrm_sales_year where YearId=".$_REQUEST['y'], $con); $resy=mysql_fetch_assoc($sqly); $fromdate=date("Y",strtotime($resy['FromDate'])); $todate=date("Y",strtotime($resy['ToDate'])); ?><option value="<?php echo $_REQUEST['y']; ?>" selected><?php echo 'Year-'.$fromdate.'-'.$todate; ?></option><?php if($_REQUEST['y']==$YearId){?><option value="<?php echo $Ymin1; ?>"><?php echo 'Year-'.$FYmin1.'-'.$TYmin1; ?></option><option value="<?php echo $Ymin2; ?>"><?php echo 'Year-'.$FYmin2.'-'.$TYmin2; ?></option>
<?php } elseif($_REQUEST['y']!=$YearId) { ?><option value="<?php echo $YearId; ?>"><?php echo 'Year-'.$FUpy.'-'.$TUpy; ?></option><?php if($_REQUEST['y']!=$Ymin1){?><option value="<?php echo $Ymin1; ?>"><?php echo 'Year-'.$FYmin1.'-'.$TYmin1; ?></option><?php } ?><?php if($_REQUEST['y']!=$Ymin2){?><option value="<?php echo $Ymin2; ?>"><?php echo 'Year-'.$FYmin2.'-'.$TYmin2; ?></option><?php } } ?></select>
   </td>
   <td></td>
    <td>
...
<?php $sqlCoc=mysql_query("select * from hrm_sales_planshow where PlanshowId=1",$con); $resCoc=mysql_fetch_assoc($sqlCoc); 
	  ...
	  if($_REQUEST['findV']=='TT' OR $_REQUEST['findV']=='AR' OR $_REQUEST['findV']=='ZZ'){ ?>
<?php $sqlRpt=mysql_query("select g.EmployeeID from hrm_employee_general g INNER JOIN hrm_employee e ON g.EmployeeID=e.EmployeeID where g.DepartmentId=6 AND e.EmpStatus='A' AND g.RepEmployeeID=".$_REQUEST['mainv'],$con); 
$rowRpt=mysql_num_rows($sqlRpt); ?>
<tr>
 <td style="width:100%;">
<?php  $sqlE=mysql_query("select EmpCode, Fname, Sname, Lname, CostCenter, Gender, Married, DR, GradeId, DesigName, HqName, StateName from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_personal p ON g.EmployeeID=p.EmployeeID INNER JOIN hrm_designation de ON g.DesigId=de.DesigId INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_state st ON hq.StateId=st.StateId where e.EmpStatus='A' AND e.CompanyId=".$CompanyId." AND e.EmployeeID=".$_REQUEST['mainv'],$con); 
...
<table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:14px;width:100%;">	
  ...
  <td colspan="26" style="height:25px;background-color:#005300;font-family:Times New Roman;font-size:16px;color:#FFFF20;font-weight:bold;" id="TD">
   &nbsp;<font color="#D7EBFF">Name:&nbsp;</font><?php echo $NameE; ?>&nbsp;
 ...
 <?php $sqlSb=mysql_query("select * from hrm_sales_emp_action where EmployeeID=".$EmployeeId." AND YearId=".$_REQUEST['y']." AND StatusA=1", $con); $rowSb=mysql_num_rows($sqlSb); ?><input type="button" style="font-family:Times New Roman;width:100px;height:25px;font-weight:bold;" value="Final Submit" onClick="ClickFSL1(<?php echo $EmployeeId.', '.$_REQUEST['y']; ?>)" <?php if($RslP=='Y'){echo 'disabled';}else{echo '';}?>/><?php } //$rowSb>0 OR  ?>
  ...
  <tr style="background-color:#D5F1D1;color:#000000;">
   <td align="center" style="width:100px;" rowspan="<?php if($rowRpt>0){echo 3;}else{echo 2;} ?>"><b>CROP</b></td>
   <td align="center" style="width:120px;" rowspan="<?php if($rowRpt>0){echo 3;}else{echo 2;} ?>"><b>VARIETY</b></td>
...
<?php 
if($_REQUEST['findV']=='AR' AND $rowRpt>0){
if($_REQUEST['ii']>0)
{
 if($rtl>0)
 {
  ...
  $sqlBta=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tl ON d.HqId=tl.TLHq where TLStatus='A' AND tl.TLRepId=".$EmployeeId." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con);
 $resAta=mysql_fetch_assoc($sqlAta); $resBta=mysql_fetch_assoc($sqlBta);
 }
 
 $sqlP2t=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_terr sdt INNER JOIN hrm_sales_seedsproduct sp ON sdt.ProductId=sp.ProductId where TerrRepEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con); 
 $sqlP3t=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_terr sdt INNER JOIN hrm_sales_seedsproduct sp ON sdt.ProductId=sp.ProductId where TerrRepEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con);  
 $sqlP2ta=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdhq INNER JOIN hrm_sales_seedsproduct sp ON sdhq.ProductId=sp.ProductId where HqRepEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sdhq.YearId=".$_REQUEST['y'], $con);
 $sqlP3ta=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdhq INNER JOIN hrm_sales_seedsproduct sp ON sdhq.ProductId=sp.ProductId where HqRepEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sdhq.YearId=".$AfterYId, $con);  
 $sqlP2tb=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaRepEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con); 
 $sqlP3tb=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaRepEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con);  
...
if($resSpEG['GradeValue']=='M2' OR $resSpEG['GradeValue']=='M3'){  
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$EmployeeId." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sda.YearId=".$_REQUEST['y'], $con);
$sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$EmployeeId." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sda.YearId=".$AfterYId, $con);
} elseif($resSpEG['GradeValue']=='M4' OR $resSpEG['GradeValue']=='M5'){ 
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_region sdr INNER JOIN hrm_sales_seedsproduct sp ON sdr.ProductId=sp.ProductId where RegionEmpID=".$EmployeeId." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con);
$sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_region sdr INNER JOIN hrm_sales_seedsproduct sp ON sdr.ProductId=sp.ProductId where RegionEmpID=".$EmployeeId." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con); }
}
else 
{
 if($_REQUEST['crop']==1 OR $_REQUEST['crop']==2)
 {
  ...
  $sqlP2tb=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaRepEmpID=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con); 
...
if($resSpEG['GradeValue']=='M2' OR $resSpEG['GradeValue']=='M3'){  
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaEmpID=".$EmployeeId." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con);
$sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaEmpID=".$EmployeeId." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con);
} elseif($resSpEG['GradeValue']=='M4' OR $resSpEG['GradeValue']=='M5'){ 
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_region sdr INNER JOIN hrm_sales_seedsproduct sp ON sdr.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where RegionEmpID=".$EmployeeId." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con);
$sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_region sdr INNER JOIN hrm_sales_seedsproduct sp ON sdr.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where RegionEmpID=".$EmployeeId." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con); }   
 }
 elseif($_REQUEST['crop']==3)
 { 
  if($rtl>0)
  {
  $sqlAta=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tl ON d.HqId=tl.TLHq where TLStatus='A' AND tl.TLRepId=".$EmployeeId." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND sd.YearId=".$_REQUEST['y'], $con); 
  $sqlBta=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tl ON d.HqId=tl.TLHq where TLStatus='A' AND tl.TLRepId=".$EmployeeId." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con);
  $resAta=mysql_fetch_assoc($sqlAta); $resBta=mysql_fetch_assoc($sqlBta);
  } 
   ...
   $sqlP3ta=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_seedsproduct sp ON sdh.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where HqRepEmpID=".$_REQUEST['mainv']." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND YearId=".$AfterYId, $con);  
   $sqlP2tb=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaRepEmpID=".$_REQUEST['mainv']." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con); 
   $sqlP3tb=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaRepEmpID=".$_REQUEST['mainv']." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND YearId=".$AfterYId, $con);  
   ...
   $sqlBt=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON ".$Ijoin." where DealerSts='A' AND (".$Wcond." OR rl.R1=".$_REQUEST['mainv']." OR rl.R2=".$_REQUEST['mainv']." OR rl.R3=".$_REQUEST['mainv']." OR rl.R4=".$_REQUEST['mainv']." OR rl.R5=".$_REQUEST['mainv'].") AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con); 
   
if($resSpEG['GradeValue']=='M2' OR $resSpEG['GradeValue']=='M3'){ $sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area  sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$EmployeeId." AND sp.ProductSts='A' AND sda.YearId=".$_REQUEST['y'], $con); $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area  sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$EmployeeId." AND sp.ProductSts='A' AND sda.YearId=".$AfterYId, $con);
} elseif($resSpEG['GradeValue']=='M4' OR $resSpEG['GradeValue']=='M5'){ $sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_region sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where RegionEmpID=".$EmployeeId." AND sp.ProductSts='A' AND sda.YearId=".$_REQUEST['y'], $con); $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_region sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where RegionEmpID=".$EmployeeId." AND sp.ProductSts='A' AND sda.YearId=".$AfterYId, $con); }     
...
$resP2tb=mysql_fetch_assoc($sqlP2tb); $resP3tb=mysql_fetch_assoc($sqlP3tb); 
...
$TotAt=$Pa1t+$Pa2t+$Pa3t+$Pa4t; $TotBt=$Pb1t+$Pb2t+$Pb3t+$Pb4t;

$rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);
$TRevAt=$rRevAt['Qa']+$rRevAt['Qb']+$rRevAt['Qc']+$rRevAt['Qd']; 
$TRevBt=$rRevBt['Qa']+$rRevBt['Qb']+$rRevBt['Qc']+$rRevBt['Qd'];

} ///////''if($_REQUEST['findV']=='AR' AND $rowRpt>0) Close 

if($_REQUEST['findV']=='AR' AND $rowRpt==0){  /////''if($_REQUEST['findV']=='AR' AND $rowRpt==0)
if($_REQUEST['ii']>0)
{ 

 $sql4tat=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ProductSts='A' AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$_REQUEST['y'], $con); 
 $sql5tat=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con); $res4tat=mysql_fetch_assoc($sql4tat); $res5tat=mysql_fetch_assoc($sql5tat); 

 $sqlAt=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sd.YearId=".$_REQUEST['y'], $con); 
 $sqlBt=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con); $resAt=mysql_fetch_assoc($sqlAt); $resBt=mysql_fetch_assoc($sqlBt);
 ...
 $sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con); 
 $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con); 
 $rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);
}
else 
{
 if($_REQUEST['crop']==1 OR $_REQUEST['crop']==2){
 $sql4tat=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND sd.YearId=".$_REQUEST['y'], $con); 
 $sql5tat=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con); 
 $res4tat=mysql_fetch_assoc($sql4tat); $res5tat=mysql_fetch_assoc($sql5tat); 

 $sqlAt=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ProductSts='A' AND si.GroupId=".$_REQUEST['crop']." AND sd.YearId=".$_REQUEST['y'], $con); 
 $sqlBt=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con); 
 $resAt=mysql_fetch_assoc($sqlAt); $resBt=mysql_fetch_assoc($sqlBt);
 $Pa1t=$resAt['sM1']+$resAt['sM2']+$resAt['sM3']; $Pa2t=$resAt['sM4']+$resAt['sM5']+$resAt['sM6']; 
 $Pa3t=$resAt['sM7']+$resAt['sM8']+$resAt['sM9']; $Pa4t=$resAt['sM10']+$resAt['sM11']+$resAt['sM12']; 
 ...
 $sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaEmpID=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$_REQUEST['y'], $con); 
 $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where AreaEmpID=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND sp.ProductSts='A' AND YearId=".$AfterYId, $con); 
 $rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);
}
 elseif($_REQUEST['crop']==3)
 { 
  $sql4tat=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND sd.YearId=".$_REQUEST['y'], $con); 
  $sql5tat=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ProductSts='A' AND (si.GroupId=1 OR si.GroupId=2) AND sd.YearId=".$AfterYId, $con); 
  $res4tat=mysql_fetch_assoc($sql4tat); $res5tat=mysql_fetch_assoc($sql5tat); 

  $sqlAt=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sp.ProductSts='A' AND (si.GroupId=1 OR si.GroupId=2) AND sd.YearId=".$_REQUEST['y'], $con); 
  $sqlBt=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND (si.GroupId=1 OR si.GroupId=2) AND sp.ProductSts='A' AND sd.YearId=".$AfterYId, $con); 
  $resAt=mysql_fetch_assoc($sqlAt); $resBt=mysql_fetch_assoc($sqlBt);
  $Pa1t=$resAt['sM1']+$resAt['sM2']+$resAt['sM3']; $Pa2t=$resAt['sM4']+$resAt['sM5']+$resAt['sM6']; 
  $Pa3t=$resAt['sM7']+$resAt['sM8']+$resAt['sM9']; $Pa4t=$resAt['sM10']+$resAt['sM11']+$resAt['sM12']; 
  $TotAt=$Pa1t+$Pa2t+$Pa3t+$Pa4t;
  ...
  $sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$_REQUEST['mainv']." AND sp.ProductSts='A' AND sda.YearId=".$_REQUEST['y'], $con); 
  $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area sda INNER JOIN hrm_sales_seedsproduct sp ON sda.ProductId=sp.ProductId where AreaEmpID=".$_REQUEST['mainv']." AND sp.ProductSts='A' AND sdaYearId=".$AfterYId, $con);  
  $rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);
  }
 }
...
<?php $TotP1t=$TotPp1+$TotPp2+$TotPp3+$TotPp4; 
      $TotP2t=$TotP2ta+$TotP2tb+$TotP2tc+$TotP2td; 
      $TotP3t=$TotP3ta+$TotP3tb+$TotP3tc+$TotP3td; ?>
<?php if($rowRpt>0){ ?>	  
...
if($rtl>0)
{
 ...
 $sqlBa=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp tl ON d.HqId=tl.TLHq where TLStatus='A' AND tl.TLRepId=".$EmployeeId." AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$AfterYId, $con);
 $resP1a=mysql_fetch_assoc($sqlP1a); $resAa=mysql_fetch_assoc($sqlAa); $resBa=mysql_fetch_assoc($sqlBa);
}

////$Gjoin='d.Terr_vc=g.EmployeeID';  $Wcond='d.Terr_vc='.$_REQUEST['mainv']; $Ijoin='d.Terr_vc=rl.EmployeeID';
//".$Ijoin." where DealerSts='A' AND (".$Wcond."

$sqlP1=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON ".$Ijoin." where DealerSts='A' AND (".$Wcond." OR rl.R1=".$_REQUEST['mainv']." OR rl.R2=".$_REQUEST['mainv']." OR rl.R3=".$_REQUEST['mainv']." OR rl.R4=".$_REQUEST['mainv']." OR rl.R5=".$_REQUEST['mainv'].") AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$BeforeYId, $con); 
$sqlP2=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_terr where TerrRepEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$_REQUEST['y'], $con); 
$sqlP3=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_terr where TerrRepEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$AfterYId, $con); 
$sqlP2a=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq where HqRepEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$_REQUEST['y'], $con); 
$sqlP3a=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq where HqRepEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$AfterYId, $con); 
$sqlP2b=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area where AreaRepEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$_REQUEST['y'], $con); 
$sqlP3b=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_area where AreaRepEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$AfterYId, $con); 

$sqlA=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON ".$Ijoin." where DealerSts='A' AND (".$Wcond." OR rl.R1=".$_REQUEST['mainv']." OR rl.R2=".$_REQUEST['mainv']." OR rl.R3=".$_REQUEST['mainv']." OR rl.R4=".$_REQUEST['mainv']." OR rl.R5=".$_REQUEST['mainv'].") AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$_REQUEST['y'], $con); 
$sqlB=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_reporting_level rl ON ".$Ijoin." where DealerSts='A' AND (".$Wcond." OR rl.R1=".$_REQUEST['mainv']." OR rl.R2=".$_REQUEST['mainv']." OR rl.R3=".$_REQUEST['mainv']." OR rl.R4=".$_REQUEST['mainv']." OR rl.R5=".$_REQUEST['mainv'].") AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$AfterYId, $con); 

$resA=mysql_fetch_assoc($sqlA); $resB=mysql_fetch_assoc($sqlB);
$resP1=mysql_fetch_assoc($sqlP1); $resP2=mysql_fetch_assoc($sqlP2); $resP3=mysql_fetch_assoc($sqlP3); 
...
$TotA=$Pa1+$Pa2+$Pa3+$Pa4; $TotB=$Pb1+$Pb2+$Pb3+$Pb4;

if($resSpEG['GradeValue']=='M2' OR $resSpEG['GradeValue']=='M3'){  
$sRevA=mysql_query("select Q1,Q2,Q3,Q4 from hrm_sales_sal_details_area where AreaEmpID=".$EmployeeId." AND ProductId=".$res['ProductId']." AND YearId=".$_REQUEST['y'], $con);
$sRevB=mysql_query("select Q1,Q2,Q3,Q4 from hrm_sales_sal_details_area where AreaEmpID=".$EmployeeId." AND ProductId=".$res['ProductId']." AND YearId=".$AfterYId, $con);
} elseif($resSpEG['GradeValue']=='M4' OR $resSpEG['GradeValue']=='M5'){ 
$sRevA=mysql_query("select Q1,Q2,Q3,Q4 from hrm_sales_sal_details_region where RegionEmpID=".$EmployeeId." AND ProductId=".$res['ProductId']." AND YearId=".$_REQUEST['y'], $con);
$sRevB=mysql_query("select Q1,Q2,Q3,Q4 from hrm_sales_sal_details_region where RegionEmpID=".$EmployeeId." AND ProductId=".$res['ProductId']." AND YearId=".$AfterYId, $con); }
$rRevA=mysql_fetch_assoc($sRevA); $rRevB=mysql_fetch_assoc($sRevB);

} //$_REQUEST['findV']=='AR' AND $rowRpt>0  CLOSE

if($_REQUEST['findV']=='AR' AND $rowRpt==0){  //($_REQUEST['findV']=='AR' AND $rowRpt>0)
 
$sqlP1=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$BeforeYId, $con); $resP1=mysql_fetch_assoc($sqlP1);

$sql4ta=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$_REQUEST['y'], $con); 
$sql5ta=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where DealerSts='A' AND ".$Wcond." AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$AfterYId, $con); 
...
$sqlA=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_employee_general g ON ".$Gjoin." where DealerSts='A' AND (".$Wcond." OR g.RepEmployeeID=".$_REQUEST['mainv'].") AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$_REQUEST['y'], $con); 
$sqlB=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_employee_general g ON ".$Gjoin." where DealerSts='A' AND (".$Wcond." OR g.RepEmployeeID=".$_REQUEST['mainv'].") AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$AfterYId, $con); 
$resA=mysql_fetch_assoc($sqlA); $resB=mysql_fetch_assoc($sqlB);
$Pa1=$resA['sM1']+$resA['sM2']+$resA['sM3']; $Pa2=$resA['sM4']+$resA['sM5']+$resA['sM6']; 
...
$sRevA=mysql_query("select * from hrm_sales_sal_details_area where AreaEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$_REQUEST['y'], $con); 
$sRevB=mysql_query("select * from hrm_sales_sal_details_area where AreaEmpID=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND YearId=".$AfterYId, $con); 
$rRevA=mysql_fetch_assoc($sRevA); $rRevB=mysql_fetch_assoc($sRevB);
} //$_REQUEST['findV']=='AR' AND $rowRpt==0  CLOSE
...
?>	
  ...
  <tr bgcolor="#EEEEEE" style="height:22px;font-size:12px;" id="TR<?php echo $Sn; ?>">  
     <td bgcolor="#FFFFFF" style="font-size:12px;"><input class="Inputt" style="width:100px;" value="<?php echo $res['ItemName']; ?>" /></td>		 
	 <td bgcolor="#FFFFFF" style="font-size:12px;"><input class="Inputt" style="width:120px;" value="<?php echo $res['ProductName']; ?>" /><input type="hidden" id="P_<?php echo $Sn; ?>" value="<?php echo $res['ProductId']; ?>" /></td>
...
<?php if($rowRpt>0){ ?>
<td align="right" bgcolor="#FFFF9B"><?php if($TotA!='' AND $TotA!=0){echo floatval($TotA);}else{echo '&nbsp;';} ?></td>
<td align="right" bgcolor="#FFFF9B"><?php if($TotB!='' AND $TotB!=0){echo floatval($TotB);}else{echo '&nbsp;';} ?></td>
<?php } ?>
<td align="right" bgcolor="#CAFF95"><?php if($TotP2!=0 AND $TotP2!=''){echo floatval($TotP2);}else{echo '&nbsp;';} ?>
<input type="hidden" id="TotP2_<?php echo $Sn; ?>" value="<?php if($TotP2!=0){echo floatval($TotP2);}else{echo 0;} ?>"/></td>
<td align="right" bgcolor="#CAFF95"><?php if($TotP3!=0 AND $TotP3!=''){echo floatval($TotP3);}else{echo '&nbsp;';} ?>
