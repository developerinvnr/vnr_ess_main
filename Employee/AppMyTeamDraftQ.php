...
if($_REQUEST['ac']=='CloseQuery' AND $_REQUEST['QCid']!='')
{
  ...
  $sqlDel=mysql_query("update hrm_employee_queryemp set QCloseStatus='Y', Level_2QStatus=3 where QueryId=".$_REQUEST['QCid'], $con); 
...
function ActionQReply(v,QI,NOfT,P,QEI,DI,SN,HYN) 
{ if(v=='P') {var agree=confirm("Are you sure you want to change status to In-Process?"); 
  if (agree) { var win = window.open("ReplyQueryApp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="AppMyTeamDraftQ.php?page="+P; } }, 1000);}}
  if(v=='R') {var agree=confirm("Are you sure you want to reply to this query...?"); 
  if (agree) { var win = window.open("ReplyQueryApp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="AppMyTeamDraftQ.php?page="+P; } }, 1000);}}
  if(v=='U') {var agree=confirm("Are you sure you want to update this query answer...?"); 
  if (agree) { var win = window.open("ReplyQueryApp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="AppMyTeamDraftQ.php?page="+P; } }, 1000);}}	
			   
  if(v=='C') { var QS=document.getElementById("QSubject_"+SN).value; var agree=confirm("Are you sure you want to close this query...?"); 
  if (agree){ window.location="AppMyTeamDraftQ.php?ac=CloseQuery&QCid="+QI+"&page="+P+"&QEI="+QEI+"&DI="+DI+"&QS="+QS+"&HYN="+HYN;} }			
  		   		    			   
  if(v=='F') {var agree=confirm("Are you sure you want to Forward to this query...?"); 
  if (agree) { var win = window.open("ReplyQueryApp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="AppMyTeamDraftQ.php?page="+P; } }, 1000);}}			   
...
<?php //*******************************************Status********** ?>       
		   ...
		   <tr>
			 <td>
			  <table border="0">
			   <tr>
             <?php if($rowApp>0) { ?>
             <td align="center"><a href="AppMyTeamDraftQ.php?page=1"><img id="Img_RepMgr1" style="display:none;" src="images/RepMgr1.png" border="0"/></a>
		     <img id="Img_RepMgr" style="display:block;" src="images/RepMgr.png" border="0"/></td><?php } ?>
			 <?php if($rowHod>0) { ?>
             <td align="center"><a href="HodMyTeamDraftQ.php?page=1"><img id="Img_Hod1" style="display:block;" src="images/RevHod1.png" border="0"/></a>
		...
		<td align="center" style="font-family:Times New Roman; color:#004A00;width:82px; font-size:15px;"><a href="javascript:window.location='AppMyTeamDraftQ.php?page=1'"><b>Refresh</b></a></td><td align="center" style="font-family:Times New Roman;width:120px;font-size:15px;"><a href="javascript:window.location='AppReplyQ.php?page=1'"><b style="color:#004A00;">Reply Query</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>	  
			   <tr>
			     <td align="center" id="QueryStatusTD" style="display:;width:1000px; margin-left:0px;" valign="top">
				    <table border="1">
					  <tr bgcolor="#7a6189" style="height:22px;">
		   <td style="color:#ffffff; font-family:Georgia; font-size:11px; width:100px;" align="center"><b>Opening Date</b></td>
		   <td style="color:#ffffff; font-family:Georgia; font-size:11px; width:100px;" align="center"><b>Closing Date</b></td>			  
...
<?php  $CurrDate=date('Y-m-d h:i:s'); $CurrDate=date('Y-m-d');
$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_2ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Level_2QStatus=0 OR Level_2QStatus=1 OR Level_2QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>=Level_2QToDT AND '".$CurrDate."'<Level_3QToDT order by QueryDT DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_2ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Level_2QStatus=0 OR Level_2QStatus=1 OR Level_2QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>=Level_2QToDT AND '".$CurrDate."'<Level_3QToDT order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);

if($total_records>0){ $Sno=1; while($resQuery=mysql_fetch_array($sql_statement)){ $sqlE=mysql_query("select EmpCode,Fname,Sname,Lname,DesigId,DepartmentId,DR,Gender,Married from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_employee.EmployeeID=".$resQuery['EmployeeID'], $con); 
...
?>
           <tr bgcolor="#FFFFFF" style="height:22px;"><input type="hidden" value="<?php echo $Sno; ?>" />
		   <td style="font-family:Times New Roman; font-size:12px; width:100px;" align="center" valign="top"><?php echo date("d-M-y", strtotime($resQuery['Level_2QToDT'])); ?></td>
		   <td style="font-family:Times New Roman; font-size:12px; width:100px;" align="center" valign="top"><?php echo date("d-M-y", strtotime($resQuery['Level_3QToDT'])); ?></td>
...
<?php $sqlD=mysql_query("select DepartmentCode from hrm_department where DepartmentId=".$resE['DepartmentId'], $con); $resD=mysql_fetch_assoc($sqlD);?>		   
		   ...
		   <a href="javascript:ReadQuery(<?php echo $resQuery['QueryId'].', '.$resQuery['EmployeeID']; ?>)">Read</a></td>
 		   <td style="font-family:Times New Roman; font-size:12px; width:80px;" align="center" valign="top"><?php if($resQuery['QStatus']==0){echo 'Draft';}elseif($resQuery['QStatus']==1){ echo '<font color="#FF2020">InProcess</font>'; } elseif($resQuery['QStatus']==2){ echo '<font color="#004000">Answer</font>'; } ?></td>
		   <td style="color:#ffffff; font-family:Georgia; font-size:12px; width:80px;" align="center" valign="top">
		   <select name="ActionQuery" id="ActionQuery" style="font-family:Georgia;font-size:12px;width:80px;height:20px;background-color:#DFFFBF;" onChange="ActionQReply(this.value,<?php echo $resQuery['QueryId'].', '.$resQuery['QueryNoOfTime'].', '.$_REQUEST['page'].', '.$resQuery['EmployeeID'].', '.$resQuery['QToDepartmentId'].', '.$Sno; ?>, '<?php echo $resQuery['HideYesNo']; ?>')"><option value="">Select</option>
		   <?php if($resQuery['QStatus']==0 OR $resQuery['QStatus']==1){ ?>
		   <option value="P">In-process</option><option value="R">Reply</option><?php if($resQuery['Level_2QFwdNoOfTime']<=1) { ?><option value="F">Forward</option><?php } ?>
		   <?php } elseif($resQuery['QStatus']==2 OR $resQuery['QCloseStatus']=='N'){ ?>
		   <option value="U">Update</option><option value="C">Close</option><?php } ?></select> </td>
		             </tr>	
<?php } } else { ?>
                     <tr>
...
<?PHP doPages($offset, 'AppMyTeamDraftQ.php', '', $total_records); ?>
</td>
</tr>					   
			 </table>
		   </td>
