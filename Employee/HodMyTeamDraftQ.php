...
{ if(v=='P') {var agree=confirm("Are you sure you want to change status to In-Process?"); 
  if (agree) { var win = window.open("ReplyQueryHod.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="HodMyTeamDraftQ.php?page="+P; } }, 1000);}}
  ...
  if (agree){ window.location="HodMyTeamDraftQ.php?ac=CloseQuery&QCid="+QI+"&page="+P+"&QEI="+QEI+"&DI="+DI+"&QS="+QS+"&HYN="+HYN;} }				   
...
<?php //*******************************************Status********** ?>       
		   ...
		   <tr>
			 <td>
			  <table border="0">
			   <tr>
             <?php if($rowApp>0) { ?>
             <td align="center"><a href="AppMyTeamDraftQ.php?page=1"><img id="Img_RepMgr1" style="display:block;" src="images/RepMgr1.png" border="0"/></a>
		     <img id="Img_RepMgr" style="display:none;" src="images/RepMgr.png" border="0"/></td><?php } ?>
			 <?php if($rowHod>0) { ?>
             <td align="center"><a href="HodMyTeamDraftQ.php?page=1"><img id="Img_Hod1" style="display:none;" src="images/RevHod1.png" border="0"/></a>
		...
		<td align="center" style="font-family:Times New Roman; color:#004A00;width:82px; font-size:15px;"><a href="javascript:window.location='HodMyTeamDraftQ.php?page=1'"><b>Refresh</b></a></td><td align="center" style="font-family:Times New Roman;width:120px;font-size:15px;"><a href="javascript:window.location='HodReplyQ.php?page=1'"><b style="color:#004A00;">Reply Query</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>	  
...
$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_3ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Level_3QStatus=0 OR Level_3QStatus=1 OR Level_3QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>=Level_3QToDT AND '".$CurrDate."'<Mngmt_QToDT order by QueryDT DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_3ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Level_3QStatus=0 OR Level_3QStatus=1 OR Level_3QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>=Level_3QToDT AND '".$CurrDate."'<Mngmt_QToDT order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
...
<?PHP doPages($offset, 'HodMyTeamDraftQ.php', '', $total_records); ?>
</td>
</tr>					   
			 </table>
		   </td>
