...
{ 
  if($_POST['t']==2){$t=$_POST['t'];}else if($_POST['t']==3){$t=3;}else{$t='';}  
  ...
  elseif($_POST['P']=='S' OR $_POST['P']=='S2' OR $_POST['P']=='S3'){ $Du=mysql_query("update hrm_employee_general set Covid_Vaccin".$t."='".$_POST['vv']."' where EmployeeID=".$_POST['E'],$con); }
 ...
 {  
  ...
  if((!empty($_FILES["Certificate"])) && ($_FILES['Certificate']['error'] == 0))
  {
   ...
   else
   {
	 ...
	 $newname = dirname(__FILE__).'/CovidCert/'.$filename; 
     if((move_uploaded_file($_FILES['Certificate']['tmp_name'],$newname)))
     { 
	   if($_POST['Person']=='S' || $_POST['Person']=='S2' || $_POST['Person']=='S3')
	   { $tbl='hrm_employee_general'; 
	     if($_POST['tt']==1){ $Qcon="Covid_Vaccin_file='".$filename."'"; }
		 elseif($_POST['tt']==2){ $Qcon="Covid_Vaccin2_file='".$filename."'"; }
		 else{ $Qcon="Covid_Vaccin3_file='".$filename."'"; }
		 $Q2con='1=1';
	   }	 
