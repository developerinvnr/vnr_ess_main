...
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, DesigCode, DesigName, GradeValue, DepartmentId, g.EmpVertical from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_designation de ON p.HR_CurrDesigId=de.DesigId INNER JOIN hrm_grade gr ON p.HR_CurrGradeId=gr.GradeId where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." AND g.DepartmentId=".$_REQUEST['FilD']." ".$ordn."", $con);
}
elseif($_REQUEST['FilD']>0 AND $_REQUEST['FilS']>0)
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, DesigCode, DesigName, GradeValue, DepartmentId, g.EmpVertical from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_designation de ON p.HR_CurrDesigId=de.DesigId INNER JOIN hrm_grade gr ON p.HR_CurrGradeId=gr.GradeId where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." AND g.DepartmentId=".$_REQUEST['FilD']." AND hq.StateId=".$_REQUEST['FilS']." ".$ordn."", $con);
}
elseif($_REQUEST['FilD']=='All' AND $_REQUEST['FilS']>0)
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, DesigCode, DesigName, GradeValue, DepartmentId, g.EmpVertical from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_designation de ON p.HR_CurrDesigId=de.DesigId INNER JOIN hrm_grade gr ON p.HR_CurrGradeId=gr.GradeId where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." AND hq.StateId=".$_REQUEST['FilS']." ".$ordn."", $con);
}
elseif(($_REQUEST['FilD']=='All' AND $_REQUEST['FilS']=='All') OR ($_REQUEST['FilD']=='' AND $_REQUEST['FilS']==''))
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, DesigCode, DesigName, GradeValue, DepartmentId, g.EmpVertical from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_designation de ON p.HR_CurrDesigId=de.DesigId INNER JOIN hrm_grade gr ON p.HR_CurrGradeId=gr.GradeId where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." ".$ordn."", $con);
}
...
{ 
 ...
 $sqVer=mysql_query("select Max(Max_GradeId) as MaxGrade from hrm_pms_vertical_increment where DepId=".$res['DepartmentId']." AND VerticalId=".$res['EmpVertical']."",$con); 
 $rowVer=mysql_num_rows($sqVer); $resVer=mysql_fetch_assoc($sqVer);
 if($rowVer>0 AND $resVer['MaxGrade']>0){ $MaxVGrade=$resVer['MaxGrade'];}
 else{ $MaxVGrade=0; }
}
else
{
...
?>
	...
	<tr style="height:24px;background-color:#FFFFFF;">
	 ...
	 <td class="tdc" style="background-color:<?php if($MaxVGrade>0 AND $res['HR_CurrGradeId']>=$MaxVGrade){echo '#FF0000';}else{echo '#FFFFFF';} ?>;"><?php echo $res['GradeValue']; ?></td>
...
<?php $sqlDm = mysql_query("SELECT DesigCode,DesigName from hrm_designation where DesigId=".$res['HR_CurrDesigId'],$con); 
	  ...
	  ?>  
     <td class="tdc" id="TD1<?php echo $sno; ?>" style="background-color:<?php echo $hodclrDe;?>;">
         
         
         <select class="tdsel" style="background-color:<?php echo $hodclrDe;?>;border:hidden;" id="DesigId_<?php echo $sno;?>" onChange="FunDesig(this.value,<?php echo $sno; ?>)" disabled><option value="<?php echo $res['Hod_EmpDesignation']; ?>"><?php echo $hodvalDe; ?></option><?php $sqlEx=mysql_query("select dgd.DesigId,DesigName,DesigCode from hrm_deptgradedesig dgd INNER JOIN hrm_designation de ON dgd.DesigId=de.DesigId where DepartmentId=".$res['DepartmentId']." AND (GradeId=".$NextGradeId." OR GradeId_2=".$NextGradeId." OR GradeId_3=".$NextGradeId." OR GradeId_4=".$NextGradeId." OR GradeId_5=".$NextGradeId." OR GradeId=".$res['HR_CurrGradeId']." OR GradeId_2=".$res['HR_CurrGradeId']." OR GradeId_3=".$res['HR_CurrGradeId']." OR GradeId_4=".$res['HR_CurrGradeId']." OR GradeId_5=".$res['HR_CurrGradeId'].") AND de.DesigStatus='A' order by DesigName ASC", $con); while($resEx=mysql_fetch_assoc($sqlEx)){ ?><option value="<?php echo $resEx['DesigId'];?>"><?php echo strtoupper($resEx['DesigName']); ?></option><?php } ?><option value="<?php echo $res['HR_CurrDesigId']; ?>"><?php echo strtoupper($resDm['DesigName']); ?></option>
...
<?php $sqlG = mysql_query("SELECT GradeValue FROM hrm_grade WHERE GradeId=".$res['Hod_EmpGrade'], $con);
	 ...
	 <td class="tdc" id="TD2<?php echo $sno; ?>" style="background-color:<?php echo $hodclrGr;?>;"><span id="SpanGarde_<?php echo $sno; ?>"><select class="tdsel" style="background-color:<?php echo $hodclrGr;?>;border:hidden;" id="GradeId_<?php echo $sno;?>" disabled><option value="<?php echo $res['Hod_EmpGrade']; ?>"><?php echo $hodvalGr; ?></option><option value="<?php echo $NextGradeId; ?>"><?php echo $NextGrade; ?></option><option value="<?php echo $res['HR_CurrGradeId']; ?>"><?php echo $resGm['GradeValue']; ?></option>
	    <option value="0"></option>
	 </select></span></td>	
