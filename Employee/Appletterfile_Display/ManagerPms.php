   ...
   <td style="width:100%;"><span id="MyTeamSpan"></span>
	...
	<thead>
	 <tr bgcolor="#7a6189" style="height:25px;">
	  ...
	  <?php if(($_SESSION['Dept']==6 AND $_SESSION['Grade']>=68) OR $EmployeeId==35 OR $EmployeeId==778 OR $EmployeeId==362 OR $EmployeeId==52 OR $EmployeeId==359 OR $EmployeeId==263 OR $EmployeeId==6 OR $EmployeeId==51 OR $EmployeeId==195 OR $EmployeeId==52 OR $EmployeeId==142 OR $EmployeeId==109 OR $EmployeeId==110 OR $EmployeeId==28 OR $EmployeeId==68 OR $EmployeeId==7 OR $EmployeeId==89 OR $EmployeeId==224 OR $EmployeeId==601 OR $EmployeeId==65 OR $EmployeeId==321 OR $EmployeeId==719 OR $EmployeeId==461 OR $EmployeeId==223 OR $EmployeeId==759){ ?>
...
{
$sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, DepartmentCode, DesigName, GradeValue, HqName, StateName, EmpPmsId, Emp_PmsStatus, Appraiser_PmsStatus from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_department d ON g.DepartmentId=d.DepartmentId INNER JOIN hrm_designation de ON g.DesigId=de.DesigId INNER JOIN hrm_grade gr ON g.GradeId=gr.GradeId INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_state st ON hq.StateId=st.StateId where e.EmpStatus='A' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.Appraiser_EmployeeID=".$EmployeeId, $con); 
}
elseif($_SESSION['eKraform']=='Y')
{
$sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, DepartmentCode, DesigName, GradeValue, HqName, StateName, EmpPmsId, Emp_PmsStatus, Appraiser_PmsStatus from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_department d ON g.DepartmentId=d.DepartmentId INNER JOIN hrm_designation de ON g.DesigId=de.DesigId INNER JOIN hrm_grade gr ON g.GradeId=gr.GradeId INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_state st ON hq.StateId=st.StateId where e.EmpStatus='A' AND p.AssessmentYear=".$_SESSION['KraYId']." AND p.Appraiser_EmployeeID=".$EmployeeId, $con);     
}
