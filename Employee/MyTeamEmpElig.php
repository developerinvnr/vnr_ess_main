...
<body class="body">
<?php 
$Sql=mysql_query("SELECT EmpCode,Fname,Sname,Lname,Married,Gender,DR FROM hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID WHERE hrm_employee.EmployeeID=".$_REQUEST['ei'], $con); $ResRep=mysql_fetch_array($Sql);
...
<?php //*************************************************************************************************************************************************** ?>	   
		     <table border="0" id="Activity">
			  <tr style="height:25px;font-family:Times New Roman;font-size:16px;" valign="middle">
			   <td>&nbsp;&nbsp;<b>[Eligibility]&nbsp;EC:<?php echo $ResRep['EmpCode'].'/ Name:'.$EmpName; ?></b></td>
			  </tr>
			  <tr>
	     ...
	     <table border="0" width="700">
			<tr>
			 <td>
			<table border="1">
<?php $SqlE=mysql_query("SELECT GradeId, EmpAddBenifit_MediInsu_value FROM hrm_employee_general INNER JOIN hrm_employee_ctc ON hrm_employee_general.EmployeeID=hrm_employee_ctc.EmployeeID WHERE hrm_employee_general.EmployeeID=".$_REQUEST['ei'], $con); $ResE=mysql_fetch_assoc($SqlE); 
...
$SqlEligEmp = mysql_query("SELECT * FROM hrm_employee_eligibility WHERE EmployeeID=".$_REQUEST['ei']." AND Status='A'", $con) or die(mysql_error());  $ResEligEmp=mysql_fetch_assoc($SqlEligEmp); 
?>
