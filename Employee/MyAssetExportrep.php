...
{ 
 ...
 if($_REQUEST['d']=='AllD') {  //----->>>(11)  
  if($_REQUEST['reqt']==1){   //---(C)
    ...
    if($_REQUEST['ast']>0)
    { 
	 if($_REQUEST['emp']>0){ $sqla2=mysql_query("select * from hrm_asset_employee_request where AssetNId=".$_REQUEST['ast']." AND PurDate>='".$fromdate."-04-01' AND PurDate<='".$todate."-03-31' AND Status=1 AND EmployeeID=".$_REQUEST['emp']." order by PurDate DESC", $con); }else{ $sqla2=mysql_query("select * from hrm_asset_employee_request where AssetNId=".$_REQUEST['ast']." AND PurDate>='".$fromdate."-04-01' AND PurDate<='".$todate."-03-31' AND Status=1 order by PurDate DESC", $con); } 
    }           
  }
...
{ 
  ...
  if($_REQUEST['reqt']==1){   //---(C)
    if($_REQUEST['ast']==0 OR $_REQUEST['ast']=='AllA')
    { 
	if($_REQUEST['emp']>0){ $sqla2=mysql_query("select * from hrm_asset_employee_request where Status=1 AND EmployeeID=".$_REQUEST['emp']." order by PurDate DESC", $con); }else{ $sqla2=mysql_query("select * from hrm_asset_employee_request where Status=1 order by PurDate DESC", $con); }
    }
    if($_REQUEST['ast']>0)
    {
...
if($resa2['PurDate']!='0000-00-00' AND $resa2['PurDate']!='1970-01-01'){$PurDate=date("d My",strtotime($resa2['PurDate']));}else{$PurDate='';} 
if($resa2['ReqDate']!='0000-00-00' AND $resa2['ReqDate']!='1970-01-01'){$ReqDate=date("d My",strtotime($resa2['ReqDate']));} else{$ReqDate='';}
$csv_output .= '"'.str_replace('"', '""', $PurDate).'",'; 
$csv_output .= '"'.str_replace('"', '""', $ReqDate).'",'; 

if($resa2['AccPayDate']!='0000-00-00' AND $resa2['AccPayDate']!='1970-01-01'){$PayDate=date("d My",strtotime($resa2['AccPayDate']));}else{$PayDate='';} 
if($resa2['AccSubDate']!='0000-00-00' AND $resa2['AccSubDate']!='1970-01-01'){$SubDate=date("d My",strtotime($resa2['AccSubDate']));} else{$SubDate='';}
$csv_output .= '"'.str_replace('"', '""', $PayDate).'",'; 
$csv_output .= '"'.str_replace('"', '""', $SubDate).'",';
...
if($resa2['AccPayDate']!='0000-00-00' AND $resa2['AccPayDate']!='1970-01-01'){ $ExpMDate=date('d My', strtotime("+36 months", strtotime($resa2['AccPayDate']))); } elseif($resa2['ReqDate']!='0000-00-00' AND $resa2['ReqDate']!='1970-01-01'){ $ExpMDate=date('d My', strtotime("+36 months", strtotime($resa2['ReqDate']))); } elseif($resa2['PurDate']!='0000-00-00' AND $resa2['PurDate']!='1970-01-01'){ $ExpMDate=date('d My', strtotime("+36 months", strtotime($resa2['PurDate']))); } else { $ExpMDate='00-00-0000';}  
$csv_output .= '"'.str_replace('"', '""', $ExpMDate).'",'; 
...
if($_REQUEST['yer']=='AllY')    ///////////////////////////////////////////////////(BBBBBBBBBBB)
{
  ...
  if($_REQUEST['allot']==1){  //---(B) 
    ...
    if($_REQUEST['ast']>0)
    {
	if($_REQUEST['emp']>0){ $sqla=mysql_query("select * from hrm_asset_employee where AssetNId=".$_REQUEST['ast']." AND EmployeeID=".$_REQUEST['emp']." order by APurDate DESC", $con); }else{ $sqla=mysql_query("select * from hrm_asset_employee where AssetNId=".$_REQUEST['ast']." order by APurDate DESC", $con); } 
	} 
  } 
...
if($resa['APurDate']!='0000-00-00' OR $resa['APurDate']!='1970-01-01'){$PurDate=date("d My",strtotime($resa['APurDate']));}else{$PurDate='';}  
$csv_output .= '"'.str_replace('"', '""', $PurDate).'",';  
