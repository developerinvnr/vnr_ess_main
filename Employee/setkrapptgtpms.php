  ...
  <td style="width:100%;" valign="top">
   <table align="center" border="0" cellspacing="1">
    ...
    <tr style="background-color:#CC9900;height:25px;">
	 ...
	 <td colspan="3" class="t"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ ?>Employee Achivement<?php }else{ ?>Sales Return Details<?php } ?></td>
	 <td colspan="3" class="t"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ ?>Appraiser Achivement<?php }else{ ?>Appraiser Sales Return<?php } ?></td>
...
<input type="hidden" name="wgt" id="wgt" value="<?php echo $_REQUEST['wgt']; ?>"/>
	...
	<tr style="background-color:#FFFFFF;height:24px;">
	 ...
	 <td class="d" align="center"><?php echo floatval($res['Wgt']); ?></td>
	 <td class="d" align="center"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ echo 100; }else{ echo '0%'; } ?><?php //echo $tgt; ?>
	 <input type="hidden" id="wgt<?php echo $i; ?>" name="wgt<?php echo $i; ?>" value="<?php echo floatval($res['Wgt']); ?>"/>
	 <input type="hidden" id="tgt<?php echo $i; ?>" name="tgt<?php echo $i; ?>" value="<?php echo floatval($res['Tgt']); ?>"/>
	 ...
	 <span onClick="FunEdit(<?php echo $i; ?>)"></span>
	 
	 </td>
	 
	 <td class="d" bgcolor="#FFFFC6" align="center"><?php if($res['Cmnt']!='' || $res['Ach']>0){ echo floatval($res['Ach']); }else{echo '';} ?>
	 ...
	 <td class="d" bgcolor="#D5FFAA" id="TD1<?php echo $i; ?>"><input type="text" name="Ach<?php echo $i; ?>" id="Ach<?php echo $i; ?>" class="d" style="width:50px;border:hidden;background-color:#D5FFAA;text-align:center;" onKeyUp="FunEnterAch(this.value,<?php echo $i; ?>)" onChange="FunEnterAch(this.value,<?php echo $i; ?>)" value="<?php if($res['AppCmnt']!='' || $res['AppAch']>0){ echo floatval($res['AppAch']); }else{echo '';} ?>" maxlength="4" onKeyPress="return isNumberKey(event)" readonly/></td>
	...
	<td align="center">
	 <?php $Nxt10Day=date("Y-m-d",strtotime('+30 day', strtotime($res['Ldate'])));  //date("2019-01-30") +14 day ?>
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ ?>
	 
	  <?php if($res['Ach']>0){ //if($res['lockk']>0) ?><span style="cursor:pointer"><img src="images/go-back-icon.png" style="display:<?php if($res['Applockk']==0 AND date("Y-m-d")<=$Nxt10Day){echo 'block';}else{echo 'none';} ?>;" onClick="funRevert(<?php echo $res['TgtDefId']; ?>)"></span><?php } ?>
	  
	  <?php //if($res['Applockk']==0 AND date("Y-m-d")<=$Nxt10Day){echo 'block';}else{echo 'none';} ?>
	  <?php //if($res['Applockk']==0){echo 'block';}else{echo 'none';} ?>
	  
	  
	  <span style="cursor:pointer"><img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($res['Applockk']==0 AND date("Y-m-d")<=$Nxt10Day) OR (($res['AppAch']==0 OR $res['AppAch']=='') AND $res['AppCmnt']=='')){echo 'block';}else{echo 'none';} ?>;" onClick="FunEdit(<?php echo $i; ?>)"/></span>
