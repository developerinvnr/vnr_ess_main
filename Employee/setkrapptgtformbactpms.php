...
{
  $sqlt=mysql_query("select SUM(AppAch) as tAch, SUM(AppLogScr) as tLogScr, SUM(AppScor) as tScor from hrm_pms_formb_tgtdefin where EmployeeID=".$_POST['eid']." AND YearId=".$_POST['yid']." AND FormBId=".$_POST['fbid'],$con); $rest=mysql_fetch_assoc($sqlt);
  echo '<input type="hidden" id="tAch" value="'.floatval($rest['tAch']).'" />';
...
{ 
  $sqlt=mysql_query("select SUM(AppAch) as tAch, SUM(AppLogScr) as tLogScr, SUM(AppScor) as tScor from hrm_pms_formb_tgtdefin where EmployeeID=".$_POST['eid']." AND YearId=".$_POST['yid']." AND FormBSubId=".$_POST['fbid'],$con); 
  $rest=mysql_fetch_assoc($sqlt);
  echo '<input type="hidden" id="tAch" value="'.floatval($rest['tAch']).'" />';
  echo '<input type="hidden" id="tLogScr" value="'.floatval($rest['tLogScr']).'" />';
  echo '<input type="hidden" id="tScor" value="'.floatval($rest['tScor']).'" />';
