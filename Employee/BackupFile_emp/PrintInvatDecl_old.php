...
function PrintIvestDecl(P,EI,M,CI,YI) 
{window.open("InvestDeclPrint.php?action=Print&EI="+EI+"&YI="+YI+"&CI="+CI+"&P="+P+"&M="+M,"PrintForm","location=no,scrollbars=no,resizable=no,menubar=no,width=820,height=750");}
</script>
</head>
<body>
...
$sql2=mysql_query("select * from hrm_investdecl_setting_limit where CompanyId=".$CI." AND Period='".$_REQUEST['P']."' AND Month=".$_REQUEST['M'], $con); $res2=mysql_fetch_array($sql2);
...
<td align="center" valign="top" style="width:90px;font-size:15px;"><?php if($res2['Handi_Limit']>0){echo intval($res['Handi_Limit']);}else{ echo '&nbsp;'; } ?></td>
