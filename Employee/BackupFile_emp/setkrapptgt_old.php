 ...
 <tr>
  <td style="width:100%;" valign="top">
   <table align="center" border="0" cellspacing="1">
    ...
    <tr><td>&nbsp;</td></tr>
	<tr><td colspan="15" style="font-size:20px;" align="center"><font color="#000000"><u>Note</u>:&nbsp;</font><font class="msg_r"><span class="blink_me">Please enter achievement numbers as per the <u><?php echo $_REQUEST['prd']; ?></u> broken targets and not the complete annual target</span></font></td></tr>
...
<?php $sql=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?>	 
	...
	<td align="center">
<?php $Nxt14Day=date("Y-m-d",strtotime('+14 day', strtotime($res['Ldate']))); //date("2018-12-14") //$res['Ldate'] ?>
	 <?php if($res['lockk']>0){ ?><span style="cursor:pointer"><img src="images/go-back-icon.png" onClick="funRevert(<?php echo $res['TgtDefId']; ?>)"></span><?php } ?>
	 </td>  
	  
	<td align="center">
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ ?>
	  <span style="cursor:pointer"><img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($res['Wgt']>0 AND $res['Tgt']>0 AND $res['Applockk']==0 AND date("Y-m-d")<=$Nxt14Day) OR ($res['Wgt']>0 AND $res['Tgt']>0 AND $res['Applockk']==0 AND $rpms['UnLckKRA']==1)){echo 'block';}else{echo 'none';} ?>" onClick="FunEdit(<?php echo $i; ?>)"/></span>
