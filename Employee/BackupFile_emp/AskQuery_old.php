...
{ 
  ...
  if($EmpAssign!=9999){$AssigEmp=$resSub['AssignEmpId'];}
...
if($resR['DR']=='Y'){$M2='Dr.';} elseif($resR['Gender']=='M'){$M2='Mr.';} elseif($resR['Gender']=='F' AND $resR['Married']=='Y'){$M2='Mrs.';} elseif($resR['Gender']=='F' AND $resR['Married']=='N'){$M2='Miss.';}  $RName=$M2.' '.$resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'];  
 $sqlH=mysql_query("select AppraiserId,HodId from hrm_employee_reporting where EmployeeID=".$AssigEmp, $con); $resH=mysql_fetch_assoc($sqlH); 
 ...
 $SqlQIns=mysql_query("insert into hrm_employee_queryemp(EmployeeID, RepMgrId, HodId, QToDepartmentId, QuerySubject, OtherSubject, AssignEmpId, HideYesNo, QueryValue, QueryDT, QueryNoOfTime, MailTo_Emp, MailTo_QueryOwner, Level_1ID, Level_1QToDT, Level_2ID, Level_2QToDT, Level_3ID, Level_3QToDT, Mngmt_ID, Mngmt_QToDT, QueryYearId)values(".$EmployeeId.", ".$_POST['RepID'].", ".$_POST['HodID'].", '".$_POST['DeptName']."', '".$QSubject."', '".$OthQSub."', ".$AssigEmp.", '".$_POST['VCheck']."', '".$EmpQ."', '".date("Y-m-d h:i:s")."', 1, 1, 1, ".$AssigEmp.", '".date("Y-m-d h:i:s")."', ".$RepId.", '".$EmpDay."', ".$HodId.", '".$AppDay."', ".$MnmtId.", '".$HodDay."', ".$YearId.")", $con); 
...
$sqlMK=mysql_query("select * from hrm_employee_querymail_key where QueryMailId=1 AND CompanyId=".$CompanyId, $con); $resMK=mysql_fetch_assoc($sqlMK);  
   ...
   {
    //$email_toT = "ajaykumar.dewangan@vnrseeds.in";
	$email_toT = $resR['EmailId_Vnr'];
    $email_fromT='admin@vnrseeds.co.in';
    //if($_POST['EmailSelf']==''){$email_fromT = $_POST['Ename'];} else {$email_fromT=$_POST['EmailSelf'];}
	...
	$email_messageT .= "<b>You</b> need to answer query in 3 working days after which it will get escalated to next level.<br><br>\n\n\n";
	$email_messageT .= "From <br><b>ADMIN ESS</b><br>";
	$email_messageT .="</body></html>";	
    $ok = @mail($email_toT, $email_subjectT, $email_messageT, $headersT);
	//echo 'TT='.$email_toT.' '.$email_subjectT.' '.$email_messageT.'<br><br>';
   }  
   ...
   if($_POST['EmailRep']!='' AND $resMK['ReportingMgr']=='Y')
   {
    ...
    //if($_POST['EmailSelf']==''){$email_fromR = $_POST['Ename'];} else {$email_fromR=$_POST['EmailSelf'];}
	...
	//echo 'RR='.$email_toR.' '.$email_subjectR.' '.$email_messageR.'<br><br>';
   }   
   ...
   if($_POST['EmailHod']!='' AND $resMK['HOD']=='Y')
   {
    ...
    //if($_POST['EmailSelf']==''){$email_fromH = $_POST['Ename'];} else {$email_fromH=$_POST['EmailSelf'];}
	...
	//echo 'HH='.$email_toH.' '.$email_subjectH.' '.$email_messageH.'<br><br>';
   }

//HR
   if($_POST['SubValue']==0 AND $resMK['HR']=='Y')
   {
    //$email_toHR = "ajaykumar.dewangan@vnrseeds.in";
...
<?php $sqlD=mysql_query("select DepartmentId,DepartmentCode from hrm_department where CompanyId=".$CompanyId." AND DepartmentId!=2 AND DepartmentId!=4 AND DepartmentId!=6 AND DepartmentId!=26 AND DepartmentId!=17 AND DepartmentId!=18 AND DeptStatus='A' order by DepartmentCode ASC", $con); 
...
<tr height="20">
 ...
 Do you want to hide your name from Reporting Mgr & HOD?&nbsp;<input type="checkbox" name="HideCheck" id="HideCheck" disabled/>
