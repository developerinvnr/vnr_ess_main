...
<?php if($rowApp>0) { ?><td align="center"><a href="ALeave.php"><img id="Img_manager1" style="display:none;" src="images/RepMgr1.png" border="0"/></a>
		     <img id="Img_manager" style="display:block;" src="images/RepMgr.png" border="0"/></td><?php } ?>
<?php if($rowHod>0) { ?><td align="center"><a href="ALeaveToHOD.php"><img id="Img_hod1" style="display:block;" src="images/RevHod1.png" border="0"/></a>
...
$sql_statement = mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp!=1 AND LeaveActionApp!=2 AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp!=1 AND LeaveActionApp!=2 AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC LIMIT " . $from . "," . $offset, $con);

//$sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp!=1 AND LeaveActionApp!=2 AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC", $con); 
