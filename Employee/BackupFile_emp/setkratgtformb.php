...
{ 
 ...
 { 
  if(document.getElementById("ChkEdit"+i).value==1)
  {
...
function FunSave(i,id)
{
 ...
 else
 {
  ...
  var url = 'setkratgtformbact.php'; var pars = 'ach='+ach+'&cmnt='+cmt+'&id='+id+'&i='+i+'&fn='+fn+'&fbid='+fbid+'&scor='+Scor+'&logscr='+LogScr;
...
function FunLk(i,id)
{
  ...
  if(agree)
  {
   var url = 'setkratgtformbact.php'; var pars = 'lockk=OkLock&id='+id+'&i='+i;
...
<?php //$sqle=mysql_query("select EmpCode,Fname,Sname,Lname from hrm_employee where EmployeeID=".$_REQUEST['e'],$con);
    ...
    $sqle=mysql_query("select EmpCode,Fname,Sname,Lname,DepartmentId from hrm_employee e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID where e.EmployeeID=".$_REQUEST['e'],$con);
...
<input type="hidden" id="NtgtN<?php echo $i; ?>" name="NtgtN<?php echo $i; ?>" value="<?php echo $i; ?>"/></td> 
	 ...
	 <td class="d" bgcolor="#D5FFAA" id="TD1<?php echo $i; ?>"><input class="input" name="Ach<?php echo $i; ?>" id="Ach<?php echo $i; ?>" class="d" style="width:50px;border:hidden;background-color:#D5FFAA;text-align:center;" onKeyUp="FunEnterAch(this.value,<?php echo $i; ?>)" onChange="FunEnterAch(this.value,<?php echo $i; ?>)" value="<?php if($res['Cmnt']!='' || $res['Ach']>0){ echo floatval($res['Ach']); }else{ echo ''; } ?>" maxlength="4" onKeyPress="return isNumberKey(event)" readonly/></td>
	 ...
	 <?php if($res['Tital']!='' AND $res['Tgt']!=''){ //AND $_REQUEST['prd']=='Monthly' ?>
	  <span style="cursor:pointer"><img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($res['lockk']==0 AND date("Y-m-d")<=$Nxt10Day) OR ($res['lockk']==0 AND $rpms['UnLckKRA']==1)){echo 'block';}else{echo 'none';} ?>" onClick="FunEdit(<?php echo $i; ?>)"/></span>
	 ...
	 <span style="cursor:pointer;"><img src="images/sub.png" border="0" id="lockk_<?php echo $i; ?>" style="display:<?php if($res['Ach']!='' AND $res['Cmnt']!=='' AND $res['lockk']==0 AND date("Y-m-d")<=$Nxt10Day){echo 'block';}else{echo 'none';} ?>;" onClick="FunLk(<?php echo $i.','.$res['TgtFbDefId']; ?>)"/></span>
	 </td>
	</tr>
    <?php } ?>
	<tr style="height:24px;">
	 ...
	 <?php $sqlt=mysql_query("select SUM(Ach) as tAch, SUM(LogScr) as tLogScr, SUM(Scor) as tScor from hrm_pms_formb_tgtdefin where ".$fn."=".$_REQUEST['fbid']." AND EmployeeID=".$_REQUEST['e']." AND YearId=".$_REQUEST['yid'],$con); 
 ...
 <tr>
  <td style="font-size:18px;color:#FFA275;">
   <?php 
	...
	echo '&nbsp;&nbsp;&nbsp;';
    echo ' The achivement is required to be entered on the last day or within few days beyand which the Skill will set auto locked.';
   ?>
