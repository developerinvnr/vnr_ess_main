...
{
 $sqlUp=mysql_query("update hrm_asset_employee_request set FwdApprovalStatus='".$_REQUEST['v']."', FwdRemark='".$_REQUEST['FRmk']."', FwdSubDate='".date("Y-m-d")."' where AssetEmpReqId=".$_REQUEST['RId']." AND EmployeeID=".$_REQUEST['EId'], $con); 
 ...
 if($sqlUp)
 {
  if($_REQUEST['v']==2 OR $_REQUEST['v']==3)
  {
   ...
   $email_subject = $Ename." asset request ".$EMsg." by first level";
   $email_txt = $Ename." asset request ".$EMsg." by first level";
   $headers = "From: ".$email_from."\r\n"; 
   $semi_rand = md5(time()); 
   $headers .= "MIME-Version: 1.0\r\n";
   $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
   $email_message .="<html><head></head><body>";
   $email_message .=$Ename." asset request ".$EMsg." by first level, kindly details log on to ESS <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>.<br>\n\n";
   ...
   $sqlItEmp=mysql_query("select Fname,Sname,Lname,Gender,DR,Married,EmailId_Vnr,RepEmployeeID from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_employee.EmployeeID=".$resIT['EmployeeID'], $con); $resItEmp=mysql_fetch_assoc($sqlItEmp); if($resItEmp['DR']=='Y'){$MMIT='Dr.';} elseif($resItEmp['Gender']=='M'){$MMIT='Mr.';} elseif($resItEmp['Gender']=='F' AND $resItEmp['Married']=='Y'){$MMIT='Mrs.';} elseif($resItEmp['Gender']=='F' AND $resItEmp['Married']=='N'){$MMIT='Miss.';} $ITEmpName=$MMIT.' '.$resItEmp['Fname'].' '.$resItEmp['Sname'].' '.$resItEmp['Lname'];
	 ...
	 if($resItEmp['EmailId_Vnr']!='')
	 {
      ...
      $email_subject22 = $Ename." asset request ".$EMsg." first level";
      $email_txt22 = $Ename." asset request ".$EMsg." first level"; 
      ...
      $email_message22 .=$Ename." asset request ".$EMsg." by first level, kindly details & your approval next level, log on to ESS <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>.<br><br>\n\n";
...
{ window.open("MyAssetEmpAnyFieldReqFwd.php?CheckAct=ExtraReqField&ID="+ID+"&Aeri="+Aeri,"AForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=600,height=400"); }

function FunClickHistoryReq(ID)
{ window.open("MyAssetEmpCHistoryReq.php?CheckAct=ExtraReqField&ID="+ID,"AForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=1000,height=400"); }

function ChangeStatus(v,RId,EId,empid,i)
{ 
  var FwdRemark=document.getElementById("FwdRemark"+i).value; var FFwdRemark=FwdRemark.replace(/[^a-zA-Z 0-9.,]+/g,'');
  if(v==1)
  { var agree=confirm("Are you sure you want to pending this request status?"); 
    if(agree){ var x="MyAssetFwdReq.php?act=true&mm=sas&mm=er4e&wew=e%e@er%rdd=012&Action=RequestSt&v="+v+"&RId="+RId+"&EId="+EId+"&FRmk="+FFwdRemark; window.location=x; }
  }
  if(v==2)
  { 
    if(FwdRemark.length===0){ alert("Please type comment field.."); document.getElementById("ActionReply"+i).value=0; return false; }
    var agree=confirm("Are you sure you want to approved this request status?"); 
    if(agree){ var x="MyAssetFwdReq.php?act=true&mm=sas&we=12345&mm=er4e&r=t5t%t5s&yy=eloi&Action=RequestSt&v="+v+"&RId="+RId+"&EId="+EId+"&FRmk="+FFwdRemark; window.location=x; }
  }
  if(v==3)
  { 
    if(FwdRemark.length===0){ alert("Please type comment field.."); document.getElementById("ActionReply"+i).value=0; return false; }
    var agree=confirm("Are you sure you want to reject this request status?"); 
    if(agree){ var x="MyAssetFwdReq.php?act=true&ask=false&rigt=checkessue&mailto=promt&Action=RequestSt&v="+v+"&RId="+RId+"&EId="+EId+"&FRmk="+FFwdRemark; window.location=x; }
  }
}
	     ...
	     <table border="0">
		    <tr>
			 ...
			 <td width="800" class="TableHead">
			   <font color="#106809" style='font-family:Times New Roman;' size="4"><i><b>Delegate Asset Request </b></i></font>
			   &nbsp;&nbsp;&nbsp;&nbsp;
...
<tr bgcolor="#7a6189" style="height:22px;">
  ...
  <td colspan="2" style="color:#FFFFFF;font-family:Times New Roman;font-size:12px;" align="center"><b>Amount</b></td>
  ...
  <td rowspan="2" style="width:200px;color:#FFFFFF;font-family:Times New Roman;font-size:12px;" align="center"><b>Comment</b></td>
...
<?php $sql_statement = mysql_query("select * from hrm_asset_employee_request where FwdEmpId=".$EmployeeId." order by ReqDate DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10; 
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}
$sql_statement = mysql_query("select * from hrm_asset_employee_request where FwdEmpId=".$EmployeeId." order by ReqDate DESC LIMIT " . $from . "," . $offset, $con);
...
<td align="center" style="font-size:12px; font-family:Times New Roman;color:<?php if($res['FwdApprovalStatus']==1){echo '#FF8C1A';}elseif($res['FwdApprovalStatus']==2){echo '#008200';}elseif($res['FwdApprovalStatus']==3){echo '#FF0000';} ?>;"><?php if($res['FwdApprovalStatus']==0){echo 'Draft';}elseif($res['FwdApprovalStatus']==1){echo 'Pending';}elseif($res['FwdApprovalStatus']==2){echo 'Approved';}elseif($res['FwdApprovalStatus']==3){echo 'Rejected';} ?></td>

  <td align="center">
   <input style="font-size:12px;font-family:Times New Roman;width:200px;" name="FwdRemark<?php echo $sno; ?>" id="FwdRemark<?php echo $sno; ?>" value="<?php echo $res['FwdRemark']; ?>" maxlength="200" />
  </td>
  <td width="100" class="TableHead1" align="center" style="font-size:12px;font-family:Times New Roman;"> 		   
<?php if($res['FwdApprovalStatus']!=2 AND $res['FwdApprovalStatus']!=3){ ?>		   
...
<option value="0">Select</option><?php if($res['FwdApprovalStatus']!=1){ ?><option value="1" style="background-color:#FFFFFF;">PENDING</option><?php } ?>
...
<?PHP doPages($offset, 'MyAssetFwdReq.php', '', $total_records); ?>
</td>
</tr>			
		 </table>
	           </td>
