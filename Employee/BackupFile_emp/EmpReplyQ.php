...
{ var QS=document.getElementById("QSubject_"+SN).value; var agree=confirm("Are you sure you want to close this query...?"); 
  if (agree) {window.location="EmpReplyQ.php?ac=CloseQuery&QCid="+i+"&page="+P+"&QEI="+QEI+"&DI="+DI+"&QS="+QS+"&HYN="+HYN;}}
</script>
</head>
<body class="body">
<table class="table">
	 ...
	 <table border="0" id="Activity">
	  <tr>
		 <td id="Anni" valign="top">
			<table border="0">
				<tr height="20"><td align="left" valign="top" width="130"><?php echo "<img width=105px height=125px src=\"show_images.php?id=".$EmployeeId."\">\n";?></td></tr>
			 </table>
		 </td>
	 </tr>
   </table>
...
<?php //*******************************************Status********** ?>       
		   ...
		   <tr>
			 <td>
			  <table border="0">
			   <tr>
		<td style="font-family:Times New Roman; color:#004A00; font-size:16px;width:800px;" align=""><b><u>Reply Query</u></b></td>
		<td align="center" style="font-family:Times New Roman; color:#004A00;width:82px; font-size:15px;"><a href="javascript:window.location='EmpReplyQ.php?page=1'"><b>Refresh</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>	  
...
$sql_statement = mysql_query("select * from hrm_employee_queryemp where QStatus=2 AND Level_1ID=".$EmployeeId." AND (Level_1QStatus=2 OR Level_1QStatus=3) order by QueryDT DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp where QStatus=2 AND Level_1ID=".$EmployeeId." AND (Level_1QStatus=2 OR Level_1QStatus=3) order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
