...
function CloseL(i)
{window.location="ALeave.php?ac=CloseLeave&LCid="+i;}
function DeleteLC(LId)
{var agree=confirm("Are you sure you want to delete this leave...?"); if (agree) { var x="ALeave.php?action=DelClose&ac=CloseLeave&L="+LId; window.location=x; }}
</script>
</head>
<body class="body">
<table class="table">
  ...
  <td style="width:100%;">
   ...
   <table border="0" style="width:90%;">
	<tr>
	 <td colspan="10">
	 <table border="0"><tr>
     <?php if($rowApp>0) { ?><td align="center"><a href="ALeave.php"><img id="Img_manager1" style="display:none;" src="images/RepMgr1.png" border="0"/></a><img id="Img_manager" style="display:block;" src="images/RepMgr.png" border="0"/></td><?php } ?>
     <?php if($rowHod>0 OR $rowRev>0) { ?><td align="center"><a href="ALeaveToHOD.php"><img id="Img_hod1" style="display:block;" src="images/RevHod1.png" border="0"/></a><img id="Img_hod" style="display:none;" src="images/RevHod.png" border="0"/></td><?php } ?>
	 <td>&nbsp;</td>
	 ...
	 <?php $CFromDate='2013-04-01'; $CToDate=date("Y").'-12-31';
		   $sqlLC=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp=1 AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC", $con); $rowLC=mysql_num_rows($sqlLC);?>
<td valign="top" style="width:150px;" align="right"><?php if($rowLC>0){?>			
	 <?php if($_REQUEST['ac']=='CloseLeave') { ?>
	 <a href="#" onClick="javascript:window.location='ALeave.php'"><font color="#464600" style='font-family:Times New Roman;' size="3"><b>Refresh</b></font></a>		             <?php } else { ?>
...
$sql_statement = mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp!=1 AND LeaveActionApp!=2 AND Apply_FromDate>='".$CFromDate."' order by Apply_Date DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp!=1 AND LeaveActionApp!=2 AND Apply_FromDate>='".$CFromDate."' order by Apply_Date DESC LIMIT " . $from . "," . $offset, $con);

//$sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND LeaveAppStatus=2 AND LeaveEmpCancelStatus='N' AND LeaveActionApp!=1 AND LeaveActionApp!=2 AND Apply_FromDate>='".$CFromDate."' order by Apply_Date DESC", $con); 
...
<?PHP doPages($offset, 'ALeave.php', '', $total_records); ?>
</td>
</tr>				
		 </table>
	           </td>
