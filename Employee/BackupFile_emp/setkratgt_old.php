...
<?php $sql=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?>		
	 ...
	 <td align="center" bgcolor="#FFFFC6" id="TDt<?php echo $i; ?>"><input type="text" class="d" style="width:60px;height:40px;border:hidden;text-align:center;background-color:#FFFFC6;" id="tgt<?php echo $i; ?>" name="tgt<?php echo $i; ?>" value="<?php if($row>0){echo floatval($res['Tgt']);}elseif($PerM==1){echo floatval($tgt);} ?>" readonly />
<input type="hidden" id="NtgtN<?php echo $i; ?>" name="NtgtN<?php echo $i; ?>" value="<?php echo $i; ?>"/></td> 
	 ...
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ //AND $_REQUEST['prd']=='Monthly' ?>
	  <span style="cursor:pointer"><img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($PerM==1 AND $res['lockk']==0 AND date("Y-m-d")<=$Nxt10Day) OR ($PerM==1 AND $res['lockk']==0 AND $rpms['UnLckKRA']==1)){echo 'block';}else{echo 'none';} ?>" onClick="FunEdit(<?php echo $i; ?>)"/></span>
