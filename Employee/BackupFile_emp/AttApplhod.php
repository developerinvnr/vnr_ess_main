 ...
 <tr>
  ...
  <table border="0" style="width:90%;">
		    <tr>
			 ...
			 <?php if($rowApp>0) { ?><td align="center"><a href="AttApplRep.php?m=<?php echo $_REQUEST['m']; ?>&Y=<?php echo $_REQUEST['Y'];?>&e=4e&w=234&tt=10234&aa=f&e=4e2&e=4e&retd=ee&wwrew=t%T@sed818&d=101&a=1"><img style="display:block;" src="images/RepMgr1.png" border="0"/></a></td><?php } ?>
...
<?php if($_REQUEST['a']==0){$a='All Status';}elseif($_REQUEST['a']==1){$a='Pending';}elseif($_REQUEST['a']==2){$a='Submitted';} ?>			 
            ...
            <tr bgcolor="#7a6189" style="height:22px;">
			 ...
			 <td class="th" style="width:80px;">Details</td>
		   </tr>
<?php   
if($_REQUEST['a']==0){$query="select * from hrm_employee_attendance_req where AttDate>='".date("Y-01-01")."' AND AttDate<='".date("Y-12-31")."' AND (HId=".$EmployeeId." OR HtId=".$EmployeeId.")"; }elseif($_REQUEST['a']==1){$query="select * from hrm_employee_attendance_req where AttDate>='".date("Y-01-01")."' AND AttDate<='".date("Y-12-31")."' AND Status=0 AND (HId=".$EmployeeId." OR HtId=".$EmployeeId.")"; }elseif($_REQUEST['a']==2){$query="select * from hrm_employee_attendance_req where AttDate>='".date("Y-01-01")."' AND AttDate<='".date("Y-12-31")."' AND Status=1 AND HId=".$EmployeeId;}
...
if($_REQUEST['a']==0){$query2="select * from hrm_employee_attendance_req where AttDate>='".date("Y-01-01")."' AND AttDate<='".date("Y-12-31")."' AND (HId=".$EmployeeId." OR HtId=".$EmployeeId.") order by AttDate DESC LIMIT " . $from . "," . $offset; }elseif($_REQUEST['a']==1){ $query2="select * from hrm_employee_attendance_req where AttDate>='".date("Y-01-01")."' AND AttDate<='".date("Y-12-31")."' AND Status=0 AND (HId=".$EmployeeId." OR HtId=".$EmployeeId.") order by AttDate DESC LIMIT " . $from . "," . $offset; }elseif($_REQUEST['a']==2){$query2="select * from hrm_employee_attendance_req where AttDate>='".date("Y-01-01")."' AND AttDate<='".date("Y-12-31")."' AND Status=1 AND (HId=".$EmployeeId." OR HtId=".$EmployeeId.") order by AttDate DESC LIMIT " . $from . "," . $offset; }
