 ...
 <tr>
  <td style="width:100%;" valign="top">
   <table align="center" border="0" cellspacing="1">
    <tr><td colspan="16" class="h"><u>Details</u></td></tr>
...
<?php $sql=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?>	 
	 ...
	 <td class="d" align="center"><?php if($_REQUEST['lgc']!='Logic6' AND $_REQUEST['lgc']!='Logic7'){ if($row>0){echo 100;} }else{ echo '0%'; } ?><?php //else{echo floatval($tgt);} ?>
	 ...
	 <?php /*?><td class="d" bgcolor="#D5FFAA" id="TD1<?php echo $i; ?>" valign="top"><textarea type="text" name="Cmnt<?php echo $i; ?>" id="Cmnt<?php echo $i; ?>" class="d" rows="2" style="width:200px;border:hidden;background-color:#D5FFAA;"><?php echo $res['RevCmnt']; ?></textarea></td> 
