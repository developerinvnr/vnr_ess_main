...
<?php //*******************************************Status********** ?>       
		   ...
		   <tr>
			 <td>
			  <table border="0">
			   <tr>
             <?php if($rowApp>0) { ?>
             <td align="center"><a href="AppMyTeamReplyQ.php?page=1"><img id="Img_RepMgr1" style="display:block;" src="images/RepMgr1.png" border="0"/></a>
		     <img id="Img_RepMgr" style="display:none;" src="images/RepMgr.png" border="0"/></td><?php } ?>
			 <?php if($rowHod>0) { ?>
             <td align="center"><a href="HodMyTeamReplyQ.php?page=1"><img id="Img_Hod1" style="display:none;" src="images/RevHod1.png" border="0"/></a>
		...
		<td align="center" style="font-family:Times New Roman; color:#004A00;width:82px; font-size:15px;"><a href="javascript:window.location='HodMyTeamReplyQ.php?page=1'"><b>Refresh</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>	  
...
$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_3ID=".$EmployeeId." AND QStatus=2 AND Level_3QStatus=2 order by QueryDT DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_3ID=".$EmployeeId." AND QStatus=2 AND Level_3QStatus=2 order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
...
<?PHP doPages($offset, 'HodMyTeamReplyQ.php', '', $total_records); ?>
</td>
</tr>					   
			 </table>
		   </td>
