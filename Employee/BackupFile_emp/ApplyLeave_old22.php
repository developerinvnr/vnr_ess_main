   ...
   $result=mysql_query("INSERT INTO hrm_employee_applyleave(EmployeeID,Apply_Date,Leave_Type,Apply_FromDate,Apply_ToDate,Apply_TotalDay,Apply_Reason,Apply_ContactNo,Apply_DuringAddress,Apply_SentToApp,Apply_SentToRev,Apply_SentToHOD,SLHodApp) VALUES(".$EmployeeId.",'".date('Y-m-d')."','".$_POST['LeaveType']."','".$AppFromDate."','".$AppToDate."','".$TotDays."','".$Reason."',".$_POST['ContactNo'].",'".$_POST['DuAdd']."',".$_POST['AppId'].",".$_POST['RevId'].",".$_POST['HodId'].",'".$SLHodApp."')", $con);
    ...
    {  
     if(($_POST['EmailRep']!='' AND $_POST['LeaveType']!='SL') OR ($_POST['EmailRep']!='' AND $_POST['LeaveType']=='SL' AND $TotsSL<=10))
     {
	  ...
	  $email_message .= $_POST['Ename']." has submitted leave application for <b>".$_POST['LeaveType']."</b> from <b>".$_POST['FromDate']."</b> to <b>".$_POST['ToDate']."</b>, reason stated as <b>" .$Reason. "</b>.<br> Login in ESS to Approve/Reject or click on the following buttons:<br><br>\n\n";
	 ...
	 if($_POST['LeaveType']=='SL' AND $TotsSL>10 AND $_POST['EmailHod']!='')
     {
	  ...
	  //$email_toa = 'ajaykumar.dewangan@vnrseeds.in';
	  /* ---------------------------------------------------------------------------------- */
	  ...
	  $email_messagea .="<html><head></head><body>";
      $email_messagea .=$_POST['Ename']." has applied SL from ".$_POST['FromDate']." to ".$_POST['ToDate']." for critical illness/long sickness. <br> Login in ESS to Approve/Reject or click on the following buttons.<br><br>\n\n";
	  $email_message .="<a href='https://www.vnrseeds.co.in/hrims/lvapi.php?viewto=".base64_encode('A')."&totd=".base64_encode($TotDays)."&ei=".base64_encode($EmployeeId)."&fd=".base64_encode($_POST['FromDate'])."&td=".base64_encode($_POST['ToDate'])."&lp=".base64_encode($_POST['LeaveType'])."&aplydt=".base64_encode(date('Y-m-d'))."&eType=".base64_encode('Hod')."&yi=".base64_encode($YearId)."' style='text-decoration:none;cursor:pointer;'><input type='button' value='Accept' style='width:90px;height:25px;' /></a>&nbsp;&nbsp;&nbsp;<a href='https://www.vnrseeds.co.in/hrims/lvapi.php?viewto=".base64_encode('R')."&totd=".base64_encode($TotDays)."&ei=".base64_encode($EmployeeId)."&fd=".base64_encode($_POST['FromDate'])."&td=".base64_encode($_POST['ToDate'])."&lp=".base64_encode($_POST['LeaveType'])."&aplydt=".base64_encode(date('Y-m-d'))."&eType=".base64_encode('Hod')."&yi=".base64_encode($YearId)."' style='text-decoration:none;cursor:pointer;'><input type='button' value='Reject' style='width:90px;height:25px;' /></a><br><br>\n\n";
	  $email_message .= " If rejected, please submit the reason.<br><br>\n\n";
	  $email_messagea .="From,<br>ADMIN ESS<br>";
	  $email_messagea .="</body></html>";
	  $ok = @mail($email_toa, $email_subjecta, $email_messagea, $headersa);
	  /* ---------------------------------------------------------------------------------- */
	  
	  
	  $email2_toa = 'vspl.attendance@gmail.com';  //vspl.hr@vnrseeds.com
	  //$email2_toa = 'ajaykumar.dewangan@vnrseeds.in';
      ...
      $headersa2 = "From: ".$email2_froma."\r\n"; 
      $semi_randa2 = md5(time()); 
      $headersa2 .= "MIME-Version: 1.0\r\n";
      $headersa2 .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
...
<td style='width:113px;'><input name="FromDate" id="FromDate" class="InputText" style="width:112px;text-align:center;height:22px;" readonly>
<script type="text/javascript">  var cal = Calendar.setup({ onSelect:  function(cal) { cal.hide()}, showTime: true }); 
cal.manageFields("FromDate", "FromDate", "%d-%m-%Y");</script></td>
...
<td style='width:113px;'><input name="ToDate" id="ToDate" class="InputText" style="width:112px;text-align:center;height:22px;" readonly>
<script type="text/javascript">  var cal = Calendar.setup({ onSelect:  function(cal) { cal.hide()}, showTime: true }); 
cal.manageFields("ToDate", "ToDate", "%d-%m-%Y");</script></td>
...
<td style='width:113px;'><select name="LeaveType" id="LeaveType" class="InputText" style="width:112px;height:22px;" onChange="CheckHoliD2(this.value)">
...
$sqlERev=mysql_query("select EmailId_Vnr from hrm_employee_general where EmployeeID=".$resApp['ReviewerId'], $con); $resERev=mysql_fetch_assoc($sqlERev);
?>			 
...
<td>&nbsp;<input type="hidden" name="AppId" id="AppId" value="<?php if($resApp['AppraiserId']==''){ echo '0'; } else {echo $resApp['AppraiserId'];} ?>" />
       ...
       <input type="hidden" name="EmailRev" value="<?php echo $resERev['EmailId_Vnr']; ?>" />
...
if(date("d")!=$date AND date("Y-m-d")!='2017-09-29'){ 
?>
...
<?php if(date("d")==$date OR date("Y-m-d")=='2017-09-29'){ ?>
<tr>
  <td></td>
...
<?php if($rowCheck>0) { $Sn=1; while($resCheck=mysql_fetch_array($sqlCheck)) { ?>
		   ...
		   <td style="color:#ffffff; font-family:Georgia; font-size:11px; width:150px;" align="center">
			...
			<?php } elseif($resCheck['LeaveEmpCancelStatus']=='Y' AND $resCheck['LeaveCancelStatus']=='N'){?><font color="#FF8000">Applied For Cancelation</font> 
...
<td colspan="9" valign="top">
 <table cellpadding="0" cellspacing="2">
  <tr bgcolor="#FFFFFF" style="height:22px;">
  <?php $sqlChk2=mysql_query("select * from hrm_employee_attendance_".date("Y")." where EmployeeID=".$EmployeeId." AND AttDate>='".date("Y")."-01-01' AND AttDate<'".$ExpMonthDate."' AND AttValue!='P' AND AttValue!='A' AND AttValue!='OD' AND AttValue!='HO' AND AttValue!='HF' order by AttDate ASC", $con); $rowChk2=mysql_num_rows($sqlChk2); 
...
<tr bgcolor="#FFFFFF" style="height:22px;">
  <?php $sqlChk2=mysql_query("select * from hrm_employee_attendance where EmployeeID=".$EmployeeId." AND AttDate>='".$ExpMonthDate."' AND AttDate<='".date("Y")."-12-31' AND AttValue!='P' AND AttValue!='A' AND AttValue!='OD' AND AttValue!='HO' AND AttValue!='HF' order by AttDate ASC", $con); $rowChk2=mysql_num_rows($sqlChk2); 
if($rowChk2>0){ $counter=0; while($resChk2=mysql_fetch_array($sqlChk2)){ 
$chk3=mysql_query("select * from hrm_employee_applyleave where '".$resChk2['AttDate']."'>=Apply_FromDate AND '".$resChk2['AttDate']."'<=Apply_ToDate AND Leave_Type='".$resChk2['AttValue']."' AND EmployeeID=".$EmployeeId,$con); $rowchk3=mysql_num_rows($chk3);
if($rowchk3==0){ $counter=$counter+1; ?>
