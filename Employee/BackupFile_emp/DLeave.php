...
{ $Fday=date("d",strtotime($_REQUEST['FD'])); $Fmonth=date("m",strtotime($_REQUEST['FD'])); $Fyear=date("Y",strtotime($_REQUEST['FD']));
  ...
  {  
   ...
   { 
     $sqlUp=mysql_query("update hrm_employee_applyleave set LeaveStatus=2, LeaveAppStatus=".$_REQUEST['v'].", LeaveAppUpDate='".date("Y-m-d")."', LeaveHodStatus=".$_REQUEST['v'].", LeaveHodUpDate='".date("Y-m-d")."', ApplyLeave_UpdatedYearId=".$YearId." where ApplyLeaveId=".$_REQUEST['lid'], $con);  
...
$sql_statement = mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND SLHodApp='N' AND (LeaveAppStatus=0 OR LeaveAppStatus=1) AND LeaveEmpCancelStatus='N' AND Apply_FromDate>='".$CFromDate."' order by Apply_Date DESC", $con);
...
$sql_statement = mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND SLHodApp='N' AND (LeaveAppStatus=0 OR LeaveAppStatus=1) AND LeaveEmpCancelStatus='N' AND Apply_FromDate>='".$CFromDate."' order by Apply_Date DESC LIMIT " . $from . "," . $offset, $con);
//So, here is the result array which will have all the rows to display on to page
	  //$sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToApp=".$EmployeeId." AND (LeaveStatus=0 OR LeaveStatus=1) AND LeaveEmpCancelStatus='N' AND Apply_FromDate>='".$CFromDate."' order by Apply_Date DESC", $con); 
	  ...
	  $sqlch=mysql_query("select * from hrm_employee_separation where EmployeeID=".$resL['EmployeeID']." AND Rep_Approved!='C' AND Hod_Approved!='C' AND HR_Approved!='C'", $con); $rowch=mysql_num_rows($sqlch);
	   ?>								  					
		   <tr style="background-color:<?php if($rowch>0){echo '#FFFF6C';}elseif($resL['Leave_Type']=='PL'){echo '#71B8FF';}else{echo '#FFFFFF';} ?>;">
			...
			<td class="tdc"><?php if($resL['Apply_FromDate']<date("Y-m-01") && $resL['Apply_ToDate']<date("Y-m-01")){echo 'Month Closed';} elseif($resL['LeaveAppStatus']==0){echo 'Draft';}if($resL['LeaveAppStatus']==1){echo '<font color="#E87400">Pending</font>';} ?></td>	
		    <td class="tdc"> 
