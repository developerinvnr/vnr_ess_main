...
<?php $lday=sprintf('%02d',$day); 
	  ...
	  if($showBtn==1 AND $rE['AttValue']!='HO' AND $rE['AttValue']!='CL' AND $rE['AttValue']!='PL' AND $rE['AttValue']!='SL' AND $rE['AttValue']!='EL' AND $rE['AttValue']!='FL' AND $rE['AttValue']!='COV' AND date("w",strtotime(date($y."-".$m."-".$day)))!=0)
	  { 
	   //date($y.'-'.$m.'-'.$day)>=date("Y-m-d") OR
	   if(($rE['AttValue']=='' OR $rE['AttValue']=='OD' OR $rE['AttValue']=='A' OR $rE['InnLate']==1 OR $rE['OuttLate']==1) AND $resCer['TimeApply']=='Y'){ ?><a style="float:right;" href="#" onClick="AttReq(<?php echo $EmployeeId.','.$day.','.$m.','.$y; ?>)"><img src="images/control.png" style="width:12px;height:12px;" style="margin-right:3px;"/></a>
	  <?php } 
