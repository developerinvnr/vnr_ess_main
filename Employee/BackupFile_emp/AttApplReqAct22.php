...
{   
	...
	$op=mysql_query("select OpeningCL,OpeningPL,CreditedCL,CreditedPL from hrm_employee_monthlyleave_balance where EmployeeID=".$_POST['ei']." AND Month='".date("m",strtotime($_POST['RDate']))."' AND Year='".date("Y",strtotime($_POST['RDate']))."'", $con); $rowp=mysql_num_rows($op); $rop=mysql_fetch_array($op); 
