...
{ window.open("MyAssetEmpAnyFieldReqAc.php?CheckAct=ExtraReqField&ID="+ID+"&Aeri="+Aeri,"AForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=600,height=400"); }
...
{ 
 if(v=='S'){ var n=document.getElementById("vname").value; var c=document.getElementById("vcode").value; window.location="MyAssetNextitLevelAct.php?MyAssetNextitLevelAct.php?act=true&we=12345&mm=er4e&r=t5t%t5s&yy=eloi&gosto=false&rigt=checkessue&mailto=promt&wew=e%e@er%rdd=012&page=1&chk=352&chk2=109&try=S&nm="+n+"&cd="+c; }
 else{ window.location="MyAssetNextitLevelAct.php?MyAssetNextitLevelAct.php?act=true&we=12345&mm=er4e&r=t5t%t5s&yy=eloi&gosto=false&rigt=checkessue&mailto=promt&wew=e%e@er%rdd=012&page=1&chk=352&chk2=109"; }
}</script>
...
<table border="1" cellspacing="0" style="width:100%;">
 <tr bgcolor="#7a6189" style="height:22px;">
  ...
  <td rowspan="2" style="width:15%;" class="th">IT Comment</td>
 </tr>		   
 <tr bgcolor="#7a6189" style="height:22px;">
...
{ 
 if($_REQUEST['nm']!=''){ $sql_statement = mysql_query("select r.* from hrm_asset_employee_request r inner join hrm_employee e on r.EmployeeID=e.EmployeeID where (e.Fname like '%".$_REQUEST['nm']."%' OR e.Sname like '%".$_REQUEST['nm']."%' OR e.Lname like '%".$_REQUEST['nm']."%') AND (ITApprovalStatus=2 OR ITApprovalStatus=3) order by ReqDate DESC", $con); }elseif($_REQUEST['cd']>0){ $sql_statement = mysql_query("select r.* from hrm_asset_employee_request r inner join hrm_employee e on r.EmployeeID=e.EmployeeID where e.EmpCode=".$_REQUEST['cd']." AND ITApprovalStatus=2 AND (AccPayStatus=2 OR AccPayStatus=3) order by ReqDate DESC", $con); }
}
else{ $sql_statement = mysql_query("select * from hrm_asset_employee_request where (ITApprovalStatus=2 OR ITApprovalStatus=3) order by ReqDate DESC", $con); }
...
if($_REQUEST['try']=='S')
{ 
 if($_REQUEST['nm']!=''){ $sql_statement = mysql_query("select r.* from hrm_asset_employee_request r inner join hrm_employee e on r.EmployeeID=e.EmployeeID where (e.Fname like '%".$_REQUEST['nm']."%' OR e.Sname like '%".$_REQUEST['nm']."%' OR e.Lname like '%".$_REQUEST['nm']."%') AND (ITApprovalStatus=2 OR ITApprovalStatus=3) order by ReqDate DESC LIMIT " . $from . "," . $offset, $con); }elseif($_REQUEST['cd']>0){ $sql_statement = mysql_query("select r.* from hrm_asset_employee_request r inner join hrm_employee e on r.EmployeeID=e.EmployeeID where e.EmpCode=".$_REQUEST['cd']." AND ITApprovalStatus=2 AND (AccPayStatus=2 OR AccPayStatus=3) order by ReqDate DESC LIMIT " . $from . "," . $offset, $con); } 
}
else{ $sql_statement = mysql_query("select * from hrm_asset_employee_request where (ITApprovalStatus=2 OR ITApprovalStatus=3) order by ReqDate DESC LIMIT " . $from . "," . $offset, $con); }
...
<tr bgcolor="#FFFFFF">
 ...
 <td class="tdc" style="color:<?php if($res['HODApprovalStatus']==1){echo '#FF8C1A';}elseif($res['HODApprovalStatus']==2){echo '#008200';}elseif($res['HODApprovalStatus']==3){echo '#FF0000';} ?>;"><?php if($res['HODApprovalStatus']==0){echo 'Draft';}elseif($res['HODApprovalStatus']==1){echo 'Pending';}elseif($res['HODApprovalStatus']==2){echo 'Approved';}elseif($res['HODApprovalStatus']==3){echo 'Reject';} if($res['HODSubDate']!='1970-01-01' && $res['HODSubDate']!='0000-00-00'){echo '&nbsp;<font color="#006FDD">'.date("d-My",strtotime($res['HODSubDate'])).'</font>'; } ?></td>
...
<?PHP doPages($offset, 'MyAssetNextitLevelAct.php', '', $total_records); ?>
</td>
</tr>			
		 </table>
	           </td>
