...
<?php $sqlK=mysql_query("select * from hrm_employee_pms_kraforma where EmpPmsId=".$_SESSION['ePmsId']." AND KRAFormAStatus='A' order by KRAFormAId ASC", $con); $Sno=1; while($resK=mysql_fetch_array($sqlK)){ 
 ...
 <tr bgcolor="#FFFFFF" style="height:40px;"> 	   
  ...
  <td class="fbody" align="center"><span style="text-decoration:<?php if($resK['Period']!='Annual' AND $resK['Period']!=''){ echo 'underline'; }else{echo 'none';}?>;color:<?php if($resK['Period']!='Annual' AND $resK['Period']!=''){ echo '#000099'; }else{echo '#000';}?>;cursor:<?php if($resK['Period']!='Annual' AND $resK['Period']!=''){echo 'pointer';} ?>;" <?php if($resK['Period']!='Annual' AND $resK['Period']!=''){?> onClick="FunTgt(<?php echo $Sno.','.$resK['KRAId']; ?>,'<?php echo $resK['Period']; ?>',<?php echo $resK['Target'].','.intval($resK['Weightage']); ?>,'<?php echo $resK['Logic']; ?>')" <?php } ?> ><?php if($rowSubK>0){echo '';}else{echo $resK['Target'];}?></span></td>
