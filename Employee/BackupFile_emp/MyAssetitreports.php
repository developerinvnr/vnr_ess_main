...
$SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['DepartmentId']; ?>"><?php echo $ResDept['DepartmentCode'];?></option><?php } ?><option value="AllD">All DEPARTMENT</option></select>
  ...
  <?php if($_REQUEST['d']>0){ ?>
   ...
   <td style="width:5px;">&nbsp;</td><td style="font-size:12px;font-family:Times New Roman;font-weight:bold;"><input type="radio" id="allotc" name="allotc" <?php if($_REQUEST['allot']==1){ echo 'checked'; } ?> onClick="FunAllot()" />&nbsp;ALLOTMENT</td>
   <td style="width:5px;">&nbsp;</td><td style="font-size:12px;font-family:Times New Roman;font-weight:bold;"><input type="radio" id="reqtc" name="reqtc" <?php if($_REQUEST['reqt']==1){ echo 'checked'; } ?> onClick="FunRequest()" />&nbsp;REQUEST</td>
