...
$sql=mysql_query("select EmpCode,Fname,Sname,Lname,EmpPmsId,Dummy_EmpRating,Dummy_AppRating,Dummy_RevRating,Dummy_HodRating,DepartmentId,HR_CurrDesigId from hrm_employee_pms INNER JOIN hrm_employee ON hrm_employee_pms.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_general ON hrm_employee_pms.EmployeeID=hrm_employee_general.EmployeeID where hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." AND hrm_employee_pms.CompanyId=".$_REQUEST['CI']." AND hrm_employee.EmpStatus='A' AND hrm_employee_general.DateJoining<='".$Yydate."' AND hrm_employee_pms.Appraiser_EmployeeID!=0 order by EmpCode ASC", $con); $no=1; while($res=mysql_fetch_array($sql)) {
 ?> 
