...
<?php if($rowHod>0 OR $rowRev>0) { ?><td align="center"><a href="DLeaveToHOD.php"><img id="Img_hod1" style="display:none;" src="images/RevHod1.png" border="0"/></a>
            ...
            <tr bgcolor="#7a6189" style="height:22px;">
			 ...
			 <td width="80" class="TableHead" align="center"><b>Status Appraiser</b></td> 
			 <td width="80" class="TableHead" align="center"><b>Status HOD</b></td>
...
{
$sql_statement = mysql_query("select * from hrm_employee_applyleave where (Leave_Type='PL' OR (Leave_Type='SL' AND SLHodApp='Y')) AND (Apply_SentToHOD=".$EmployeeId." OR Apply_SentToRev=".$EmployeeId.") AND (LeaveStatus=0 OR LeaveStatus=1) AND (LeaveHodStatus=0 OR LeaveHodStatus=1) AND LeaveEmpCancelStatus='N' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}
$sql_statement = mysql_query("select * from hrm_employee_applyleave where (Leave_Type='PL' OR (Leave_Type='SL' AND SLHodApp='Y')) AND (Apply_SentToHOD=".$EmployeeId." OR Apply_SentToRev=".$EmployeeId.") AND (LeaveStatus=0 OR LeaveStatus=1) AND (LeaveHodStatus=0 OR LeaveHodStatus=1) AND LeaveEmpCancelStatus='N' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC LIMIT " . $from . "," . $offset, $con);
}
elseif($rowRev>0)
{
$sql_statement = mysql_query("select * from hrm_employee_applyleave where Leave_Type='PL' AND Apply_SentToRev=".$EmployeeId." AND (LeaveStatus=0 OR LeaveStatus=1) AND (LeaveHodStatus=0 OR LeaveHodStatus=1) AND LeaveEmpCancelStatus='N' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}
$sql_statement = mysql_query("select * from hrm_employee_applyleave where Leave_Type='PL' AND Apply_SentToRev=".$EmployeeId." AND (LeaveStatus=0 OR LeaveStatus=1) AND (LeaveHodStatus=0 OR LeaveHodStatus=1) AND LeaveEmpCancelStatus='N' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_Date DESC LIMIT " . $from . "," . $offset, $con);
}


//So, here is the result array which will have all the rows to display on to page 
	  //$sqlCheck=mysql_query("select * from hrm_employee_applyleave where Apply_SentToHOD=".$EmployeeId." AND (LeaveStatus=0 OR LeaveStatus=1) AND LeaveEmpCancelStatus='N' AND (Apply_Date>='".$CFromDate."' AND Apply_Date<='".$CToDate."') order by Apply_FromDate ASC", $con); 
      ...
      $resE=mysql_fetch_assoc($sqlE); $EmpName=$resE['Fname'].' '.$resE['Sname'].' '.$resE['Lname']; ?>								  					
		   <tr <?php if($resL['Leave_Type']=='PL'){ echo 'bgcolor="#71B8FF"'; } else {echo 'bgcolor="#FFFFFF"';}?>>
			...
			<td width="80" class="TableHead1" align="center" bgcolor="#FFFFFF"><?php if($resL['LeaveAppStatus']==0){echo 'Draft';}elseif($resL['LeaveAppStatus']==1){echo '<font color="#E87400">Pending</font>';}elseif($resL['LeaveAppStatus']==2){echo '<font color="#005900">Approved</font>';}?></td>	
			<td width="80" class="TableHead1" align="center" bgcolor="#FFFFFF"><?php if($resL['Leave_Type']=='PL'){if($resL['LeaveHodStatus']==0){echo 'Draft';}elseif($resL['LeaveHodStatus']==1){echo '<font color="#E87400">Pending</font>';} } else {echo '&nbsp;';}?></td>
