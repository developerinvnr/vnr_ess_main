...
{
 $sqlUp=mysql_query("update hrm_asset_employee_request set ITId=".$EmployeeId.", ITApprovalStatus='".$_REQUEST['v']."', ITRemark='".$_REQUEST['ITRmk']."', ITSubDate='".date("Y-m-d")."' where AssetEmpReqId=".$_REQUEST['RId']." AND EmployeeID=".$_REQUEST['EId'], $con); 
 ...
 if($sqlUp)
 {
  if($_REQUEST['v']==2 OR $_REQUEST['v']==3)
  {
   ...
   $email_subject = $Ename." asset request ".$EMsg." by IT department";
   $email_txt = $Ename." asset request ".$EMsg." by IT department";
   $headers = "From: ".$email_from."\r\n"; 
   $semi_rand = md5(time()); 
   $headers .= "MIME-Version: 1.0\r\n";
   $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
   $email_message .="<html><head></head><body>";
   $email_message .=$Ename." asset request ".$EMsg." by IT department, kindly details log on to ESS <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>.<br><br>\n\n";
   $email_message .= "From <br><b>ADMIN ESS</b><br>";
   $email_message .="</body></html>";	      
   $ok = @mail($email_to, $email_subject, $email_message, $headers); 
   
   $sqlAc=mysql_query("select EmployeeID from hrm_asset_dept_access where DepartmentId=8", $con); $resAc=mysql_fetch_assoc($sqlAc);
   $sqlAcEmp=mysql_query("select Fname,Sname,Lname,Gender,DR,Married,EmailId_Vnr,RepEmployeeID from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_employee.EmployeeID=".$resAc['EmployeeID'], $con); $resAcEmp=mysql_fetch_assoc($sqlAcEmp); if($resAcEmp['DR']=='Y'){$MMAc='Dr.';} elseif($resAcEmp['Gender']=='M'){$MMAc='Mr.';} elseif($resAcEmp['Gender']=='F' AND $resAcEmp['Married']=='Y'){$MMAc='Mrs.';} elseif($resAcEmp['Gender']=='F' AND $resAcEmp['Married']=='N'){$MMAc='Miss.';} $AcEmpName=$MMIT.' '.$resAcEmp['Fname'].' '.$resAcEmp['Sname'].' '.$resAcEmp['Lname'];
   
	 if($resAcEmp['EmailId_Vnr']!='')
	 {
	  $email_to22 = $resAcEmp['EmailId_Vnr'];
      $email_from22='admin@vnrseeds.co.in';
      $email_subject22 = $Ename." asset request ".$EMsg." by IT department";
      $email_txt22 = $Ename." asset request ".$EMsg." by IT department"; 
	  ...
	  $email_message22 .= "Dear <b>".$AcEmpName."</b> <br><br>\n\n\n";
      $email_message22 .=$Ename." asset request ".$EMsg." by IT department, kindly details & your approval next level, log on to ESS <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>.<br><br>\n\n";
...
{ window.open("MyAssetEmpAnyFieldReqIt.php?CheckAct=ExtraReqField&ID="+ID+"&Aeri="+Aeri,"AForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=600,height=400"); }
...
{ 
  var ITRemark=document.getElementById("ITRemark"+i).value; var FITRemark=ITRemark.replace(/[^a-zA-Z 0-9.,]+/g,''); 
  if(v==1)
  { var agree=confirm("Are you sure you want to pending this request status?"); 
    if(agree){ var x="MyAssetNextitLevel.php?act=true&mm=sas&mm=er4e&wew=e%e@er%rdd=012&Action=RequestSt&v="+v+"&RId="+RId+"&EId="+EId+"&ITRmk="+FITRemark; window.location=x; }
  }
  if(v==2)
  { 
    if(ITRemark.length===0){ alert("Please type comment field.."); document.getElementById("ActionReply"+i).value=0; return false; }	
    var agree=confirm("Are you sure you want to approved this request status?"); 
    if(agree){ var x="MyAssetNextitLevel.php?act=true&mm=sas&we=12345&mm=er4e&r=t5t%t5s&yy=eloi&Action=RequestSt&v="+v+"&RId="+RId+"&EId="+EId+"&ITRmk="+FITRemark; window.location=x; }
  }
  if(v==3)
  { 
    if(ITRemark.length===0){ alert("Please type comment field.."); document.getElementById("ActionReply"+i).value=0; return false; }
    var agree=confirm("Are you sure you want to reject this request status?"); 
    if(agree){ var x="MyAssetNextitLevel.php?act=true&ask=false&rigt=checkessue&mailto=promt&Action=RequestSt&v="+v+"&RId="+RId+"&EId="+EId+"&ITRmk="+FITRemark; window.location=x; }
  }
}
...
<?php //echo "<img width=105px height=125px src=\"show_images.php?id=".$EmployeeId."\">\n";?></td>
  ...
  <td rowspan="2" style="width:100px;color:#FFFFFF;font-family:Times New Roman;font-size:12px;" align="center"><b>First Level</b></td>
  <td rowspan="2" style="width:100px;color:#FFFFFF;font-family:Times New Roman;font-size:12px;" align="center"><b>Second Level</b></td>
  <td rowspan="2" style="width:150px;color:#FFFFFF;font-family:Times New Roman;font-size:12px;" align="center"><b>Comment</b></td>
...
<?php $sql_statement = mysql_query("select * from hrm_asset_employee_request where (HODApprovalStatus=2 OR FwdApprovalStatus=2) AND Status=1 AND (ITApprovalStatus=0 OR ITApprovalStatus=1) order by ReqDate DESC", $con);
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10; 
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}
$sql_statement = mysql_query("select * from hrm_asset_employee_request where (HODApprovalStatus=2 OR FwdApprovalStatus=2) AND (ITApprovalStatus=0 OR ITApprovalStatus=1) AND Status=1 order by ReqDate DESC LIMIT " . $from . "," . $offset, $con);
...
<td align="center" style="font-size:12px; font-family:Times New Roman;color:<?php if($res['ITApprovalStatus']==1){echo '#FF8C1A';}elseif($res['ITApprovalStatus']==2){echo '#008200';}elseif($res['ITApprovalStatus']==3){echo '#FF0000';} ?>;"><?php if($res['ITApprovalStatus']==0){echo 'Draft';}elseif($res['ITApprovalStatus']==1){echo 'Pending';}elseif($res['ITApprovalStatus']==2){echo 'Approved';}elseif($res['ITApprovalStatus']==3){echo 'Rejected';} ?></td>
  ...
  <td align="center">
   <input style="font-size:12px;font-family:Times New Roman;width:160px;" name="ITRemark<?php echo $sno; ?>" id="ITRemark<?php echo $sno; ?>" value="<?php echo $res['ITRemark']; ?>" maxlength="200" />
  </td>
		   <td width="80" class="TableHead1" align="center" style="font-size:12px;font-family:Times New Roman;"> 		   
<?php if($res['ITApprovalStatus']!=2 AND $res['ITApprovalStatus']!=3){ ?>		   
...
<?PHP doPages($offset, 'MyAssetNextitLevel.php', '', $total_records); ?>
</td>
</tr>			
		 </table>
	           </td>
