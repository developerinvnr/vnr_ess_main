...
{ 
  ...
  else if(lgc=='Logic6')
  {
   ...
   else if(AppKRARating>=0 && AppKRARating<=30){var EScore=document.getElementById("AppKRALogic"+n).value=ScoAct;}
   else{var EScore=document.getElementById("AppKRALogic"+n).value=0;}
   var MScore=document.getElementById("AppKRAScore"+n).value=Math.round(((EScore/KRATarget)*KRAWeight)*100)/100;
   var Score=document.getElementById("KraScore"+n).value=MScore;
  }
...
{ 
  ...
  else if(lgc=='Logic6')
  {
   ...
   else if(Rating>=0 && Rating<=30){var EScore=document.getElementById("AppKRALogic"+i+"_"+j).value=ScoAct;}
   else{var EScore=document.getElementById("AppKRALogic"+i+"_"+j).value=0;}
   var MScore=document.getElementById("AppKRAScore"+i+"_"+j).value=Math.round(((EScore/Target)*Weight)*100)/100;
   if(MScore>0){document.getElementById("AppKRAScore"+i+"_"+j).value=MScore; document.getElementById("KraScoreSub"+i+"_"+j).value=MScore;}else{document.getElementById("AppKRAScore"+i+"_"+j).value=0;}
  }
...
<?php $sql2=mysql_query("select * from hrm_employee_pms_kraforma where KRAFormAStatus='A' AND EmpPmsId=".$resApp['EmpPmsId']." order by KRAFormAId ASC", $con); $Sno=1; while($res2=mysql_fetch_array($sql2)){ 
     ...
     <tr bgcolor="#FFFFFF" style="height:25px;">
      ...
      <td class="fbodyc" style="background-color:<?php if($res2['Period']!='Annual' AND $res2['Period']!=''){echo '#B9DCFF';}else{echo '#FFFFFF';} ?>;"><span style="text-decoration:<?php if($res2['Period']!='Annual' AND $res2['Period']!=''){ echo 'underline'; }else{echo 'none';}?>;color:<?php if($res2['Period']!='Annual' AND $res2['Period']!=''){ echo '#000099'; }else{echo '#000';}?>;cursor:<?php if($res2['Period']!='Annual' AND $res2['Period']!=''){echo 'pointer';} ?>;" maxlength="8" <?php if($res2['Period']!='Annual' AND $res2['Period']!=''){?> onClick="FunTgt(<?php echo $Sno.','.$res2['KRAId']; ?>,'<?php echo $res2['Period']; ?>',<?php echo $res2['Target'].','.intval($res2['Weightage']); ?>,'<?php echo $res2['Logic']; ?>')" <?php } ?> ><?php if($rowSubK>0){echo '';}else{echo $res2['Target'];}?></span></td>
	  ...
	  <?php if($_SESSION['eAppform']=='Y' OR $_SESSION['eMidform']=='Y'){ ?>
      <td class="fbodyc" style="color:<?php if($rowSubK>0){echo '#FFFFFF';}?>;"><?php if($_SESSION['eAppform']=='Y'){ if($n==1){echo $rest['tELogScr'];}else{echo $res2['SelfRating'];} }elseif($_SESSION['eMidform']=='Y'){ echo $res2['Mid_SelfRating']; } ?></td>
...
<?php if($m==1){$n='(a)';}elseif($m==2){$n='(b)';}elseif($m==3){$n='(c)';}elseif($m==4){$n='(d)';}elseif($m==5){$n='(e)';} ?>
	 ...
	 <tr style="background-color:#FFFFFF;">  
      ...
      <td class="fbodyc" style="background-color:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){echo '#B9DCFF';}else{echo '#FFFFFF';} ?>;"><span style="text-decoration:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){ echo 'underline'; }else{echo 'none';}?>;color:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){ echo '#000099'; }else{echo '#000';}?>;cursor:<?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){echo 'pointer';} ?>;" maxlength="8" <?php if($rSubK['Period']!='Annual' AND $rSubK['Period']!=''){ ?> onClick="FunSubTgt(<?php echo $Sno.','.$m.','.$rSubK['KRASubId']; ?>,'<?php echo $rSubK['Period']; ?>',<?php echo $rSubK['Target'].','.intval($rSubK['Weightage']); ?>,'<?php echo $rSubK['Logic']; ?>')" <?php } ?> ><?php echo $rSubK['Target']; ?></span></td>
