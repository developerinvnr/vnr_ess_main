...
if($_REQUEST['ac']=='CloseQuery' AND $_REQUEST['QCid']!='')
{
  ...
  $sqlDel=mysql_query("update hrm_employee_queryemp set QCloseStatus='Y', Level_1QStatus=3 where QueryId=".$_REQUEST['QCid'], $con); 
...
{ if(v=='P') {var agree=confirm("Are you sure you want to change status to In-Process?"); 
  if (agree) { var win = window.open("ReplyQueryEmp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="EmpDraftQ.php?page="+P; } }, 1000);}}
  if(v=='R') {var agree=confirm("Are you sure you want to reply to this query...?"); 
  if (agree) { var win = window.open("ReplyQueryEmp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="EmpDraftQ.php?page="+P; } }, 1000);}}
  if(v=='U') {var agree=confirm("Are you sure you want to update this query...?"); 
  if (agree) { var win = window.open("ReplyQueryEmp.php?action="+v+"&QR="+QI+"&NOfT="+NOfT,"QForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=430,height=350");
               var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="EmpDraftQ.php?page="+P; } }, 1000);}}		
			   
  if(v=='C') { var QS=document.getElementById("QSubject_"+SN).value; var agree=confirm("Are you sure you want to close this query...?"); 
  if (agree){ window.location="EmpDraftQ.php?ac=CloseQuery&QCid="+QI+"&page="+P+"&QEI="+QEI+"&DI="+DI+"&QS="+QS+"&HYN="+HYN;} }				   
...
<?php //*******************************************Status********** ?>       
		...
		<td align="center" style="font-family:Times New Roman;width:82px; font-size:15px;"><a href="javascript:window.location='EmpDraftQ.php?page=1'"><b>Refresh</b></a></td>   <td align="center" style="font-family:Times New Roman;width:120px;font-size:15px;"><a href="javascript:window.location='EmpReplyQ.php?page=1'"><b style="color:#004A00;">Reply Query</b></a></td>
			  </tr>
			 </table>
			</td>
		   </tr>
...
<?php  $CurrDate=date('Y-m-d H:i:s'); //$CurrDate=date('Y-m-d 24:i:s');

$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_1ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Level_1QStatus=0 OR Level_1QStatus=1 OR Level_1QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>=Level_1QToDT AND '".$CurrDate."'<=Level_2QToDT order by QueryDT DESC", $con);
...
$sql_statement = mysql_query("select * from hrm_employee_queryemp where Level_1ID=".$EmployeeId." AND (QStatus=0 OR QStatus=1 OR QStatus=2) AND QCloseStatus='N' AND (Level_1QStatus=0 OR Level_1QStatus=1 OR Level_1QStatus=2) AND QueryStatus_Emp!=3 AND QueryStatus_Emp!=4 AND '".$CurrDate."'>=Level_1QToDT AND '".$CurrDate."'<=Level_2QToDT order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
...
?>
		   ...
		   <td style="font-family:Georgia; font-size:12px; width:100px; overflow:hidden;" valign="top"><?php echo $sub; ?><input type="hidden" id="QSubject_<?php echo $Sno; ?>" value="<?php if($resQuery['QuerySubject']=='N'){echo $resQuery['OtherSubject'];}else{echo $resQuery['QuerySubject'];} ?>" />
		  </td>  
...
<?php $sqlD=mysql_query("select DepartmentCode from hrm_department where DepartmentId=".$resE['DepartmentId'], $con); $resD=mysql_fetch_assoc($sqlD);?>		   
		   ...
		   <option value="C">Close</option><option value="U">Update</option><?php } ?></select></td>
		             </tr>	
<?php $Sno++;} } else { ?>
                     <tr>
