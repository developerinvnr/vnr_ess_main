...
<td>
 ...
 <?php $sqlMK=mysql_query("select WarmWelCome from hrm_employee_key where CompanyId=1", $con); 
...
<?php if(date("m")>=2 && date("m")<=12){ $PrevMnt=date("m")-1; $PrevYer=date("Y"); }
	  ...
	  //$PrevMnt=01; $PrevYer=2017;
	  
      $mkdate = mktime(0,0,0, $PrevMnt, 1, $PrevYer); 
      $days = date('t',$mkdate);

$SqlEmp=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, DepartmentName, DesigName, HqName, StateName, Gender, DR, Married, DateJoining, DOB, RepEmployeeID, Warm_WelC_Oth, e.CompanyId from hrm_employee e inner join hrm_employee_general g ON e.EmployeeID=g.EmployeeID inner join hrm_employee_personal p on e.EmployeeID=p.EmployeeID inner join hrm_designation de on g.DesigId=de.DesigId inner join hrm_department d on g.DepartmentId=d.DepartmentId inner join hrm_headquater hq on g.HqId=hq.HqId inner join hrm_state s on g.CostCenter=s.StateId where g.DateJoining between '".date($PrevYer."-".$PrevMnt."-01")."' and '".date($PrevYer."-".$PrevMnt."-".$days)."' and e.EmpStatus='A' and e.CompanyId=1 order by e.EmpCode ASC", $con); 
while ($ResEmp=mysql_fetch_assoc($SqlEmp))
{ 
...
$sqlQ=mysql_query("select Fname,Sname,Lname,DesigName,DR,Gender,Married from hrm_employee_general g inner join hrm_employee e on g.EmployeeID=e.EmployeeID inner join hrm_employee_personal p on g.EmployeeID=p.EmployeeID inner join hrm_designation de on g.DesigId=de.DesigId where g.EmployeeID=".$ResEmp['RepEmployeeID'],$con); $resQ=mysql_fetch_assoc($sqlQ); 
...
<?php if($ResEmp['Gender']=='F'){echo 'She';}else{echo 'He';}?> has earlier worked with <?php  $j=1; while($resExp=mysql_fetch_assoc($sqlExp)){ echo $resExp['ExpComName']; $rowExp2=$rowExp-1; if($j<$rowExp2){echo ', ';}elseif($j==$rowExp2){echo ' & ';} $j++; } ?>.</td>
 ...
 <tr><td colspan="3" style="height:15px;border:hidden;"></td></tr> 
<tr>
  <td colspan="3" style="text-align:center;font-size:15px;"><b>We welcome him<?php if($rowF2>0 || $rowF3>0 || $rowF4>0){ echo ', ';}else{echo ' & ';}?>his spouse -  <?php $resF=mysql_fetch_assoc($sqlF); echo ucwords(strtolower($resF['HW_SN'])).'. '.$resF['HusWifeName']; ?>
  
  <?php if($rowF2>0)
        { 
		 if($rowF3>0 || $rowF4>0){echo ', ';}else{echo ' & ';} $k=1; echo 'Son - '; 
