...
function FunDetailsSubm(ID,EI,CI,YI,PI){ window.open("InvestSubForm.php?action=Print&ID="+ID+'&EI='+EI+'&CI='+CI+'&YI='+YI+'&PI='+PI,"Form","location=no,scrollbars=yes,resizable=no,menubar=no,width=970,height=700"); }
...
<?php $Ms2=mysql_query("select IvstDecIdSb,Inv_Date,FormSubmit,SubmittedDate from hrm_employee_investment_submission where EmployeeID=".$res['EmployeeID']." AND Period='".$_REQUEST['prd']."' ", $con); $rowMs2=mysql_num_rows($Ms2); $resMs2=mysql_fetch_array($Ms2); if($rowMs2>0){$IdSub=$resMs2['IvstDecIdSb'];}else{$IdSub=0;} ?>
	...
	<td align="center" class="All_80"><?php if($resMs2['FormSubmit']=='N'){ echo 'NO ACTION';}elseif($resMs2['FormSubmit']=='Y'){ echo '<font color="#FF8000">PENDING</font>';}if($resMs2['FormSubmit']=='YY'){ echo '<font color="#008000">OK</font>';} ?></td><font color=""></font>
	<td align="center" class="All_80"><?php //if($resMs2['FormSubmit']=='YY'){?><a href="#" onClick="FunDetailsSubm(<?php echo $IdSub.', '.$res['EmployeeID'].', '.$CompanyId.', '.$YearId; ?>,'<?php echo $_REQUEST['prd']; ?>')" style="color:<?php if($resMs2['FormSubmit']=='YY'){echo '#004F9D';}else{echo '#00CE00';}?>;">Click</a><?php //} ?></td>
