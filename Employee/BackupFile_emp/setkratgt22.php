...
function EditTgtDef()
{
 ...
 { document.getElementById("Remark"+i).readOnly=false; document.getElementById("Remark"+i).style.background='#FFF';
   document.getElementById("TDD"+i).style.background='#FFF'; document.getElementById("TDw"+i).style.background='#FFF';
   document.getElementById("TDt"+i).style.background='#FFF'; document.getElementById("wgt"+i).style.background='#FFF';
...
{
 for(var i=1; i<=document.getElementById("numr").value; i++)
 {if(document.getElementById("Remark"+i).value==''){alert("please enter activity performed for row "+i+"."); return false;}}
 
 var twgt=document.getElementById("twgt").value; var ttgt=document.getElementById("ttgt").value; 
 ...
 if(num==2)
 {
  ...
  if(wgt!=twgt){alert("Error! distributed weightage not equal to total weightage"); return false;}
  if(tgt!=ttgt){alert("Error! distributed target not equal to total target"); return false;}
 }
 else if(num==4)
 {
  var a1= parseFloat(document.getElementById("wgt"+1).value); var a2= parseFloat(document.getElementById("wgt"+2).value); 
  ...
  var wgt=Math.round(a1+a2+a3+a4); var tgt=Math.round(s1+s2+s3+s4);
  if(wgt!=twgt){alert("Error! distributed weightage not equal to total weightage"); return false;}
  if(tgt!=ttgt){alert("Error! distributed target not equal to total target"); return false;}
 }
 else if(num==12)
  ...
  <td style="width:100%;" valign="top">
   <table align="center" border="0" cellspacing="1">
    ...
    <tr style="background-color:#CC9900;height:25px;">
	 ...
	 <td colspan="4" class="t" style="width:60px;">Achivement</td>
...
?>
	...
	<tr style="background-color:#FFFFFF;height:24px;">
	 ...
	 <td align="center" bgcolor="#FFFFC6" id="TDw<?php echo $i; ?>"><input type="text" class="d" style="width:60px;height:40px;border:hidden;text-align:center;background-color:#FFFFC6;" id="wgt<?php echo $i; ?>" name="wgt<?php echo $i; ?>" value="<?php echo $wgt; ?>"/></td>
	 <td align="center" bgcolor="#FFFFC6" id="TDt<?php echo $i; ?>"><input type="text" class="d" style="width:60px;height:40px;border:hidden;text-align:center;background-color:#FFFFC6;" id="tgt<?php echo $i; ?>" name="tgt<?php echo $i; ?>" value="<?php echo $tgt; ?>"/>
...
<?php $sql=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?>  
	 ...
	 <td class="d" bgcolor="#FFFFC6" id="TDD<?php echo $i; ?>" valign="top"><textarea type="text" name="Remark<?php echo $i; ?>" id="Remark<?php echo $i; ?>" class="d" rows="2" style="width:300px;border:hidden;background-color:#FFFFC6;"><?php if($res['Remark']!=''){echo $res['Remark'];}else{echo $resk['KRA'];}?></textarea>
	 ...
	 <td class="d" bgcolor="#D5FFAA" id="TD2<?php echo $i; ?>"><input type="text" name="LogScr<?php echo $i; ?>" id="LogScr<?php echo $i; ?>" class="d" style="width:60px;border:hidden;background-color:#D5FFAA;text-align:center;" value="<?php if($res['LogScr']!=''){ echo $res['LogScr'];}else{echo 0;} ?>" readonly/></td>
	 ...
	 <td align="center">
	 <?php $Nxt10Day=date("Y-m-d",strtotime('+10 day', strtotime($res['Ldate']))); ?>
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ ?>
	  <span style="cursor:pointer"><img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if($res['lockk']==0 AND date("Y-m-d")<=$Nxt10Day){echo 'block';}else{echo 'none';} ?>" onClick="FunEdit(<?php echo $i; ?>)"/></span>
...
<?php $sqlt=mysql_query("select SUM(Ach) as tAch, SUM(LogScr) as tLogScr, SUM(Scor) as tScor from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid'],$con); $rest=mysql_fetch_assoc($sqlt); ?>	 
	 ...
	 <td class="d" align="" bgcolor="#FFFFFF"></td>	 
	 <td class="d" align="center" bgcolor="#FFFFFF"><b><input type="text" id="TotLogScr" style="width:50px;font-weight:bold; text-align:center;background-color:#FFFFFF;border:hidden;" value="<?php if($rest['tLogScr']>0){echo floatval($rest['tLogScr']);} ?>" readonly/></b></td>	 	 
