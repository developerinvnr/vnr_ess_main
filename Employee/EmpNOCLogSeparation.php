...
{ 
  var win=window.open("SepLogClearForm.php?act=act&v=v&ss=vty&cc=it@~t~1212&p=value&a=app&true=false&si="+SId+"&st="+St+"&ei="+ei,"leaveForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=850,height=670");
  var timer = setInterval( function() { if(win.closed) {  clearInterval(timer);  window.location="EmpNOCLogSeparation.php?e=4e&w=234&y=10234&e=4e2&e=4e&w=234&y=110022344&retd=ee&rr=09drfGe&S=eewwqq&wwrew=t%T@sed818&d=101"; } }, 1000);
...
<table border="0" cellspacing="5">
 <tr>
  <td style="display:block;"> <?php //if($_REQUEST['a']=='hod'){echo 'block';}elseif($_REQUEST['a']=='app'){echo 'none';}?>
   <table>
      <tr><td height="30" colspan="7" valign="top" style="font-size:18px;font-family:Times New Roman;"><b>Departmental NOC Clearance(Logistics) </b>&nbsp;&nbsp;
  ...
  <td>
   <table border="1">
     <tr bgcolor="#7a6189" style="height:22px;">
	  ...
	  <td width="80" class="TableHead" align="center"><b>Reporting Clearance</b></td>
	  <td width="80" class="TableHead" align="center"><b>Logistics Clearance</b></td>
	  <td width="80" class="TableHead" align="center"><b>Clearance Form</b></td>
     </tr>
<?php $sql_statement = mysql_query("select * from hrm_employee_separation where Log_EmployeeID=".$EmployeeId, $con); $total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  $offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}
$sql_statement = mysql_query("select * from hrm_employee_separation where Log_EmployeeID=".$EmployeeId." order by ResignationStatus ASC, Emp_ResignationDate DESC LIMIT " . $from . "," . $offset, $con);  $rowCheck=mysql_num_rows($sql_statement); if($rowCheck>0){ 
...
?>	 
	 <tr bgcolor="#FFFFFF">
	  ...
	  <td width="80" class="TableHead1" align="center"><?php if($res['Log_NOC']=='N'){echo '<font color="#F97C00">PENDING</font>';}elseif($res['Log_NOC']=='Y'){ echo '<font color="#006F00">SUBMITTED</font>';}?></td>
	  <td width="80" class="TableHead1" align="center">
	  <?php if($res['Rep_NOC']=='Y'){ ?><a href="javascript:OpenClearanceF(<?php echo $res['EmpSepId'].', '.$EmployeeId;?>,'<?php echo $res['Log_NOC'];?>')"><font color="#008000"><b>Click</b></font></a>
	  <?php } else { echo ''; }?>
...
<?PHP doPages($offset, 'EmpNOCLogSeparation.php', '', $total_records); ?>
 </td>
</tr>  
   </table>
  </td>
