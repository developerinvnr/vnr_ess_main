...
$sql = mysql_query("SELECT * from hrm_user WHERE User_name='".$_POST['username']."' AND User_pass='".$_POST['userpass']."' AND CompanyId='".$_POST['companyadmin']."' AND User_status='A' AND (User_type='M' OR User_type='S' OR User_type='A' OR User_type='U')", $con); 
   $row = mysql_fetch_assoc($sql);
   if(mysql_num_rows($sql)==1)
     { $_SESSION['login'] = true; $_SESSION['AXAUESRUserId']=$row['AXAUESRUser_Id']; $_SESSION['CompanyId']=$row['CompanyId'];
    
       if(($row['User_name']==$_POST['username']) AND ($row['User_pass']==$_POST['userpass']) AND ($row['User_status']=='A') AND ($row['User_type']=='M' OR $row['User_type']=='S' OR $row['User_type']=='A' OR $row['User_type']=='U'))
          {header('Location:AdminUser/Index.php?log='.$_POST['Login']);}
	 }
   elseif(($row['User_name']!=$_POST['username'] AND $row['User_pass']!=$_POST['userpass'] AND $row['CompanyId']!=$_POST['companyadmin']) OR ($row['User_name']==$_POST['username'] AND $row['User_pass']!=$_POST['userpass'] AND $row['CompanyId']!=$_POST['companyadmin']) OR ($row['User_name']!=$_POST['username'] AND $row['User_pass']==$_POST['userpass'] AND $row['CompanyId']!=$_POST['companyadmin']) OR ($row['User_name']==$_POST['username'] AND $row['User_pass']==$_POST['userpass'] AND $row['CompanyId']!=$_POST['companyadmin']) OR ($row['User_name']==$_POST['username'] AND $row['User_pass']==$_POST['userpass'] AND $row['CompanyId']==$_POST['companyadmin'] AND $row['User_status']=='D'))
...
$sql = mysql_query("SELECT * FROM hrm_employee WHERE EmpCode='".$_POST['empcode']."' AND EmpPass='".$_POST['emppass']."' AND CompanyId='".$_POST['companyemp']."' AND EmpStatus='A' AND (EmpType='E' OR EmpType='M')", $con);
   ...
   if(mysql_num_rows($sql)==1)
     { $_SESSION['login'] = true; $_SESSION['EmpCode']=$row2['EmpCode'];  $_SESSION['EmployeeID']=$row2['EmployeeID']; $_SESSION['CompanyId']=$row2['CompanyId'];
      ...
      if($row2['EmpCode']==$_POST['empcode'] AND $row2['EmpPass']==$_POST['emppass'] AND ($row2['EmpType']=='E' OR $row2['EmpType']=='M')  AND $row2['EmpStatus']=='A')
      { header('Location:Employee/EmpHome.php?log='.$_POST['Login']); }
    }   
   elseif(($row2['EmpCode']!=$_POST['empcode'] AND  $row2['EmpPass']!=$_POST['emppass'] AND $row2['CompanyId']!=$_POST['companyemp']) OR ($row2['EmpCode']==$_POST['empcode'] AND $row2['EmpPass']!=$_POST['emppass'] AND $row2['CompanyId']!=$_POST['companyemp']) OR ($row2['EmpCode']!=$_POST['empcode'] AND $row2['EmpPass']==$_POST['emppass'] AND $row2['CompanyId']!=$_POST['companyemp']) OR ($row2['EmpCode']==$_POST['empcode'] AND $row2['EmpPass']==$_POST['emppass'] AND $row2['CompanyId']!=$_POST['companyemp']) OR ($row2['EmpCode']==$_POST['empcode'] AND $row2['EmpPass']==$_POST['emppass'] AND $row2['CompanyId']==$_POST['companyemp'] AND $row2['EmpStatus']=='D'))
