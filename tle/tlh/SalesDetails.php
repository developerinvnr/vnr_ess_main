...
<option value="<?php echo $Ymin2; ?>"><?php echo 'Year-'.$FYmin2.'-'.$TYmin2; ?></option><?php } elseif($_REQUEST['y']!=$YearId) { ?><option value="<?php echo $YearId; ?>"><?php echo 'Year-'.$FUpy.'-'.$TUpy; ?></option><?php if($_REQUEST['y']!=$Ymin1){?><option value="<?php echo $Ymin1; ?>"><?php echo 'Year-'.$FYmin1.'-'.$TYmin1; ?></option><?php } ?><?php if($_REQUEST['y']!=$Ymin2){?><option value="<?php echo $Ymin2; ?>"><?php echo 'Year-'.$FYmin2.'-'.$TYmin2; ?></option><?php } } ?></select>
   ...
   <td></td>
    <td>
     <select style="font-family:Georgia;font-size:15px;width:px;background-color:#E4E0FC;height:23px;width:150px;" id="CropV" onChange="ChangeCrop(<?php echo $EmployeeId; ?>)">
   ...
   <td></td>
	<td>
     <select id="ItemV" id="ItemV" style="font-family:Georgia;font-size:14px;width:px;background-color:#E4E0FC;height:23px;width:140px;" onChange="ChangeTT(<?php echo $EmployeeId; ?>)" <?php if($_REQUEST['crop']==0){echo 'disabled';} ?>>
...
<table border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:14px;width:<?php if($rowRpt>0){ echo 1300;}else{echo 1000;} ?>px;">	
  <tr>
 ...
 <?php $sqlSb=mysql_query("select * from hrm_sales_emp_action_tle where TLEId=".$EmployeeId." AND YearId=".$_REQUEST['y']." AND StatusA=1", $con); $rowSb=mysql_num_rows($sqlSb); ?><input type="button" style="font-family:Times New Roman;width:100px;height:25px;font-weight:bold;" value="Final Submit" onClick="ClickFSL1(<?php echo $EmployeeId.', '.$_REQUEST['y']; ?>)" <?php if($RslP=='Y'){echo 'disabled';}else{echo '';}?>/><?php } //$rowSb>0 OR  ?>
  ...
  <tr style="background-color:#D5F1D1;color:#000000;">
   <td align="center" style="width:80px;" rowspan="2"><b>CROP</b></td>
   <td align="center" style="width:150px;" rowspan="2"><b>VARIETY</b></td>
   <td align="center" style="width:50px;" rowspan="2"><b>&nbsp;TYPE&nbsp;</b></td>
   <td colspan="2" align="center"><b>Quarter 1</b></td>
   <td colspan="2" align="center"><b>Quarter 2</b></td>
   <td colspan="2" align="center"><b>Quarter 3</b></td>
   <td colspan="2" align="center"><b>Quarter 4</b></td>
   <td colspan="2" align="center" bgcolor="#D9D9FF"><b>TOTAL</b></td>
   <td colspan="2" rowspan="0" align="center" bgcolor="#D9D9FF"><b>REVISED</b></td>
  </tr>	
   <tr style="background-color:#D5F1D1;color:#000000;">
   <?php for($i=1; $i<=5; $i++){ ?>
...
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_seedsproduct sp ON sdh.ProductId=sp.ProductId INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND YearId=".$_REQUEST['y'], $con); $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_seedsproduct sp ON sdh.ProductId=sp.ProductId INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND sp.ItemId=".$_REQUEST['ii']." AND YearId=".$AfterYId, $con); 
$rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);
}
else {
if($_REQUEST['crop']==1 OR $_REQUEST['crop']==2){
$sqlP1t=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp hqt ON d.HqId=hqt.TLHq where TLStatus='A' AND hqt.TLEId=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND sd.YearId=".$BeforeYId, $con); $resP1t=mysql_fetch_assoc($sqlP1t);
$sql4tat=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details _".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp hqt ON d.HqId=hqt.TLHq where TLStatus='A' AND hqt.TLEId=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND sd.YearId=".$_REQUEST['y'], $con); 
$sql5tat=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp hqt ON d.HqId=hqt.TLHq where TLStatus='A' AND hqt.TLEId=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND sd.YearId=".$AfterYId, $con); 
...
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_seedsproduct sp ON sdh.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND YearId=".$_REQUEST['y'], $con); 
$sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_seedsproduct sp ON sdh.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND si.GroupId=".$_REQUEST['crop']." AND YearId=".$AfterYId, $con); 
$rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);

}
elseif($_REQUEST['crop']==3){ $sqlP1t=mysql_query("select SUM(M1_Ach) as sM1,SUM(M2_Ach) as sM2,SUM(M3_Ach) as sM3,SUM(M4_Ach) as sM4,SUM(M5_Ach) as sM5,SUM(M6_Ach) as sM6,SUM(M7_Ach) as sM7,SUM(M8_Ach) as sM8,SUM(M9_Ach) as sM9,SUM(M10_Ach) as sM10,SUM(M11_Ach) as sM11,SUM(M12_Ach) as sM12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp hqt ON d.HqId=hqt.TLHq where TLStatus='A' AND hqt.TLEId=".$_REQUEST['mainv']." AND (si.GroupId=1 OR si.GroupId=2) AND sd.YearId=".$BeforeYId, $con); 
$resP1t=mysql_fetch_assoc($sqlP1t);
$sql4tat=mysql_query("select SUM(M1) as sM1,SUM(M2) as sM2,SUM(M3) as sM3,SUM(M4) as sM4,SUM(M5) as sM5,SUM(M6) as sM6,SUM(M7) as sM7,SUM(M8) as sM8,SUM(M9) as sM9,SUM(M10) as sM10,SUM(M11) as sM11,SUM(M12) as sM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp hqt ON d.HqId=hqt.TLHq where TLStatus='A' AND hqt.TLEId=".$_REQUEST['mainv']." AND (si.GroupId=1 OR si.GroupId=2) AND sd.YearId=".$_REQUEST['y'], $con); 
$sql5tat=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_sales_tlemp hqt ON d.HqId=hqt.TLHq where TLStatus='A' AND hqt.TLEId=".$_REQUEST['mainv']." AND (si.GroupId=1 OR si.GroupId=2) AND sd.YearId=".$AfterYId, $con); 
$res4tat=mysql_fetch_assoc($sql4tat); $res5tat=mysql_fetch_assoc($sql5tat); 
...
$sRevAt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND YearId=".$_REQUEST['y'], $con); $sRevBt=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND YearId=".$AfterYId, $con); 
$rRevAt=mysql_fetch_assoc($sRevAt); $rRevBt=mysql_fetch_assoc($sRevBt);
...
<?php /*Total Close **********************************/ ?>	
<?php 
if($_REQUEST['ii']>0){ $sql = mysql_query("select sp.ItemId,ProductId,ProductName,ItemName,ItemCode,TypeName from hrm_sales_seedsproduct sp INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_seedtype st ON sp.TypeId=st.TypeId where sp.ItemId=".$_REQUEST['ii']." order by ItemName ASC, ProductName ASC, TypeName ASC", $con);}
else {
 if($_REQUEST['crop']==1 OR $_REQUEST['crop']==2){$sql = mysql_query("select sp.ItemId,ProductId,ProductName,ItemName,ItemCode,TypeName from hrm_sales_seedsproduct sp INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_seedtype st ON sp.TypeId=st.TypeId where si.GroupId=".$_REQUEST['crop']." order by ItemName ASC, ProductName ASC, TypeName ASC", $con); }
 elseif($_REQUEST['crop']==3){$sql = mysql_query("select sp.ItemId,ProductId,ProductName,ItemName,ItemCode,TypeName from hrm_sales_seedsproduct sp INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_seedtype st ON sp.TypeId=st.TypeId order by ItemName ASC, ProductName ASC, TypeName ASC", $con); } 
}
...
$sRevA=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND hqt.TLStatus='A' AND YearId=".$_REQUEST['y'], $con); 
$sRevB=mysql_query("select SUM(Q1) as Qa,SUM(Q2) as Qb,SUM(Q3) as Qc,SUM(Q4) as Qd from hrm_sales_sal_details_hq sdh INNER JOIN hrm_sales_tlemp hqt ON sdh.HqId=hqt.TLHq where TLEId=".$_REQUEST['mainv']." AND ProductId=".$res['ProductId']." AND hqt.TLStatus='A' AND YearId=".$AfterYId, $con); 
