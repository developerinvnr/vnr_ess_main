...
<?php if($branch>0){ $sbranch=mysql_query("select BranchName from erp_branch where BranchId=".$branch,$con2); $rbranch=mysql_fetch_assoc($sbranch); ?><option value='<?php echo $branch;?>' selected><?php echo $rbranch['BranchName'];?></option><?php }else{ ?><option value="0" selected>select</option><?php } $s2branch=mysql_query("select b.BranchId,BranchName from erp_branch b inner join erp_branch_state bs on b.BranchId=bs.BranchId inner join erp_mrp_empstate es on bs.StateId=es.StateId where es.EmpId=".$EmployeeId." AND es.TlEmp='Y' AND es.Sts='A' group by b.BranchId order by b.BranchName ASC",$con2);
...
$sstate=mysql_query("select * from erp_mrp_empstate where EmpId=".$EmployeeId." AND TlEmp='Y' AND Sts='A'",$con2); $rowst=mysql_num_rows($sstate); $ww=$rowst*10; ?>
 <tr>
 ...
 <tr>
  ...
  <?php $ss=mysql_query("select ms.StateId,StateCode from erp_mrp_empstate ms inner join erp_state s on ms.StateId=s.StateId where ms.EmpId=".$EmployeeId." AND ms.TlEmp='Y' AND ms.Sts='A' order by StateCode ASC",$con2);
 ...
 <tr>
  ...
  <?php if($_REQUEST['branch']>0){ ?>
   ...
   <tr>
    ...
    <td class="td1" style="width:15%;">Variety</td>
    <td class="td1" style="width:5%;">Size</td>
    <?php $ss=mysql_query("select ms.StateId,StateCode from erp_mrp_empstate ms inner join erp_state s on ms.StateId=s.StateId where ms.EmpId=".$EmployeeId." AND ms.Sts='A' AND ms.TlEmp='Y' order by StateCode ASC",$con2);
  while($rs=mysql_fetch_assoc($ss)){ ?> 
    <td class="td1" style="width:6%;"><?php echo $rs['StateCode']; ?></td>
    <?php } ?>
   </tr> 
