...
$sql = mysql_query("select hrm_sales_sal_details.ProductId,hrm_sales_sal_details.DealerId,M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach,ProductName,ItemName,ItemCode,TLName,TLRepId from hrm_sales_sal_details INNER JOIN hrm_sales_seedsproduct ON hrm_sales_sal_details.ProductId=hrm_sales_seedsproduct.ProductId INNER JOIN hrm_sales_seedsitem ON hrm_sales_seedsproduct.ItemId=hrm_sales_seedsitem.ItemId INNER JOIN hrm_sales_dealer ON hrm_sales_sal_details.DealerId=hrm_sales_dealer.DealerId INNER JOIN hrm_sales_tlemp ON hrm_sales_dealer.HqId=hrm_sales_tlemp.TLHq where hrm_sales_tlemp.TLStatus='A' AND hrm_sales_tlemp.TLEId=".$_REQUEST['ei']." AND hrm_sales_sal_details.YearId=".$_REQUEST['y']." AND (M1_Ach!=0 OR M2_Ach!=0 OR M3_Ach!=0 OR M4_Ach!=0 OR M5_Ach!=0 OR M6_Ach!=0 OR M7_Ach!=0 OR M8_Ach!=0 OR M9_Ach!=0 OR M10_Ach!=0 OR M11_Ach!=0 OR M12_Ach!=0) order by hrm_sales_seedsitem.ItemName ASC,hrm_sales_seedsproduct.ProductName ASC", $con); $Sn=1; while($res=mysql_fetch_array($sql)){ 
