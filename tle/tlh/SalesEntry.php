...
<input type="hidden" id="Reporting" value="<?php echo $resE['TLRepId']; ?>" />
<?php $sHq2=mysql_query("select * from hrm_sales_tlemp where TLEId=".$EmployeeId, $con); $rowHq2=mysql_num_rows($sHq2); echo '<input type="hidden" id="Rowhq2" value="'.$rowHq2.'" />';
...
<td>&nbsp;</td>
<?php if($resE['TLHq']>0){  $sqlHQ=mysql_query("select HqName,StateName from hrm_headquater h inner join hrm_state s on h.StateId=s.StateId where HqId=".$resE['TLHq'],$con); $resHQ=mysql_fetch_assoc($sqlHQ); $sn=1; ?>
<td align="center" valign="bottom">
<a href="#" onClick="ChangeHq(<?php echo $resE['TLHq'].', '.$_REQUEST['y'].','.$sn; ?>)" style="text-decoration:none;">
...
function ChangeDealer(De,hq,v,no)
{
  var myh=1; var rowHq=document.getElementById("Rowhq2").value;  window.location="SalesEntry.php?ern1=r114&ern2w=234&ern3y=10234&ern=4e2&erne=4e&ernw=234&erney=110022344&ernretd=ee&rernr=09drfGe&ernS=eewwqq&act="+document.getElementById("act").value+"&hq="+hq+"&y="+v+"&q="+document.getElementById("q").value+"&ii="+document.getElementById("ii").value+"&vc="+document.getElementById("vc").value+"&fc="+document.getElementById("fc").value+"&cc="+document.getElementById("cc").value+"&di="+document.getElementById("di").value+"&gi="+document.getElementById("gi").value+'&EHq='+document.getElementById("EHq").value+'&myh='+myh+'&myt='+document.getElementById("myt").value+'&no='+no+'&SelectV=DealerNo&DealerV='+De;
}

</script>

   <td>&nbsp;</td><td align="center" valign="bottom">
         <a href="#" onClick="ChangeTotal(<?php echo $_REQUEST['hq'].', '.$_REQUEST['y'].', '.$_REQUEST['no']; ?>)"><font style="color:#fff;font-family:Times New Roman;font-size:16px;font-weight:bold;">
 <u>TOTAL HEAD-QUARTER</u></font></a></td>
         <td>&nbsp;</td>
		 
		 <td align="center" valign="top">
         <select style="font-family:Times New Roman;font-size:14px;width:200px;background-color:#C4FFC4;height:23px;font-weight:bold;" id="ItemV2" onChange="ChangeDealer(this.value,<?php echo $_REQUEST['hq'].', '.$_REQUEST['y'].', '.$_REQUEST['no']; ?>)"><option>Select Dealer Name</option><?php  $sqlDeal = mysql_query("select * from hrm_sales_dealer where HqId=".$_REQUEST['hq']." order by DealerId ASC", $con); while($resDeal=mysql_fetch_assoc($sqlDeal)){ ?>
		 <option value="<?php echo $resDeal['DealerId']; ?>"><?php echo $resDeal['DealerName']; ?></option>
		 <?php } ?>
		 </select>
		 </td>
...
<table class="bluetable" border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:14px;width:<?php if($_REQUEST['ii']=='All_1' OR $_REQUEST['ii']=='All_2' OR $_REQUEST['ac']==1){ echo '1300px';}else{echo '1200px';}?>;">
  ...
  <td colspan="60" style="height:25px;background-color:#005300;font-family:Times New Roman;font-size:16px;color:#FFFF20;font-weight:bold;" id="TD_<?php echo $counter; ?>" valign="top">
   ...
   <?php $sqlSb=mysql_query("select * from hrm_sales_emp_action_tle where TLEId=".$EmployeeId." AND YearId=".$_REQUEST['y']." AND StatusA=1", $con); $rowSb=mysql_num_rows($sqlSb); ?><input type="button" style="font-family:Times New Roman;width:100px;height:25px;font-weight:bold;" value="Final Submit" onClick="ClickFSL1(<?php echo $EmployeeId.', '.$_REQUEST['y']; ?>)" <?php if($rowSb>0 OR $RslP=='Y'){echo 'disabled';}else{echo '';}?>/><?php } ?>
   </td>
  </tr>
...
<td colspan="5" align="center"><b>APR</b></td><td colspan="4" align="center"><b>MAY</b></td><td colspan="4" align="center"><b>JUN</b></td><td colspan="4" align="center"><b>JUL</b></td><td colspan="4" align="center"><b>AUG</b></td><td colspan="4" align="center"><b>SEP</b></td><td colspan="4" align="center"><b>OCT</b></td><td colspan="4" align="center"><b>NOV</b></td><td colspan="4" align="center"><b>DEC</b></td><td colspan="4" align="center"><b>JAN</b></td><td colspan="4" align="center"><b>FEB</b></td><td colspan="4" align="center"><b>MAR</b></td>
...
<?php if($_REQUEST['ii']!='All_1' AND $_REQUEST['ii']!='All_2' AND $_REQUEST['ii']!=0){$sql = mysql_query("select sp.ItemId,ProductId,ProductName,ItemName,TypeName from hrm_sales_seedsproduct sp INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_seedtype st ON sp.TypeId=st.TypeId where si.ItemId=".$_REQUEST['ii']." order by ItemName ASC, TypeName ASC, ProductName ASC", $con);}
 ...
 $sqlP3=mysql_query("select SUM(M1_Proj) as sM1,SUM(M2_Proj) as sM2,SUM(M3_Proj) as sM3,SUM(M4_Proj) as sM4,SUM(M5_Proj) as sM5,SUM(M6_Proj) as sM6,SUM(M7_Proj) as sM7,SUM(M8_Proj) as sM8,SUM(M9_Proj) as sM9,SUM(M10_Proj) as sM10,SUM(M11_Proj) as sM11,SUM(M12_Proj) as sM12 from hrm_sales_sal_details_".$AfterYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND sd.ProductId=".$res['ProductId']." AND sd.YearId=".$AfterYId, $con); 
 $resP0=mysql_fetch_assoc($sqlP0); $resP1=mysql_fetch_assoc($sqlP1); $resP2=mysql_fetch_assoc($sqlP2); $resP2ch=mysql_fetch_assoc($sqlP2ch); $resP3=mysql_fetch_assoc($sqlP3); 
...
<?php /********************************** Open Overall******************************************/ ?>
<td><input class="Input" id="M1_D<?php echo $Sn; ?>" value="<?php if($resP0['sM1']!=0){echo floatval($resP0['sM1']);}?>" readonly/></td>
<td><input class="Input" id="M1_A<?php echo $Sn; ?>" value="<?php if($resP1['sM1']!=0){echo floatval($resP1['sM1']);}?>" readonly/></td>
<td><input class="Input" id="M1_B<?php echo $Sn; ?>" value="<?php if($resP2['sM1']!=0){echo floatval($resP2['sM1']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M1_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM1']!=0){echo floatval($resP2ch['sM1']);}?>" readonly/></td><?php } ?>
<?php if($EntP=='Y'){ ?><td><input class="Input" id="M1_C<?php echo $Sn; ?>" value="<?php if($resP3['sM1']!=0){echo floatval($resP3['sM1']);}?>" readonly/></td><?php } ?>


<td><input class="Input" id="M2_D<?php echo $Sn; ?>" value="<?php if($resP0['sM2']!=0){echo floatval($resP0['sM2']);}?>" readonly/></td>
<td><input class="Input" id="M2_A<?php echo $Sn; ?>" value="<?php if($resP1['sM2']!=0){echo floatval($resP1['sM2']);}?>" readonly/></td>
<td><input class="Input" id="M2_B<?php echo $Sn; ?>" value="<?php if($resP2['sM2']!=0){echo floatval($resP2['sM2']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M2_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM2']!=0){echo floatval($resP2ch['sM2']);}?>" readonly/></td><?php } ?>
<?php if($EntP=='Y'){ ?><td><input class="Input" id="M2_C<?php echo $Sn; ?>" value="<?php if($resP3['sM2']!=0){echo floatval($resP3['sM2']);}?>" readonly/></td><?php } ?>


<td><input class="Input" id="M3_D<?php echo $Sn; ?>" value="<?php if($resP0['sM3']!=0){echo floatval($resP0['sM3']);}?>" readonly/></td>
<td><input class="Input" id="M3_A<?php echo $Sn; ?>" value="<?php if($resP1['sM3']!=0){echo floatval($resP1['sM3']);}?>" readonly/></td>
<td><input class="Input" id="M3_B<?php echo $Sn; ?>" value="<?php if($resP2['sM3']!=0){echo floatval($resP2['sM3']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M3_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM3']!=0){echo floatval($resP2ch['sM3']);}?>" readonly/></td><?php } ?>
<?php if($EntP=='Y'){ ?><td><input class="Input" id="M3_C<?php echo $Sn; ?>" value="<?php if($resP3['sM3']!=0){echo floatval($resP3['sM3']);}?>" readonly/></td><?php } ?>

<td><input class="Input" id="M4_D<?php echo $Sn; ?>" value="<?php if($resP0['sM4']!=0){echo floatval($resP0['sM4']);}?>" readonly/></td>
<td><input class="Input" id="M4_A<?php echo $Sn; ?>" value="<?php if($resP1['sM4']!=0){echo floatval($resP1['sM4']);}?>" readonly/></td>
<td><input class="Input" id="M4_B<?php echo $Sn; ?>" value="<?php if($resP2['sM4']!=0){echo floatval($resP2['sM4']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M4_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM4']!=0){echo floatval($resP2ch['sM4']);}?>" readonly/></td><?php } ?>
...
<td><input class="Input" id="M5_D<?php echo $Sn; ?>" value="<?php if($resP0['sM5']!=0){echo floatval($resP0['sM5']);}?>" readonly/></td>
<td><input class="Input" id="M5_A<?php echo $Sn; ?>" value="<?php if($resP1['sM5']!=0){echo floatval($resP1['sM5']);}?>" readonly/></td>
<td><input class="Input" id="M5_B<?php echo $Sn; ?>" value="<?php if($resP2['sM5']!=0){echo floatval($resP2['sM5']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M5_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM5']!=0){echo floatval($resP2ch['sM5']);}?>" readonly/></td><?php } ?>
...
<td><input class="Input" id="M6_D<?php echo $Sn; ?>" value="<?php if($resP0['sM6']!=0){echo floatval($resP0['sM6']);}?>" readonly/></td>
<td><input class="Input" id="M6_A<?php echo $Sn; ?>" value="<?php if($resP1['sM6']!=0){echo floatval($resP1['sM6']);}?>" readonly/></td>
<td><input class="Input" id="M6_B<?php echo $Sn; ?>" value="<?php if($resP2['sM6']!=0){echo floatval($resP2['sM6']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M6_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM6']!=0){echo floatval($resP2ch['sM6']);}?>" readonly/></td><?php } ?>
...
<td><input class="Input" id="M7_D<?php echo $Sn; ?>" value="<?php if($resP0['sM7']!=0){echo floatval($resP0['sM7']);}?>" readonly/></td>
<td><input class="Input" id="M7_A<?php echo $Sn; ?>" value="<?php if($resP1['sM7']!=0){echo floatval($resP1['sM7']);}?>" readonly/></td>
<td><input class="Input" id="M7_B<?php echo $Sn; ?>" value="<?php if($resP2['sM7']!=0){echo floatval($resP2['sM7']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M7_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM7']!=0){echo floatval($resP2ch['sM7']);}?>" readonly/></td><?php } ?>
<?php if($EntP=='Y'){ ?><td><input class="Input" id="M7_C<?php echo $Sn; ?>" value="<?php if($resP3['sM7']!=0){echo floatval($resP3['sM7']);}?>" readonly/></td><?php } ?>

<td><input class="Input" id="M8_D<?php echo $Sn; ?>" value="<?php if($resP0['sM8']!=0){echo floatval($resP0['sM8']);}?>" readonly/></td>
<td><input class="Input" id="M8_A<?php echo $Sn; ?>" value="<?php if($resP1['sM8']!=0){echo floatval($resP1['sM8']);}?>" readonly/></td>
<td><input class="Input" id="M8_B<?php echo $Sn; ?>" value="<?php if($resP2['sM8']!=0){echo floatval($resP2['sM8']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M8_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM8']!=0){echo floatval($resP2ch['sM8']);}?>" readonly/></td><?php } ?>
<?php if($EntP=='Y'){ ?><td><input class="Input" id="M8_C<?php echo $Sn; ?>" value="<?php if($resP3['sM8']!=0){echo floatval($resP3['sM8']);}?>" readonly/></td><?php } ?>

<td><input class="Input" id="M9_D<?php echo $Sn; ?>" value="<?php if($resP0['sM9']!=0){echo floatval($resP0['sM9']);}?>" readonly/></td>
<td><input class="Input" id="M9_A<?php echo $Sn; ?>" value="<?php if($resP1['sM9']!=0){echo floatval($resP1['sM9']);}?>" readonly/></td>
<td><input class="Input" id="M9_B<?php echo $Sn; ?>" value="<?php if($resP2['sM9']!=0){echo floatval($resP2['sM9']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M9_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM9']!=0){echo floatval($resP2ch['sM9']);}?>" readonly/></td><?php } ?>
...
<td><input class="Input" id="M10_D<?php echo $Sn; ?>" value="<?php if($resP0['sM10']!=0){echo floatval($resP0['sM10']);}?>" readonly/></td>
<td><input class="Input" id="M10_A<?php echo $Sn; ?>" value="<?php if($resP1['sM10']!=0){echo floatval($resP1['sM10']);}?>" readonly/></td>
<td><input class="Input" id="M10_B<?php echo $Sn; ?>" value="<?php if($resP2['sM10']!=0){echo floatval($resP2['sM10']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M10_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM10']!=0){echo floatval($resP2ch['sM10']);}?>" readonly/></td><?php } ?>
...
<td><input class="Input" id="M11_D<?php echo $Sn; ?>" value="<?php if($resP0['sM11']!=0){echo floatval($resP0['sM11']);}?>" readonly/></td>
<td><input class="Input" id="M11_A<?php echo $Sn; ?>" value="<?php if($resP1['sM11']!=0){echo floatval($resP1['sM11']);}?>" readonly/></td>
<td><input class="Input" id="M11_B<?php echo $Sn; ?>" value="<?php if($resP2['sM11']!=0){echo floatval($resP2['sM11']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M11_C<?php echo $Sn; ?>" value="<?php if($resP2ch['sM11']!=0){echo floatval($resP2ch['sM11']);}?>" readonly/></td><?php } ?>
...
<td><input class="Input" id="M12_D<?php echo $Sn; ?>" value="<?php if($resP0['sM12']!=0){echo floatval($resP0['sM12']);}?>" readonly/></td>
<td><input class="Input" id="M12_A<?php echo $Sn; ?>" value="<?php if($resP1['sM12']!=0){echo floatval($resP1['sM12']);}?>" readonly/></td>
<td><input class="Input" id="M12_B<?php echo $Sn; ?>" value="<?php if($resP2['sM12']!=0){echo floatval($resP2['sM12']);}?>" readonly/></td>
<?php if($RslP=='Y'){ ?><td><input class="Inputc" id="M12_C<?php echo $Sn; ?>" style="width:50px;" value="<?php if($resP2ch['sM12']!=0){echo floatval($resP2ch['sM12']);}?>" readonly/></td><?php } ?>
...
<td><input class="InputB" id="TotP0_<?php echo $Sn; ?>" value="<?php if($TotP0!=0){echo floatval($TotP0);}else{echo '';} ?>" readonly/></td>
<td><input class="InputB" id="TotP1_<?php echo $Sn; ?>" value="<?php if($TotP1!=0){echo floatval($TotP1);}else{echo '';} ?>" readonly/></td>
...
$sqlP0t=mysql_query("select SUM(M1_Ach) as tsM1,SUM(M2_Ach) as tsM2,SUM(M3_Ach) as tsM3,SUM(M4_Ach) as tsM4,SUM(M5_Ach) as tsM5,SUM(M6_Ach) as tsM6,SUM(M7_Ach) as tsM7,SUM(M8_Ach) as tsM8,SUM(M9_Ach) as tsM9,SUM(M10_Ach) as tsM10,SUM(M11_Ach) as tsM11,SUM(M12_Ach) as tsM12 from hrm_sales_sal_details_".$Before2YId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$Before2YId, $con); $resP0t=mysql_fetch_assoc($sqlP0t);

$sqlP1t=mysql_query("select SUM(M1_Ach) as tsM1,SUM(M2_Ach) as tsM2,SUM(M3_Ach) as tsM3,SUM(M4_Ach) as tsM4,SUM(M5_Ach) as tsM5,SUM(M6_Ach) as tsM6,SUM(M7_Ach) as tsM7,SUM(M8_Ach) as tsM8,SUM(M9_Ach) as tsM9,SUM(M10_Ach) as tsM10,SUM(M11_Ach) as tsM11,SUM(M12_Ach) as tsM12 from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$BeforeYId, $con); $resP1t=mysql_fetch_assoc($sqlP1t);

$sqlP2t=mysql_query("select SUM(M1) as tsM1,SUM(M2) as tsM2,SUM(M3) as tsM3,SUM(M4) as tsM4,SUM(M5) as tsM5,SUM(M6) as tsM6,SUM(M7) as tsM7,SUM(M8) as tsM8,SUM(M9) as tsM9,SUM(M10) as tsM10,SUM(M11) as tsM11,SUM(M12) as tsM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$_REQUEST['y'], $con); $resP2t=mysql_fetch_assoc($sqlP2t);

$sqlP2tch=mysql_query("select SUM(M1_Ach) as tsM1,SUM(M2_Ach) as tsM2,SUM(M3_Ach) as tsM3,SUM(M4_Ach) as tsM4,SUM(M5_Ach) as tsM5,SUM(M6_Ach) as tsM6,SUM(M7_Ach) as tsM7,SUM(M8_Ach) as tsM8,SUM(M9_Ach) as tsM9,SUM(M10_Ach) as tsM10,SUM(M11_Ach) as tsM11,SUM(M12_Ach) as tsM12 from hrm_sales_sal_details_".$_REQUEST['y']." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND sp.ItemId=".$_REQUEST['ii']." AND sd.YearId=".$_REQUEST['y'], $con); $resP2tch=mysql_fetch_assoc($sqlP2tch);
...
<?php if($_REQUEST['SelectV']=='DealerNo' AND $_REQUEST['DealerV']!='') {?> 
 
 <table border="0" cellpadding="0" cellspacing="2" cellpadding="0">
<?php $sqlDeal = mysql_query("select * from hrm_sales_dealer where DealerId=".$_REQUEST['DealerV']." order by DealerId ASC", $con); $resDealRow=mysql_num_rows($sqlDeal);  
...
<?php 
/////////////////////////////////////////////////
if($_REQUEST['ii']!='All_1' AND $_REQUEST['ii']!='All_2'){ $sqlDealChk = mysql_query("select count(*) as rtotal from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where si.ItemId=".$_REQUEST['ii']." AND YearId=".$BeforeYId." AND YearId!=".$_REQUEST['y']." AND YearId!=".$AfterYId." AND DealerId=".$resDeal['DealerId'], $con); }
elseif($_REQUEST['ii']=='All_1'){$sqlDealChk = mysql_query("select count(*) as rtotal from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where si.GroupId=1 AND YearId=".$BeforeYId." AND YearId!=".$_REQUEST['y']." AND YearId!=".$AfterYId." AND DealerId=".$resDeal['DealerId'], $con); }
elseif($_REQUEST['ii']=='All_2'){$sqlDealChk = mysql_query("select count(*) as rtotal from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId where si.GroupId=2 AND YearId=".$BeforeYId." AND YearId!=".$_REQUEST['y']." AND YearId!=".$AfterYId." AND DealerId=".$resDeal['DealerId'], $con); }
...
<div class="outerbox">
<div class="innerbox">	  
<table class="bluetable" border="1" cellpadding="0" cellspacing="0" style="font-family:Times New Roman;font-size:14px;width:<?php if($_REQUEST['ii']=='All_1' OR $_REQUEST['ii']=='All_2'){ echo '1390px';}else{echo '1290px';}?>;">	
  <tr>
...
<td colspan="4" align="center"><b>APR</b></td><td colspan="4" align="center"><b>MAY</b></td><td colspan="4" align="center"><b>JUN</b></td><td colspan="4" align="center"><b>JUL</b></td><td colspan="4" align="center"><b>AUG</b></td><td colspan="4" align="center"><b>SEP</b></td><td colspan="4" align="center"><b>OCT</b></td><td colspan="4" align="center"><b>NOV</b></td><td colspan="4" align="center"><b>DEC</b></td><td colspan="4" align="center"><b>JAN</b></td><td colspan="4" align="center"><b>FEB</b></td><td colspan="4" align="center"><b>MAR</b></td>
...
$sqlP0d=mysql_query("select M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach from hrm_sales_sal_details_".$Before2YId." where YearId=".$Before2YId." AND ProductId=".$res['ProductId']." AND DealerId=".$resDeal['DealerId'], $con); $resP0d=mysql_fetch_assoc($sqlP0d); 
$sqlP1d=mysql_query("select M1_Ach,M2_Ach,M3_Ach,M4_Ach,M5_Ach,M6_Ach,M7_Ach,M8_Ach,M9_Ach,M10_Ach,M11_Ach,M12_Ach from hrm_sales_sal_details_".$BeforeYId." where YearId=".$BeforeYId." AND ProductId=".$res['ProductId']." AND DealerId=".$resDeal['DealerId'], $con); $resP1d=mysql_fetch_assoc($sqlP1d);
...
/////////////////////////////////////////////////
$sqlP0dhq=mysql_query("select * from hrm_sales_sal_details_".$Before2YId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND ProductId=".$res['ProductId']." AND YearId=".$Before2YId, $con); 
$sqlP1dhq=mysql_query("select * from hrm_sales_sal_details_".$BeforeYId." sd INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId where d.HqId=".$_REQUEST['hq']." AND ProductId=".$res['ProductId']." AND YearId=".$BeforeYId, $con);
...
$rowP0dhq=mysql_num_rows($sqlP0dhq); $rowP1dhq=mysql_num_rows($sqlP1dhq); 
...
<?php } ?>		 
	 ...
	 <td align="center" valign="middle" bgcolor="#FFFFFF" id="TD<?php echo $resDeal['DealerId'].'_'.$Sn; ?>">
	   <?php //if($resE['EditPerMi']=='Y' AND $EntP=='Y'){ ?>
...
<td><input class="Input" id="<?php echo $DeId; ?>M10_De<?php echo $Sn; ?>" value="<?php if($resP0d['M10_Ach']!=0){echo floatval($resP0d['M10_Ach']);} ?>" readonly/></td>
<td><input class="Input" id="<?php echo $DeId; ?>M10_Ae<?php echo $Sn; ?>" value="<?php if($resP1d['M10_Ach']!=0){echo floatval($resP1d['M10_Ach']);} ?>" readonly/></td>
...
<td><input class="Input" id="<?php echo $DeId; ?>M11_De<?php echo $Sn; ?>" value="<?php if($resP0d['M11_Ach']!=0){echo floatval($resP0d['M11_Ach']);} ?>" readonly/></td>
<td><input class="Input" id="<?php echo $DeId; ?>M11_Ae<?php echo $Sn; ?>" value="<?php if($resP1d['M11_Ach']!=0){echo floatval($resP1d['M11_Ach']);} ?>" readonly/></td>
