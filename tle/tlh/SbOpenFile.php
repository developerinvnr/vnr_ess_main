...
{
 $sqlchk=mysql_query("select * from hrm_sales_emp_action_tle where TLEId=".$_POST['En']." AND YearId=".$_POST['Yn'], $con); $row=mysql_num_rows($sqlchk);
 if($row==0){ $sqlIns=mysql_query("insert into hrm_sales_emp_action_tle(TLEId,YearId,StatusA) values(".$_POST['En'].",".$_POST['Yn'].",1)", $con);}
 elseif($row>0)
 { $sqlIns=mysql_query("update hrm_sales_emp_action_tle set StatusA=1 where TLEId=".$_POST['En']." AND YearId=".$_POST['Yn'], $con);
   $res=mysql_fetch_assoc($sqlchk);
   if($res['StatusA']==1 OR $res['StatusB']==1)
   { $sqlCPerMi=mysql_query("update hrm_sales_tlemp set EditPerMi='N' where TLEId=".$_POST['En'],$con); }
 }
