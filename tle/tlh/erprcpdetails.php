...
<?php $sv=mysql_query("select sd.ProductId,si.ItemId,".$mf.",ProductName,ItemName from hrm_sales_sal_details sd inner join hrm_sales_seedsproduct sp on sd.ProductId=sp.ProductId inner join hrm_sales_seedsitem si on sp.ItemId=si.ItemId where si.GroupId=".$grp." AND sd.YearId=".$y." AND ".$mf."!=0 AND sd.DealerId=".$di,$con); 
...
$s2v=mysql_query("select SUM(".$mf.") as M from hrm_sales_sal_details sd inner join hrm_sales_seedsproduct sp on sd.ProductId=sp.ProductId inner join hrm_sales_seedsitem si on sp.ItemId=si.ItemId where si.GroupId=".$grp." AND sd.YearId=".$y." AND ".$mf."!=0 AND sd.DealerId=".$di,$con); $r2v=mysql_fetch_assoc($s2v); ?>  
 <?php /*?><tr>
