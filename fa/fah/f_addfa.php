...
if(isset($_POST['btnsave']))
{
  ...
  { $photoN=strtolower(basename($_FILES['photo']['name']));
	...
	$photoPath='../../Employee/sp/user/f_fafile/'.$photoName;
  ...
  { $dlN=strtolower(basename($_FILES['updl']['name']));
	...
	$dlName='dl'.$nMx.'.'.$dlExt;
	$dlPath='../../Employee/sp/user/f_fafile/'.$dlName;
  ...
  { $aadharN=strtolower(basename($_FILES['upaadhar']['name']));
	...
	$aadharPath='../../Employee/sp/user/f_fafile/'.$aadharName;
  ...
  { $panN=strtolower(basename($_FILES['uppan']['name']));
	...
	$panPath='../../Employee/sp/user/f_fafile/'.$panName;
  ...
  { $resumeN=strtolower(basename($_FILES['upresume']['name']));
	...
	$resumePath='../../Employee/sp/user/f_fafile/'.$resumeName;
  ...
  { $voterN=strtolower(basename($_FILES['upvotercard']['name']));
	...
	$voterPath='../../Employee/sp/user/f_fafile/'.$voterName;
  ...
  { $an1N=strtolower(basename($_FILES['upan1']['name']));
	...
	$an1Path='../../Employee/sp/user/f_fafile/'.$an1Name;
  ...
  { $an2N=strtolower(basename($_FILES['upan2']['name']));
	...
	$an2Path='../../Employee/sp/user/f_fafile/'.$an2Name;
...
function validate(reqform) 
{
 ...
 else if(document.getElementById("rep").value==0){ alert("Please select reporting name."); return false; }
...
<?php if($_REQUEST['opt']>0){ ?><option value='<?php echo $res['HqId'];?>' selected><?php echo ucfirst(strtolower($rc['HqName']));?></option><?php } else { ?><option value="0">Select</option><?php $sqlhq2=mysql_query("select * from hrm_headquater hq inner join hrm_sales_dealer d on (hq.HqId=d.Hq_vc OR hq.HqId=d.Hq_fc) where ".$state." group by HqName order by HqName asc",$con); while($reshq2=mysql_fetch_assoc($sqlhq2)){ ?><option value="<?php echo $reshq2['HqId']; ?>"><?php echo ucfirst(strtolower($reshq2['HqName'])); ?></option><?php }}?></select></span><input type="hidden" name="HeadQuarter" id="HeadQuarter" value="<?php if($_REQUEST['opt']>0){ echo $res['HqId']; } else {echo 0;} ?>" /></td>
	...
	<tr>
	 ...
	 <td><select style="width:200px;" class="InputSel" id="Crop" name="Crop" required <?php if($_REQUEST['opt']==0){echo 'disabled';} ?>>
	...
	<tr>
	 ...
	 <td class="tdf" align="center">:</td>
	 <td><span id="spanRep"><input style="width:200px;" class="InputType" id="rep" name="rep" readonly value="<?php if($_REQUEST['opt']>0){ $e=mysql_query("select Fname,Sname,Lname from hrm_employee where EmployeeID=".$res['Reporting'],$con); $re=mysql_fetch_assoc($e); echo ucfirst(strtolower($re['Fname'].' '.$re['Sname'].' '.$re['Lname'])); } ?>"/></span>
	 
	 <input type="hidden" name="repI" id="repI" value="0" /><input type="hidden" name="tl" id="tl" value="0" /></td>
	...
	<tr>
	 ...
	 <td class="tdf" align="center">:</td><td><input style="width:120px;" class="InputType" id="ohq" name="ohq" <?php if($_REQUEST['opt']==0){echo 'readonly';} ?> /><input type="hidden" style="width:120px;text-align:right;" class="InputType" id="expences" name="expences" <?php if($_REQUEST['opt']==0){echo 'readonly';} ?> value="0"/></td>
	</tr>
	<tr>
	 ...
	 <td class="tdf" align="center">:</td><td><select style="width:120px;" class="InputSel" id="jobstatus" name="jobstatus" <?php if($_REQUEST['opt']==0){echo 'disabled';} ?>><option value="Temporary">Temporary</option><option value="Permanent" selected>Permanent</option></select></td>
	</tr> 
	<tr>
	 <td class="tdf">&nbsp;DOJ</td>
	 <td class="tdf" align="center">:</td><td><input style="width:120px;text-align:center;" class="InputType" id="doj" name="doj" maxlength="10" placeholder="dd-mm-yyyy"/><button id="f_btn1" class="CalenderButton"></button><script type="text/javascript">var cal = Calendar.setup({ onSelect: function(cal) { cal.hide()}, showTime: true }); cal.manageFields("f_btn1", "doj", "%d-%m-%Y");</script></td>
	 <td class="tdf">&nbsp;DOB</td>
	 <td class="tdf" align="center">:</td><td><input style="width:120px;text-align:center;" class="InputType" id="dob" name="dob" maxlength="10" placeholder="dd-mm-yyyy"/><button id="f_btn2" class="CalenderButton"></button><script type="text/javascript">var cal = Calendar.setup({ onSelect: function(cal) { cal.hide()}, showTime: true }); cal.manageFields("f_btn2", "dob", "%d-%m-%Y");</script></td>
	</tr>
	<tr>
	...
	<tr>
	 ...
	 <td class="tdf" align="center">:</td><td><select style="width:120px;" class="InputSel" id="quali" name="quali" <?php if($_REQUEST['opt']==0){echo 'disabled';} ?>><option value="0" selected>Select</option><option value="High School">High School</option><option value="Higher Secondary">Higher Secondary</option><option value="Graduate">Graduate</option><option value="Post Graduate">Post Graduate</option></select></td>
	</tr>
	<tr>
...
<?php $sql=mysql_query("select * from fa_details where FaStatus='A' order by FaId DESC LIMIT 0,5",$con); 
    ...
    <tr style="height:22px;">
	  ...
	  <?php /*<td class="tdf2"><input class="InputType" style="width:150px;" value="<?php $crp=mysql_query("select ItemName from fa_details_crop rc inner join hrm_sales_seedsitem si on rc.ItemId=si.ItemId where FaId=".$res['FaId']." order by ItemName ASC",$con); while($rcrp=mysql_fetch_assoc($crp)){ echo ucfirst(strtolower($rcrp['ItemName'])).', '; } ?>" /></td>*/?>
	  <td class="tdf2"><input class="InputType" style="width:150px;" value="<?php echo $res['Crop'];?>" /></td>
