...
<?php $sql=mysql_query("select e.EmployeeID,Fname,Sname,Lname,DesigName from hrm_employee e inner join hrm_employee_general eg on e.EmployeeID=eg.EmployeeID inner join hrm_designation d on eg.DesigId=d.DesigId where eg.departmentId=6 AND e.EmpStatus='A' AND eg.GradeId>=67 AND eg.GradeId<=75 order by Fname ASC",$con); $sn=1; while($res=mysql_fetch_assoc($sql)){ $En=strtoupper($res['Fname'].' '.$res['Sname'].' '.$res['Lname']); ?>    
 <tr bgcolor="#FFFFF">
