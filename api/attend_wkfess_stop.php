<?php 
 ...
 /******************************* Joining Employee Open *************************************/
  ...
  if($json!=null)
  {
    ...
    //$ec_s = $json->NAME;
    $ec_s = $json->EmployeeID; //Mobile No
	...
	$attv=''; if($attv_s=='A'){ $attv='P'; }else{ $attv=''; }
	...
	$sqlE=mysqli_query($con, "select EmployeeID from hrm_employee_general where (MobileNo_Vnr=".trim($ec)." OR MobileNo2_Vnr=".trim($ec)." OR AttMobileNo1=".trim($ec)." OR AttMobileNo2=".trim($ec).") AND CompanyId=".$com); 
