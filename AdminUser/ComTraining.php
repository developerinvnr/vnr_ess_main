...
{
 $sql=mysql_query("select TrainingId from hrm_company_training_participant where TrainingId=".$_REQUEST['id'], $con);
 $row=mysql_num_rows($sql); 
 if($row>0)
 {
  $sqlc=mysql_query("select Duration from hrm_company_training where TrainingId=".$_REQUEST['id'], $con); $resc=mysql_fetch_assoc($sqlc);
  $TotManDay=$resc['Duration']*$row; 
  $sqlU=mysql_query("UPDATE hrm_company_training SET TotalParticipant=".$row.", Man_Day=".$TotManDay." WHERE TrainingId=".$_REQUEST['id'], $con);
 }
 elseif($row==0)
 {
  $sqlU=mysql_query("UPDATE hrm_company_training SET TotalParticipant=".$row.", Man_Day=0 WHERE TrainingId=".$_REQUEST['id'], $con);
 }
...
{ var Y = document.getElementById("Year").value; var x = 'ComTraining.php?Y='+Y+'&page=1'; window.location=x;}
								  
function ClickNew(c,u,yi,y,p)
{ var win=window.open("NewTrainingProg.php?c="+c+"&u="+u+"&yi="+yi,"leaveForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=380,height=290");
  var timer = setInterval( function() { if(win.closed) { clearInterval(timer);  window.location="ComTraining.php?Y="+y+"&page="+p; } }, 1000);
}

function EditTra(c,u,id,y,p,yi)
{ var win=window.open("TrainingProgDetails.php?c="+c+"&u="+u+"&id="+id+"&yi="+yi,"leaveForm","menubar=no,scrollbars=yes,resizable=no,directories=no,width=1040,height=550");
  var timer = setInterval( function() { if(win.closed) { clearInterval(timer);  window.location="ComTraining.php?Y="+y+"&action=ManDay&id="+id+"&page="+p; } }, 1000);
}

function PrintData(v)
{ var ComId=document.getElementById("ComId").value; 
  window.open("PrintTraData.php?v="+v+"&C="+ComId,"PrintForm","menubar=no,scrollbars=no,resizable=no,directories=no,width=50,height=50");
}
  
function ExportData(v)
{ var ComId=document.getElementById("ComId").value;   
  window.open("ExportTraData.php?v="+v+"&C="+ComId,"PrintForm","menubar=no,scrollbars=no,resizable=no,directories=no,width=20,height=20");
}
...
<table border="0" style="margin-top:0px; width:100%; height:350px;">
 <tr>
  <td align="left" height="20" valign="top" colspan="3">
   <table border="0">
    <tr>
	  <td style="width:140px;" align="">&nbsp;<font color="#2D002D" style='font-family:Times New Roman;' size="4"><b>Training : Year</b></font></td>
...
<?php } ?>
	  ...
	  <td style="font-size:14px;height:20px;font-family:Times New Roman;"><a href="NewTraParticular.php">Add New Particular</a></td>
	  <td>&nbsp;</td>
 ...
 <tr>
  <td valign="top">
   <table border="1" valign="top" style="width:1200px;">
 <tr bgcolor="#7a6189">
	...
	<td align="center" style="color:#FFFFFF;" class="All_150"><b>Institute</b></td>
	<td align="center" style="color:#FFFFFF;" class="All_150"><b>Trainer</b></td>
...
<?php
if($_REQUEST['Y']!='All'){ $sql_statement = mysql_query("SELECT * from hrm_company_training WHERE TraYear=".$_REQUEST['Y']." AND CompanyId=".$CompanyId." order by TraFrom DESC", $con);}
if($_REQUEST['Y']=='All'){ $sql_statement= mysql_query("SELECT * from hrm_company_training WHERE CompanyId=".$CompanyId." order by TraFrom DESC", $con); }
...
$from = 0;
 ...
 if($_REQUEST['Y']!='All'){ $sqlT = mysql_query("SELECT * from hrm_company_training WHERE TraYear=".$_REQUEST['Y']." AND CompanyId=".$CompanyId." order by TraFrom DESC LIMIT ".$from.",".$offset, $con); }
 if($_REQUEST['Y']=='All'){ $sqlT = mysql_query("SELECT * from hrm_company_training WHERE CompanyId=".$CompanyId." order by TraFrom DESC LIMIT ".$from.",".$offset, $con); }
 if($total_records>0) 
...
?> 	 
 <tr bgcolor="<?php if($Sno%2){ echo '#FFFFFF'; } else {echo '#D9D1E7';}?>">
	...
	<td align="" style="" class="All_150" valign="top"><?php echo $resT['TraTitle']; ?></td>
	<td align="center" style="" class="All_60" valign="top"><?php echo date("d-M-y",strtotime($resT['TraFrom'])); ?></td>
	<td align="center" style="" class="All_80" valign="top"><?php echo $resT['Duration']; ?>&nbsp;<?php if($resT['Duration']>1){echo 'Days';}else{echo 'Day';}; ?></td>
	<td align="" style="" class="All_150" valign="top"><?php echo $resT['Institute']; ?></td>
	<td align="" style="" class="All_150" valign="top"><?php echo $resT['TrainerName']; ?></td>
	...
	<td align="center" style="" class="All_50" valign="top">
        <a href="#"><img src="images/edit.png" border="0" alt="Edit" onClick="EditTra(<?php echo $CompanyId.', '.$UserId.', '.$resT['TrainingId']; ?>, '<?php echo $_REQUEST['Y']; ?>', <?php echo $_REQUEST['page'].', '.$YearId; ?>)"></a></td>
 </tr>
<?php $Sno++; }?> 
<tr>
  <td class="fontButton" align="center" colspan="9" style="font-family:Times New Roman; font-size:15px; font-weight:bold;">
  <?PHP doPages($offset, 'ComTraining.php', '', $total_records); ?>
  </td>
