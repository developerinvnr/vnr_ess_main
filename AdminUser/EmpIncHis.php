...
$sn_ctc=mysql_query("select * from hrm_employee_ctc where CtcCreatedDate='".$res['SalaryChange_Date']."' AND CtcId=(select Max(CtcId) from hrm_employee_ctc where EmployeeID=".$resE['EmployeeID']." AND CtcCreatedDate='".$res['SalaryChange_Date']."')",$con); $rn_ctc=mysql_fetch_assoc($sn_ctc); ?>  	
   <tr bgcolor="#ffffff">
	...
	<td class="tdc"><?php if($_REQUEST['U']==4 || $_REQUEST['U']==5 || $_REQUEST['U']==11 || $_REQUEST['U']==15 || $_REQUEST['U']==16 || $_REQUEST['U']==19 || $_REQUEST['U']==38 || $_REQUEST['U']==39 || $_REQUEST['U']==49){?><img src="images/delete.png" onClick="DelFun(<?php echo $res['AppHistoryId'].','.$_REQUEST['V'].','.$_REQUEST['C'].','.$_REQUEST['U']; ?>)" /><?php } ?></td>
