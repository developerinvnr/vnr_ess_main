...
<html>
<head>
<title>Investment Declaration Form 2013-14</title>
...
function PrintIvestDecl(E,Y,C) 
{window.open("InvestDeclPrint.php?action=Print&EI="+E+"&YI="+Y+"&CI="+C,"PrintForm","location=no,scrollbars=no,resizable=no,menubar=no,width=820,height=750");}
</script>
</head>
<body>
...
<a href="#" onClick="PrintIvestDecl(<?php echo $_REQUEST['EI'].','.$_REQUEST['YI'].','.$_REQUEST['CI']; ?>)">Print</a>&nbsp;&nbsp;</td></tr>
<?php 
$EI=$_REQUEST['EI']; $YI=$_REQUEST['YI']; $CI=$_REQUEST['CI'];
$sqlE=mysql_query("select Fname,Sname,Lname,EmpCode,DesigId,DepartmentId,Gender,PanNo,Married,DR from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_employee.EmployeeID=".$EI, $con); $resE=mysql_fetch_assoc($sqlE); 
...
if($resE['DR']=='Y'){$M='Dr.';} elseif($resE['Gender']=='M'){$M='Mr.';} elseif($resE['Gender']=='F' AND $resE['Married']=='Y'){$M='Mrs.';} elseif($RresE['Gender']=='F' AND $resE['Married']=='N'){$M='Miss.';}  
...
$SqlIvst=mysql_query("select * from hrm_employee_investment_declaration where EmployeeID=".$EI." AND YearId=".$YI." AND Status='A'", $con); 
$rowIvst=mysql_num_rows($SqlIvst); if($rowIvst>0) { $resIvst=mysql_fetch_assoc($SqlIvst); 
$SqlY=mysql_query("select FromDate,ToDate from hrm_year where YearId=".$YI, $con); $ResY=mysql_fetch_assoc($SqlY);
   ...
   <?php //*************************************************************************************************************************************************** ?>	   
	     ...
	     <table border="0" width="850">	 
		    <tr>
			 ...
			 <font color="#106809" style='font-family:Times New Roman;' size="4"><b>Investment Declaration Form 2014-15</b></font>
			 </td>
			</tr>
			<tr>
			 <td>
...
<tr>
  <td align="center" style="width:850px;font-size:15px; height:18px;" colspan="4">&nbsp;<b>(To be used to declare investment for income Tax that will be made during the period 2014-2015)</b></td>
</tr>
...
<?php $SqlCom=mysql_query("select CompanyName from hrm_company_basic where CompanyId=".$CI."", $con); $resCom=mysql_fetch_assoc($SqlCom); ?> 
...
<td align="center" valign="top" style="width:110px;font-size:15px;"><?php if($resIvst['HRA']==''){echo $resHRA['HRA_Value'];} else {echo $resIvst['HRA'];} ?></td>
</tr>
<tr>
