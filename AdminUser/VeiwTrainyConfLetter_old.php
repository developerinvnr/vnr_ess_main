...
$rg=mysql_fetch_assoc(mysql_query("select g.GradeId,GradeValue from hrm_grade gr inner join hrm_employee_general g on gr.GradeId=g.GradeId where g.EmployeeId=".$_REQUEST['E'],$con));
$rde=mysql_fetch_assoc(mysql_query("select g.DesigId,DesigName from hrm_designation de inner join hrm_employee_general g on de.DesigId=g.DesigId where g.EmployeeId=".$_REQUEST['E'],$con));
	  ...
	  <td style="font-family:Times New Roman;color:#000000; width:785;" align="center">
	    <table border="0">
		  ...
		  <td style="width:685px;font-size:18px;text-align:justify;">
           ...
           We are now pleased to confirm your services with effect from <b><?php echo date("d-m-Y", strtotime($res2['ConfDate'])); //$ConfDate; ?></b> and your designation has been revised from <b><?php echo ucwords(strtolower($rde['DesigName'])); ?> Trainee</b> to <b><?php echo ucwords(strtolower($rde['DesigName'])); ?></b> at <b><?php if($res['GradeId']!=$rg['GradeId']){echo 'Grade-'.$res['GradeValue'].' to ';}echo 'Grade-'.$rg['GradeValue']; ?></b>.<p> 
   ...
   <tr>
	  <td style="font-family:Times New Roman;color:#000000; width:785;" align="center">
	    <table border="0">
		  ...
		  <tr><td style="width:100px;">&nbsp;</td><td style="width:343px;font-size:18px;font-weight:bold;">VNR <?php if($res['CompanyId']==1){echo 'Seeds';}elseif($res['CompanyId']==3){echo 'Nursery'; } ?> Pvt. Ltd</td><td style="width:50px;">&nbsp;</td></tr>
		</table>
	  </td>
	 </tr>	 	 
   </table>
