...
//****************************************************************************************************************
if(isset($_POST['SaveRepRevHod']))
{ 
  for($i=1; $i<=$_POST['Sn']; $i++)
  { $Sql=mysql_query("update hrm_employee_reporting_pmskra set AppraiserId=".$_POST['SelRep_'.$i].", ReviewerId=".$_POST['SelRev_'.$i].", HodId=".$_POST['SelHod_'.$i]." where EmployeeID=".$_POST['EI_'.$i], $con); }
...
{ var url = 'ReportingPmskra.php?v='+value; window.location=url; }
...
{ document.getElementById('ChangeE').style.display='none'; document.getElementById('EditE').style.display='block';
  ...
  for(var i=1; i<=no; i++)
  { document.getElementById('SelRep_'+i).disabled=false; document.getElementById('SelRev_'+i).disabled=false; document.getElementById('SelHod_'+i).disabled=false;}
}
...
<tr>
 <td>
  <table>
    <tr>
     <td align="left" class="heading">Assign Appraiser/Reviewer/HOD For PMS-KRA Process</td>
...
<?php if($_REQUEST['v']!=''){ $sql = mysql_query("SELECT hrm_employee_reporting_pmskra.*, EmpCode,Fname,Sname,Lname,DesigId,DR,Gender,Married FROM hrm_employee_reporting_pmskra INNER JOIN hrm_employee_general ON hrm_employee_reporting_pmskra.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee ON hrm_employee_reporting_pmskra.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee_reporting_pmskra.EmployeeID=hrm_employee_personal.EmployeeID WHERE hrm_employee.EmpStatus='A' AND hrm_employee.EmpType='E' AND hrm_employee.CompanyId=".$CompanyId." AND hrm_employee_general.DepartmentId=".$_REQUEST['v']." order by EmpCode ASC", $con); $Sno=1;  while($res = mysql_fetch_assoc($sql)) { 
...
$NameE=$resE['Fname'].' '.$resE['Sname'].' '.$resE['Lname'].'-'.$resE['EmpCode']; ?> 
 <option style="background-color:#FFFFFF;" value="<?php echo $resE['EmployeeID']; ?>"><?php echo $NameE; ?></option><?php } ?>
    ...
    <td align="center" style="color:#FFFFFF;" class="All_200"><select <?php if($res['ReviewerId']==0 OR $res['ReviewerId']==''){echo 'style="background-color:#FFFFFF;"'; } if($res['ReviewerId']!=0 AND $res['ReviewerId']!='') {echo 'style="background-color:#FFE8DD;"'; } ?> name="SelRev_<?php echo $Sno; ?>" id="SelRev_<?php echo $Sno; ?>" class="All_190" disabled>
 <?php if($res['HodId']==0 OR $res['HodId']==''){?><option value="0">Select HOD</option><?php } else { $sqlRe = mysql_query("SELECT EmpCode,Fname,Sname,Lname,DR,Gender,Married FROM hrm_employee INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID WHERE hrm_employee.EmployeeID=".$res['ReviewerId'], $con); 
$resRe = mysql_fetch_assoc($sqlRe);  if($resRe['DR']=='Y'){$MRe='Dr.';} elseif($resRe['Gender']=='M'){$MRe='Mr.';} elseif($resRe['Gender']=='F' AND $resRe['Married']=='Y'){$MRe='Mrs.';} elseif($resRe['Gender']=='F' AND $resRe['Married']=='N'){$MRe='Miss.';}  $NameRe=$resRe['EmpCode'].'-'.$MRe.' '.$resRe['Fname'].' '.$resRe['Sname'].' '.$resRe['Lname']; ?>
 <option value="<?php echo $res['ReviewerId']; ?>"><?php echo $NameRe; ?></option><?php } ?>
<?php $sqlE3=mysql_query("select hrm_employee.EmployeeID,EmpCode,Fname,Sname,Lname,DR,Gender,Married from hrm_employee INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_employee.EmpStatus='A' AND hrm_employee.CompanyId=".$CompanyId." order by Fname ASC", $con); 
while($resE3=mysql_fetch_array($sqlE3)){ //if($resE3['DR']=='Y'){$ME3='Dr.';} elseif($resE3['Gender']=='M'){$ME3='Mr.';} elseif($resE3['Gender']=='F' AND $resE3['Married']=='Y'){$ME3='Mrs.';} elseif($resE3['Gender']=='F' AND $resE3['Married']=='N'){$ME3='Miss.';} 
$NameE3=$resE3['Fname'].' '.$resE3['Sname'].' '.$resE3['Lname'].'-'.$resE3['EmpCode']; ?> 
 <option style="background-color:#FFFFFF;" value="<?php echo $resE3['EmployeeID']; ?>"><?php echo $NameE3; ?></option><?php } ?>
 </select></td>
...
<?php if($_SESSION['User_Permission']=='Edit'){?>
	<table border="0"><tr>
    <td align="right" style="width:90px;" ><input type="button" name="ChangeE" id="ChangeE" style="width:90px;display:block;" value="edit" onClick="EditAppRev()" disabled>
	<input type="submit" name="SaveRepRevHod" id="EditE" style="width:90px;display:none;" value="save"></td>
    <td align="right" style="width:90px;"><input type="button" value="refresh" style="width:90px;" onClick="javascript:window.location='ReportingPmsKra.php?v=<?php echo $_REQUEST['v']; ?>'"/></td></tr>
    </table>
<?php } ?>
    </td>
   </tr>
