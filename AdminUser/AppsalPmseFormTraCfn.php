...
function SelectYear(v){window.location='AppsalPmseFormTraCfn.php?YI='+v;}
function SelectEKra(v,yi,e,a)
{ if(e=='d'){var ee='Dept';}else if(e=='a'){var ee='App';}else if(e=='r'){var ee='Rev';}else if(e=='h'){var ee='Hod';} 
  window.location='AppsalPmseFormTraCfn.php?action=EmpTf&ee='+ee+'&value='+v+'&YI='+yi+'&a='+a;}
    
function PrintTf(v,yi,ee,a)
{var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;
 window.open("PrintPMSFormTf.php?action=Score&value="+v+"&c="+ComId+"&YI="+yi+"&ee="+ee+'&a='+a,"PrintForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=50,height=50");}

function ExportTf(v,yi,ee,a)
{ var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;   
  window.open("ExportPmsFormTf.php?action=FormTfExport&value="+v+"&c="+ComId+"&YI="+yi+"&ee="+ee+'&a='+a,"ExportForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");}
		...
		<td align="" style="width:100%;" valign="top">
		   <table border="0" style="width:100%;">
       		 <tr><td colspan="2">
			       <table border="0">
                     <tr>
					  <td style="width:20%;" align="left" class="heading">Trainig/ Conference  &nbsp;<span id="ReturnValue">&nbsp;</span></td>
					  <td>
				    <table border="0">
                    <tr>
...
<tr>
 <td>
   ...
   <thead>
     <tr>
  <td colspan="<?php if($_REQUEST['a']==0){echo 8;}else{echo 11;} ?>" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Trainig/ Conference <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2.'-'.$PRD; ?>&nbsp;)&nbsp;&nbsp;&nbsp;<a href="#" onClick="ExportTf(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>',<?php echo $_REQUEST['a']; ?>)" style="color:#F9F900; font-size:12px;">Export Excel</a>
	  </td>
	  <?php /*?><a href="#" onClick="PrintTf(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>',<?php echo $_REQUEST['a']; ?>)" style="color:#F9F900; font-size:12px;">Print</a><?php */?>
	  
	 </tr>
	 <tr bgcolor="#7a6189">
	    <td rowspan="2" class="th" style="width:2%;">&nbsp;</td>
		<td rowspan="2" class="th" style="width:3%;"><b>SNo.</b></td>
		<td rowspan="2" class="th" style="width:5%;"><b>EC</b></td>
		<td rowspan="2" class="th" style="width:17%;"><b>Name</b></td>	
		<td colspan="2" class="th"><b>Appraiser</b></td>	
		<td colspan="2" class="th"><b>Reviewer</b></td>
  	</tr>
	<tr bgcolor="#7a6189">
		<td class="th" style="width:19%;"><b>Soft Skill</b></td>	
		<td class="th" style="width:19%;"><b>Technincal Skill</b></td>
		<td class="th" style="width:19%;"><b>Soft Skill</b></td>	
		<td class="th" style="width:19%;"><b>Technincal Skill</b></td>
  	</tr>
	</thead>
   ...
   <tbody>
	 <tr bgcolor="#FFFFFF" id="TR<?php echo $Sno; ?>">
		...
		<?php for($i=1; $i<=5; $i++){ 
		    ...
		    {
		    $sS1 = mysql_query("SELECT Tid,Category,Topic,Description FROM hrm_pms_training WHERE Tid='".$res['Appraiser_SoftSkill_'.$i]."'",$con); $rS1 = mysql_fetch_assoc($sS1); echo $rS1['Topic']; if($i<=4 && $rS1['Tid']>0){echo ', '; } 
		    }      
		    elseif($res['Appraiser_SoftSkill_'.$i]==69){
		       echo '<b>Other:</b> '. $res["Appraiser_SoftSkill_Oth"]; 
		    }
		 } ?>
		<?php //echo $res['Appraiser_SoftSkill_1'].', '.$res['Appraiser_SoftSkill_2']; ?></td>	
		...
		<?php for($j=1; $j<=5; $j++){ 
		    ...
		    $sS1 = mysql_query("SELECT Tid,Category,Topic,Description FROM hrm_pms_training WHERE Tid='".$res['Appraiser_TechSkill_'.$j]."'",$con); $rS1 = mysql_fetch_assoc($sS1); echo $rS1['Topic'];  if($j<=4 && $rS1['Tid']>0){echo ', '; } 
		    }   
		    elseif($res['Appraiser_TechSkill_'.$j]==70){
		       echo '<b>Other:</b> '. $res["Appraiser_TechSkill_Oth"]; 
		    }
		        
		    } ?>
		<?php //echo $res['Appraiser_TechSkill_1'].', '.$res['Appraiser_TechSkill_2']; ?></td>
		<td class="tdl">
		<?php for($i=1; $i<=5; $i++){ 
		    if($res['Reviewer_SoftSkill_'.$i]>0 && res['Reviewer_SoftSkill_'.$i]!=69)
		    {
		    $sS1 = mysql_query("SELECT Tid,Category,Topic,Description FROM hrm_pms_training WHERE Tid='".$res['Reviewer_SoftSkill_'.$i]."'",$con); $rS1 = mysql_fetch_assoc($sS1); echo $rS1['Topic']; if($i<=4 && $rS1['Tid']>0){echo ', '; } 
		    }      
		    elseif($res['Reviewer_SoftSkill_'.$i]==69){
		       echo '<b>Other:</b> '. $res["Appraiser_SoftSkill_Oth"]; 
		    }
		 } ?>
		<?php //echo $res['Reviewer_SoftSkill_1'].', '.$res['Reviewer_SoftSkill_2']; ?></td>	
		...
		<?php for($j=1; $j<=5; $j++){ 
		    ...
		    $sS1 = mysql_query("SELECT Tid,Category,Topic,Description FROM hrm_pms_training WHERE Tid='".$res['Reviewer_TechSkill_'.$j]."'",$con); $rS1 = mysql_fetch_assoc($sS1); echo $rS1['Topic'];  if($j<=4 && $rS1['Tid']>0){echo ', '; } 
		    }   
		    elseif($res['Reviewer_TechSkill_'.$j]==70){
		       echo '<b>Other:</b> '. $res["Reviewer_TechSkill_Oth"]; 
		    }
		        
		    } ?>
		<?php //echo $res['Reviewer_TechSkill_1'].', '.$res['Reviewer_TechSkill_2']; ?></td>		
	 </tr>
	 </tbody>
	 </div>
	 <?php $Sno++; } ?>
