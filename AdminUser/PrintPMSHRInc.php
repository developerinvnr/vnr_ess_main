...
<tr>
 <td>
   <table border="1" width="1800">
     <tr>
  <td colspan="11" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Full & Final Increment <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2.'-'.$PRD; ?>&nbsp;)&nbsp;&nbsp;&nbsp;
<a href="#" onClick="PrintHRInc(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>')" style="color:#F9F900; font-size:12px;">Print</a>&nbsp;&nbsp;<a href="#" onClick="ExportHRInc(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>')" style="color:#F9F900; font-size:12px;">Export Excel</a>
	  </td>
...
{ $sql=mysql_query("select hrm_employee.*,EmpPmsId,Emp_TotalFinalScore,Emp_TotalFinalRating,EmpCurrGrossPM,HR_CurrDesigId,HR_CurrGradeId,HR_Curr_DepartmentId,HR_Score,HR_Rating,HR_DepartmentId,HR_DesigId,HR_GradeId,HR_ProIncSalary,HR_Percent_ProIncSalary,HR_ProCorrSalary,HR_Percent_ProCorrSalary,HR_IncNetMonthalySalary,HR_Percent_IncNetMonthalySalary,HR_GrossMonthlySalary,HR_GrossAnnualSalary,HR_CTC from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_pms ON hrm_employee.EmployeeID=hrm_employee_pms.EmployeeID where hrm_employee.CompanyId=".$CompanyId." AND hrm_employee.EmpStatus='A' AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." AND hrm_employee_pms.HOD_EmployeeID=".$_REQUEST['value']." AND hrm_employee_pms.Appraiser_EmployeeID!=0 order by EmpCode ASC", $con);
 ...
 ?>	   
	 <tr bgcolor="#FFFFFF" id="TR<?php echo $Sno; ?>">
		...
		<td align="center" class="recCls"><?php echo $res['HR_Score']; ?></td> 
		<td align="center" class="recCls"><?php echo $res['HR_Rating']; ?></td> 
<?php if($res['HR_DesigId']!=$res['HR_CurrDesigId']){ $sqlDesigH=mysql_query("select DesigName,DesigCode from hrm_designation where DesigId=".$res['HR_DesigId']." AND CompanyId=".$CompanyId, $con); $resDesigH=mysql_fetch_assoc($sqlDesigH); $DesigH=$resDesigH['DesigName']; }else{$DesigH='';}
if($res['HR_GradeId']!=$res['HR_CurrGradeId']){ $sqlGH=mysql_query("select GradeValue from hrm_grade where GradeId=".$res['HR_GradeId']." AND CompanyId=".$CompanyId, $con); $resGH=mysql_fetch_assoc($sqlGH); $GradeH=$resGH['GradeValue']; }else{$GradeH='';} ?>	
		...
		<td class="recCls" align="right"><?php echo $res['HR_ProIncSalary']; ?></td>
		<td class="recCls" align="center"><?php echo $res['HR_Percent_ProIncSalary']; ?></td>
		<td class="recCls" align="right"><?php echo $res['HR_ProCorrSalary']; ?></td>
        <td class="recCls" align="center"><?php echo $res['HR_Percent_ProCorrSalary']; ?></td> 		
        <td class="recCls" align="right"><?php echo $res['HR_IncNetMonthalySalary']; ?></td>
        <td class="recCls" align="center"><?php echo $res['HR_Percent_IncNetMonthalySalary']; ?></td>
		<td class="recCls" align="right"><?php echo $res['HR_GrossMonthlySalary']; ?></td> 		
	 </tr>
	 <?php $Sno++; } ?>
</table>
 </td>
