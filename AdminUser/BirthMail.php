...
$chkWishM=mysql_query("select * from hrm_wishmail_checker where WDate='".date("Y-m-d")."' AND WCheck=1 AND CompanyId=1",$con); 
...
$sqlEa=mysql_query("select e.EmployeeID,e.EmpCode,Fname,Sname,Lname,Gender,Married,DR,DepartmentCode,HqName from hrm_employee_general g inner join hrm_employee e on g.EmployeeID=e.EmployeeID inner join hrm_employee_personal p on g.EmployeeID=p.EmployeeID inner join hrm_department d on g.DepartmentId=d.DepartmentId inner join hrm_headquater hq on g.HqId=hq.HqId where e.EmpStatus='A' AND e.CompanyId=1 AND e.EmployeeID!=6 AND e.EmployeeID!=914 AND p.Married='Y' AND p.MarriageDate_dm='".date("0000-m-d")."' order by g.GradeId DESC",$con); $rowEa=mysql_num_rows($sqlEa);
...
if($dy==0){ $ImgA='https://www.vnrseeds.co.in/AdminUser/AnnBirth/AA_7.jpg'; $ImgB='https://www.vnrseeds.co.in/AdminUser/AnnBirth/BB_7.jpg'; }
else{ $ImgA='https://www.vnrseeds.co.in/AdminUser/AnnBirth/AA_'.$dy.'.jpg'; $ImgB='https://www.vnrseeds.co.in/AdminUser/AnnBirth/BB_'.$dy.'.jpg'; }
...
if($rowE>0 || $rowEa>0)
{
 ...
 /*
 $sql=mysql_query("select g.EmployeeID,EmailId_Vnr from hrm_employee_general g inner join hrm_employee e on g.EmployeeID=e.EmployeeID where e.EmpStatus='A' AND e.CompanyId=1 AND g.EmailId_Vnr!='' AND g.EmployeeID=169 order by e.EmployeeID ASC",$con); //AND (g.DepartmentId=1 OR g.EmployeeID=169) AND (g.DepartmentId=1 OR g.DepartmentId=9)
 //order by e.EmployeeID ASC limit 0,90
