...
//echo 'kk='.$YId;
$SqlPay=mysql_query("SELECT * FROM ".$PayTable." WHERE EmployeeID=".$_REQUEST['EI']." AND Month=".$_REQUEST['m']." AND YEAR=".$_REQUEST['Y']."", $con); 

$RowPay=mysql_num_rows($SqlPay);
 if($RowPay==0)
 { 
  $SqlPay=mysql_query("SELECT * FROM hrm_employee_monthlypayslip WHERE EmployeeID=".$_REQUEST['EI']." AND Month=".$_REQUEST['m']." AND Year=".$_REQUEST['Y']."", $con); $ResPay=mysql_fetch_assoc($SqlPay);
