...
while($rMain=mysql_fetch_assoc($sMain))
{
 ...
 $sPmsId=mysql_query("select EmpPmsId,Hod_TotalFinalRating,HR_CurrDesigId,HR_CurrGradeId,Hod_EmpDesignation,Hod_EmpGrade,HR_DesigId,HR_GradeId from hrm_employee_pms where AssessmentYear='".$rSet['CurrY']."' AND EmployeeID=".$rMain['EmployeeID'],$con); 
 $rPm=mysql_fetch_assoc($sPmsId);
 
 
 if($rPm['HR_GradeId']>0){$EGr=$rPm['HR_GradeId'];}
 elseif($rPm['Hod_EmpGrade']>0){$EGr=$rPm['HR_GradeId']; }
 else{$EGr=$rPm['HR_CurrGradeId'];}
 
 
 if($rPm['HR_DesigId']>0){$qryD="DesigId=".$rPm['HR_DesigId'];}else{$qryD="DesigId=".$rPm['Hod_EmpDesignation'];}
 if($rPm['HR_GradeId']>0){ $qryG="GradeId=".$rPm['HR_GradeId'];}else{$qryG="GradeId=".$rPm['Hod_EmpGrade']; }

 $sqlD=mysql_query("select DesigName,DesigId from hrm_designation where ".$qryD,$con); 
 $sqlG=mysql_query("select GradeValue from hrm_grade where CompanyId=".$_REQUEST['C']." AND ".$qryG,$con);
...
$suqry="Rating=".$RNew." AND MgmtId=".$ResPMS['HOD_EmployeeID']." AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'];
...
if($_REQUEST['C']==1){ $qry="cpms.CtcCreatedDate='".date("Y-01-01")."'"; }
elseif($_REQUEST['C']==3){ $qry="cpms.CtcCreatedDate='".date("Y-04-01")."'"; }

if($_REQUEST['action']=='All') { $SqlCtc=mysql_query("SELECT e.*, cpms.*, Gender, DR, Married FROM hrm_employee e INNER JOIN hrm_employee_personal p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_employee_ctc_pms cpms ON e.EmployeeID=cpms.EmployeeID WHERE e.EmployeeID=".$ENew." AND ".$qry." AND e.CompanyId=".$_REQUEST['C'], $con); $ResCtc=mysql_fetch_assoc($SqlCtc); 
...
$SqlEligEmp = mysql_query("SELECT * FROM hrm_employee_eligibility_pms WHERE EmployeeID=".$ENew." AND Status='A'", $con) or die(mysql_error());  $ResEligEmp=mysql_fetch_assoc($SqlEligEmp); 
$SqlCtc = mysql_query("SELECT ESCI FROM hrm_employee_ctc_pms WHERE EmployeeID=".$ENew." AND Status='A'", $con); $ResCtc=mysql_fetch_assoc($SqlCtc);
