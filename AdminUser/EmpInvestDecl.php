...
{ $sqlE=mysql_query("update hrm_employee_investment_declaration set FormSubmit='Y' where EmployeeID=".$_REQUEST['EI']." AND Period='".$_REQUEST['P']."' AND Month=".$_REQUEST['M'], $con); 
}
?>
<html>
<head>
...
{ var YI=document.getElementById("YearId").value; var CI=document.getElementById("ComId").value;
  window.open("InvestDeclPrint.php?action=Print&EI="+EI+"&YI="+YI+"&CI="+CI+"&P="+P+"&M="+M,"PrintForm","location=no,scrollbars=yes,resizable=no,menubar=no,width=950,height=700");
}
...
<?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?>
					   ...
					   <option value="All">&nbsp;All</option>
                                           ...
                                           <td align="center" style="" class="All_100"><a href="#" onClick="ExpIvtFormate('<?php echo $resMs['Period']; ?>',<?php echo $resMs['Month'].','.$CompanyId; ?>)"><i><b>Export-Report(All)</b></i></a></td>
										   <script type="text/javascript">
										   function ExpIvtFormate(p,m,c)
										   { 
										    
										    window.open("EmpInvestDeclExp.php?action=Export&P="+p+"&M="+m+"&CI="+c,"ExpForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20"); 
...
<tr bgcolor="#7a6189">
    ...
    <td align="center" style="color:#FFFFFF;" class="All_100"><b>Status</b></td>
    <td align="center" style="color:#FFFFFF;" class="All_100"><b>Details</b></td>
...
<?php $sql33=mysql_query("select C_YearId from hrm_investdecl_setting where CompanyId=".$CompanyId." AND Status='A'", $con); $res33=mysql_fetch_array($sql33);
	  ...
	  <a href="#"><img src="images/go-back-icon.png" onClick="ReDir('<?php echo $resMs2['Period']; ?>',<?php echo $res['EmployeeID'].', '.$resMs2['Month'].', '.$_REQUEST['mxid'].', '.$_REQUEST['dd']; ?>)" border="0"></a>
	  <?php } ?>
<?php } ?>
	</td>
</tr>
