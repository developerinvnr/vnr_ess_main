...
.All_150{font-size:11px; height:20px; width:150px;}.All_170{font-size:11px; height:20px; width:170px;}.All_180{font-size:11px; height:20px; width:180px;}.All_190{font-size:11px; height:20px; width:190px;} .All_200{font-size:11px; height:20px; width:200px;} .All_450{font-size:11px; height:20px; width:450px;}.All_360{font-size:11px; height:20px; width:350px;}.All_540{font-size:11px; height:20px; width:540px;}.All_400{font-size:11px; height:20px; width:400px;} .All_600{font-size:11px; height:20px; width:600px;}.recCls{font-family:Georgia; font-size:12px;}.All_500{font-size:11px; height:20px; width:500px;}
...
function SelectYear(v){window.location='AppsalPmseFormKra.php?YI='+v;}
function SelectEKra(v,yi,e,a)
{ if(e=='d'){var ee='Dept';}else if(e=='a'){var ee='App';}else if(e=='r'){var ee='Rev';}else if(e=='h'){var ee='Hod';} 
  window.location='AppsalPmseFormKra.php?action=EmpKra&ee='+ee+'&value='+v+'&YI='+yi+'&a='+a;}
    
function PrintKra(v,yi,ee,a)
{var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;
 window.open("PrintPMSFormKra.php?action=Score&value="+v+"&c="+ComId+"&YI="+yi+"&ee="+ee+'&a='+a,"PrintForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=50,height=50");}

function ExportKra(v,yi,ee,a)
{ var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;   
  window.open("ExportPmsFormKra.php?action=FormKraExport&value="+v+"&c="+ComId+"&YI="+yi+"&ee="+ee+'&a='+a,"ExportForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");}
		...
		<td align="" style="width:100%;" valign="top">
		   <table border="0" style="width:100%;">
       		 <tr><td colspan="2">
			       <table border="0">
                     <tr>
					  <td style="width:10%;" align="left" class="heading">KRA List &nbsp;<span id="ReturnValue">&nbsp;</span></td>
					  <td>
				    <table border="0">
                    <tr>
...
<td class="td1" style="font-size:12px;" align="center"><select style="font-size:12px; width:158px; height:20px; background-color:#DDFFBB;" name="DeptScore" id="DeptScore" onChange="SelectEKra(this.value,<?php echo $_REQUEST['YI']; ?>,'d',0)"><option value="" style="margin-left:0px;" selected>SELECT DEPARTMENT</option><?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['DepartmentId']; ?>"><?php echo '&nbsp;'.$ResDept['DepartmentCode'];?></option><?php } ?><option value="0">&nbsp;All</option></select></td>
<td>&nbsp;</td>
<td class="td1" style="font-size:12px; width:150px;" align="center"><select style="font-size:12px; width:158px; height:20px; background-color:#DDFFBB;" name="DeptScore" id="DeptScore" onChange="SelectEKra(this.value,<?php echo $_REQUEST['YI']; ?>,'d',1)"><option value="" style="margin-left:0px;" selected>NAME WISE</option><?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['DepartmentId']; ?>"><?php echo '&nbsp;'.$ResDept['DepartmentCode'];?></option><?php } ?><option value="0">&nbsp;All</option></select></td>
...
<tr>
 <td style="width:100%;">
   ...
   <thead>
     <tr>
  <td colspan="<?php if($_REQUEST['a']==0){echo 7;}else{echo 11;} ?>" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;KRA List <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2.'-'.$PRD; ?>&nbsp;)&nbsp;&nbsp;&nbsp;<a href="#" onClick="ExportKra(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>',<?php echo $_REQUEST['a']; ?>)" style="color:#F9F900; font-size:12px;">Export Excel</a>
  
  <?php /*?><a href="#" onClick="PrintKra(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>',<?php echo $_REQUEST['a']; ?>)" style="color:#F9F900; font-size:12px;">Print</a><?php */?>
	  </td>
	 </tr>
	 <tr bgcolor="#7a6189">
	    <td style="width:2%;">&nbsp;</td>
...
<?php if($_REQUEST['a']==1){ ?>
		...
		<td class="th" style="width:15%;"><b>Designation</b></td>	
<?php } ?>
		<td class="th" style="width:10%;"><b>KRA</b></td>
		<td class="th" style="width:28%;"><b>Description</b></td>
		<td class="th" style="width:5%;"><b>Measure</b></td>	
		<td class="th" style="width:5%;"><b>Unit</b></td>	
  	</tr>
	</thead>
	</div>
<?php 
...
{  
  if($_REQUEST['value']==0)
  { $sql=mysql_query("SELECT * from hrm_pms_kra WHERE (KRAStatus='A' OR KRAStatus='R') AND CompanyId=".$CompanyId." AND YearId=".$_REQUEST['YI'], $con); }
  else{ $sql=mysql_query("SELECT * from hrm_pms_kra WHERE (KRAStatus='A' OR KRAStatus='R') AND CompanyId=".$CompanyId." AND YearId=".$_REQUEST['YI']." AND DepartmentId=".$_REQUEST['value'], $con); }
}
if($_REQUEST['ee']=='Dept' AND $_REQUEST['a']==1)
{  
  if($_REQUEST['value']==0)
  { $sql=mysql_query("SELECT EmpCode,Fname,Sname,Lname,DesigId,hrm_employee_general.DepartmentId,KRA,KRA_Description,Measure,Unit,Weightage,Target FROM hrm_pms_kra INNER JOIN hrm_employee ON hrm_pms_kra.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_general ON hrm_pms_kra.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_pms_kra.YearId=".$_REQUEST['YI']."  AND hrm_pms_kra.CompanyId=".$CompanyId." AND (hrm_pms_kra.KRAStatus='A' OR hrm_pms_kra.KRAStatus='R') AND hrm_employee_general.DateJoining<='".$YYear."-06-30' AND hrm_employee.EmpCode<=11000 order by hrm_employee.EmpCode ASC, hrm_pms_kra.KRAId ASC", $con); }
  else{ $sql=mysql_query("SELECT EmpCode,Fname,Sname,Lname,DesigId,hrm_employee_general.DepartmentId,KRA,KRA_Description,Measure,Unit,Weightage,Target FROM hrm_pms_kra INNER JOIN hrm_employee ON hrm_pms_kra.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_general ON hrm_pms_kra.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_pms_kra.DepartmentId=".$_REQUEST['value']." AND hrm_pms_kra.YearId=".$_REQUEST['YI']."  AND hrm_pms_kra.CompanyId=".$CompanyId." AND (hrm_pms_kra.KRAStatus='A' OR hrm_pms_kra.KRAStatus='R') AND hrm_employee_general.DateJoining<='".$YYear."-06-30' AND hrm_employee.EmpCode<=11000 order by hrm_employee.EmpCode ASC, hrm_pms_kra.KRAId ASC", $con); }
}
...
<?php } ?>		
		<td class="tdl"><?php echo $res['KRA']; ?></td>	
		<td class="tdl"><?php echo $res['KRA_Description']; ?></td>
		<td class="tdc"><?php echo $res['Measure']; ?></td>		
		<td class="tdc"><?php echo $res['Unit']; ?></td>		
	 </tr>
</tbody>
</div>	 
	 <?php $Sno++; } ?>
