...
<?php $SqlEmp=mysql_query("select e.*,g.* from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND g.DepartmentId=".$_REQUEST['d']." AND e.CompanyId=".$CompanyId." order by e.EmpCode ASC", $con); $Sno=1; $SqlRows=mysql_num_rows($SqlEmp); echo '<input type="hidden" name="EmpRows" id="EmpRows" value='.$SqlRows.' />';  while($ResEmp=mysql_fetch_array($SqlEmp)){ $ename=$ResEmp['Fname'].' '.$ResEmp['Sname'].' '.$ResEmp['Lname']; 
