...
if(isset($_POST['id']) && $_POST['dept']!=""){ $id=$_POST['id']; $dept=$_POST['dept']; $YId=$_POST['YId']; $UId=$_POST['UId']; $CId=$_POST['CId'];

$sqlCheck=mysql_query("select * from hrm_pms_appraiser where DepartmentId=".$dept." AND EmployeeID=".$id, $con);  $row=mysql_num_rows($sqlCheck);
 if($row==0)
  { $sql=mysql_query("insert into hrm_pms_appraiser(YearId,DepartmentId,EmployeeID,CompanyId,CreatedBy,CreatedDate) values(".$YId.",".$dept.",".$id.",".$CId.",".$UId.",'".date("Y-m-d")."')", $con);  }
...
<?php $sqlRev=mysql_query("select * from hrm_pms_appraiser where DepartmentId=".$dept." Order By AppraiserId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlRev)) { ?>
 <tr bgcolor="#FFFFFF">
...
<?php $sqlE=mysql_query("select EmpCode,Fname,Sname,Lname,DesigId,DesigId2,Gender,Married from hrm_pms_appraiser INNER JOIN hrm_employee ON hrm_pms_appraiser.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_pms_appraiser.EmployeeID=".$resApp['EmployeeID'], $con); 
  ...
  <td class="font1" style="width:50px;" align="center">
   <a href="#"><img src="images/delete.png" alt="Delete" border="0" onClick="DelApp(<?php echo $resApp['AppraiserId'].','.$dept; ?>)"></a>
  </td>  
 </tr>
<?php $no++; } ?>
</table>
