...
{ 
  ...
  if($row==0)
  { $sqlIns=mysql_query("insert into hrm_employee_separation_exitint(EmpSepId, Rep_EligForReHire, Rep_Rating, Rep_ReasonsLeaving, Rep_ReasonsLeaving_2, Rep_CulturePolicy, Rep_CulturePolicy_2, Rep_SuggImp, Rep_SuggImp_2, RepExitIntStatus) values(".$_POST['SepId'].", '".$_POST['ReHire']."', ".$_POST['ReRating'].", '".$ReasonsLeaving."', '".$ReasonsLeaving2."', '".$CulturePolicy."', '".$CulturePolicy2."', '".$SuggImp."', '".$SuggImp2."', 1)", $con); }
  elseif($row>0)
  { $sqlIns=mysql_query("update hrm_employee_separation_exitint set Rep_EligForReHire='".$_POST['ReHire']."', Rep_Rating=".$_POST['ReRating'].", Rep_ReasonsLeaving='".$ReasonsLeaving."', Rep_ReasonsLeaving_2='".$ReasonsLeaving2."', Rep_CulturePolicy='".$CulturePolicy."', Rep_CulturePolicy_2='".$CulturePolicy2."', Rep_SuggImp='".$SuggImp."', Rep_SuggImp_2='".$SuggImp2."', RepExitIntStatus=1 where EmpSepId=".$_POST['SepId'], $con);}
...
{ 
  ...
  $sqlIns=mysql_query("update hrm_employee_separation_exitint set Rep_EligForReHire='".$_POST['ReHire']."', Rep_Rating=".$_POST['ReRating'].", Rep_ReasonsLeaving='".$ReasonsLeaving."', Rep_ReasonsLeaving_2='".$ReasonsLeaving2."', Rep_CulturePolicy='".$CulturePolicy."', Rep_CulturePolicy_2='".$CulturePolicy2."', Rep_SuggImp='".$SuggImp."', Rep_SuggImp_2='".$SuggImp2."', RepExitIntStatus=2 where EmpSepId=".$_POST['SepId'], $con);
...
{ document.getElementById("editBtn").style.display='none'; document.getElementById("saveExitInt").style.display='block'; 
  ...
  document.getElementById("Yes_1").disabled=false; document.getElementById("No_1").disabled=false; document.getElementById("ReRating").disabled=false;
  ...
  document.getElementById("ReRating").readOnly=false;
}

function validate(formname)
{
  var ReHire = formname.ReHire.value; var ReRating = formname.ReRating.value; var ReasonsLeaving = formname.ReasonsLeaving.value; 
  ...
  if(ReRating==''){ alert("Please Type Performance Rating."); return false;} 
  if(ReRating!=1 && ReRating!=2 && ReRating!=3 && ReRating!=4 && ReRating!=5){ alert("Please Type Only Performance Rating Between 1 to 5."); return false;} 
...
function printp(si){ window.location="SepRepMgrExitIntFormprint.php?act=act&si="+si; }
</script>	
</head>
<body bgcolor="#E0DBE3">
  ...
  <tr>
   <td align="Right" class="fontButton">
	 ...
	 <tr>
      ...
      <td> <input type="button" name="Refrash" value="refresh" onClick="javascript:window.location='SepRepMgrExitIntForm.php?act=act&v=v&ss=vty&cc=it@~t~1212&p=value&a=app&true=false&si=<?php echo $_REQUEST['si']; ?>&st=<?php echo $_REQUEST['st']; ?>&ei=<?php echo $_REQUEST['ei']; ?>'" style="display:block;"/></td>
      <?php } ?>
     </tr> 
	</table>
   </td>
