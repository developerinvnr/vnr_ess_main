...
{  
 $SqlInseart = mysql_query("INSERT INTO hrm_bonus_wages(YearId, Category, PerDayApr, PerMonthApr, PerDayOct, PerMonthOct, BWageStatus, CompanyId, CrBy, CrDate) VALUES(".$_POST['ey'].",'".$_POST['Category']."', '".$_POST['PerDayApr']."', '".$_POST['PerMonthApr']."', '".$_POST['PerDayOct']."', '".$_POST['PerMonthOct']."', '".$_POST['BWageStatus']."', ".$CompanyId.", ".$UserId.", '".date('Y-m-d')."')", $con);  
...
{ 
 ...
 $SqlUpdate = mysql_query("UPDATE hrm_bonus_wages SET PerDayApr='".$_POST['PerDayApr']."', PerMonthApr='".$_POST['PerMonthApr']."', PerDayOct='".$_POST['PerDayOct']."', PerMonthOct='".$_POST['PerMonthOct']."', BWageStatus='".$_POST['BWageStatus']."', UpdBy=".$UserId.", UpdDate='".date("Y-m-d")."' WHERE BWageId=".$_POST['BWageId'], $con) or die(mysql_error());  
...
<table border="1" width="100%" bgcolor="#FFFFFF" cellspacing="1">
 <tr bgcolor="#7a6189" style="height:25px;">
  ...
  <td rowspan="2" class="th" style="width:10%;"><b>Status</b></td>
  <td rowspan="2" class="th" style="width:5%;"><b>Action</b></td>
 </tr>
 <tr bgcolor="#7a6189" style="height:25px;">
...
<?php $sqlRat=mysql_query("select * from hrm_bonus_wages where BWageStatus!='De' AND CompanyId=".$CompanyId." AND YearId=".$_REQUEST['ey']." order by BWageId ASC", $con); $SNo=1; while($resRat=mysql_fetch_array($sqlRat)) {
  if(isset($_REQUEST['action']) && $_REQUEST['action']=="edit" && $_REQUEST['eid']==$resRat['BWageId']){ ?>
...
<?php if($_SESSION['User_Permission']=='Edit'){ ?>&nbsp;<input type="submit" name="SaveEdit"  value="" class="SaveButton">&nbsp;<input type="hidden" name="BWageId" id="BWageId" value="<?php echo $_REQUEST['eid']; ?>"/><?php } ?></td>
 </tr>
</form>
...
<tr style="height:24px;">
 ...
 <td class="tdc"><?php if($_SESSION['User_Permission']=='Edit'){ ?><a href="#"><img src="images/edit.png" border="0" alt="Edit" onClick="edit(<?php echo $resRat['BWageId'].', '.$_REQUEST['ey'].', '.$_REQUEST['ed']; ?>)"></a><?php /*?>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#"><img src="images/delete.png" alt="Delete" border="0" onClick="del(<?php echo $resRat['BWageId'].', '.$_REQUEST['ey']; ?>)"></a><?php */?><?php } ?></td>
</tr>
<?php } $SNo++;} ?>
...
<tr>
 <td class="tdc"><?php echo $SNo; ?></td>
 <td class="tdc"><input name="Category" id="Category" class="EditInput" style="width:99%;" maxlength="25"/></td>
...
<?php $sqlEmp=mysql_query("select e.EmployeeID,e.EmpCode,Fname,Sname,Lname,de.DepartmentCode,gr.GradeValue,c.BWageId from hrm_employee e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID inner join hrm_employee_ctc c on e.EmployeeID=c.EmployeeID inner join hrm_department de on g.DepartmentId=de.DepartmentId inner join hrm_grade gr on g.GradeId=gr.GradeId where e.EmpStatus='A' AND c.Status='A' AND c.BAS_Value<='".$rsMxb['Lumpsum_MaxBonus']."' AND e.CompanyId=".$CompanyId." AND g.DepartmentId=".$_REQUEST['ed']." order by e.EmpCode ASC", $con); $no=1; while($resEmp=mysql_fetch_array($sqlEmp)) { ?>
