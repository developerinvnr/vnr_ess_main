...
if($_REQUEST['YI']<=7)
{ $qry='SUM(EmpCurrGrossPM) as E_GROSS, SUM(EmpCurrIncentivePM) as EINC_GROSS, SUM(Hod_ProIncSalary) as H_IS, SUM(Hod_ProCorrSalary) as H_SC, SUM(Hod_Incentive) as H_Inc, DepartmentId, SUM(Hod_IncNetMonthalySalary) as Tinc, HOD_EmployeeID'; }
else
...
{ $sqlT=mysql_query("select ".$qry." from hrm_employee_pms p INNER JOIN hrm_employee e ON p.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND e.CompanyId=".$_REQUEST['c']." AND p.AssessmentYear=".$_REQUEST['YI']." AND p.HOD_EmployeeID>0 AND p.Appraiser_EmployeeID!=0", $con); //g.DateJoining<='".$Y."-06-30'
 ...
 $ISPer=$Diff/$One; $SCPer=$resT['H_SC']/$One; $INCPer=$incH/$One;
  ...
  $sqlTE=mysql_query("select * from hrm_employee_pms p INNER JOIN hrm_employee e ON p.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND e.CompanyId=".$_REQUEST['c']." AND p.HOD_EmployeeID>0 AND p.AssessmentYear=".$_REQUEST['YI']." AND p.Appraiser_EmployeeID!=0", $con); $rowTNOE=mysql_num_rows($sqlTE); //g.DateJoining<='".date($FD.'-06-30')."'
