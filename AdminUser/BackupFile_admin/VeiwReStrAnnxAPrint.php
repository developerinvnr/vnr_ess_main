...
<table>
<?php if($_REQUEST['action']=='ReStrAnnxA') { $SqlE=mysql_query("SELECT hrm_employee.*,Gender,DR,Married FROM hrm_employee INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_employee.EmployeeID=".$_REQUEST['e'], $con); $ResE=mysql_fetch_assoc($SqlE); 
