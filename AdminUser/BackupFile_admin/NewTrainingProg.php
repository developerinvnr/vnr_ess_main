...
{ 
 ...
 $sqlIns=mysql_query("insert into hrm_company_training(CompanyId, TraTitle, TraYear, TraFrom, TraFrom_HF, TraTo, TraTo_HF, Duration, TrainerName, Location, Institute, CreatedBy, CreatedDate, YearId) values(".$_POST['ComId'].", '".$_POST['Tital']."', ".date("Y",strtotime($_POST['DateFrom'])).", '".date("Y-m-d",strtotime($_POST['DateFrom']))."', '".$_POST['FromHF']."', '".date("Y-m-d",strtotime($_POST['DateTo']))."', '".$_POST['ToHF']."', '".$Duration."', '".$_POST['Trainer']."', '".$_POST['Vanue']."', '".$_POST['Inst']."', ".$_POST['UId'].", '".date("Y-m-d")."', ".$_POST['YId'].")", $con);
 if($sqlIns)
...
{ 
  ...
  var Vanue = formName.Vanue.value; var filter=/^[a-zA-Z.&()-, /]+$/; var test_bool2 = filter.test(Vanue);
  if (Vanue.length === 0) { alert("You must enter a Vanue Name.");  return false; }
  //if(test_bool2==false) { alert('Please Enter Only Alphabets in the Venue Name Field');  return false; } 
  
  var Trainer = formName.Trainer.value; var filter=/^[a-zA-Z.&()-, /]+$/; var test_bool3 = filter.test(Trainer);
  if (Trainer.length === 0) { alert("You must enter a Trainer Name.");  return false; }
  //if(test_bool3==false) { alert('Please Enter Only Alphabets in the Trainer Name Field');  return false; } 
  
  var Inst = formName.Inst.value; var filter=/^[a-zA-Z.&()-, /]+$/; var test_bool4 = filter.test(Inst);
  ...
  var DF = formName.DateFrom.value; var filter=/^[a-zA-Z.&()-, /]+$/; var test_bool5 = filter.test(DF);
  if (DF.length === 0) { alert("You must enter a Date From Field.");  return false; }
 
  var DT = formName.DateTo.value; var filter=/^[a-zA-Z.&()-, /]+$/; var test_bool6 = filter.test(DT);
  ...
  var agree=confirm("Are you sure you want to add new training program.")
...
<table style="vertical-align:top;width:350px;" align="center" border="0">
 <tr><td align="center" style="font-family:Times New Roman;font-size:18px;height:16px;"><b><u>Training Details</u></b></td></tr>
 <tr><td>&nbsp;</td></tr>
 <tr><td>&nbsp;</td></tr>
 <tr>
 <td>
  <table border="0" style="vertical-align:top;width:350px;">
   ...
   <tr><td class="head" style="width:100px;">Trainer :</td><td style="width:240px;"><input name="Trainer" id="Trainer" class="textInput" style="width:240px;" readonly/></td></tr>
   <tr><td class="head" style="width:100px;">Institute :</td><td style="width:240px;"><input name="Inst" id="Inst" class="textInput" style="width:240px;" readonly/></td></tr>
