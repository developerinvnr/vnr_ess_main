...
?>
<table style="vertical-align:top;" width="220" align="center" border="1">
<?php /*
<tr>
...
$sql1 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=1 AND YearId=".$_REQUEST['Y'], $con);  
$row1 = mysql_num_rows($sql1); if($row1>0){ $res1 = mysql_fetch_assoc($sql1);}
$sql2 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=2 AND YearId=".$_REQUEST['Y'], $con);  
$row2 = mysql_num_rows($sql2); if($row2>0){ $res2 = mysql_fetch_assoc($sql2);}     
$sql3 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=3 AND YearId=".$_REQUEST['Y'], $con);  
$row3 = mysql_num_rows($sql3); if($row3>0){ $res3 = mysql_fetch_assoc($sql3);}
$sql4 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=4 AND YearId=".$_REQUEST['Y'], $con);  
$row4 = mysql_num_rows($sql4); if($row4>0){ $res4 = mysql_fetch_assoc($sql4);}
$sql5 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=5 AND YearId=".$_REQUEST['Y'], $con);  
$row5 = mysql_num_rows($sql5); if($row5>0){ $res5 = mysql_fetch_assoc($sql5);}
$sql6 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=6 AND YearId=".$_REQUEST['Y'], $con);  
$row6 = mysql_num_rows($sql6); if($row6>0){ $res6 = mysql_fetch_assoc($sql6);}
$sql7 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=7 AND YearId=".$_REQUEST['Y'], $con);  
$row7 = mysql_num_rows($sql7); if($row7>0){ $res7 = mysql_fetch_assoc($sql7);}
$sql8 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=8 AND YearId=".$_REQUEST['Y'], $con);  
$row8 = mysql_num_rows($sql8); if($row8>0){ $res8 = mysql_fetch_assoc($sql8);}
$sql9 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=9 AND YearId=".$_REQUEST['Y'], $con);  
$row9 = mysql_num_rows($sql9); if($row9>0){ $res9 = mysql_fetch_assoc($sql9);}
$sql10 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=10 AND YearId=".$_REQUEST['Y'], $con);  
$row10 = mysql_num_rows($sql10); if($row10>0){ $res10 = mysql_fetch_assoc($sql10);}
$sql11 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=11 AND YearId=".$_REQUEST['Y'], $con);  
$row11 = mysql_num_rows($sql11); if($row11>0){ $res11 = mysql_fetch_assoc($sql11);}
$sql12 = mysql_query("SELECT EduCess_Amt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Month=12 AND YearId=".$_REQUEST['Y'], $con);  
$row12 = mysql_num_rows($sql12); if($row12>0){ $res12 = mysql_fetch_assoc($sql12);}

$sqlTot = mysql_query("SELECT SUM(EduCess_Amt) as TotEduCessSAmt FROM hrm_emplyee_tds_maincompo WHERE EmployeeID=".$_REQUEST['E']." AND Period='".$FD."-".$TD."' AND YearId=".$_REQUEST['Y'], $con);  $rowTot = mysql_num_rows($sqlTot); if($rowTot>0){ $resTot = mysql_fetch_assoc($sqlTot);}
...
<table border="0">
 ...
 <td bgcolor="#D9D1E7">
 <table border="1">
 <tr><td colspan="2" align="center" style="font-family:Times New Roman;font-size:12px;width:200px;background-color:#7a6189;color:#FFFFFF;"><b> Education Cess (Year:&nbsp;<?php echo $FD."-".$TD; ?> )</b></td></tr> 
 <tr>
