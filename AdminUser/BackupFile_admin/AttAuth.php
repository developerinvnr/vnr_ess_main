...
<?php /*?><script type="text/javascript" src="js/stuHover.js" ></script>
<script type="text/javascript" src="js/Prototype.js"></script><?php */?>
...
.CalenderButton {background-image:url(images/CalenderBtn.jpeg); width:16px; height:16px; background-color:#E0DBE3; border-color:#FFFFFF;}.inner_table{height:450px;overflow-y:auto;width:auto;}</style>
<script src="js/jquery-1.9.1.js"></script>
<script type="text/javascript" src="js/jquery.freezeheader.js"></script>
<Script type="text/javascript">window.history.forward(1);</script>
<script language="javascript">
$(document).ready(function () { $("#table1").freezeHeader({ 'height': '450px' }); })
function ActClick(Id,m,y,a,emp)
{ //alert(Id+"-"+m+"-"+y+"-"+a);
 ...
 var timer = setInterval( function() { if(win.closed){ clearInterval(timer);  window.location="AttAuth.php?m="+m+"&Y="+y+"&e=4e&w=234&tt=10234&a=f&e=4e2&e=4e&w=234&act=1&retd=ee&wwrew=t%T@sed818&d=101&a="+a+"&emp="+emp; } }, 1000);
 
}

function FunY(y,a,m,emp)
{ window.location="AttAuth.php?m="+m+"&Y="+y+"&e=4e&w=234&tt=10234&a=f&e=4e2&e=4e&w=234&tt=110022344&retd=ee&wwrew=t%T@sed818&d=101&a="+a+"&emp="+emp; }

function FunChang(a,m,y,emp)
{ window.location="AttAuth.php?m="+m+"&Y="+y+"&e=4e&w=234&tt=10234&a=f&e=4e2&e=4e&w=234&tt=110022344&retd=ee&wwrew=t%T@sed818&d=101&a="+a+"&emp="+emp; }

function FunEmpChang(emp,a,m,y)
{ window.location="AttAuth.php?m="+m+"&Y="+y+"&e=4e&w=234&tt=10234&a=f&e=4e2&e=4e&w=234&tt=110022344&retd=ee&wwrew=t%T@sed818&d=101&a="+a+"&emp="+emp; }


function Export(m,y,e,s,c)
{   
  window.open("AttAuthExp.php?action=Export&m="+m+"&y="+y+"&e="+e+"&s="+s+"&c="+c,"PrintForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");} 
   ...
   <table border="0">
    <tr>
	 ...
	 <?php if($_REQUEST['a']==0){$a='All Status';}elseif($_REQUEST['a']==1){$a='Peding';}elseif($_REQUEST['a']==2){$a='Submitted';} ?>
	         <td><select style="width:100px;font-size:14px;font-family:Times New Roman;" onChange="FunY(this.value,<?php echo $_REQUEST['a'].','.$_REQUEST['m'].','.$_REQUEST['emp']; ?>)"><option value="<?php echo $_REQUEST['Y']; ?>" selected><?php echo $_REQUEST['Y']; ?></option>
<?php for($i=date("Y"); $i>=2016; $i--){?>
<?php if($i!=$_REQUEST['Y']){?><option value="<?php echo $i; ?>"><?php echo $i; ?></option><?php } ?>
<?php } ?></select></td>			 
			 <td><select style="width:100px;font-size:14px;font-family:Times New Roman;" onChange="FunChang(this.value,<?php echo $_REQUEST['m'].','.$_REQUEST['Y'].','.$_REQUEST['emp']; ?>)"><option value="<?php echo $_REQUEST['a']; ?>" selected><?php echo $a; ?></option>
			 ...
			 <td>
			     ...
			     <select style="width:250px;font-size:14px;font-family:Times New Roman;" onChange="FunEmpChang(this.value,<?php echo $_REQUEST['a'].','.$_REQUEST['m'].','.$_REQUEST['Y']; ?>)">
			 
			 <?php $sqE=mysql_query("select EmployeeID,EmpCode,Fname,Sname,Lname from hrm_employee where EmpStatus='A' AND CompanyId=".$CompanyId."",$con); while($reE=mysql_fetch_assoc($sqE)){ ?>
			 
			 <option value="<?=$reE['EmployeeID']?>" <?php if($_REQUEST['emp']==$reE['EmployeeID']){echo 'selected';} ?>><?=$reE['Fname'].' '.$reE['Sname'].' '.$reE['Lname'].'-'.$reE['EmpCode']?></option>
			 <?php } ?>   
			 <option value="0" <?php if($_REQUEST['emp']==0){echo 'selected';} ?>>All</option>
			 </select></td>
			 
			 <td>
			     &nbsp;<a href="#" onClick="Export(<?=$_REQUEST['m'].','.$_REQUEST['Y']?>,'<?=$_REQUEST['emp']?>','<?=$_REQUEST['a']?>',<?=$CompanyId?>)" style="font-size:12px;">Export</a>
			 </td>
	</tr>
   </table>
  </td>
...
<td style="width:50px;" valign="top" align="center">&nbsp;</td>
  <td align="left" id="ReplyToQuery" valign="top" style="display:block;">             
   ...
   <table border="0">
	<tr>  
	  <td>
		<table border="1" id="table1" cellspacing="1">
		<div class="thead">
        <thead>
            <tr bgcolor="#7a6189" style="height:22px;">
			 ...
			 <td width="120" class="TableHead" align="center"><b>Created<br> Date/Time</b></td>
...
if($_REQUEST['emp']==0 OR $_REQUEST['emp']==''){ $qemp='1=1'; }
elseif($_REQUEST['emp']>0){ $qemp='rq.EmployeeID='.$_REQUEST['emp']; }

if($_REQUEST['a']==0){$query="select rq.* from hrm_employee_attendance_req rq inner join hrm_employee e on rq.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND ".$qemp." AND AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31'"; }elseif($_REQUEST['a']==1){$query="select rq.* from hrm_employee_attendance_req rq inner join hrm_employee e on rq.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND ".$qemp." AND Status=0 AND AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31'"; }elseif($_REQUEST['a']==2){$query="select rq.* from hrm_employee_attendance_req rq inner join hrm_employee e on rq.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND ".$qemp." AND Status=1 AND AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31'";}
...
if($_REQUEST['a']==0){$query2="select rq.* from hrm_employee_attendance_req rq inner join hrm_employee e on rq.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND ".$qemp." AND AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' order by AttDate DESC LIMIT " . $from . "," . $offset; }elseif($_REQUEST['a']==1){$query2="select rq.* from hrm_employee_attendance_req rq inner join hrm_employee e on rq.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND ".$qemp." AND Status=0 AND AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' order by AttDate DESC LIMIT " . $from . "," . $offset; }elseif($_REQUEST['a']==2){$query2="select rq.* from hrm_employee_attendance_req rq inner join hrm_employee e on rq.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND ".$qemp." AND Status=1 AND AttDate>='".$_REQUEST['Y']."-01-01' AND AttDate<='".$_REQUEST['Y']."-12-31' order by AttDate DESC LIMIT " . $from . "," . $offset; }
...
<td class="TableHead1" align="center"><?php if($res['CrDate']!='0000-00-00'){ echo date("d-m-Y", strtotime($res['CrDate'])); } if($res['CrTime']!='00:00:00'){echo '<font color="#FF0000">/</font>'.$res['CrTime']; } ?></td>
...
<td class="TableHead1" align="center"><span style="cursor:pointer;color:#0061C1;" onClick="ActClick(<?php echo $res['AttRqId'].','.$_REQUEST['m'].','.$_REQUEST['Y'].','.$_REQUEST['a'].','.$_REQUEST['emp']; ?>)"><u>Click</u></span></td>
