...
echo "\tHOD Remark\tActual(%)\tProrata(%)\tHR Score\tHR Rating\tHR Grade\tHR Designation\tHR Department\tVertical";

if($_REQUEST['YI']<=7){ echo "\tHR PIS\tHR % PIS\tHR PSC\tHR % PSC\tHR TISPM\tHR % TISPM\tHR TPGSPM"; }
else{ echo "\tHR Proposed CTC\t%% HR PRO. CTC\tHR CTC Corr.\t% HR Corr.\tHR Total Increment\tHR % Total\tHR Total Proposed CTC\tActual(%)\tProrata(%)"; }
...
{ $sql=mysql_query("select EmpCode,Fname,Sname,Lname,hrm_employee_pms.* from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_pms ON hrm_employee.EmployeeID=hrm_employee_pms.EmployeeID where hrm_employee.CompanyId=".$_REQUEST['c']." AND hrm_employee.EmpStatus='A' AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." AND hrm_employee_pms.HOD_EmployeeID=".$_REQUEST['value']." AND hrm_employee_pms.Appraiser_EmployeeID!=0 order by EmpCode ASC", $con);
 ...
 if($_REQUEST['YI']<=5)
 { 
  $sqlC = mysql_query("SELECT * from hrm_employee_ctc_pms WHERE EmployeeID=".$res['EmployeeID']." AND CtcYearId=".$_REQUEST['YI']." AND CtcCreatedDate>='".date($FD."-10-01")."' AND CtcCreatedDate<='".date($FD."-12-31")."'", $con); 
  $sqlEl = mysql_query("SELECT * from hrm_employee_eligibility_pms WHERE EmployeeID=".$res['EmployeeID']." AND EligYearId=".$_REQUEST['YI']." AND EligCreatedDate>='".date($FD."-10-01")."' AND EligCreatedDate<='".date($FD."-12-31")."'", $con); 
 }
 else
 {  
  $sqlC = mysql_query("SELECT * from hrm_employee_ctc_pms WHERE EmployeeID=".$res['EmployeeID']." AND CtcYearId=".$_REQUEST['YI']." AND CtcCreatedDate='".date($TD."-01-01")."'", $con);
  $sqlEl = mysql_query("SELECT * from hrm_employee_eligibility_pms WHERE EmployeeID=".$res['EmployeeID']." AND EligYearId=".$_REQUEST['YI']." AND EligCreatedDate='".date($TD."-01-01")."'", $con); 
 }
...
if($res['Hod_EmpGrade']!=$res['HR_CurrGradeId']){ $sqlGH=mysql_query("select GradeValue from hrm_grade where GradeId=".$res['Hod_EmpGrade']." AND CompanyId=".$_REQUEST['c'], $con); $resGH=mysql_fetch_assoc($sqlGH); $GradeH=$resGH['GradeValue']; }else{$GradeH='';}   
  ...
  $schema_insert .= $res['Hod_ActualInc_Per'].$sep;
  $schema_insert .= $res['Hod_ProRataInc_Per'].$sep;
...
if($res['HR_GradeId']!=$res['HR_CurrGradeId']){ $sqlGH=mysql_query("select GradeValue from hrm_grade where GradeId=".$res['HR_GradeId']." AND CompanyId=".$_REQUEST['c'], $con); $resGH=mysql_fetch_assoc($sqlGH); $GradeH=$resGH['GradeValue']; }else{$GradeH='';}	  
  ...
  $HRActPer=''; if($res['Hod_ActualInc_Per']==$res['HR_ActualInc_Per']){ $HRActPer=$res['HR_ActualInc_Per']; }
  $HRProPer=''; if($res['Hod_ProRataInc_Per']==$res['HR_ProRataInc_Per']){ $HRProPer=$res['HR_ProRataInc_Per']; }
  $schema_insert .= $HRActPer.$sep;
  $schema_insert .= $HRProPer.$sep;
