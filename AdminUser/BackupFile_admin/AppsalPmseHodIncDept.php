...
<?php 
if($_REQUEST['YI']<=7)
{ $qry='SUM(EmpCurrGrossPM) as E_GROSS, SUM(EmpCurrIncentivePM) as EINC_GROSS, SUM(Hod_ProIncSalary) as H_IS, SUM(Hod_ProCorrSalary) as H_SC, SUM(Hod_Incentive) as H_Inc, SUM(Hod_IncNetMonthalySalary) as Tinc, DepartmentId, HOD_EmployeeID'; }
else
{ $qry='SUM(EmpCurrCtc) as E_GROSS, SUM(Hod_ProIncCTC) as H_IS, SUM(Hod_ProCorrCTC) as H_SC, SUM(Hod_IncNetCTC) as Tinc, DepartmentId, HOD_EmployeeID';} 
?>			 
...
?>	 
	 ...
	 <tr style="height:20px; background-color:#FF6CB6;"align="middle">
	  <td colspan="3" class="tdr">Total :&nbsp;&nbsp;</td>
<?php $sqlTE=mysql_query("select * from hrm_employee_pms p INNER JOIN hrm_employee e ON p.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where p.EmployeeID!=6 AND p.EmployeeID!=7 AND p.EmployeeID!=51 AND p.EmployeeID!=223 AND p.EmployeeID!=224 AND p.EmployeeID!=461 AND ".$sq." AND e.EmpStatus='A' AND e.CompanyId=".$CompanyId." AND p.HOD_EmployeeID>0 AND p.AssessmentYear=".$_REQUEST['YI']." AND p.Appraiser_EmployeeID!=0", $con); 
