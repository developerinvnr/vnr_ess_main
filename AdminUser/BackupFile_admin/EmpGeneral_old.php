...
{ $sql = mysql_query("select hrm_employee.*,hrm_employee_general.* from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID where hrm_employee.EmployeeID=".$EMPID, $con); $res=mysql_fetch_assoc($sql);
 ...
 if($_POST['RetiStatus']=='Y'){ $RiteDate=date("Y-m-d",strtotime($_POST['RetiDate'])); $RetiNCode=$_POST['RetiNewCode']; $RetiOldCode=$_POST['RetiOldCode'];}
	   ...
	   $SqlUpGen = mysql_query("UPDATE hrm_employee SET EmpStatus='".$_POST['EmpStatus']."',RetiStatus='".$_POST['RetiStatus']."',RetiDate='".$RiteDate."',RetiNewCode=".$RetiNCode.",RetiOldCode=".$RetiOldCode.",Fname='".$Fname."',Sname='".$Sname."',Lname='".$Lname."', DateOfResignation='".date("Y-m-d",strtotime($_POST['Resig']))."', DateOfSepration='".date("Y-m-d",strtotime($_POST['Sep']))."', CreatedBy=".$UserId.",CreatedDate='".date('Y-m-d')."',YearId=".$YearId." WHERE EmployeeID=".$EMPID, $con);
	   ...
	   $SqlUpRe = mysql_query("UPDATE hrm_employee_reporting SET AppraiserId=".$_POST['RepName']." WHERE EmployeeID=".$EMPID, $con);}
...
<tr>
  ...
  <td class="All_100" valign="top">Official Mob-2 :</td><td class="All_125"><input class="All_120" maxlength="10" value="<?php if($ResEmp['MobileNo2_Vnr']==0) echo ''; else {echo $ResEmp['MobileNo2_Vnr'];} ?>" readonly></td>
   <td colspan="2" class="All_100" valign="top"></td>
</tr>
...
<tr><td class="All_100" valign="top">Esic No. :</td><td class="All_125"><input class="All_120" name="ESICNo" id="ESICNo1" value="<?php if($ResEmp['EsicNo']>0){echo $ResEmp['EsicNo'];} else { echo ''; } ?>" readonly>
<?php $sqlEsic=mysql_query("select Esic_EmployerCode from hrm_company_statutory_esic", $con); $resEsic=mysql_fetch_assoc($sqlEsic);?> 
