...
function FunClick()  
{
 ...
 else{ window.location='Attendance.php?ls=10&wer=123grtd&se=reew&w=ee102&m='+document.getElementById("Month").value+'&Y='+document.getElementById("Year").value+'&ee=s1s&d='+document.getElementById("Department").value+'&dd=truevalu&fals=truefalse'; }
}
...
<?php $SqlD2=mysql_query("select DepartmentId,DepartmentCode from hrm_department where CompanyId=".$CompanyId." AND DeptStatus='A' order by DepartmentCode ASC", $con); 
      while($ResD2=mysql_fetch_array($SqlD2)) { ?><option value="<?php echo $ResD2['DepartmentId']; ?>"><?php echo $ResD2['DepartmentCode'];?></option><?php } ?><option value='all'>All</option></option></select></td>
...
<?php if($_REQUEST['d']>0){ $SqlEmp=mysql_query("select e.*,g.* from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND g.DepartmentId=".$_REQUEST['d']." AND e.CompanyId=".$CompanyId." order by e.EmpCode ASC", $con); }
...
$Sno=1; $SqlRows=mysql_num_rows($SqlEmp); echo '<input type="hidden" name="EmpRows" id="EmpRows" value='.$SqlRows.' />';  while($ResEmp=mysql_fetch_array($SqlEmp)){ $ename=$ResEmp['Fname'].' '.$ResEmp['Sname'].' '.$ResEmp['Lname']; 
