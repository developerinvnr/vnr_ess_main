...
function SelectYear(v){window.location='AppsalRatGraphEmp.php?YI='+v;}
</Script>
</head>
...
<tr>
 <td>
   <table border="0" width="width="<?php if($_REQUEST['YI']>=2){echo '900';}else{echo '400';} ?>"">
     <tr>
  <td colspan="8" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Normal Distribustion Rating Graph (ALL EMP)&nbsp;&nbsp;&nbsp;
...
<?php } ?>	 	 
	...
	<tr>
	 <td bgcolor="#7a6189" align="Right" style="font-family:Times New Roman;color:#FFFFFF; font-size:13px;"><b>Employee</b>&nbsp;</td>
<?php $sqlR=mysql_query("select * from hrm_pms_rating where YearId=".$_REQUEST['YI']." AND CompanyId=".$CompanyId." AND RatingStatus='A' order by Rating ASC", $con); 
      $sn=1; while($resR=mysql_fetch_array($sqlR)){ 
...
$RowE=mysql_num_rows($SqlE); $vE=number_format($RowE, 2, '.', '');	?>	 
	 <td bgcolor="#FFCAFF" align="Center" style="font-family:Times New Roman;color:#OOOOOO; font-size:13px;"><?php if($vE!=0){echo $vE;}else{echo '';} //echo $vE; ?></td>
<?php } ?>	
	</tr>
	<tr>
	 <td bgcolor="#7a6189" align="Right" style="font-family:Times New Roman;color:#FFFFFF; font-size:13px;"><b>Appraiser</b>&nbsp;</td>
<?php $sqlR=mysql_query("select * from hrm_pms_rating where YearId=".$_REQUEST['YI']." AND CompanyId=".$CompanyId." AND RatingStatus='A' order by Rating ASC", $con); 
      $sn=1; while($resR=mysql_fetch_array($sqlR)){ 
$SqlA=mysql_query("select EmpPmsId from hrm_employee_pms INNER JOIN hrm_employee ON hrm_employee_pms.EmployeeID=hrm_employee.EmployeeID where hrm_employee.EmpStatus='A' AND hrm_employee.CompanyId=".$CompanyId." AND AssessmentYear=".$_REQUEST['YI']." AND Appraiser_TotalFinalRating=".$resR['Rating'], $con); $RowA=mysql_num_rows($SqlA); $vA=number_format($RowA, 2, '.', '');	?>	 
	 <td bgcolor="#379BFF" align="Center" style="font-family:Times New Roman;color:#OOOOOO; font-size:13px;"><?php if($vA!=0){echo $vA;}else{echo '';} //echo $vA; ?></td>
<?php } ?>		 
	</tr>
	<tr>
	 <td bgcolor="#7a6189" align="Right" style="font-family:Times New Roman;color:#FFFFFF; font-size:13px;"><b>Reviewer</b>&nbsp;</td>
<?php $sqlR=mysql_query("select * from hrm_pms_rating where YearId=".$_REQUEST['YI']." AND CompanyId=".$CompanyId." AND RatingStatus='A' order by Rating ASC", $con); 
      $sn=1; while($resR=mysql_fetch_array($sqlR)){ 
$SqlR=mysql_query("select EmpPmsId from hrm_employee_pms INNER JOIN hrm_employee ON hrm_employee_pms.EmployeeID=hrm_employee.EmployeeID where hrm_employee.EmpStatus='A' AND hrm_employee.CompanyId=".$CompanyId." AND AssessmentYear=".$_REQUEST['YI']." AND Reviewer_TotalFinalRating=".$resR['Rating'], $con);
 $RowR=mysql_num_rows($SqlR); $vR=number_format($RowR, 2, '.', '');	?>		 
	 <td bgcolor="#BFFFBF" align="Center" style="font-family:Times New Roman;color:#OOOOOO; font-size:13px;"><?php if($vR!=0){echo $vR;}else{echo '';} //echo $vR; ?></td>
<?php } ?>		 
	</tr>
	<tr>
	 <td bgcolor="#7a6189" align="Right" style="font-family:Times New Roman;color:#FFFFFF; font-size:13px;"><b>HOD</b>&nbsp;</td>
<?php $sqlR=mysql_query("select * from hrm_pms_rating where YearId=".$_REQUEST['YI']." AND CompanyId=".$CompanyId." AND RatingStatus='A' order by Rating ASC", $con); 
      $sn=1; while($resR=mysql_fetch_array($sqlR)){ 
$SqlH=mysql_query("select EmpPmsId from hrm_employee_pms INNER JOIN hrm_employee ON hrm_employee_pms.EmployeeID=hrm_employee.EmployeeID where hrm_employee.EmpStatus='A' AND hrm_employee.CompanyId=".$CompanyId." AND AssessmentYear=".$_REQUEST['YI']." AND Hod_TotalFinalRating=".$resR['Rating'], $con);
 $RowH=mysql_num_rows($SqlH); $vH=number_format($RowH, 2, '.', '');	?> 
	 <td bgcolor="#FFA579" align="Center" style="font-family:Times New Roman;color:#OOOOOO; font-size:13px;"><?php if($vH!=0){echo $vH;}else{echo '';} //echo $vH; ?></td>
<?php } ?>		 
	</tr>
   </table>
  </td>
 ...
 <tr>
     <td colspan="20" style="width:800px; height:300px; border:0; border-style:hidden;" valign="top" align="left">
	  <iframe name="Giframe" src="AppsalRatGraphEmpImg.php?action=LinGraph&YI=<?php echo $_REQUEST['YI']; ?>" style="width:750px; height:300px; border:0; border-style:hidden;"></iframe></td>
	  </td>
	 </tr>
   </table>
 </td>
