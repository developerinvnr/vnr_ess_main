...
//****************************************************************************************************************
if($_REQUEST['act']=='ReDir' AND $_REQUEST['ID']!='' AND $_REQUEST['EI']!='')
{ $sqlE=mysql_query("update hrm_employee_investment_submission set FormSubmit='Y' where IvstDecIdSb=".$_REQUEST['ID']." AND EmployeeID=".$_REQUEST['EI'], $con); }
?>
<html>
<head>
...
function FunDetailsSubm(ID,EI,CI){ window.open("InvestSubForm.php?action=Print&ID="+ID+'&EI='+EI+'&CI='+CI,"Form","location=no,scrollbars=yes,resizable=no,menubar=no,width=970,height=700"); }
...
<?php $Ms2=mysql_query("select IvstDecIdSb,Inv_Date,FormSubmit,SubmittedDate from hrm_employee_investment_submission where EmployeeID=".$res['EmployeeID']." AND Period='".$_REQUEST['prd']."' ", $con); $rowMs2=mysql_num_rows($Ms2); $resMs2=mysql_fetch_array($Ms2); ?>
	...
	<td align="center" class="All_80"><?php if($resMs2['FormSubmit']=='YY'){?><a href="#" onClick="FunDetailsSubm(<?php echo $resMs2['IvstDecIdSb'].', '.$res['EmployeeID'].', '.$CompanyId; ?>)">Click</a><?php } ?></td>
