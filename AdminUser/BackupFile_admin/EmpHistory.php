...
{ 
 echo '<img width="105px" height="125px" border=1 src="EmpImg'.$resE['CompanyId'].'Emp/'.$resE['EmpCode'].'.jpg" />';
}
elseif($EmployeeId==944 OR $EmployeeId==945)
{
 echo '<img width="105px" height="125px" border=1 src="EmpImg'.$resE['CompanyId'].'Emp/'.$resE['EmpCode'].'_NewEmp.jpg" />';
}   
?>	         
...
elseif($months>=72 AND $months<84){$m1=$months-72; $l=strlen($m1);if($l==1){$mnt='6.0'.$m1;}else{$mnt='6.'.$m1;} }
elseif($months>=84 AND $months<96){$m1=$months-84; $l=strlen($m1);if($l==1){$mnt='7.0'.$m1;}else{$mnt='7.'.$m1;} }
elseif($months>=96 AND $months<108){$m1=$months-96; $l=strlen($m1);if($l==1){$mnt='8.0'.$m1;}else{$mnt='8.'.$m1;} } //echo 'bb='.$m1;
$str_a = explode('.',$mnt); //echo 'aa='.$str_a[0].'-'.$years.'-'.$str_a[1];
    ...
    <td style="vertical-align:top;width:900px;" valign="top">
	  <table border="1">
	   <tr bgcolor="#7a6189" style="height:22px;">
		...
		<td align="center" class="head" style="color:#FFFFFF;width:100px;">CTC</td>
...
<?php $sqlHi2=mysql_query("select EmpPmsId,SalaryChange_Date, Previous_GrossSalaryPM, Proposed_Designation, Proposed_Grade, TotalProp_GSPM, Proposed_GrossSalaryPM, TotalProp_PerInc_GSPM, Rating from hrm_pms_appraisal_history where EmpCode=".$resE['EmpCode']." AND CompanyId=".$resE['CompanyId']." AND SalaryChange_Date>='".date('2012-01-01')."' order by SalaryChange_Date DESC", $con); 
      $Sno2=1; while($resHi2=mysql_fetch_array($sqlHi2)){ 
	  
	  $sctc=mysql_query("select * from hrm_employee_ctc where CtcCreatedDate='".$resHi2['SalaryChange_Date']."' AND CtcId=(select Max(CtcId) from hrm_employee_ctc where EmployeeID=".$EmployeeId." AND CtcCreatedDate='".$resHi2['SalaryChange_Date']."')",$con); $rctc=mysql_fetch_assoc($sctc);
	  
	  
	  //$sctc=mysql_query("select Tot_CTC from hrm_employee_ctc where EmployeeID=".$EmployeeId." AND (CtcCreatedDate='".$resHi2['SalaryChange_Date']."' OR SalChangeDate='".$resHi2['SalaryChange_Date']."')",$con); $rctc=mysql_fetch_assoc($sctc);
	  ...
	  ?>
       ...
       <tr bgcolor="#FFFFFF" style="height:22px;">
		...
		<td align="right" class="data"><?php if($resHi2['TotalProp_GSPM']<$resHi2['Previous_GrossSalaryPM']){echo $resHi2['Proposed_GrossSalaryPM'];}else{echo $resHi2['TotalProp_GSPM']; } ?>&nbsp;</td>
		
		<td align="right" class="data"><?php echo floatval($rctc['Tot_CTC']); ?>&nbsp;</td>
		
		<td align="center" class="data">
<?php 
      if($rctc['Tot_CTC']==0 AND $rctc['Tot_CTC']==''){echo '0'; }
      else if($resHi2['TotalProp_PerInc_GSPM']==0)
      { 
...
<?php //} // if($rctc['Tot_CTC']>0 AND $rctc['Tot_CTC']!='')	   
