 ...
 <td>
  <table border="0" style="vertical-align:top;width:700px;">
   <tr>
	...
	<input type="text" name="Tital" id="Tital" class="textInput" style="width:240px;" value='<?php echo $resT['TraTitle']; ?>' readonly/> </td>
   ...
   <tr>
	...
	<input name="DateFrom" id="DateFrom" class="textInput" style="width:60px;" value="<?php echo date("d-m-Y",strtotime($resT['TraFrom'])); ?>" readonly/>
	...
	<input name="DateTo" id="DateTo" class="textInput" style="width:60px;" value="<?php echo date("d-m-Y",strtotime($resT['TraTo'])); ?>" readonly/>
...
{ if(document.getElementById("OthName").value==''){ alert("please type participant name"); return false;}
  ...
  var url = 'GetParticularEmp.php';	var pars = 'Act=OthEmp&Val='+OthName+'&ID='+ID;
...
{
  var url = 'GetParticularEmp.php';	var pars = 'Act2=DelOthEmp&v='+v+'&ID='+ID;
...
$sn2=1; while($resP2=mysql_fetch_array($sqlP2)){ ?>			 
		  <tr bgcolor="#FFFFFF">
		    ...
		    <td style="width:50px;" align="center"><img src="images/delete.png" border="0" onClick="OthDel(<?php echo $resP2['TraEmpId'].', '.$_REQUEST['id']; ?>)"/></td>
		  </tr>
<?php $sn2++;} ?>	
          <tr bgcolor="#FFFFFF">
		    <td style="width:50px;font-family:Times New Roman;font-size:12px;" align="center"><?php echo $sn2; ?></td>
...
<?php $sqlD=mysql_query("select DepartmentId,DepartmentCode,DepartmentName from hrm_department where DeptStatus='A' AND CompanyId=2 AND DepartmentCode!='MANAGEMENT' order by DepartmentId ASC", $con); 
      while($resD=mysql_fetch_array($sqlD)) { ?>
	 <tr bgcolor="#7a6189">
	  <td class="body" align="center" style="width:30px;">
	  <a href="#" style="text-decoration:none;">
	...
	<?php /* <input type="checkbox" id="CDept_<?php echo $resD['DepartmentId']; ?>" onClick="ClickDept(<?php echo $resD['DepartmentId']; ?>)" /> */ ?> </td>
	  <td class="head" align="" style="width:270px;color:#FFFFFF;" bgcolor="#0080FF">&nbsp;<?php echo ucfirst(strtolower($resD['DepartmentName'])); ?></td>
	 </tr>	
     <tr>
	  <td colspan="2">
	   <table id="TR_<?php echo $resD['DepartmentId']; ?>" style="display:none;" border="1">
<?php $sqlE=mysql_query("select hrm_employee.EmployeeID,EmpCode,Fname,Sname,Lname from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID where hrm_employee_general.DepartmentId=".$resD['DepartmentId']." AND hrm_employee.CompanyId=2 order by EmpCode ASC", $con); while($resE=mysql_fetch_array($sqlE)) { 
$sqlc=mysql_query("select TrainingId from hrm_company_training_participant where TrainingId=".$_REQUEST['id']." AND EmployeeID=".$resE['EmployeeID'], $con);
$row=mysql_num_rows($sqlc);
?>	   
	   <tr id=ETR_<?php echo $resE['EmployeeID']; ?> bgcolor="<?php if($row>0){echo '#FFD5BF';}else{echo '#FFFFFF';} ?>">	   
...
<?php $sqlD=mysql_query("select DepartmentId,DepartmentCode,DepartmentName from hrm_department where DeptStatus='A' AND CompanyId=3 AND DepartmentCode!='MANAGEMENT' order by DepartmentId ASC", $con); 
      while($resD=mysql_fetch_array($sqlD)) { ?>
	 <tr bgcolor="#7a6189">
	  <td class="body" align="center" style="width:30px;">
	  <a href="#" style="text-decoration:none;">
	...
	<?php /* <input type="checkbox" id="CDept_<?php echo $resD['DepartmentId']; ?>" onClick="ClickDept(<?php echo $resD['DepartmentId']; ?>)" /> */ ?> </td>
     ...
     <tr>
	  <td colspan="2">
	   <table id="TR_<?php echo $resD['DepartmentId']; ?>" style="display:none;" border="1">
<?php $sqlE=mysql_query("select hrm_employee.EmployeeID,EmpCode,Fname,Sname,Lname from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID where hrm_employee_general.DepartmentId=".$resD['DepartmentId']." AND hrm_employee.CompanyId=3 order by EmpCode ASC", $con); while($resE=mysql_fetch_array($sqlE)) { 
$sqlc=mysql_query("select TrainingId from hrm_company_training_participant where TrainingId=".$_REQUEST['id']." AND EmployeeID=".$resE['EmployeeID'], $con);
$row=mysql_num_rows($sqlc);
?>	   
	   <tr id=ETR_<?php echo $resE['EmployeeID']; ?> bgcolor="<?php if($row>0){echo '#FFD5BF';}else{echo '#FFFFFF';} ?>">	   
