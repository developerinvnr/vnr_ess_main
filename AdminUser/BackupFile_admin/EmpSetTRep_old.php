 ...
 { 
   ...
   {  
	 ...
	 $Out_15 = strtotime(date($r2E['O'.$dv])) - (15 * 60); $nO15=date('H:i',$Out_15);
	  ...
	  //echo $In.' '.$Out.' '.$nI15.' '.$nO15.' '.$In_15.' '.$Out_15;
...
<?php /*?><script type="text/javascript" src="js/MasterAjaxCall.js"></script>
...
function FunClick()  
{
 ...
 else{ window.location='EmpSetTRep.php?ls=10&wer=123grtd&se=reew&w=ee102&m='+document.getElementById("Month").value+'&Y='+document.getElementById("Year").value+'&ee=s1s&d='+document.getElementById("Department").value+'&dd=truevalu&fals=truefalse'; }
}
...
<?php if($_REQUEST['d']=='All'){$dept='All';}else{ $SqlD=mysql_query("select DepartmentCode from hrm_department where DepartmentId=".$_REQUEST['d'], $con); $ResD=mysql_fetch_array($SqlD); } ?>	
...
<td align="center" class="tdf"><?php if($_REQUEST['m']==date("m") OR $_REQUEST['m']==$PreMonth){?><img src="images/go-back-icon.png" onClick="RefFun(<?php echo $ResEmp['EmployeeID'].','.$_REQUEST['m'].','.$_REQUEST['Y'].','.$_REQUEST['d']; ?>)"/><?php } ?></td>
