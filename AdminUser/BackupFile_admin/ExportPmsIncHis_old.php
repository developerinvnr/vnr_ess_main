...
header("Expires: 0");
$sep = "\t"; 
echo "Sn\tEmpCode\tName\tCurr Grade\tPro Grade\tDepartment\tCurr Designation\tPro Designation\tChange Date\tBasic\tStipend\tHRA\tCA\tVA\tSA\tPI\tIBM\tPre GrossPM\tCurr GrossPM\tPIS\t% PIS\tPSC\t% PSC\tTGSPM\t% TGSPM\tIncentive\tBonus\tScore\tRating";
print("\n");
...
{ $sql=mysql_query("select * from hrm_pms_appraisal_history INNER JOIN hrm_employee ON hrm_pms_appraisal_history.EmpCode=hrm_employee.EmpCode INNER JOIN hrm_employee_pms ON hrm_employee.EmployeeID=hrm_employee_pms.EmployeeID where hrm_pms_appraisal_history.CompanyId=".$_REQUEST['c']." AND hrm_employee_pms.HOD_EmployeeID=".$_REQUEST['value']." AND hrm_employee.CompanyId=".$_REQUEST['c']." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." order by hrm_pms_appraisal_history.EmpCode ASC, hrm_pms_appraisal_history.SalaryChange_Date ASC", $con); 
 ...
 if($resCheck['EmpStatus']=='A')
 {
  ...
  $schema_insert .= $res['Salary_Basic'].$sep;
  $schema_insert .= $res['Salary_Stipend'].$sep;
  $schema_insert .= $res['Salary_HRA'].$sep;
  $schema_insert .= $res['Salary_CA'].$sep;
  $schema_insert .= $res['Salary_VA'].$sep;
  $schema_insert .= $res['Salary_SA'].$sep;
  $schema_insert .= $res['Salary_PI'].$sep;
  $schema_insert .= $res['Industry_Bench_Markinge'].$sep;
  $schema_insert .= $res['Previous_GrossSalaryPM'].$sep;
  $schema_insert .= $res['Current_GrossSalaryPM'].$sep;
  $schema_insert .= $res['Proposed_GrossSalaryPM'].$sep;
  $schema_insert .= $res['Prop_PeInc_GSPM'].$sep;
  $schema_insert .= $res['PropSalary_Correction'].$sep;
  $schema_insert .= $res['TotalProp_GSPM'].$sep; 	
  $schema_insert .= $res['TotalProp_PerInc_GSPM'].$sep;
  $schema_insert .= $res['Incentive'].$sep;
  $schema_insert .= $res['BonusAnnual_September'].$sep;
...
$sqlE2=mysql_query("select Fname, Sname, Lname from hrm_employee where EmpCode=".$res['EmpCode'], $con); 
