...
{
$sqlU=mysql_query("update hrm_pms_appraisal_history set SalaryChange_Date='".date("Y-m-d",strtotime($_REQUEST['DV']))."' where AppHistoryId=".$_REQUEST['ID'], $con);
...
var DV=document.getElementById("DateEdit_"+Sn).value; 
var agree=confirm("Are you sure you want to edit?");  
if (agree) { window.location="EmpCheckDate.php?ID="+HI+"&Sn="+Sn+"&EI="+EI+"&DV="+DV; }
}
</script>
 ...
 <table border="0">
  ...
  <tr>
    <td style="vertical-align:top;width:900px;" valign="top">
	  <table border="0">
	   <tr>
	     <td align="center" style="width:150px;" valign="top"><?php echo "<img width=105px height=120px border=1 src=\"show_images.php?id=".$EmployeeId."\">\n";?></td>
    ...
    <td style="vertical-align:top;width:900px;" valign="top">
	  <table border="1">
	   <tr bgcolor="#7a6189" style="height:22px;">
		...
		<td align="center" class="head" style="color:#FFFFFF;width:100px;">Monthly_Gross</td>
...
<?php $sqlHi2=mysql_query("select AppHistoryId,SalaryChange_Date,Proposed_Designation,Proposed_Grade,TotalProp_GSPM, TotalProp_PerInc_GSPM from hrm_pms_appraisal_history where EmpCode=".$resE['EmpCode']." AND CompanyId=".$resE['CompanyId']." AND SalaryChange_Date>='".date('2012-01-01')."' order by SalaryChange_Date DESC", $con); 
      $Sno2=1; while($resHi2=mysql_fetch_array($sqlHi2)){ ?>	
       <tr bgcolor="#FFFFFF" style="height:22px;">
		...
		<td align="center" class="data" style="width:100px;"><input name="DateEdit_<?php echo $Sno2; ?>" id="DateEdit_<?php echo $Sno2; ?>" style="font-family:Times New Roman, Times, serif; font-size:11px; width:98px;" value="<?php echo date("d-m-Y",strtotime($resHi2['SalaryChange_Date'])); ?>" /></td>
		...
		<td align="right" class="data" style="width:100px;"><?php echo $resHi2['TotalProp_GSPM']; ?>&nbsp;</td>
		<td align="center" class="data" style="width:100px;"><?php echo $resHi2['TotalProp_PerInc_GSPM']; ?></td>	
		<td align="center" class="data" style="width:50px;"><input type="button" name="SaveEdit"  value="" onClick="Editbtn(<?php echo $resHi2['AppHistoryId'].', '.$Sno2.', '.$EmployeeId; ?>)" class="SaveButton"></td>	
	   </tr>   	
<?php $Sno2++; } $Sno=$Sno2; ?>	     	   
<?php $sqlHi=mysql_query("select AppHistoryId,SalaryChange_Date, Current_Designation, Current_Grade, Previous_GrossSalaryPM, Prop_PeInc_GSPM from hrm_pms_appraisal_history where EmpCode=".$resE['EmpCode']." AND CompanyId=".$resE['CompanyId']." AND SalaryChange_Date<'".date('2012-01-01')."' order by SalaryChange_Date DESC", $con); 
       while($resHi=mysql_fetch_array($sqlHi)){ ?>	
       <tr bgcolor="#FFFFFF" style="height:22px;">
		<td align="center" class="data" style="width:50px;"><?php echo $Sno; ?></td>
		<td align="center" class="data" style="width:100px;"><input name="DateEdit_<?php echo $Sno; ?>" id="DateEdit_<?php echo $Sno; ?>" style="font-family:Times New Roman, Times, serif; font-size:11px; width:98px;" value="<?php echo date("d-m-Y",strtotime($resHi['SalaryChange_Date'])); ?>" /> </td>
		...
		<td align="right" class="data" style="width:100px;"><?php echo $resHi['Previous_GrossSalaryPM']; ?>&nbsp;</td>
		<td align="center" class="data" style="width:100px;"><?php echo $resHi['Prop_PeInc_GSPM']; ?></td>	
