		...
		<td align="" width="100%" valign="top">
		   <table border="0">
       		 <tr><td colspan="2">
			       <table border="0">
                     <tr>
					  <td colspan="12" align="left" class="heading">Achievement List &nbsp;<span id="ReturnValue">&nbsp;</span></td>
					  <td>			
				   </td>                           			 
		           </tr>
                  </table>
...
<tr>
 <td>
   <table border="1" width="1000">
     <tr> 
  <td colspan="<?php if($_REQUEST['a']==0){echo 3;}else{echo 7;} ?>" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Achievement List <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2; ?>&nbsp;)&nbsp;&nbsp;&nbsp;
...
<?php } */ ?>
		<td align="center" class="All_50" style="color:#FFFFFF; width:900px;" ><b>Achievement</b></td>
     </tr>
<?php 
...
{  
  if($_REQUEST['value']==0)
  { $sql=mysql_query("SELECT EmpCode,Fname,Sname,Lname,DesigId,hrm_employee_general.DepartmentId,KRA,KRA_Description,Measure,Unit,Weightage,Target FROM hrm_pms_kra INNER JOIN hrm_employee ON hrm_pms_kra.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_general ON hrm_pms_kra.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_pms_kra.YearId=".$_REQUEST['YI']."  AND hrm_pms_kra.CompanyId=".$CompanyId." AND (hrm_pms_kra.KRAStatus='A' OR hrm_pms_kra.KRAStatus='R') AND hrm_employee_general.DateJoining<='".$Y."-06-30' AND hrm_employee.EmpCode<=11000 order by hrm_employee.EmpCode ASC, hrm_pms_kra.KRAId ASC", $con); }
  else{ $sql=mysql_query("SELECT EmpCode,Fname,Sname,Lname,DesigId,hrm_employee_general.DepartmentId,KRA,KRA_Description,Measure,Unit,Weightage,Target FROM hrm_pms_kra INNER JOIN hrm_employee ON hrm_pms_kra.EmployeeID=hrm_employee.EmployeeID INNER JOIN hrm_employee_general ON hrm_pms_kra.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_pms_kra.DepartmentId=".$_REQUEST['value']." AND hrm_pms_kra.YearId=".$_REQUEST['YI']."  AND hrm_pms_kra.CompanyId=".$CompanyId." AND (hrm_pms_kra.KRAStatus='A' OR hrm_pms_kra.KRAStatus='R') AND hrm_employee_general.DateJoining<='".$Y."-06-30' AND hrm_employee.EmpCode<=11000 order by hrm_employee.EmpCode ASC, hrm_pms_kra.KRAId ASC", $con); }
}
*/
...
<?php } */ ?>		
		<td class="recCls" valign="top"><?php echo $res['Achivement']; ?></td>	
	 </tr>
	 <?php $Sno++; } ?>
