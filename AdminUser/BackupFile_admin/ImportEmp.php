...
if(isset($_POST['SaveEmpContact']))
{
 if ($_FILES['Contact_csv_file']['error'] > 0) {
  ...
  if (($handle = fopen($_FILES['Contact_csv_file']['tmp_name'], "r")) !== FALSE) {
   ...
   fclose($handle);
   if($sql){ $msgCon= 'Employee Contact Details Uploaded sSuccessfully...';}
  }
 }
}
...
//====================================================Import Training=============================================================//
if(isset($_POST['SaveEmpTra']))
{
 if ($_FILES['Tra_csv_file']['error'] > 0) {
  echo "Error: " . $_FILES['Tra_csv_file']['error'] . "<br />"; 
 }else{  
  if (($handle = fopen($_FILES['Tra_csv_file']['tmp_name'], "r")) !== FALSE) {
   $ctr = 1; // used to exclude the CSV header
   while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) { 
   $c0 = mysql_real_escape_string($data[0]); $c1 = mysql_real_escape_string($data[1]); $c2 = mysql_real_escape_string($data[2]); $c3 = mysql_real_escape_string($data[3]);
   $c4 = mysql_real_escape_string($data[4]); $c5 = mysql_real_escape_string($data[5]); $c6 = mysql_real_escape_string($data[6]); $c7 = mysql_real_escape_string($data[7]);
   $c8 = mysql_real_escape_string($data[8]); $c9 = mysql_real_escape_string($data[9]); $c10 = mysql_real_escape_string($data[10]); 

    if ($ctr > 1 AND ($c0!='' OR $c0!=0)) { 
	$sqlE=mysql_query("select EmployeeID from hrm_employee where CompanyId=".$CompanyId." AND EmpCode=".$c0."", $con); $resE=mysql_fetch_assoc($sqlE);
	$sql=mysql_query("insert into hrm_employee_training(EmployeeID, EC, TraName, TraDate, FromDate, ToDate, TraDuration, Institute, TrainerName, Location, CreatedDate, YearId) values(".$resE['EmployeeID'].", ".$c0.", '".$c1."', '".date("Y-m-d",strtotime($c2))."', '".date("Y-m-d",strtotime($c2))."', '".date("Y-m-d",strtotime($c3))."', ".$c4.", '".$c5."', '".$c6."', '".$c7."', '".date("Y-m-d")."', ".$YearId.")", $con); }
    else { $ctr++; }
   }
   fclose($handle);
   if($sql){ $msgTra= 'Employee Training Details Uploaded Successfully...';}
  }
 }
}

//====================================================Import Conference=============================================================//
if(isset($_POST['SaveEmpConf']))
{
 if ($_FILES['Conf_csv_file']['error'] > 0) {
  echo "Error: " . $_FILES['Conf_csv_file']['error'] . "<br />"; 
 }else{  
  if (($handle = fopen($_FILES['Conf_csv_file']['tmp_name'], "r")) !== FALSE) {
   $ctr = 1; // used to exclude the CSV header
   while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) { 
   $c0 = mysql_real_escape_string($data[0]); $c1 = mysql_real_escape_string($data[1]); $c2 = mysql_real_escape_string($data[2]); $c3 = mysql_real_escape_string($data[3]);
   $c4 = mysql_real_escape_string($data[4]); $c5 = mysql_real_escape_string($data[5]); $c6 = mysql_real_escape_string($data[6]); $c7 = mysql_real_escape_string($data[7]);
   $c8 = mysql_real_escape_string($data[8]); $c9 = mysql_real_escape_string($data[9]); $c10 = mysql_real_escape_string($data[10]); 
    ...
    if ($ctr > 1 AND ($c0!='' OR $c0!=0)) { 
	 $sqlE=mysql_query("select EmployeeID from hrm_employee where CompanyId=".$CompanyId." AND EmpCode=".$c0."", $con); $resE=mysql_fetch_assoc($sqlE);
	$sql=mysql_query("insert into hrm_employee_conference(EmployeeID, EC, ConfName, ConfDate, FromDate, ToDate, ConfDuration, ConductedBy, Location, CreatedDate, YearId) values(".$resE['EmployeeID'].", ".$c0.", '".$c1."', '".date("Y-m-d",strtotime($c2))."', '".date("Y-m-d",strtotime($c2))."', '".date("Y-m-d",strtotime($c3))."', ".$c4.", '".$c5."', '".$c6."', '".date("Y-m-d")."', ".$YearId.")", $con); }
    else { $ctr++; }
   }
   fclose($handle);
   if($sql){ $msgConf= 'Employee Conference Details Uploaded Successfully...';}
  }
 }
}
...
//====================================================Import Pf_Uan=============================================================//
if(isset($_POST['SaveUan']))
{
 if ($_FILES['Uan_csv_file']['error'] > 0) {
  echo "Error: " . $_FILES['Uan_csv_file']['error'] . "<br />"; 
 }else{  
  if (($handle = fopen($_FILES['Uan_csv_file']['tmp_name'], "r")) !== FALSE) {
   $ctr = 1; // used to exclude the CSV header
   while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) { 
   $c0 = mysql_real_escape_string($data[0]); $c1 = mysql_real_escape_string($data[1]); $c2 = mysql_real_escape_string($data[2]); $c3 = mysql_real_escape_string($data[3]);
   $c4 = mysql_real_escape_string($data[4]); $c5 = mysql_real_escape_string($data[5]); 
    ...
    if ($ctr > 1 AND ($c0!='' OR $c0!=0)) { 
	 $sqlE=mysql_query("select EmployeeID from hrm_employee where CompanyId=".$CompanyId." AND EmpCode=".$c3."", $con); $resE=mysql_fetch_assoc($sqlE);
	 $sql=mysql_query("update hrm_employee_general set PF_UAN=".$c1." where EmployeeID=".$resE['EmployeeID'], $con); }
    else { $ctr++; }
   }
   fclose($handle);
   if($sql){ $msgUan= 'Employee UAN Details Uploaded Successfully...';}
  }
 }
}
...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="delete")
{
  ...
  if($SqlDelete) { echo '<script> alert("Data has been deleted successfully..."); window.location="ImportEmp.php";</script>'; }
}
?>
<html>
<head>
...
if (agree) { var x = "ImportEmp.php?action=delete&did="+v;  window.location=x;}}
...
{ document.getElementById("IUnDel").style.display='block'; document.getElementById("IDel").style.display='none'; 
  ...
  Gen_UnDel(); Per_UnDel(); Con_UnDel(); Fam_UnDel(); All_UnDel(); Tra_UnDel(); Conf_UnDel();
}
function UnDel()
...
{ document.getElementById("Gen_IUnDel").style.display='block'; document.getElementById("Gen_IDel").style.display='none';  
  ...
  UnDel(); Per_UnDel(); Con_UnDel(); Fam_UnDel(); All_UnDel(); Tra_UnDel(); Conf_UnDel();
}
function Gen_UnDel()
...
{ document.getElementById("Per_IUnDel").style.display='block'; document.getElementById("Per_IDel").style.display='none';  
  ...
  UnDel(); Gen_UnDel(); Con_UnDel(); Fam_UnDel(); All_UnDel(); Tra_UnDel(); Conf_UnDel();
}
function Per_UnDel()
...
{ document.getElementById("Con_IUnDel").style.display='block'; document.getElementById("Con_IDel").style.display='none';  
  ...
  UnDel(); Gen_UnDel(); Per_UnDel(); Fam_UnDel(); All_UnDel(); Tra_UnDel(); Conf_UnDel();
}
function Con_UnDel()
...
{ document.getElementById("Fam_IUnDel").style.display='block'; document.getElementById("Fam_IDel").style.display='none';  
  ...
  UnDel(); Gen_UnDel(); Per_UnDel(); Con_UnDel(); All_UnDel(); Tra_UnDel(); Conf_UnDel();
}
function Fam_UnDel()
...
{ document.getElementById("All_IUnDel").style.display='block'; document.getElementById("All_IDel").style.display='none';  
  ...
  UnDel(); Gen_UnDel(); Per_UnDel(); Con_UnDel(); Fam_UnDel(); Tra_UnDel(); Conf_UnDel();
}
function All_UnDel()
...
function Tra_Del()
{ document.getElementById("Tra_IUnDel").style.display='block'; document.getElementById("Tra_IDel").style.display='none';  
  document.getElementById("Tra_csv_file").disabled=false; document.getElementById("Tra_DeleteEmp").disabled=true;  document.getElementById("Tra_Up").disabled=false;
  UnDel(); Gen_UnDel(); Per_UnDel(); Con_UnDel(); Fam_UnDel(); All_UnDel(); Conf_UnDel();
}
function Tra_UnDel()
{ document.getElementById("Tra_IUnDel").style.display='none'; document.getElementById("Tra_IDel").style.display='block';  
  document.getElementById("Tra_csv_file").disabled=true; document.getElementById("Tra_DeleteEmp").disabled=true;  document.getElementById("Tra_Up").disabled=true;}  
  
function Conf_Del()
{ document.getElementById("Conf_IUnDel").style.display='block'; document.getElementById("Conf_IDel").style.display='none';  
  document.getElementById("Conf_csv_file").disabled=false; document.getElementById("Conf_DeleteEmp").disabled=true;  document.getElementById("Conf_Up").disabled=false;
  UnDel(); Gen_UnDel(); Per_UnDel(); Con_UnDel(); Fam_UnDel(); All_UnDel(); Tra_UnDel();
}
function Conf_UnDel()
{ document.getElementById("Conf_IUnDel").style.display='none'; document.getElementById("Conf_IDel").style.display='block';  
  document.getElementById("Conf_csv_file").disabled=true; document.getElementById("Conf_DeleteEmp").disabled=true;  document.getElementById("Conf_Up").disabled=true;}       
...
<table border="0" style="margin-top:0px; width:100%; height:300px;">
 <tr>
  <td align="left" height="20" valign="top" colspan="3">
   <table border="0">
    <tr>
	  <td align="" width="400" class="heading">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<u>Import CSV File (Employee File)</u> </td>
...
<tr>
  <td align="left" width="1150">
	 ...
	 <table width="" border="0">
	   ...
	   <form name="FormTraining" method="POST" enctype="multipart/form-data" >
	   <tr style="height:20px;">
	     <td style="font-family:Times New Roman; font-size:15px; color:#003300; width:150px; font-weight:bold;">(6) Training Details:</td>
	     <td style="width:285px;"><input type="file" name="Tra_csv_file" id="Tra_csv_file" style="width:200px; background-color:#CEFFCE;" size="30" disabled/></td>
		 <td style="width:75px;"><input type="submit" name="SaveEmpTra" value="Upload" id="Tra_Up" disabled/></td>
		 <td width="1">&nbsp;</td>
		 <td style="font-family:Times New Roman; font-size:15px; color:#003300; width:70px; font-weight:bold;">Deleted :</td>
		 <td style="width:70px;"><input type="button" name="Tra_DeleteEmp" id="Tra_DeleteEmp" value="Delete" onClick="Tra_DeletedEmp(<?php echo $CompanyId; ?>)" disabled/></td>
		 <td><img src="images/checkbox_UnCheck.png" border="0" id="Tra_IUnDel" onClick="Tra_UnDel()" style="display:none;"/>
		     <img src="images/checkbox.png" border="0" id="Tra_IDel" onClick="Tra_Del()"/></td>
		 <td class="font4" style=" color:#B00000;">&nbsp;<b><i><?php echo $msgTra; ?></i></b></td>		  
	   </tr>		
	   </form>  
	   <form name="FormConference" method="POST" enctype="multipart/form-data" >
	   <tr style="height:20px;">
	     <td style="font-family:Times New Roman; font-size:15px; color:#003300; width:180px; font-weight:bold;">(6) Conference Details:</td>
	     <td style="width:285px;"><input type="file" name="Conf_csv_file" id="Conf_csv_file" style="width:200px; background-color:#CEFFCE;" size="30" disabled/></td>
		 <td style="width:75px;"><input type="submit" name="SaveEmpConf" value="Upload" id="Conf_Up" disabled/></td>
		 <td width="1">&nbsp;</td>
		 <td style="font-family:Times New Roman; font-size:15px; color:#003300; width:70px; font-weight:bold;">Deleted :</td>
		 <td style="width:70px;"><input type="button" name="Conf_DeleteEmp" id="Conf_DeleteEmp" value="Delete" onClick="Conf_DeletedEmp(<?php echo $CompanyId; ?>)" disabled/></td>
		 <td><img src="images/checkbox_UnCheck.png" border="0" id="Conf_IUnDel" onClick="Conf_UnDel()" style="display:none;"/>
		     <img src="images/checkbox.png" border="0" id="Conf_IDel" onClick="Conf_Del()"/></td>
		 <td class="font4" style=" color:#B00000;">&nbsp;<b><i><?php echo $msgConf; ?></i></b></td>		  
	   </tr>		
	   </form> 
            <form name="FormUAN" method="POST" enctype="multipart/form-data" >
	   <tr style="height:20px;">
	     <td style="font-family:Times New Roman; font-size:15px; color:#003300; width:180px; font-weight:bold;">(7) PF UAN:</td>
	     <td style="width:285px;"><input type="file" name="Uan_csv_file" id="uan_csv_file" style="width:200px; background-color:#CEFFCE;" size="30" disabled/></td>
		 <td style="width:75px;"><input type="submit" name="SaveUan" value="Upload" id="Uan_Up" disabled/></td>
		 <td colspan="5" class="font4" style=" color:#B00000;">&nbsp;<b><i><?php echo $msgUan; ?></i></b></td>	
	   </tr>		
	   </form>   
	 </table>
