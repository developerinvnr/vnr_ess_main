...
elseif($_REQUEST['v']==3) //Grade-Desig
{
 ...
 print("\n");
 $sql=mysql_query("select e.EmployeeID,e.EmpCode,HR_Curr_DepartmentId,HR_GradeId,HR_DesigId from hrm_employee_pms p inner join hrm_employee e on p.EmployeeID=e.EmployeeID where p.AssessmentYear=".$_REQUEST['yi']." AND e.EmpStatus='A' AND e.CompanyId=".$_REQUEST['ci'], $con);
 $no=1;
 while($res=mysql_fetch_array($sql))
 {
