...
$EmployeeId=$_REQUEST['EI']; $Y=$_REQUEST['Y'];
?>
...
function SelectYear(v){ var EI=document.getElementById("EI").value; window.location='EmpCheckLeaveReg.php?Y='+v+"&EI="+EI;}

function PrintForm(EI,Y) 
{window.open("EmpCheckLeaveRegPrint.php?action=Form&EI="+EI+"&Y="+Y,"LeaveForm","location=no,scrollbars=yes,resizable=no,menubar=no,width=50,height=50");}
</script>
...
/*
$y2y=date("Y")-2; $y1y=date("Y")-1;
if($y2y<=$_REQUEST['Y'] OR ($y1y==$_REQUEST['Y'] && date("m")!='01' && date("m")!=02))
{ $yerY=date($_REQUEST['Y']."-12-31"); }
else{ $yerY=date("Y-m-d"); }

$ExpMonthDate=date('Y-m-d', strtotime("-2 months", strtotime($yerY))); $pp=strtotime($ExpMonthDate);
$ExpMonth=date('m', strtotime("-2 months", strtotime($yerY)));
$ExpYear=date('Y', strtotime("-2 months", strtotime($yerY)));
*/
...
<td style="width:150px;font-size:15px;font-family:Times New Roman;">&nbsp;<b><u>Date of separation</u>:</b></td>
 ...
 <td style="width:300px;" align="right"><b>Select Year:</b>
  <select style="font-size:11px; width:60px; height:18px; background-color:#DDFFBB;" name="YearID" id="YearID" onChange="SelectYear(this.value)"><option value="<?php echo $_REQUEST['Y']; ?>" style="margin-left:0px;" selected>&nbsp;<?php echo $_REQUEST['Y']; ?></option>
  <?php for($i=date("Y"); $i>=2013; $i--){?><option value="<?php echo $i; ?>">&nbsp;<?php echo $i; ?></option><?php } ?></select>
  &nbsp;<a href="#" onClick="PrintForm(<?php echo $_REQUEST['EI'].','.$_REQUEST['Y']; ?>)" style="text-decoration:none;"><font id="FonPC" style="color:#000099;">Print</font></a>
 </td>
</tr>
	</table>
   </td>
...
$TML2=mysql_query("select * from ".$AttTable." where EmployeeID=".$EmployeeId." AND AttValue='ML' AND AttDate>='".$ExpMonthDate."' AND AttDate<='".$ytT."' GROUP BY AttDate", $con); $TC2_ML=mysql_num_rows($TML2); 
 ...
 $TCountML=($TC_ML+$TC2_ML); ?><td align="center" class="data" style="width:100px;height:22px;font-weight:<?php if($TCountML>0){echo 'bold';}else{echo '';}?>;"><?php if($TCountML>0){echo $TCountML;} ?></td>
...
$TTL2=mysql_query("select * from ".$AttTable." where EmployeeID=".$EmployeeId." AND AttValue='TL' AND AttDate>='".$ExpMonthDate."' AND AttDate<='".$ytT."' GROUP BY AttDate", $con); $TC2_TL=mysql_num_rows($TTL2); 
 ...
 $TCountTL=($TC_TL+$TC2_TL); ?><td align="center" class="data" style="width:100px;height:22px;font-weight:<?php if($TCountTL>0){echo 'bold';}else{echo '';}?>;"><?php if($TCountTL>0){echo $TCountTL;} ?></td>	
...
$TCOV2=mysql_query("select * from ".$AttTable." where EmployeeID=".$EmployeeId." AND AttValue='COV' AND AttDate>='".$ExpMonthDate."' AND AttDate<='".$ytT."' GROUP BY AttDate", $con); $TC2_COV=mysql_num_rows($TCOV2); 
 ...
 $TCountCOV=($TC_COV+$TC2_COV); ?><td align="center" class="data" style="width:100px;height:22px;font-weight:<?php if($TCountCOV>0){echo 'bold';}else{echo '';}?>;"><?php if($TCountCOV>0){echo $TCountCOV;} ?></td>	 
...
?>	
	...
	<td rowspan="6" colspan="4" align="center" class="data" style="width:100px;height:22px;">&nbsp;</td>
	<?php /*?><td rowspan="5" align="center" class="data" style="width:100px;height:22px;">&nbsp;</td>		
...
?>	
	...
	<?php /*?><td align="center" class="data" style="width:100px;height:22px;" bgcolor="#FFFFFF">&nbsp;</td>
	<td align="center" class="data" style="width:100px;height:22px;" bgcolor="#FFFFFF">&nbsp;</td>	
	<td align="center" class="data" style="width:100px;height:22px;" bgcolor="#FFFFFF">&nbsp;</td>
...
<?php if($TCB_EL>30){$EnCashEL=$TCB_EL-30;} ?>	
	...
	<td class="data" style="width:100px;height:22px;font-weight:bold;" bgcolor="#FFFFFF">&nbsp;</td>
	<td class="data" style="width:100px;height:22px;font-weight:bold;" bgcolor="#FFFFFF">&nbsp;</td>
	<td class="data" style="width:100px;height:22px;font-weight:bold;" bgcolor="#FFFFFF">&nbsp;</td><?php */?>
   </tr>
<?php /********************************************* Earned leave *******************************************/ ?>
