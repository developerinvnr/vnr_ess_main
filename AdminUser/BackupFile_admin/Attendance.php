...
{ var x='Attendance.php?ls=10&wer=123grtd&se=reew&w=ee102&m='+v+'&Y='+y+'&ee=s1s&d='+d+'&dd=truevalu&fals=truefalse'; window.location=x; }
function SelectYear(v,m,d)
{ var x='Attendance.php?ls=10&wer=123grtd&se=reew&w=ee102&m='+m+'&Y='+v+'&ee=s1s&d='+d+'&dd=truevalu&fals=truefalse'; window.location=x; }
function SelectDept(v,m,y)
{ var x='Attendance.php?ls=10&wer=123grtd&se=reew&w=ee102&m='+m+'&Y='+y+'&ee=s1s&d='+v+'&dd=truevalu&fals=truefalse'; window.location=x; } */
...
function FunClick()  
{
 ...
 else{ window.location='Attendance.php?ls=10&wer=123grtd&se=reew&w=ee102&m='+document.getElementById("Month").value+'&Y='+document.getElementById("Year").value+'&ee=s1s&d='+document.getElementById("Department").value+'&seh='+document.getElementById("Searchkey").value+'&dd=truevalu&fals=truefalse'; }
}
	  ...
	  <td width="90%">
	    <table border="0">
		 <tr>
		   ...
		   <select style="font-size:11px; width:100px; height:19px; background-color:#DDFFBB; display:block;" name="Month" id="Month" onChange="SelectMonth(this.value, <?php echo $_REQUEST['Y'].', '.$_REQUEST['d']; ?>)"><option value="<?php echo $_REQUEST['m']; ?>"><?php echo date("F",strtotime(date("Y-".$_REQUEST['m']."-01"))); ?></option>
...
<?php $SqlD2=mysql_query("select DepartmentId,DepartmentCode from hrm_department where CompanyId=".$CompanyId." AND DeptStatus='A' order by DepartmentCode ASC", $con); 
   ...
   <td align="left" style="width:70px;"><input type="button" name="Refresh" id="Refresh" style="width:90px;" value="Refresh" onClick="javascript:window.location='Attendance.php?ls=10&wer=123grtd&se=reew&w=ee102&m=<?php echo $_REQUEST['m']; ?>&Y=<?php echo $_REQUEST['Y']; ?>&ee=s1s&d=<?php echo $_REQUEST['d']; ?>&dd=truevalu&fals=truefalse'"/></td> 
		  
		  <td align="left" class="cell3" style="background-color:#FFFF6C;text-align:center;width:200px;">Submission of Resignation</td>
		  
		  <td width="500" align="left" class="cell3">&nbsp;&nbsp;&nbsp;
...
{ $AttTable='hrm_employee_attendance'; $AttTable2='hrm_employee_attendance_'.$_REQUEST['Y']; 
  $LeaveTable='hrm_employee_monthlyleave_balance_'.$_REQUEST['Y']; $AppLeaveTable='hrm_employee_applyleave_'.$_REQUEST['Y'];  }
else
{$AttTable='hrm_employee_attendance'; $AttTable2='hrm_employee_attendance_'.$_REQUEST['Y']; 
 $LeaveTable='hrm_employee_monthlyleave_balance_'.$_REQUEST['Y']; $AppLeaveTable='hrm_employee_applyleave_'.$_REQUEST['Y'];  }
 ...
 <tr>
  ...
  <thead>
   <tr bgcolor="#7a6189">	 	
    ...
    <td colspan="5" class="cell" style="width:100px;"><b>Opening leave</b></td>
    <td rowspan="2" class="cell" style="width:30px;"><b>Edit</b></td>
    <td class="cell" colspan="31" style="text-align:left;">&nbsp;&nbsp;<b>Month :&nbsp;<font color="#EAEF18"><?php echo date("F",strtotime(date("Y-".$_REQUEST['m']."-01"))); ?></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Department :&nbsp;<font color="#EAEF18"><?php echo $ResD['DepartmentCode']; ?></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Year :&nbsp;<font color="#EAEF18"><?php echo $_REQUEST['Y']; ?></font></b></td>
   </tr>
   <tr bgcolor="#7a6189">	 
    <td class="cell" style="width:20px;">CL</td>
    <td class="cell" style="width:20px;">SL</td>
    <td class="cell" style="width:20px;">PL</td>
    <td class="cell" style="width:20px;">EL</td>
    <td class="cell" style="width:20px;">FL</td>
    <?php /*?><td class="cell" style="width:20px;">ML</td><?php */?>
    <?php for($i=1; $i<=date("t",strtotime(date("Y-".$_REQUEST['m']."-01"))); $i++) { ?>
...
$sql_statement=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname,DepartmentId from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND ".$ConDQ." AND e.CompanyId=".$CompanyId." order by e.EmpCode ASC", $con);
...
$SqlEmp=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname,DepartmentId from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND ".$ConDQ." AND e.CompanyId=".$CompanyId." order by e.EmpCode ASC LIMIT ".$from.",".$offset, $con);

/*  
if($_REQUEST['d']>0){ $SqlEmp=mysql_query("select e.*,g.* from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND g.DepartmentId=".$_REQUEST['d']." AND e.CompanyId=".$CompanyId." order by e.EmpCode ASC", $con); }
    elseif($_REQUEST['d']=='all'){ $SqlEmp=mysql_query("select e.*,g.* from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND e.CompanyId=".$CompanyId." order by e.EmpCode ASC", $con); }
*/
...
?>		
  ...
  <tbody>
   <tr id="tr_<?php echo $Sno; ?>" bgcolor="<?php if($rowch>0){echo '#FFFF6C';}elseif($Sno%2==0){echo '#FFFFFF';}else{echo '#D9D1E7';}?>">
	...
	<?php $SqlEmp3=mysql_query("SELECT * FROM ".$LeaveTable." WHERE EmployeeID=".$ResEmp['EmployeeID']." AND Month=".$m." AND Year=".$Y, $con); $ResEmp3=mysql_fetch_array($SqlEmp3); ?>
  <td class="cell2" align="center"><?php if($m==1){echo $ResEmp3['TotCL'];}else{echo floatval($ResEmp3['OpeningCL']);}?></td>
  <td class="cell2" align="center"><?php if($m==1){echo $ResEmp3['TotSL'];}else{echo floatval($ResEmp3['OpeningSL']);}?></td>
  <td class="cell2" align="center"><?php if($m==1){echo $ResEmp3['TotPL'];}else{echo floatval($ResEmp3['OpeningPL']);}?></td>
  <td class="cell2" align="center"><?php if($m==1){echo $ResEmp3['TotEL'];}else{echo floatval($ResEmp3['OpeningEL']);}?></td>
  <td class="cell2" align="center"><?php if($m==1){echo $ResEmp3['TotOL'];}else{echo floatval($ResEmp3['OpeningOL']);}?></td>
<?php /*?><td class="cell2" align="center"><?php if($m==1){echo $ResEmp3['TotML'];}else{echo floatval($ResEmp3['OpeningML']);}?></td><?php */?>
   
    <?php $PreMonth=$ExpMDate=date('m', strtotime("-1 months", strtotime(date("Y-".date("m")."-d")))); ?>
    <td bgcolor="#FFFFFF" align="center" id="btnTd_<?php echo $ResEmp['EmployeeID']; ?>">
    <?php //if($_SESSION['User_Permission']=='Edit'){ if($m==date("m") OR $m==$PreMonth){ ?>
    <img src="images/edit.png" border="0" id="<?php echo $ResEmp['EmployeeID']; ?>_BtnEdit" onClick="ClickEdit(<?php echo $ResEmp['EmployeeID'].', '.$m.', '.$Y; ?>)" style="display:block;"><img src="images/save.gif" border="0" id="<?php echo $ResEmp['EmployeeID']; ?>_BtnSave" onClick="ClickSave(<?php echo $ResEmp['EmployeeID'].', '.$m.', '.$Y.', '.$YearId; ?>)" style="display:none;">
    <?php //} 
    //} ?>
    </td>	
...
<?php } //if($ConDQ!='') ?>
 ...
 <td align="center" colspan="10" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
  <?PHP doPages($offset, 'Attendance.php', '', $total_records); ?></td>
 </tr>
 </table>
 </form>     
</td>
