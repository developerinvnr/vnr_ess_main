...
if($_REQUEST['actionv']=='revert')
{
 ...
 $sqlu=mysql_query("update hrm_pms_formb_tgtdefin set lockk=0,Applockk=0,Revlockk=0 where TgtFbDefId=".$_REQUEST['revertId'],$con); 
 if($sqlu){ echo '<script>alert("Revert Successfully!"); window.location="setformbtgt.php?act=setkratgt&n='.$_REQUEST['n'].'&valueins=true&checkalpha=false&d=12&pa=4&y=2&fbid='.$_REQUEST['fbid'].'&prd='.$_REQUEST['prd'].'&yy=t1t&tgt='.$_REQUEST['tgt'].'&c='.$_REQUEST['c'].'&e='.$_REQUEST['e'].'&yid='.$_REQUEST['yid'].'&lgc='.$_REQUEST['lgc'].'&wgt='.$_REQUEST['wgt'].'"; </script>'; }
}
...
function funRevert(id)
{
 ...
 if(agree)
 {
window.location="setformbtgt.php?act=setkratgt&n="+document.getElementById("nn").value+"&valueins=true&checkalpha=false&d=12&pa=4&y=2&fbid="+document.getElementById("bid").value+"&prd="+document.getElementById("prd").value+"&yy=t1t&tgt="+document.getElementById("tgt").value+"&actionv=revert&revertId="+id+"&eid="+document.getElementById("eid").value+"&e="+document.getElementById("eid").value+"&c="+document.getElementById("c").value+"&yid="+document.getElementById("yid").value+"&lgc="+document.getElementById("lgc").value+"&wgt="+document.getElementById("wgt").value; 
 ...
 <tr>
  <td style="width:100%;"  valign="top">
   <table align="center" border="0" cellspacing="1">
	...
	<tr><td colspan="15"><font color="#9BFFFF">Logic:</font>&nbsp;<font color="#FFFFFF"><?php echo $_REQUEST['lgc']; ?></font>&nbsp;,&nbsp;<font color="#9BFFFF">Skill:</font>&nbsp;<font color="#FFFFFF"><?php echo $resk['Skill']; ?></font>&nbsp;&nbsp;&nbsp;&nbsp;<font color="#9BFFFF">Description:</font>&nbsp;<font color="#FFFFFF"><?php echo $resk['SkillComment']; ?></font></td></tr>
    <tr style="background-color:#CC9900;height:25px;">
	 ...
	 <td colspan="2" class="t">Reporting Rating Deatis</td>
	 <td rowspan="2" class="t" style="width:50px;">Edit</td>
	</tr>
	<tr style="height:25px;background-color:#CC9900;">
	 <td class="t" style="width:50px;">Self Rating</td>
	 ...
	 <td class="t" style="width:50px;">Rep Rating</td>
	 <!--<td class="t" style="width:200px;">Remark</td>
	 <td class="t" style="width:50px;">Allow Logic</td>-->
	 <td class="t" style="width:50px;">Score</td>
	</tr>
...
<input type="hidden" name="bid" id="bid" value="<?php echo $_REQUEST['fbid']; ?>"/>
...
<input type="hidden" name="eid" id="eid" value="<?php echo $_REQUEST['e']; ?>" />
...
<?php $sql=mysql_query("select * from hrm_pms_formb_tgtdefin where ".$fn."=".$_REQUEST['fbid']." AND EmployeeID=".$_REQUEST['e']."  AND Tital='".$t."' AND NtgtN='".$i."'",$con); $row=mysql_num_rows($sql); $res=mysql_fetch_assoc($sql); ?>	 
	 <input type="hidden" id="wgt<?php echo $i; ?>" name="wgt<?php echo $i; ?>" value="<?php if($res['Wgt']!=''){echo floatval($res['Wgt']);}else{echo floatval($wgt);} ?>"/><input type="hidden" id="tgt<?php echo $i; ?>" name="tgt<?php echo $i; ?>" value="<?php if($res['Tgt']!=''){echo floatval($res['Tgt']);}else{echo floatval($tgt);} ?>"/><input type="hidden" id="NtgtN<?php echo $i; ?>" name="NtgtN<?php echo $i; ?>" value="<?php echo $i; ?>"/><input type="hidden" name="ld<?php echo $i; ?>" value="<?php if($row>0 AND $res['Ldate']!='0000-00-00' AND $res['Ldate']!='1970-01-01'){ echo $res['Ldate']; }else{ echo $ld; } ?>"/>
	 ...
	 <td class="dc"><?php if($row>0){echo floatval($res['Tgt']);}//else{echo floatval($tgt);} ?></td>  
...
<td class="dc" bgcolor="#FFFFC6"><span style="cursor:pointer;"><?php if($res['lockk']>0){?><img src="images/go-back-icon.png" onClick="funRevert(<?php echo $res['TgtFbDefId']; ?>)"><?php } ?></span></td>
 ...
 <?php /*?><td class="dc" bgcolor="#D5FFAA"><input class="input" name="Cmnt<?php echo $i; ?>" id="Cmnt<?php echo $i; ?>" class="d" style="width:200px;border:hidden;background-color:#D5FFAA;"><?php echo $res['AppCmnt']; ?></textarea> */ ?>
	...
	<td align="center">
	 ...
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ ?>
	  ...
	  <span style="cursor:pointer"><img src="../Employee/images/Floppy-Small-icon.png" border="0" id="save_<?php echo $i; ?>" style="display:none;" onClick="FunSave(<?php echo $i.','.$res['TgtFbDefId']; ?>)"/></span>
	 <?php } ?> 
	 </td>
	...
	<tr style="height:24px;background-color:#FFFFFF;">
	 ...
	 <td class="d" align="center"><b><?php echo $_REQUEST['tgt']; ?></b></td>
	 <td class="d">
...
<?php $sqlt=mysql_query("select SUM(Ach) as tAch, SUM(LogScr) as tLogScr, SUM(Scor) as tScor, SUM(AppAch) as tAppAch, SUM(AppLogScr) as tAppLogScr, SUM(AppScor) as tAppScor from hrm_pms_formb_tgtdefin where ".$fn."=".$_REQUEST['fbid']." AND EmployeeID=".$_REQUEST['e']." AND YearId=".$_REQUEST['yid'],$con); $rest=mysql_fetch_assoc($sqlt); ?>	 
	 ...
	 <td><input class="input" id="totAch" style="width:50px;font-weight:bold;text-align:center;" value="<?php if($rest['tAch']>0){echo floatval($rest['tAch']);} ?>" readonly/></td>
	  <?php /*?><td class="d"></td>*/ ?>
     ...
     <td><input class="input" id="TotScore" style="width:50px;font-weight:bold;text-align:center;" value="<?php if($rest['tScor']>0){echo floatval($rest['tScor']);} ?>" readonly/></td>


     <td class="d"></td>
	 <td><input class="input" id="tot2Ach" style="width:50px;font-weight:bold;text-align:center;" value="<?php if($rest['tAppAch']>0){echo floatval($rest['tAppAch']);} ?>" readonly/></td>
	 ...
	 <?php /*?><td class="d" align="center" bgcolor="#FFFFFF"><b><input type="text" id="TotLogScr" style="width:50px;font-weight:bold; text-align:center;background-color:#FFFFFF;border:hidden;" value="<?php if($rest['tAppLogScr']>0){echo floatval($rest['tAppLogScr']);} ?>" readonly/></b></td>	<?php */?>
	  ...
	  <td><input class="input" id="Tot2Score" style="width:50px;font-weight:bold;text-align:center;" value="<?php if($rest['tAppScor']>0){echo floatval($rest['tAppScor']);} ?>" readonly/></b></td>
