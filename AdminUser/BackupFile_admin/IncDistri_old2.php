...
{ 
 $sql5w=mysql_query("select * from hrm_pms_increment_dishod2 where CompanyId=".$CompanyId." AND YearId=".$_POST['yer5']." AND Rating=".$_POST['Rat5']." AND HodId=".$_POST['hod5']." AND Hod2Id=".$_POST['hod25'],$con); $row5w=mysql_num_rows($sql5w);
 if($row5w>0){ $SqlU5w = mysql_query("UPDATE hrm_pms_increment_dishod2 SET IncDistriFrom=".$_POST['IncF5'].", IncDistriTo=".$_POST['IncT5'].", CreatedBy=".$UserId.", CreatedDate='".date("Y-m-d")."' where CompanyId=".$CompanyId." AND YearId=".$_POST['yer5']." AND Rating=".$_POST['Rat5']." AND HodId=".$_POST['hod5']." AND Hod2Id=".$_POST['hod25'], $con); }
 else { $SqlU5w = mysql_query("insert into hrm_pms_increment_dishod2(YearId, Rating, IncDistriFrom, IncDistriTo, HodId, Hod2Id, IncDisStatus, CompanyId, CreatedBy, CreatedDate) values(".$_POST['yer5'].", ".$_POST['Rat5'].", ".$_POST['IncF5'].", ".$_POST['IncT5'].", ".$_POST['hod5'].", ".$_POST['hod25'].", 'A', ".$CompanyId.", ".$UserId.", '".date("Y-m-d")."')",$con); }  if($SqlU5w){$msg="Data has been Updeted successfully...";}
}

if(isset($_POST['Save6Edit']))
{ 
 $sql6w=mysql_query("select * from hrm_pms_increment_disbuc where CompanyId=".$CompanyId." AND YearId=".$_POST['yer6']." AND Rating=".$_POST['Rat6']." AND HodId=".$_POST['hod6']." AND DeptId=".$_POST['dept6']." AND BUCId=".$_POST['buc6'],$con); $row6w=mysql_num_rows($sql6w);
 if($row6w>0){ $SqlU6w = mysql_query("UPDATE hrm_pms_increment_disbuc SET IncDistriFrom=".$_POST['IncF6'].", IncDistriTo=".$_POST['IncT6'].", CreatedBy=".$UserId.", CreatedDate='".date("Y-m-d")."' where CompanyId=".$CompanyId." AND YearId=".$_POST['yer6']." AND Rating=".$_POST['Rat6']." AND HodId=".$_POST['hod6']." AND DeptId=".$_POST['dept6']." AND BUCId=".$_POST['buc6'], $con); }
 else { $SqlU6w = mysql_query("insert into hrm_pms_increment_disbuc(YearId, Rating, IncDistriFrom, IncDistriTo, HodId, DeptId,  BUCId, IncDisStatus, CompanyId, CreatedBy, CreatedDate) values(".$_POST['yer6'].", ".$_POST['Rat6'].", ".$_POST['IncF6'].", ".$_POST['IncT6'].", ".$_POST['hod6'].", ".$_POST['dept6'].", ".$_POST['buc6'].", 'A', ".$CompanyId.", ".$UserId.", '".date("Y-m-d")."')",$con); }  if($SqlU6w){$msg="Data has been Updeted successfully...";}
}
...
<?php //*************** Open All All All All All ************************?>
 ...
 <table border="1" width="220" cellpadding="2" cellspacing="1" style="background-color:#FFFFAA;">
  <tr>
...
<?php $sqlRat6=mysql_query("select * from hrm_pms_increment_dis where IncDisStatus='A' AND YearId=".$_REQUEST['ey']." AND CompanyId=".$CompanyId." order by Rating DESC",$con); $SNo=1; while($rs6=mysql_fetch_array($sqlRat6)) { $s6=mysql_query("select IncDistriFrom,IncDistriTo,CreatedDate from hrm_pms_increment_disbuc where Rating=".$rs6['Rating']." AND YearId=".$_REQUEST['ey']." AND HodId=".$_REQUEST['h']." AND DeptId=".$_REQUEST['d']." AND BUCId=".$_REQUEST['v'], $con); $row6=mysql_num_rows($s6); $r6=mysql_fetch_assoc($s6);
