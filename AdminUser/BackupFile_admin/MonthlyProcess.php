...
{ if(date("m")==01){ if($_REQUEST['v']==12){$Y=date("Y")-1;}else{$Y=date("Y");} }  //EmployeeID>601 AND EmployeeID<=650
  ...
  //echo "select EmployeeID from hrm_employee where CompanyId=".$CompanyId." AND (EmpStatus='A' OR (EmpStatus='D' AND (DateOfSepration='0000-00-00' OR DateOfSepration='1970-01-01' OR DateOfSepration>='".date($Y."-".$_REQUEST['v']."-01")."')))";
  
  $sqlE=mysql_query("select EmployeeID from hrm_employee where CompanyId=".$CompanyId." AND (EmpStatus='A' OR (EmpStatus='D' AND (DateOfSepration='0000-00-00' OR DateOfSepration='1970-01-01' OR DateOfSepration>='".date($Y."-".$_REQUEST['v']."-01")."'))) order by EmployeeID ASC", $con);
...
if($_REQUEST['action']=='UpAttMonth' && $_REQUEST['v']!='')
{
 ...
 $SqlCOV=mysql_query("select count(DISTINCT AttDate)as COV from hrm_employee_attendance where AttValue='COV' AND EmployeeID=".$resEI['EmployeeID']." AND (AttDate between '".date($Y."-".$M."-1")."' AND '".date($Y."-".$M."-31")."')", $con); 
...
/********************************************** Open 1 ***********/
  ...
  //echo "select * from hrm_employee_monthlyleave_balance where EmployeeID=".$resEI['EmployeeID']." AND Month='".$M."' AND Year='".$Y."'";
   ...
   //echo "UPDATE hrm_employee_monthlyleave_balance set AvailedCL='".$TotalCL."', AvailedSL='".$TotalSL."', AvailedPL='".$TotalPL."', AvailedEL='".$TotalEL."', AvailedML='".$TotalML."', AvailedOL='".$TotalFL."', AvailedTL='".$TotalTL."', BalanceCL='".$TotBalCL."', BalanceSL='".$TotBalSL."', BalancePL='".$TotBalPL."', BalanceEL='".$TotBalEL."', BalanceML='".$TotBalML."', BalanceOL='".$TotBalFL."', MonthAtt_TotLeave='".$TotalLeaveCount."', MonthAtt_TotOD='".$TotalOnDuties."', MonthAtt_TotHO='".$TotalHoliday."', MonthAtt_TotPR='".$TotalPR."', MonthAtt_TotAP='".$TotalAbsent."', MonthAtt_TotWorkDay='".$TotalWorkingDay."', MonthAtt_TotPaidDay='".$TotalPaidDay."' where EmployeeID=".$resEI['EmployeeID']." AND Month='".$M."' AND Year='".$Y."'";
...
{   
  ...
  $sd=$Y."-".$m."-01"; $ed=$Y."-".$m."-31";
  $sqlE=mysql_query("select e.EmployeeID,g.DateJoining,g.DateConfirmationYN from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.CompanyId=".$CompanyId." AND (e.EmpStatus='A' OR (e.EmpStatus='D' AND e.DateOfSepration>='".$sd."')) ", $con);
  ...
  { 
   ...
   { 
	...
	{ 
	 ...
	 { $resME2=mysql_fetch_assoc($sqlME2); $oppl=$resME2['CreditedPL']; $opfl=$resME2['CreditedOL']; }
	 else{ $oppl=0; $opfl=0; }
...
$SqlCOV=mysql_query("select count(AttValue)as COV from hrm_employee_attendance where AttValue='COV' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); 
...
$SqlPresent=mysql_query("select count(AttValue)as Present from hrm_employee_attendance where (AttValue='P' OR AttValue='WFH') AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlAbsent=mysql_query("select count(AttValue)as Absent from hrm_employee_attendance where AttValue='A' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlOnDuties=mysql_query("select count(AttValue)as OnDuties from hrm_employee_attendance where AttValue='OD' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')", $con); $SqlHoliday=mysql_query("select count(AttValue)as Holiday from hrm_employee_attendance where AttValue='HO' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlELSun=mysql_query("select count(CheckSunday)as SUN from hrm_employee_attendance where CheckSunday='Y' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con);
   ...
   $ResELSun=mysql_fetch_array($SqlELSun); $ResCOV=mysql_fetch_array($SqlCOV);
   ...
   $TotalLeaveCount=$TotalCL+$TotalSL+$TotalPL+$TotalEL+$TotalFL+$TotalTL+$TotalML+$TotalCOV; $TotELSun=$ResELSun['SUN'];
...
if($_REQUEST['action']=='PayMonth' && $_REQUEST['v']!='')
{
 ...
 //$sqlE=mysql_query("select e.EmployeeID,EmpCode,DepartmentId,DesigId,GradeId,DateJoining,HqId,CostCenter,DateOfSepration from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.CompanyId=".$CompanyId." AND e.EmpStatus='A' AND g.DateJoining!='0000-00-00' AND g.DateJoining!='1970-01-01' AND g.DateJoining<='".date($Y.'-'.$_REQUEST['v'].'-31')."' ", $con);
 
 
  $m=$_REQUEST['v'];
  
  $sql=mysql_query("select b.EmployeeID from hrm_employee_monthlyleave_balance b inner join hrm_employee e on b.EmployeeID=e.EmployeeID where e.CompanyId=".$CompanyId." AND b.Year=".$Y." AND b.Month=".$m." AND b.MonthAtt_TotPaidDay>0 order by b.EmployeeID", $con); 
  
  while($res=mysql_fetch_assoc($sql))
  {  
    $id=$res['EmployeeID'];
    $sqlE=mysql_query("select e.EmployeeID,EmpCode,DepartmentId,DesigId,GradeId,DateJoining,HqId,CostCenter,DateOfSepration from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.CompanyId=".$CompanyId." AND e.EmployeeID=".$id, $con); 
...
/*********************************************************************************/	 
	...
	*/
	...
	/**************************************************/
	    $rowchk=0;
	    $sd=$Y."-".$m."-01"; $ed=$Y."-".$m."-31";
	   $chk=mysql_query("select count(*) as totrow from hrm_employee_general g inner join hrm_employee e on g.EmployeeID=e.EmployeeID where g.EmployeeID=".$id." AND ((g.DateJoining>='".$sd."' AND g.DateJoining<='".$ed."') OR (e.DateOfSepration>='".$sd."' AND e.DateOfSepration<='".$ed."')) ",$con); $reschk=mysql_fetch_assoc($chk);
       $rowchk=$reschk['totrow'];   
	  
       if($rowchk>0)
       {
         if($PrDay<=26){$PaidDay=$PrDay;}else{$PaidDay=$WorkDay;} //$aa=1;
       }
       else
       {
        $TWD=$WorkDay;  //26
        if($resME['MonthAtt_TotAP']>0){ $ActPD=$TWD-$resME['MonthAtt_TotAP']; }else{ $ActPD=$TWD; }
		if($PrDay>0){ $PaidDay=$ActPD; }else{ $PaidDay=0; }   //$aa=2;
       }
     /**************************************************/
     
     
	//echo $resE['EmpCode'].'-'.$aa.'-'.$PrDay.'-'.$TotalWork.'-'.$resME['MonthAtt_TotAP'].'-'.$PaidDay.'<br>';
  ...
  //if($Basic<=$ResLumSum['Pf_MaxSalPf']){ $Emp_PF=round(($PFBasic*12)/100);} else {$Emp_PF=round(($ResLumSum['Pf_MaxSalPf']*12)/100);}
	 ...
	 if($resCTC['NPS_Value']>0){ $NPS=$resCTC['NPS_Value']; }else{ $NPS=0; }
