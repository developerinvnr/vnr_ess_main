...
$sql_statement=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname,DepartmentId from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND ".$ConDQ." AND e.CompanyId=".$CompanyId." order by EmpCode ASC", $con);
...
$SqlEmp=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname,DepartmentId from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where e.EmpStatus='A' AND ".$ConDQ." AND e.CompanyId=".$CompanyId." order by EmpCode ASC LIMIT ".$from.",".$offset, $con);

/*
if($_REQUEST['d']>0){$SqlEmp=mysql_query("select hrm_employee.*,hrm_employee_general.* from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID where hrm_employee.EmpStatus='A' AND hrm_employee_general.DepartmentId=".$_REQUEST['d']." AND hrm_employee.CompanyId=".$CompanyId." order by EmpCode ASC", $con);}
...
$SqlRows=mysql_num_rows($SqlEmp); echo '<input type="hidden" name="EmpRows" id="EmpRows" value='.$SqlRows.' />'; 

while($ResEmp=mysql_fetch_array($SqlEmp))
{
$EId=$ResEmp['EmployeeID']; $ename=$ResEmp['Fname'].' '.$ResEmp['Sname'].' '.$ResEmp['Lname']; 
...
<?php } //if($ConDQ!='') ?>
 ...
 <td align="center" colspan="10" style="font-family:Times New Roman;font-size:15px;font-weight:bold;">
  <?PHP doPages($offset, 'AttendanceWeek.php', '', $total_records); ?></td>
 </tr>
...
$i = $current-1;
echo '<a href="'.$thepage.'?page='.$i.$query.'&ls='.$_REQUEST['ls'].'&wer=123grtd&se=reew&w=ee102&m='.$_REQUEST['m'].'&Y='.$_REQUEST['Y'].'&ee=s1s&d='.$_REQUEST['d'].'&trt=1f&tt=345&dd=truevalu&wk='.$_REQUEST['wk'].'&tyt=0" class="prn" rel="nofollow" title="go to page '.$i.'">&lt; Previous</a>&nbsp;';
echo '<span class="prn">...</span>&nbsp;';
}
if($start > 1) {
$i = 1;
echo '<a href="'.$thepage.'?page='.$i.$query.'&ls='.$_REQUEST['ls'].'&wer=123grtd&se=reew&w=ee102&m='.$_REQUEST['m'].'&Y='.$_REQUEST['Y'].'&ee=s1s&d='.$_REQUEST['d'].'&trt=1f&tt=345&dd=truevalu&wk='.$_REQUEST['wk'].'&tyt=0" title="go to page '.$i.'">'.$i.'</a>&nbsp;';
}
...
$i = $current+1;
echo '<span class="prn">...</span>&nbsp;';
echo '<a href="'.$thepage.'?page='.$i.$query.'&ls='.$_REQUEST['ls'].'&wer=123grtd&se=reew&w=ee102&m='.$_REQUEST['m'].'&Y='.$_REQUEST['Y'].'&ee=s1s&d='.$_REQUEST['d'].'&trt=1f&tt=345&dd=truevalu&wk='.$_REQUEST['wk'].'&tyt=0" class="prn" rel="nofollow" title="go to page '.$i.'">Next &gt;</a>&nbsp;';
} else {
