...
{  $SqlInseart = mysql_query("INSERT INTO hrm_company_conference_particular(Particular,CompanyId,CreatedBy,CreatedDate,YearId) VALUES('".$_POST['Particular']."','".$_POST['c']."', '".$_POST['u']."','".date('Y-m-d')."','".$_POST['yi']."')", $con); 
   if($SqlInseart){ $msg = "Data Save Successfully..."; }
}
if(isset($_POST['SaveEdit']))
{  $SqlUpdate = mysql_query("UPDATE hrm_company_conference_particular SET Particular='".$_POST['Particular']."', CreatedBy='".$_POST['u']."', CreatedDate='".date('Y-m-d')."', YearId='".$_POST['yi']."' WHERE ParticularId=".$_POST['ParticularId'], $con);
...
if (agree) { var x = "New2ConfParticular.php?action=edit&eid="+value+"&c="+c+"&u="+u+"&yi="+yi;  window.location=x;}}
function del(value,c,u,yi) { var agree=confirm("Are you sure you want to delete this record?");
if (agree) { var x = "New2ConfParticular.php?action=delete&did="+value+"&c="+c+"&u="+u+"&yi="+yi;  window.location=x;}}
function newsave(c,u,yi) { var x = "New2ConfParticular.php?action=newsave&c="+c+"&u="+u+"&yi="+yi;  window.location=x;}
</SCRIPT> 
</head>
 ...
 <tr>
  <td align="center" height="20" valign="top" colspan="3">
   <table border="0" align="left">
    <tr>
	  <td align="left" width="250" class="heading">&nbsp;&nbsp;&nbsp;Conference Particular</td>
...
<?php $sqlP=mysql_query("select * from hrm_company_conference_particular where CompanyId=".$_REQUEST['c'], $con); $SNo=1; while($resP=mysql_fetch_array($sqlP)) {
