...
{ $AfterFiveDay=date("Y-m-d",strtotime('+5 day')); $After30Day=date("Y-m-d",strtotime('+30 day'));
  ...
  if($rowch==0){ $sqlIns=mysql_query("insert into hrm_employee_separation(EmployeeID, ResigSubmit_HR, Emp_ResignationDate, Emp_RelievingDate, Emp_Reason, RetainQAns1, RetainQAns2, RetainQAns3, NoticeDay, Emp_Resignation_Status, Emp_SaveDate, Rep_EmployeeID, Log_EmployeeID, HOD_Date, Hod_EmployeeID, HR_Date, ResignationStatus, YearId) values(".$EmployeeId.", 'Y', '".date("Y-m-d", strtotime($_POST['ResDate']))."', '".date("Y-m-d", strtotime($_POST['RelDate']))."', '".$_POST['Reason']."', '".$_POST['RetainQAns1']."', '".$_POST['RetainQAns2']."', '".$_POST['RetainQAns3']."', ".$_POST['NoticeDay'].", 'Y', '".date("Y-m-d")."', ".$resE['AppraiserId'].", ".$logEmp.", '".$AfterFiveDay."', ".$resE['HodId'].", '".$After30Day."', 1, ".$YearId.")", $con); }
  if($sqlIns)
  { 
...
$email_subject = "Resignation has been submitted by HR";
$email_txt = "Resignation has been submitted by HR";
$headers = "From: ".$email_from."\r\n"; 
$semi_rand = md5(time()); 
$headers .= "MIME-Version: 1.0\r\n";
$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
$email_message .="The separation of ".$Ename." has been submitted in ESS by HR. Needs to be approved within 5 working days. Kindly log on to ESS for approval. <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>. \n\n";
...
$email_txt = "Apply For Resignation";
$headers = "From: ".$email_from."\r\n"; 
$semi_rand = md5(time()); 
$headers .= "MIME-Version: 1.0\r\n";
$headers .= "MIME-Version: 1.0\r\n";
$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
$email_message2 .="The separation of ".$Ename." has been submitted in ESS by HR. For more details kindly log on to ESS <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>  .\n\n";
...
$headers .= "MIME-Version: 1.0\r\n";
$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n"; 
$email_message3 .="The separation of ".$Ename." has been submitted in ESS by HR . Kindly log on to ESS for details <a href='https://www.vnrseeds.co.in'>www.vnrseeds.co.in</a>  .\n\n";
$email_message3 .="Thanks & Regards\n";
$email_message3 .="HR\n\n";
$ok = @mail($email_to, $email_subject, $email_message3, $headers);
} 
   $msg="Resignation save successfully.";
  }
}
...
<table border="0" cellspacing="5">
 <tr>
  <td colspan="2" class="heading">
   ...
   <input type="button" value="Cancel Resignation" style="background-color:<?php if($_REQUEST['a']=='c'){echo '#FFFFFF';}else{echo '#2D96FF';}?>;color:<?php if($_REQUEST['a']=='c'){echo '#C7C7C7';}else{echo '#FFFFFF';}?>;font-weight:bold;font-family:Geneva, Arial, Helvetica, sans-serif;" <?php if($_REQUEST['a']=='c'){echo 'disabled';} ?> onClick="javascript:window.location='FillEmpResignation.php?ee=4e&w=234&y=10234&a=c&ee=4e2&e=4e&w=234&y=110022344&retd=ee&wwrew=t%T@sed818&d=101&e=<?php echo $_REQUEST['e']; ?>&DI=<?php echo $_REQUEST['DI']; ?>'"/>
   <?php } */ ?>
   &nbsp;&nbsp;
   ...
   <?php } ?>
   &nbsp;&nbsp;<br><font style="color:#EC0000;font-size:16px;font-family:Times New Roman;font-weight:bold;"><blink>
   <?php if($rowch>0 AND $resch['Extend']=='N' AND $resch['HR_Approved']=='N' AND $resch['Emp_ExitInt']=='N' AND $resch['Emp_CancelResig']==0){ echo 'your resignation request is under process.'; }
         //elseif($rowch>0 AND $resch['Extend']=='N' AND $resch['HR_Approved']=='Y' AND $resch['Emp_ExitInt']=='N' AND $resch['Emp_CancelResig']==0){ echo 'please fill <u>exit interview</u> form for further process.'; }
		 elseif($rowch>0 AND $resch['Extend']=='N' AND $resch['HR_Approved']=='Y' AND $resch['Emp_ExitInt']=='Y' AND $resch['Emp_CancelResig']==0){ echo 'The exit interview form has been submitted. Your departmental clearance has been initiated; kindly submit all your assets and documents to the reporting manager before the relieving date'; }
...
<td style="display:<?php if($_REQUEST['a']=='f'){echo 'block';}elseif($_REQUEST['a']=='e' OR $_REQUEST['a']=='c'){echo 'none';}?>;">
 ...
 <tr><td colspan="7" valign="top" style='font-family:Times New Roman; font-size:17px; color:#008800' align="right">
  ...
  <?php
  $sqlEmp=mysql_query("select EmpCode,DepartmentId from hrm_employee_general g INNER JOIN hrm_employee e ON g.EmployeeID=e.EmployeeID where g.EmployeeID=".$_REQUEST['e'], $con); $resEmp=mysql_fetch_assoc($sqlEmp); 
  ...
  <td align="left" class="font2" style="width:120px;">&nbsp;&nbsp;Expected Relieving Date</td><td style="width:10px;"><b>&nbsp;:&nbsp;</b></td>
  ...
  <td colspan="7" align="Right" class="fontButton">
   <table>
	<tr> 
	 ...
	 <td>
	 <input type="button" name="Refrash" value="refresh" onClick="javascript:window.location='FillEmpResignation.php?ee=4e&w=234&y=10234&a=f&ee=4e2&e=4e&w=234&y=110022344&retd=ee&wwrew=t%T@sed818&d=101&e=<?php echo $_REQUEST['e']; ?>&DI=<?php echo $_REQUEST['DI']; ?>'" <?php if($rowch>0){echo 'disabled';} ?>/>
     </td>
	</tr>
   </table>
  </td>
