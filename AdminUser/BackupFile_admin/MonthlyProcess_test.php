...
{ if(date("m")==01){ if($_REQUEST['v']==12){$Y=date("Y")-1;}else{$Y=date("Y");} }  //EmployeeID>601 AND EmployeeID<=650
  ...
  { $m=$_REQUEST['v'];  
    ...
    { $sAtt=mysql_query("select AttValue from hrm_employee_attendance where EmployeeID=".$resE['EmployeeID']." AND AttDate='".date($Y.'-'.$m.'-'.$i)."'", $con); 
      //echo "select AttValue from hrm_employee_attendance where EmployeeID=".$resE['EmployeeID']." AND AttDate='".date($Y.'-'.$m.'-'.$i)."'";
...
if($_REQUEST['action']=='UpAttMonth' && $_REQUEST['v']!='')
{
 ...
 $SqlPresent=mysql_query("select count(DISTINCT AttDate)as Present from hrm_employee_attendance where AttValue='P' AND EmployeeID=".$resEI['EmployeeID']." AND (AttDate between '".date($Y."-".$M."-1")."' AND '".date($Y."-".$M."-31")."')", $con); 
...
$SqlHf=mysql_query("select count(AttValue)as Hf from hrm_employee_attendance where AttValue='HF' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlPresent=mysql_query("select count(AttValue)as Present from hrm_employee_attendance where AttValue='P' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlAbsent=mysql_query("select count(AttValue)as Absent from hrm_employee_attendance where AttValue='A' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlOnDuties=mysql_query("select count(AttValue)as OnDuties from hrm_employee_attendance where AttValue='OD' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')", $con); $SqlHoliday=mysql_query("select count(AttValue)as Holiday from hrm_employee_attendance where AttValue='HO' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con); $SqlELSun=mysql_query("select count(CheckSunday)as SUN from hrm_employee_attendance where CheckSunday='Y' AND EmployeeID=".$id." AND (AttDate between '".date($NY."-".$NM."-1")."' AND '".date($NY."-".$NM."-".$day)."')",$con);
   ...
   $CountCL=$ResCL['CL']; $CountCH=$ResCH['CH']/2; $TotalCL=$CountCL+$CountCH; $CountSL=$ResSL['SL']; 
   $CountSH=$ResSH['SH']/2; $TotalSL=$CountSL+$CountSH; $CountPH=$ResPH['PH']/2; 
   $TotalPL=$ResPL['PL']+$CountPH; $TotalEL=$ResEL['EL']; $TotalML=$ResML['ML']; $TotalFL=$ResFL['FL']; 
   $TotalTL=$ResTL['TL']; 
   ...
   { $RL=mysql_fetch_assoc($SL); 
    ...
    {  
       ...
       */
	   ...
	   $TW=mysql_query("select * from ".$AttT." where AttValue='P' AND ".$Cond1." GROUP BY AttDate", $con); 
	   ...
	   $TW2=mysql_query("select * from ".$AttT2." where AttValue='P' AND ".$Cond2." GROUP BY AttDate", $con); 
	   $TOD2=mysql_query("select * from ".$AttT2." where AttValue='OD' AND ".$Cond2." GROUP BY AttDate", $con); 
	   $TCH2=mysql_query("select * from ".$AttT2." where AttValue='CH' AND ".$Cond2." GROUP BY AttDate", $con); 
	   $TSH2=mysql_query("select * from ".$AttT2." where AttValue='SH' AND ".$Cond2." GROUP BY AttDate", $con); 
	   $TPH2=mysql_query("select * from ".$AttT2." where AttValue='PH' AND ".$Cond2." GROUP BY AttDate", $con); 
       ...
       if(($resE['DateJoining']<=date($Y.'-01-01')) OR ($TTotW_NoHO>=240)) //$TTotW
       { 
...
/*********************************************************************************/
	  ...
	  if($MaxCtcId!=''){$sqlCTC=mysql_query("select * from hrm_employee_ctc where EmployeeID=".$id." AND CtcId=".$MaxCtcId, $con);}elseif($MaxCtcId==''){$sqlCTC=mysql_query("select * from hrm_employee_ctc where EmployeeID=".$id." AND Status='A'", $con);} $resCTC=mysql_fetch_assoc($sqlCTC); 
/*********************************************************************************/	 
...
{ if(v!=''){document.getElementById("BtnsubUpAtt").disabled=false; 
  window.location="MonthlyProcess_test.php?act=SelUpAttMonth&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r"; } 
...
{ var value=document.getElementById("CMonthUpAtt").value;  
  ...
  { var agree2=confirm("Confirmation-2: Are you sure?");
	...
	{ var agree3=confirm("Confirmation-3: Are you sure?");
	  if(agree3)
	  { var x = "MonthlyProcess_test.php?action=UpAttMonth&v="+value;  window.location=x; }
	}
  }
}

function MonthPay(v)
{ if(v!=''){document.getElementById("BtnsubPay").disabled=false; var x = "MonthlyProcess_test.php?act=SelMonth&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r";   window.location=x;} 
  if(v==''){document.getElementById("BtnsubPay").disabled=true; var x = "MonthlyProcess_test.php?act=SelMonth&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r"; window.location=x;} 
}
...
{ var value=document.getElementById("CMonthPay").value;  
  ...
  { var agree2=confirm("Confirmation-2: Are you sure?");
	...
	{ var agree3=confirm("Confirmation-3: Are you sure?");
	  if(agree3)
	  { var x = "MonthlyProcess_test.php?action=PayMonth&v="+value;  window.location=x; }
	}
  }
}

function MonthLeave(v)
{ if(v!=''){document.getElementById("BtnsubLeave").disabled=false; var x = "MonthlyProcess_test.php?act=SelMonthLeave&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r"; window.location=x;} 
  if(v==''){document.getElementById("BtnsubLeave").disabled=true; var x = "MonthlyProcess_test.php?act=SelMonthLeave&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r"; window.location=x;} 
}
...
{ var value=document.getElementById("CMonthLeave").value;  
  ...
  { var agree2=confirm("Confirmation-2: Are you sure?");
	...
	{ var agree3=confirm("Confirmation-3: Are you sure?");
	  if(agree3)
	  { var x = "MonthlyProcess_test.php?action=LeaveMonth&v="+value;  window.location=x; }
	}
  }
}

function MonthAtt(v)
{ if(v!=''){document.getElementById("BtnsubAtt").disabled=false; var x = "MonthlyProcess_test.php?act=SelMonthAtt&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r"; window.location=x;} 
  if(v==''){document.getElementById("BtnsubAtt").disabled=true; var x = "MonthlyProcess_test.php?act=SelMonthAtt&v="+v+"&ls=10&wer=123grtd&se=reew&w=ee102&tt=false&true=r%r"; window.location=x;} 
}
...
{ var value=document.getElementById("CMonthAtt").value;  
  ...
  { var agree2=confirm("Confirmation-2: Are you sure?");
	...
	{ var agree3=confirm("Confirmation-3: Are you sure?");
	  if(agree3)
	  { var x = "MonthlyProcess_test.php?action=AttMonth&v="+value;  window.location=x; }
	}
  }
}
...
<?php if($_SESSION['User_Permission']=='Edit'){?><input type="button" id="BtnsubAtt" value="submit" onClick="SelMonthAtt()" <?php if($_REQUEST['act']=='SelMonthAtt' AND $_REQUEST['v']!=''){ echo '';} else {echo 'disabled';} ?> style="width:80px; "/>&nbsp;&nbsp;&nbsp;<?php echo $msgAtt; ?><?php } ?></td>
