...
<div class="tbody">
 ...
 <tr bgcolor="<?php if($Sno%2){ echo '#FFFFFF'; } else {echo '#D9D1E7';}?>"> 
	   ...
	   <td class="tdc" bgcolor="<?php if($resDP['EmpStatus']=='D'){echo '#FBE4BB';}?>"><?php echo $resDP['EmpStatus'];?></td>
		<td class="tdc"><?php if($_SESSION['User_Permission']=='Edit'){?><a href="EmpConfFormoK.php?e=<?php echo $resDP['EmployeeID']; ?>&c=1&cd=0000000&d=<?php echo $ResRep['DepartmentId']; ?>"><blink>Form</blink></a><?php } ?></td>
		
		<?php $sqf=mysql_query("select ConfLetterId,Recommendation,Status,EmpShow,EmpShow_Trr,EmpShow_Ext from hrm_employee_confletter where SubmitStatus='Y' AND EmployeeID=".$resDP['EmployeeID'], $con); $rref=mysql_num_rows($sqf); $ref=mysql_fetch_assoc($sqf); ?>
