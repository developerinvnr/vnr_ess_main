We are extremely concerned that your performance for the assessment period <b><?php echo $Ass2Year; ?></b> has been rated as <?php if($ResE['DepartmentId']==2 AND $resG['GradeValue']!='L1' AND $resG['GradeValue']!='L2' AND $resG['GradeValue']!='L3' AND $resG['GradeValue']!='L4' AND $resG['GradeValue']!='L5' AND $resG['GradeValue']!='MG'){ ?>"<b>2</b>". The given rating is based on Individual performance, Group performance and Departmental performance.<?php }else{ ?><b>"<?php echo $ResR['RatingName']; ?>"</b> with "<b>Rating-2</b>".<?php } ?><?php /*?><b>"<?php echo $ResR['RatingName']; ?>"</b> with "<b>Rating-2</b>".<?php */?> <p> 

           Your performance is below exceedingly expectation and you are being given a last chance to improve your performance. You shall be administered a <b>"Performance Improvement Plan"</b> (PIP) through which your performance shall be monitored against the objectives set by your manager for next 90 days. Your continuity in the organization post PIP completion shall be guided by the PIP policy. <p><?php $ProGrossAnnual=$ResPMS['HR_ProIncSalary']*12-$resS['Previous_GrossSalaryPM']*12; ?>
...
?>
           There is an increase in your Gross Annual Salary by <b>Rs. <?php echo $ProGrossAnnual; ?>/- </b>i.e. by <b><?php echo $ResPMS['HR_Percent_ProIncSalary']; ?> % </b>.<?php if($_REQUEST['C']==1 AND $ResE['DateJoining']>=$FD2.'-01-01' AND $ResE['DateJoining']<=$FD2.'-06-30') { ?> As your service is less than a year in the assessment cycle <?php echo $AssYear; ?>, your increment % is calculated on pro-rata basis as per the duration served during this period against the actual increment of <b><?php echo $ResPMS['HR_Percent_IncNetMonthalySalary']; ?> % </b> as per your performance rating.<p><?php } ?><?php if($_REQUEST['C']==1 AND $ResE['DateJoining']>=$FD1.'-04-01' AND $ResE['DateJoining']<=$FD1.'-09-30') { ?> As your assessment tenure is more than a year (DOJ on or after 1st July <?php echo $FD1; ?>), your increment % is calculated on pro-rata basis for the duration served since your joining against the actual annual increment for assessment cycle<?php //echo $AssYear; ?> that is <b><?php echo $resMaxMin['IncDistriFrom']; ?> %</b> as per your performance rating.<p><?php } ?>
