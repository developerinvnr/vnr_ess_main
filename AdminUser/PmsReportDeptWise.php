...
function SelectIncAllowPMS(v){window.location='PmsReportDeptWise.php?action=AllowIncPms&value='+v;}
function Click(I,N){ 
...
<td colspan="2" bgcolor="#7a6189" style="width:150px; height:20px; font-size:12px; font-family:Georgia; color:#FFFFFF; font-weight:bold;" align="center">Rating Graph</td>
				    ...
				    <table border="0">
					<tr>
				<td bgcolor="#7a6189" style="width:150px; height:20px; font-size:12px; font-family:Georgia; color:#FFFFFF; font-weight:bold;" align="center">Allow Increment</td>
					</tr>					
                    <tr bgcolor="#DDFFBB"> 
					 <td class="td1" style="font-size:11px; width:150px;" align="center">
                     <select style="font-size:11px; width:148px; height:18px; background-color:#DDFFBB;" name="IncAllowPMS" id="IncAllowPMS" onChange="SelectIncAllowPMS(this.value)">
					 <option value="" style="margin-left:0px;" selected>Select Department</option>
<?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?>
                       <option value="<?php echo $ResDept['DepartmentId']; ?>"><?php echo '&nbsp;'.$ResDept['DepartmentCode'];?></option><?php } ?>
					   <option value="All">&nbsp;All</option>
					   </select></td>				 
...
<tr>
 <td>
   <table border="1" width="1400">
     <tr>
<?php if($_REQUEST['value']!='All') {$sqlA=mysql_query("select DepartmentName from hrm_department where DepartmentId=".$_REQUEST['value'], $con);  $resA=mysql_fetch_assoc($sqlA); } ?>	 	  <td colspan="13" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Georgia; font-weight:bold;">&nbsp;PMS Status Department Wise :
...
<?php if($_REQUEST['value']=='All') { if($YearId==1){$Y=2012;}elseif($YearId==2){$Y=2013;}elseif($YearId==3){$Y=2014;}elseif($YearId==4){$Y=2015;}elseif($YearId==5){$Y=2016;}elseif($YearId==6){$Y=2017;}elseif($YearId==7){$Y=2018;}elseif($YearId==8){$Y=2019;}elseif($YearId==9){$Y=2020;}
$sql = mysql_query("SELECT hrm_employee.*, hrm_employee_general.*, hrm_employee_personal.*, hrm_employee_pms.* FROM hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_pms ON hrm_employee.EmployeeID=hrm_employee_pms.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID WHERE EmpStatus='A' AND hrm_employee_general.DateJoining<='".$Y."-03-31' AND hrm_employee_pms.AssessmentYear=".$YearId." AND (hrm_employee_pms.Appraiser_EmployeeID!='' OR hrm_employee_pms.Appraiser_EmployeeID!=0) AND hrm_employee.CompanyId=".$CompanyId, $con); }
      else { if($YearId==1){$Y=2012;}elseif($YearId==2){$Y=2013;}elseif($YearId==3){$Y=2014;}elseif($YearId==4){$Y=2015;}elseif($YearId==5){$Y=2016;}elseif($YearId==6){$Y=2017;}elseif($YearId==7){$Y=2018;}elseif($YearId==8){$Y=2019;}elseif($YearId==9){$Y=2020;}
$sql = mysql_query("SELECT hrm_employee.*, hrm_employee_general.*, hrm_employee_personal.*, hrm_employee_pms.* FROM hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_pms ON hrm_employee.EmployeeID=hrm_employee_pms.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID WHERE EmpStatus='A' AND hrm_employee_general.DateJoining<='".$Y."-03-31' AND hrm_employee_pms.AssessmentYear=".$YearId." AND (hrm_employee_pms.Appraiser_EmployeeID!='' OR hrm_employee_pms.Appraiser_EmployeeID!=0) AND hrm_employee.CompanyId=".$CompanyId." AND hrm_employee_general.DepartmentId=".$_REQUEST['value'], $con); }
...
<tr>
 <td>
   <table border="1" width="700">
     <tr>
	  <td colspan="13" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Georgia; font-weight:bold;">&nbsp;Allow Increment PMS Without Score Rating : </td>
	 </tr>
<tr bgcolor="#7a6189">
    <td align="center" style="color:#FFFFFF;" class="All_40"><b>SNo.</b></td>
    <td align="center" style="color:#FFFFFF;" class="All_60"><b>EmpCode</b></td>
	<td align="center" style="color:#FFFFFF;" class="All_150"><b>Department</b></td>
    <td align="center" style="color:#FFFFFF;" class="All_250"><b>Name</b></td>
	<td align="center" style="color:#FFFFFF;" class="All_80"><b>Check</b></td>
 </tr>
<?php if($YearId==1){$Y=2012;}elseif($YearId==2){$Y=2013;}elseif($YearId==3){$Y=2014;}elseif($YearId==4){$Y=2015;}elseif($YearId==5){$Y=2016;}elseif($YearId==6){$Y=2017;}elseif($YearId==7){$Y=2018;}elseif($YearId==8){$Y=2019;}elseif($YearId==9){$Y=2020;}
$Sql=mysql_query("SELECT hrm_employee.*, DepartmentId from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_employee.CompanyId=".$CompanyId." AND hrm_employee.EmpStatus='A' AND hrm_employee_general.DateJoining<='".$Y."-03-31' AND hrm_employee_general.DepartmentId=".$_REQUEST['value']." ORDER BY hrm_employee.EmpCode ASC", $con); 
	  $no=1; while($Res=mysql_fetch_array($Sql)) { $Ename=$Res['Fname'].' '.$Res['Sname'].' '.$Res['Lname'];  
	 $sqlCh=mysql_query("select * from hrm_pms_allow_inc where EmployeeID=".$Res['EmployeeID']." AND CompanyId=".$CompanyId." AND AssesmentYear=".$YearId, $con); 
...
<?php $sqlDept=mysql_query("select DepartmentCode from hrm_department where DepartmentId=".$Res['DepartmentId'], $con);  $resDept=mysql_fetch_assoc($sqlDept); ?> 
	<td align="" style="" class="All_150">&nbsp;<?php echo $resDept['DepartmentCode']; ?></td>
    <td align="" style="" class="All_250">&nbsp;<?php echo $Ename ?></td>
	<td style="width:80px; background-color:#9FCFFF;" align="center"><input type="checkbox" style="height:10px;" name="CheckAllowIncPMS_<?php echo $no; ?>" id="CheckAllowIncPMS_<?php echo $no; ?>" <?php if($RowCh>0){echo 'checked';}?> onClick="FunIncClick(<?php echo $Res['EmployeeID'].','.$CompanyId.','.$YearId.','.$no; ?>)" />
	 </td>
 </tr>
<?php $no++;} ?> 
   </table>
