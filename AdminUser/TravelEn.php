  ...
  <form  name="formDept" method="post" onSubmit="return validate(this)">
   <table border="0" width="770">
	...
	<tr>
      ...
      <?php 
if($CompanyId==1){$sqlTravelEn=mysql_query("select * from hrm_travelentitle where CompanyId=".$CompanyId." AND CreatedDate>='2014-01-21' order by GradeId DESC", $con); }
else{$sqlTravelEn=mysql_query("select * from hrm_travelentitle where CompanyId=".$CompanyId." order by GradeId DESC", $con);}
      ...
      if(isset($_REQUEST['action']) && $_REQUEST['action']=="edit" && $_REQUEST['eid']==$resTravelEn['TravelEntitleId']){ ?>
          ...
          <tr>
		   ...
		   <option value="AC">AC</option><option value="AC-I">AC-I</option><option value="AC-II">AC-II</option> 
