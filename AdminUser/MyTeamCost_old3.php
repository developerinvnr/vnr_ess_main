   ...
   <tr>
    ...
    <?php if($_REQUEST['action']=='Over') { $Action='ORGANIZATION'; }
     ...
     <tr bgcolor="#7a6189" style="height:24px;">
      ...
      <td class="th" style="width:6%;">Proposed<br><?php if($_REQUEST['YI']>=8){echo 'CTC';}?></td>
...
// $IncD=($Pre_GrossValue*$res2I['PerInc'])/100;   
 $IncD=($Pre_HodCtc*$res2I['PerInc'])/100;
 $GrossD=$Pre_HodCtc+$IncD;
 $OnePerD=($Pre_HodCtc*1)/100;
...
?>	
      ...
      <td class="tdc"><input class="inpr" style="color:#0080FF;font-weight:bold;" id="PIS_<?php echo $sn; ?>" name="PIS_<?php echo $sn; ?>" value="<?php echo floatval($HTotCtc); ?>" <?php if($_REQUEST['YI']==$YearId){ ?> onKeyDown="Cal_OneProInc(<?php echo $sn; ?>)" onClick="Cal_OneProInc(<?php echo $sn; ?>)" onChange="Cal_OneProInc(<?php echo $sn; ?>)" <?php } ?> disabled/></td>   
...
<?php $sql4I=mysql_query("select TIncGross,TPerInc,TInc from hrm_pms_dummy_increment_untot where YearId=".$_REQUEST['YI']." AND ".$ExtQuery." AND CompanyId=".$CompanyId, $con); $res4I=mysql_fetch_array($sql4I);
	  $sqlD=mysql_query("select SUM(EmpCurrGrossPM)as DeptTotGross, SUM(EmpCurrIncentivePM) as DeptInct, SUM(EmpCurrCtc) as HotDeptCtc from hrm_employee_pms p INNER JOIN hrm_employee_general g ON p.EmployeeID=g.EmployeeID INNER JOIN hrm_employee e ON p.EmployeeID=e.EmployeeID where e.EmpStatus='A' AND p.AssessmentYear=".$_REQUEST['YI']." AND p.CompanyId=".$CompanyId." ".$query." ".$query_y." AND (p.Dummy_EmpRating!=0 AND p.Dummy_AppRating!=0 AND p.Dummy_RevRating!=0 AND p.Dummy_HodRating!=0) AND p.Appraiser_EmployeeID!=0", $con); $resD=mysql_fetch_assoc($sqlD);
	   ...
	   {
		$IncD=($Pre_Ctc*$res4I['TPerInc'])/100;
		$GrossD=$Pre_Ctc+$IncD;
		$OnePerD=($Pre_Ctc*1)/100;
		if($OnePerD>0){$PerD=$IncD/$OnePerD;}else{$PerD=0;}
...
?>			   	
	  ...
	  <td class="tdc"><input class="inpr" style="font-weight:bold;background-color:#0076EC;color:#FFFFFF;" id="IncST" name="IncST" value="<?php echo $H444I; ?>aaa" readonly/></td>
