...
$SqlEar=mysql_query("select SUM(Tot_Gross)as Gross, SUM(VarRemburmnt)as VaReb, SUM(Bonus)as Bonus, SUM(DA) as DA, SUM(LeaveEncash) as EnCash, SUM(Arreares) as Arr, SUM(Incentive) as Inc, SUM(VariableAdjustment) as Vd, SUM(PerformancePay) as PP, SUM(CCA) as CCA, SUM(RA) as RA, SUM(Arr_Basic) as ArrBas, SUM(Arr_Hra) as ArrHra, SUM(Arr_Spl) as ArrSpl, SUM(Arr_Conv) as ArrConv, SUM(YCea) as YCea, SUM(YMr) as YMr, SUM(YLta) as YLta, SUM(Tot_Deduct)as Ded, SUM(TDS) as Tds, SUM(ESCI_Employee) as Esic, SUM(Arr_Pf) as ArrPf, SUM(Arr_Esic) as ArrEsic, SUM(VolContrib) as VolCont, SUM(DeductAdjmt) as DedAj, SUM(RecConAllow) as RecConAllow, SUM(Car_Allowance) as CarAllow, SUM(Bonus_Month)as Bonus_Month, SUM(Car_Allowance_Arr)as Car_Allowance_Arr, SUM(Arr_LvEnCash)as Arr_LvEnCash, SUM(Arr_Bonus)as Arr_Bonus, SUM(Arr_LTARemb)as Arr_LTARemb, SUM(Arr_RA)as Arr_RA, SUM(Arr_PP)as Arr_PP,SUM(Bonus_Adjustment)as Bonus_Adjustment,SUM(PP_Inc)as PP_Inc from hrm_employee_monthlypayslip where ((Year='".$FromY."' AND (Month=4 OR Month=5 OR Month=6 OR Month=7 OR Month=8 OR Month=9 OR Month=10 OR Month=11 OR Month=12)) OR (Year='".$ToY."' AND (Month=1 OR Month=2 OR Month=3))) AND EmployeeID=".$res['EmployeeID']); 
$ResEar=mysql_fetch_array($SqlEar);
$TotG=$ResEar['Gross']+$ResEar['VaReb']+$ResEar['Bonus']+$ResEar['DA']+$ResEar['Arr']+$ResEar['EnCash']+$ResEar['Inc']+$ResEar['Vd']+$ResEar['pp']+$ResEar['CCA']+$ResEar['RA']+$ResEar['ArrBas']+$ResEar['ArrHra']+$ResEar['ArrSpl']+$ResEar['ArrConv']+$ResEar['YCea']+$ResEar['YMr']+$ResEar['YLta']+$ResEar['Bonus_Month']+$ResEar['Car_Allowance_Arr']+$ResEar['Arr_LvEnCash']+$ResEar['Arr_Bonus']+$ResEar['Arr_LTARemb']+$ResEar['Arr_RA']+$ResEar['Arr_PP']+$ResEar['Bonus_Adjustment']+$ResEar['PP_Inc'];
