...
if($_POST['Action']=='Add') 
{
//$sqlc=mysql_query("select ConferenceId from hrm_company_conference_participant where ConferenceId=".$_POST['ID']." AND EmployeeID=".$_POST['EI'], $con);
//$row=mysql_num_rows($sqlc); if($row==0){
$sql = mysql_query("insert into hrm_company_conference_participant(ConferenceId,EmployeeID) values(".$_POST['ID'].", ".$_POST['EI'].")", $con) or die(mysql_error()); //} 
}
...
//$sqlc=mysql_query("select ConferenceId from hrm_company_conference_participant where ConferenceId=".$_POST['ID']." AND EmployeeID=".$_POST['EI'], $con);
//$row=mysql_num_rows($sqlc); if($row>0){
$sql = mysql_query("delete from hrm_company_conference_participant where EmployeeID=".$_POST['EI']." AND ConferenceId=".$_POST['ID'], $con) or die(mysql_error()); //} 
}

$sqlPP=mysql_query("select ConferenceId from hrm_company_conference_participant where ConferenceId=".$_POST['ID'], $con);
...
<?php $sqlP=mysql_query("select EmpCode,Fname,Sname,Lname,DepartmentCode,DR,Married,Gender from hrm_company_conference_participant p INNER JOIN hrm_employee_general g ON p.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_personal pp ON p.EmployeeID=pp.EmployeeID INNER JOIN hrm_employee e ON p.EmployeeID=e.EmployeeID INNER JOIN hrm_department d ON g.DepartmentId=d.DepartmentId where p.ConferenceId=".$_POST['ID'], $con); 
...
//$sql=mysql_query("select ConferenceId from hrm_company_conference_participant where ConferenceId=".$_POST['ID']." AND OtherName='".$_POST['Val']."' ", $con);
//$row=mysql_num_rows($sql); if($row==0){
$sql = mysql_query("insert into hrm_company_conference_participant(ConferenceId,OtherName) values(".$_POST['ID'].", '".$_POST['Val']."')", $con); //} 
$sqlPP=mysql_query("select ConferenceId from hrm_company_conference_participant where ConferenceId=".$_POST['ID'], $con);
$rowPP=mysql_num_rows($sqlPP); echo '<input type="hidden" id="TotEmpParti" value="'.$rowPP.'" />'; 
?>
...
<?php $sqlP2=mysql_query("select * from hrm_company_conference_participant where EmployeeID='' AND OtherName!='' AND ConferenceId=".$_POST['ID']." order by ConfEmpId ASC", $con); 
$sn2=1; while($resP2=mysql_fetch_array($sqlP2)){  ?>			 
		  <tr bgcolor="#FFFFFF">
		   ...
		   <td style="width:50px;" align="center"><img src="images/delete.png" border="0" onClick="OthDel(<?php echo $resP2['ConfEmpId'].', '.$_POST['ID']; ?>)"/></td>
		  </tr>
<?php $sn2++;} ?>	
          <tr bgcolor="#FFFFFF">
		    <td style="width:50px;font-family:Times New Roman;font-size:12px;" align="center"><?php echo $sn2; ?></td>
...
$sql=mysql_query("delete from hrm_company_conference_participant where ConfEmpId=".$_POST['v']." AND ConferenceId=".$_POST['ID'], $con);
$sqlPP=mysql_query("select ConferenceId from hrm_company_conference_participant where ConferenceId=".$_POST['ID'], $con);
$rowPP=mysql_num_rows($sqlPP); echo '<input type="hidden" id="TotEmpParti" value="'.$rowPP.'" />'; 
?>
<table style="width:600px;" border="1" cellpadding="0" cellspacing="0">
