...
////////////////<<<<<<<<<<<<<<<<<<<////////////
	...
	{  
	  ...
	  {  
	   ...
	   {   
	    ...
	    //echo "insert into hrm_employee_attendance(EmployeeID, AttValue, AttDate, Year, YearId) values(".$resE['EmployeeID'].",'".$c6."', '".$i."', '".$_POST['Year']."', ".$YearId.")";
          ...
          $sqlm=mysql_query("Select MorEveRepId from hrm_employee_moreve_report_".$_POST['Year']." WHERE EmployeeID=".$resE['EmployeeID']." AND MorEveDate='".$i."'",$con); $rowsm = mysql_num_rows($sqlm);  
	  if($rowsm>0)
	  { $resm=mysql_fetch_assoc($sqlm); if($resm['Att']!='CL' AND $resm['Att']!='CH' AND $resm['Att']!='SL' AND $resm['Att']!='SH' AND $resm['Att']!='PL' AND $resm['Att']!='PH' AND $resm['Att']!='EL' AND $resm['Att']!='FL' AND $resm['Att']!='TL'){ $sqlUp = mysql_query("UPDATE hrm_employee_moreve_report_".$_POST['Year']." SET MorDateTime='".$c1."', MorReports='".$MrRep."', Att='".$c6."', UpAtt='Y' WHERE EmployeeID=".$resE['EmployeeID']." AND MorEveDate='".$i."'", $con); }
	  } 
	  ...
	  { 
	    $sqlUp = mysql_query("insert into hrm_employee_moreve_report_".$_POST['Year']."(EmployeeID, MorEveDate, MorDateTime, MorReports, Att, UpAtt) values(".$resE['EmployeeID'].", '".$i."', '".$c1."', '".$MrRep."', '".$c6."', 'Y')", $con); 
	  }  
...
/**************************** Check Leave Query C ******************************/	
   ...
   elseif(($c6=='' OR $c6=='00:00' OR $c6=='00:00:00') AND $c7!='' AND $c7!='00:00' AND $c7!='00:00:00')
   {
   ...
   elseif($c6!='' AND $c6!='00:00' AND $c6!='00:00:00' AND ($c7=='' OR $c7=='00:00' OR $c7=='00:00:00'))
   {
...
$sqlDel=mysql_query("delete from hrm_employee_moreve_report_".$_REQUEST['Y6']." where MorEveDate>='".$_REQUEST['Y6']."-".$_REQUEST['m6']."-01' AND MorEveDate<='".$_REQUEST['Y6']."-".$_REQUEST['m6']."-31'", $con); if($sqlDel){ $MsgEntryDel='Mor-Eve reports of month '.$Month6.' deleted successfully'; }
}
