...
{ 
 ...
 //FF_EmpVnrExp
   ...
   $sqlUp=mysql_query("update hrm_employee_separation set HR_RelievingDate='".date("Y-m-d",strtotime($_POST['HR_RelievingDate']))."', FullFinalDate='".date("Y-m-d",strtotime($_POST['FullFinalDate']))."', ChequeNo='".$_POST['ChequeNo']."', MICRNo='".$_POST['MICRNo']."', NoticeBankName='".$_POST['NoticeBankName']."', NoticeAmount='".$_POST['NoticeAmount']."', NoticeRemark='".addslashes($_POST['NoticeRemark'])."', Note='".addslashes($_POST['Note'])."', TotalEarn='".$_POST['TotalEarn']."', TotalDeduct='".$_POST['TotalDeduct']."', TotalPayable='".$_POST['TotalPayable']."', HR_Fvalue='".$_POST['Fvalue']."', HR_Finaly_Other_Remark='".addslashes($_POST['OtheFinRemark'])."', FF_EmpVnrExp=".$_POST['FF_EmpVnrExp']." where EmpSepId=".$_POST['SId'], $con);
...
{ 
 ...
 { $search =  '!"#$%\'' ; $search = str_split($search); $FRimark=str_replace($search, "", $_POST['OtheFinRemark']); } else{$FRimark='';}
   ...
   $sqlUp=mysql_query("update hrm_employee_separation set HR_RelievingDate='".date("Y-m-d",strtotime($_POST['HR_RelievingDate']))."', FullFinalDate='".date("Y-m-d",strtotime($_POST['FullFinalDate']))."', ChequeNo='".$_POST['ChequeNo']."', MICRNo='".$_POST['MICRNo']."', NoticeBankName='".$_POST['NoticeBankName']."', NoticeAmount='".$_POST['NoticeAmount']."', NoticeRemark='".addslashes($_POST['NoticeRemark'])."', Note='".addslashes($_POST['Note'])."', TotalEarn='".$_POST['TotalEarn']."', TotalDeduct='".$_POST['TotalDeduct']."', TotalPayable='".$_POST['TotalPayable']."', HR_Fvalue='".$_POST['Fvalue']."', HR_Finaly_Other_Remark='".addslashes($_POST['OtheFinRemark'])."', HR_FullFinal_Submit='Y', FF_EmpVnrExp=".$_POST['FF_EmpVnrExp']." where EmpSepId=".$_POST['SId'], $con);
 if($sqlUp)
 {
	  ...
	  <td>
	   <table border="1">
		...
		<tr bgcolor="#FFFFFF">
		  ...
		  <td class="Text" style="width:90px;" align="">&nbsp;<?php if($resSE['TerMination']=='Y'){ echo 'Termination';}else{echo 'Relieving'; } ?> : </td><td class="Text" style="width:140px;">&nbsp;<input name="HR_RelievingDate" id="HR_RelievingDate" maxlength="25" style="font-family:Georgia; font-size:12px;width:80px; height:20px; border-style:hidden;" value="<?php if($resSE['HR_RelievingDate3']!='0000-00-00' AND $resSE['HR_RelievingDate3']!='1970-01-01'){echo date("d-m-Y",strtotime($resSE['HR_RelievingDate3']));}elseif($resSE['HR_RelievingDate2']!='0000-00-00' AND $resSE['HR_RelievingDate2']!='1970-01-01'){echo date("d-m-Y",strtotime($resSE['HR_RelievingDate2']));}elseif($resSE['HR_RelievingDate']!='0000-00-00' AND $resSE['HR_RelievingDate']!='1970-01-01'){echo date("d-m-Y",strtotime($resSE['HR_RelievingDate']));} ?>" readonly><button id="f_btn2" class="CalenderButton" disabled></button>
   <script type="text/javascript">  var cal = Calendar.setup({ onSelect:  function(cal) { cal.hide()}, showTime: true }); 
...
$TotalDeduct=$resHr['TotDeduct']+$resAcc['TotAmt2']+$resIt['TotItAmt']+$TotRepAmt+$resAdmin['TotAdminAmt']; //echo $resHr['TotDeduct']."+".$resAcc['TotAmt2']."+".$resIt['TotItAmt']."+".$TotRepAmt."+".$resAdmin['TotAdminAmt']; ?>          		     		   
		   <tr bgcolor="#8F8F8F">
		    <td style="width:320px;" class="Text" align="">&nbsp;<b>(B) Total Deductions</b></td>
