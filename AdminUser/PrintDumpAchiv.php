...
<tr>
 <td>
   ...
   <tr>
	  <td colspan="8" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Georgia; font-weight:bold;">&nbsp;Employee Achivement:
	 ...
	 <tr bgcolor="#7a6189">
	   ...
	   <td style="color:#ffffff; font-family:Georgia; font-size:12px; width:800px;" valign="top" align="center"><b>Achivement</b></td>
	</tr>
<?php if($_REQUEST['value']=='All') {$sqlAc = mysql_query("SELECT * from hrm_employee_pms_achivement INNER JOIN hrm_employee_pms ON hrm_employee_pms_achivement.EmpPmsId=hrm_employee_pms.EmpPmsId INNER JOIN hrm_employee_general ON hrm_employee_pms.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_employee_pms.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$YearId." AND hrm_employee_pms_achivement.Achivement!='' order by AchivementId ASC", $con) or die(mysql_error()); } else {$sqlAc = mysql_query("SELECT * from hrm_employee_pms_achivement INNER JOIN hrm_employee_pms ON hrm_employee_pms_achivement.EmpPmsId=hrm_employee_pms.EmpPmsId INNER JOIN hrm_employee_general ON hrm_employee_pms.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_employee_pms.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$YearId." AND hrm_employee_pms_achivement.Achivement!='' AND hrm_employee_pms.HR_Curr_DepartmentId=".$_REQUEST['value']." order by AchivementId ASC", $con) or die(mysql_error()); } $Sno=1;  while($resAc = mysql_fetch_assoc($sqlAc)) { ?> 
	    <tr bgcolor="#FFFFFF">
