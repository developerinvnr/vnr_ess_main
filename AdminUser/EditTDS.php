...
{ $PayTable='hrm_employee_monthlypayslip_'.$_POST['Y']; }
 ...
 if($rowMP>0){ 
 $UpSal=mysql_query("update ".$PayTable." set TDS='".$_POST['YTds']."' where EmployeeID=".$_POST['E']." AND Month=".$_POST['M']." AND Year=".$_POST['Y']."", $con);}
 else{$UpSal=mysql_query("insert into ".$PayTable."(EmployeeID, Month, Year, TDS) values(".$_POST['E'].", ".$_POST['M'].", ".$_POST['Y'].", '".$_POST['YTds']."')", $con);}
...
<html>
<head>
<title><?php include_once('../Title.php'); ?>&nbsp;TDS</title>
...
{document.getElementById("Save").style.display='block'; document.getElementById("Change").style.display='none'; 
 document.getElementById("YTds").readOnly=false; document.getElementById("YTds").style.background='#FFFFFF'; 
...
<tr>
 ...
 <tr>
   ...
   <td align="center" class="Des" style="font-family:Times New Roman;font-size:14px;width:150px;background-color:#7a6189;color:#FFFFFF;"><b>TDS</b></td>
 </tr>
<?php if($_REQUEST['M']==1){$SelM='January';} elseif($_REQUEST['M']==2){$SelM='February';} elseif($_REQUEST['M']==3){$SelM='March';} 
 ...
 <tr id="TRAmt" style="background-color:#DDDDDD;">
   ...
   <td align="center" class="Amt"><input style="text-align:right;font-family:Times New Roman;font-size:12px;width:150px;border-style:hidden;background-color:#DDDDDD;" id="YTds" name="YTds" value="<?php echo intval($res['TDS']); ?>" readonly maxlength="8"/></td>
 ...
 <tr bgcolor="#7a6189">
   <td align="right" colspan="6">
    <table>
	 ...
	 <td align="right" style="width:90px;"><input type="button" name="Refresh" id="Refresh" style="width:90px;" value="refresh" onClick="javascript:window.location='EditTDS.php?E=<?php echo $_REQUEST['E']; ?>&C=<?php echo $_REQUEST['C']; ?>&YI=<?php echo $_REQUEST['YI']; ?>&U=<?php echo $_REQUEST['U']; ?>&M=<?php echo $_REQUEST['M']; ?>&Y=<?php echo $_REQUEST['Y']; ?>'"/> </td>				   
	</tr>
	</table>
	</td>
	</tr>
