...
$SetPeD=$rSet['Prod_EffectedDate']; $Ass2Year=$rSet['LettPeriod']; $chke=1;

$sqlGap=mysql_query("select Gap1,Gap2 from hrm_pms_setting where Process='PMS' AND CompanyId=".$_REQUEST['C'], $con); 
$resGap=mysql_fetch_assoc($sqlGap);

$SqlY=mysql_query("select FromDate,ToDate from hrm_year where YearId=".$rSet['CurrY'], $con); $ResY=mysql_fetch_assoc($SqlY);
$From=date("Y",strtotime($ResY['FromDate'])); $To=date("Y",strtotime($ResY['ToDate'])); 
$ToTo=$From-1; $AssYear=$ToTo.'-'.date("y",strtotime($ResY['FromDate'])); $FD1=$From-1; $FD2=$From; 
...
<?php if($_REQUEST['action']=='All') { $SqlE=mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, Gender, DR, Married, DepartmentId, HqId, DateJoining, RepEmployeeID, EmpVertical FROM hrm_employee e INNER JOIN hrm_employee_personal p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID WHERE e.EmployeeID=".$ENew." AND e.CompanyId=".$_REQUEST['C'], $con); $ResE=mysql_fetch_assoc($SqlE); 
...
$SqlR=mysql_query("SELECT EmpCode, Fname, Sname, Lname, Gender, DR, Married FROM hrm_employee e INNER JOIN hrm_employee_personal p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID WHERE e.EmployeeID=".$ResE['RepEmployeeID']." AND e.CompanyId=".$_REQUEST['C'], $con); $ResR=mysql_fetch_assoc($SqlR); 
if($ResR['DR']=='Y'){$Mr='Dr.';} elseif($ResR['Gender']=='M'){$Mr='Mr.';} elseif($ResR['Gender']=='F'){$Mr='Ms.';}  
$NameR=$Mr.' '.$ResR['Fname'].'&nbsp;'.$ResR['Sname'].'&nbsp;'.$ResR['Lname'];
...
$sqlD=mysql_query("select DesigName from hrm_designation where DesigId=".$DNew, $con); 
$sqlG=mysql_query("select GradeValue from hrm_grade where GradeValue='".$GNew."' AND CompanyId=".$_REQUEST['C'], $con); 
$resDept=mysql_fetch_assoc($sqlDept); $resD=mysql_fetch_assoc($sqlD); $resG=mysql_fetch_assoc($sqlG);
...
$sqlPer=mysql_query("select IncDistriFrom from hrm_pms_increment_dis where Rating=".$RNew." AND YearId=".$YNew." AND CompanyId=".$_REQUEST['C'], $con); $resPer=mysql_fetch_assoc($sqlPer);  
...
$ss02= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_disbuc WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND DeptId=".$ResPMS['HR_Curr_DepartmentId']." AND BUCId=".$ResE['EmpVertical']." AND Hod2Id=0 AND RevId=".$ResPMS['Reviewer_EmployeeID']." AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con); $rr02=mysql_num_rows($ss02);
if($rr02>0){ $resMaxMin = mysql_fetch_assoc($ss02); }
else
{ 
/* ppppppppppppppp */
$ss01= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_disbuc WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND DeptId=".$ResPMS['HR_Curr_DepartmentId']." AND BUCId=".$ResE['EmpVertical']." AND Hod2Id=".$ResPMS['Rev2_EmployeeID']." AND RevId=0 AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con); $rr01=mysql_num_rows($ss01);
if($rr01>0){ $resMaxMin = mysql_fetch_assoc($ss01); }
else
{ 
/* oooooooooooooooo */
$ss0= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_disbuc WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND DeptId=".$ResPMS['HR_Curr_DepartmentId']." AND BUCId=".$ResE['EmpVertical']." AND Hod2Id=0 AND RevId=0 AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con); $rr0=mysql_num_rows($ss0);
if($rr0>0){ $resMaxMin = mysql_fetch_assoc($ss0); }
else
{ 
 $ss= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_disrev WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND DeptId=".$ResPMS['HR_Curr_DepartmentId']." AND (RevId=".$ResPMS['Appraiser_EmployeeID']." OR RevId=".$ResPMS['Reviewer_EmployeeID'].") AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con); $rr=mysql_num_rows($ss);
 if($rr>0){ $resMaxMin = mysql_fetch_assoc($ss); }
 else
 {
  $ss1= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_dishod2 WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND DeptId=".$ResPMS['HR_Curr_DepartmentId']." AND Hod2Id=".$ResPMS['Rev2_EmployeeID']." AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con); $rr1=mysql_num_rows($ss1);
  if($rr1>0){ $resMaxMin = mysql_fetch_assoc($ss1); }
  else
  {
   $ss2= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_disdept WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND DeptId=".$ResPMS['HR_Curr_DepartmentId']." AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con); 
   $rr2=mysql_num_rows($ss2);
   if($rr2>0){ $resMaxMin = mysql_fetch_assoc($ss2); }
   else
   {
    $ss3= mysql_query("SELECT IncDistriFrom,IncDistriTo FROM hrm_pms_increment_dishod WHERE Rating=".$RNew." AND HodId=".$ResPMS['HOD_EmployeeID']." AND YearId=".$YNew." AND IncDistriFrom>0 AND CompanyId=".$_REQUEST['C'], $con);
