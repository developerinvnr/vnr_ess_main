...
{  $SqlInseart = mysql_query("INSERT INTO hrm_company_conference_particular(Particular,CompanyId,CreatedBy,CreatedDate,YearId) VALUES('".$_POST['Particular']."','".$CompanyId."', '".$UserId."','".date('Y-m-d')."','".$YearId."')", $con); 
   if($SqlInseart){ $msg = "Data has been Inserted successfully..."; }
}
if(isset($_POST['SaveEdit']))
{  $SqlUpdate = mysql_query("UPDATE hrm_company_conference_particular SET Particular='".$_POST['Particular']."', CreatedBy='".$UserId."', CreatedDate='".date('Y-m-d')."', YearId='".$YearId."' WHERE ParticularId=".$_POST['ParticularId'], $con);
...
if (agree) { var x = "NewConfParticular.php?action=edit&eid="+value;  window.location=x;}}
function del(value) { var agree=confirm("Are you sure you want to delete this record?");
if (agree) { var x = "NewConfParticular.php?action=delete&did="+value;  window.location=x;}}
function newsave() { var x = "NewConfParticular.php?action=newsave";  window.location=x;}
</SCRIPT> 
</head>
<body class="body">
<table class="table">
...
<table border="0" style="margin-top:0px; width:100%; height:300px;">
 <tr>
  <td align="left" height="20" valign="top" colspan="3">
   <table border="0">
    <tr>
	  <td align="" width="250" class="heading">&nbsp;&nbsp;&nbsp;Conference Particular</td>
...
<?php $sqlP=mysql_query("select * from hrm_company_conference_particular where CompanyId=".$CompanyId, $con); $SNo=1; while($resP=mysql_fetch_array($sqlP)) {
...
<?php } $SNo++;} ?>
   ...
   <tr>
      <td align="Right" class="fontButton"><table border="0" width="350">
		...
		<input type="button" name="back" id="back" style="width:60px;" value="back" onClick="javascript:window.location='ComConference.php?Y=<?php echo date("Y"); ?>&page=1'"/>
		<input type="button" style="width:60px;" name="Refrash" value="refresh" onclick="javascript:window.location='NewConfParticular.php'"/>&nbsp;
			     </td>
		</tr></table>
      </td>
   </tr>
