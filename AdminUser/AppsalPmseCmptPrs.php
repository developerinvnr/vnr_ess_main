...
function ExportCmptPrs_pmsT(v,yi,ee)
{ var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;   
  window.open("ExportPmsCmptPrs_pmsT.php?action=CmptPrsIncExport&value="+v+"&c="+ComId+"&YI="+yi+"&ee="+ee,"ExportForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");}  
...
<tr>
 <td>
  ...
  <td colspan="14" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Complete PMS Process Details <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2.'-'.$PRD; ?>&nbsp;)&nbsp;&nbsp;&nbsp;<a href="#" onClick="ExportCmptPrs(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>')" style="color:#F9F900; font-size:12px;">Export Excel</a>
      ...
      &nbsp;&nbsp;&nbsp;<a href="#" onClick="ExportCmptPrs_pmsT(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>')" style="color:#F9F900; font-size:12px;">Export Excel Oth_Tbl</a>
	  ...
	  <td colspan="14" class="th">HOD</td>
	  <td colspan="14" class="th">HR</td>
	  <td colspan="17" class="th">SALARY</td>
	  <td colspan="16" class="th">ELIGIBILITY</td>
	 </tr>
	 <tr bgcolor="#7a6189">
		...
		<td class="th" style="width:60px;">Actual(%)</td>
		<td class="th" style="width:60px;">Prorata(%)</td>
		...
		<td class="th" style="width:150px;">VERTICAL</td>
		...
		<td class="th" style="width:60px;">CAR ALLOW</td>
		...
		<td class="th" style="width:60px;">ESIC</td>
		<td class="th" style="width:60px;">PF</td>
		<td class="th" style="width:60px;">NPS</td>
		...
		<td class="th" style="width:60px;">ESIC CONTRI<sup>n</sup></td>
		<td class="th" style="width:60px;">PF CONTRI<sup>n</sup></td>
		<?php if($_REQUEST['YI']<=7){?>
		<td class="th" style="width:60px;">BONUS</td>
		<?php } ?>
...
if($res['Hod_EmpGrade']!=$res['HR_CurrGradeId']){ $sqlGH=mysql_query("select GradeValue from hrm_grade where GradeId=".$res['Hod_EmpGrade']." AND CompanyId=".$CompanyId, $con); $resGH=mysql_fetch_assoc($sqlGH); $GradeH=$resGH['GradeValue']; }else{$GradeH='';} ?>	
		...
		<td class="tdc"><?php echo $res['Hod_ActualInc_Per']; ?></td>
		<td class="tdc"><?php echo $res['Hod_ProRataInc_Per']; ?></td>
...
if($res['HR_GradeId']!=$res['HR_CurrGradeId']){ $sqlGH=mysql_query("select GradeValue from hrm_grade where GradeId=".$res['HR_GradeId']." AND CompanyId=".$CompanyId, $con); $resGH=mysql_fetch_assoc($sqlGH); $GradeH=$resGH['GradeValue']; }else{$GradeH='';} ?>	
		...
		<td class="tdl"><?php echo $resHRDept['DepartmentCode']; ?></td>
		
		<?php $sqlVer=mysql_query("select VerticalName from hrm_department_vertical where VerticalId=".$res['EmpVertical']."", $con); $resVer=mysql_fetch_array($sqlVer); ?>
		<td class="tdl"><?php echo $resVer['VerticalName']; ?></td>
		...
		<td class="tdc"><?php if($res['Hod_ActualInc_Per']==$res['HR_ActualInc_Per']){echo $res['HR_ActualInc_Per'];}?></td>
		<td class="tdc"><?php if($res['Hod_ProRataInc_Per']==$res['HR_ProRataInc_Per']){echo $res['HR_ProRataInc_Per'];}?></td>
			
		<?php /* SALARY */ ?>
		<td class="tdr"><?php echo $ResC['BAS_Value']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResC['HRA_Value']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResC['CONV_Value']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResC['CAR_ALL_Value']; ?>&nbsp;</td>
		<td class="tdr"><?php echo $ResC['Bonus_Month']; ?>&nbsp;</td>
	    ...
	    <td class="tdr"><?php echo $ResC['ESCI']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResC['PF_Employee_Contri_Value']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResC['NPS_Value']; ?>&nbsp;</td>
	    ...
	    <td class="tdr"><?php echo $ResC['AnnualESCI']; ?>&nbsp;</td>
		...
		<?php /* Eligibility */ ?>
		<td class="tdr"><?php if($ResEl['Travel_TwoWeeKM']>0){echo $ResEl['Travel_TwoWeeKM'];}else{echo 'NA';}?>&nbsp;</td>	
	    <td class="tdr"><?php if($ResEl['Travel_FourWeeKM']>0){ echo $ResEl['Travel_FourWeeKM'];}else{echo 'NA';}?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResEl['DA_Inside_Hq']; ?></td>
	    <td class="tdr"><?php echo $ResEl['DA_Outside_Hq']; ?></td>
	    <td class="tdr"><?php echo $ResEl['Lodging_CategoryA']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResEl['Lodging_CategoryB']; ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResEl['Lodging_CategoryC']; ?>&nbsp;</td>
	    <td class="tdc"><?php if($ResEl['Mode_Travel_Outside_Hq']!=''){echo $ResEl['Mode_Travel_Outside_Hq'];}else{echo 'NA';} ?>&nbsp;</td>
	    <td class="tdc"><?php if($ResEl['TravelClass_Outside_Hq']!=''){echo $ResEl['TravelClass_Outside_Hq'];}else{echo 'NA';} ?>&nbsp;</td>
	    <td class="tdr"><?php if($ResEl['Mobile_Exp_Rem_Rs']>0){echo $ResEl['Mobile_Exp_Rem_Rs'];}else{echo 'NA';}?>&nbsp;</td>
	    <td class="tdr"><?php if($ResEl['Mobile_Hand_Elig_Rs']>0){echo $ResEl['Mobile_Hand_Elig_Rs'];} else {echo 'NA';} ?>&nbsp;</td>
	    <td class="tdr"><?php echo $ResEl['Health_Insurance']; ?>&nbsp;</td>
