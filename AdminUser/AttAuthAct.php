...
{ 
  ...
  elseif($_POST['Inchk']==0 AND $_POST['Outchk']==1 AND $_POST['chk']==0)
  {
   ...
   $InCnt='Y';
    ...
    elseif($_POST['chk']==1)
  {
   $Attv=mysql_query("select AttValue from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate='".$_POST['RDate']."'", $con); $rAttv=mysql_fetch_array($Attv);
   ...
   { 
       ...
       //if($_POST['Reason']=='WFH'){$chkAtt='WFH';}elseif($rAttv['AttValue']=='A' OR $rAttv['AttValue']=='OD'){$chkAtt='OD';}elseif($rAttv['AttValue']=='WFH'){$chkAtt='WFH';}else{$chkAtt='P';} 
   ...
   { if($rAttv['AttValue']!='P' AND $rAttv['AttValue']!='A' AND $rAttv['AttValue']!='' AND $rAttv['AttValue']!='OD' AND $rAttv['AttValue']!='WFH'){$chkAtt=$rAttv['AttValue'];}
	...
	$ssCH=mysql_query("select Count(AttValue) as aaCH from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate between '".date("Y-m-01",strtotime($_POST['RDate']))."' AND '".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttDate!='".$_POST['RDate']."' AND AttValue='CH'", $con); $rrCH=mysql_fetch_array($ssCH); $CountCH=$rrCH['aaCH']/2; $tCL=$rCL['aCL']+$rrCL['aaCL']+$CountCH;
	...
	$ssPH=mysql_query("select Count(AttValue) as aaPH from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate between '".date("Y-m-01",strtotime($_POST['RDate']))."' AND '".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttDate!='".$_POST['RDate']."' AND AttValue='PH'", $con); $rrPH=mysql_fetch_array($ssPH); $CountPH=$rrPH['aaPH']/2; $tPL=$rPL['aPL']+$rrPL['aaPL']+$CountPH;
	...
	$ssSH=mysql_query("select Count(AttValue) as aaSH from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate>='".date("Y-m-01",strtotime($_POST['RDate']))."' AND AttDate<='".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttValue='SH'", $con); $rrSH=mysql_fetch_array($ssSH);
 ...
 $Up=mysql_query("update hrm_employee_attendance_req set InStatus=".$_POST['MyActIn'].", OutStatus=".$_POST['MyActOut'].", SStatus=".$_POST['MyAct'].", H_Remark='".$Remark."', Status=1 where EmployeeID=".$_POST['ei']." AND AttDate='".$_POST['RDate']."'",$con);
...
/************************************/
 ...
 $sUp=mysql_query("UPDATE ".$AttTable." SET AttValue='".$Att."', InnCnt='".$InCnt."', OuttCnt='".$OutCnt."', Relax='".$Relax."', Allow='".$Allow."', Af15=".$Af15." where AttId=".$_POST['AttId']." AND EmployeeID=".$_POST['ei'], $con);
...
/************/
	...
	$ssCH=mysql_query("select Count(AttValue) as aaCH from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate between '".date("Y-m-01",strtotime($_POST['RDate']))."' AND '".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttDate!='".$i."' AND AttValue='CH'", $con); $rrCH=mysql_fetch_array($ssCH); $CountCH=$rrCH['aaCH']/2; $tCL=$rCL['aCL']+$rrCL['aaCL']+$CountCH;
	//Check Total PL Availed in month
	$sPL=mysql_query("select SUM(Apply_TotalDay) as aPL from ".$AppLeaveTable." where LeaveStatus!=4 AND LeaveStatus!=3 AND LeaveStatus!=2 AND EmployeeID=".$_POST['ei']." AND Apply_FromDate>='".date("Y-m-01",strtotime($_POST['RDate']))."' AND Apply_ToDate<='".date("Y-m-31",strtotime($_POST['RDate']))."' AND (Leave_Type='PL' OR Leave_Type='PH')", $con); 
	...
	$ssPH=mysql_query("select Count(AttValue) as aaPH from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate between '".date("Y-m-01",strtotime($_POST['RDate']))."' AND '".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttDate!='".$i."' AND AttValue='PH'", $con); $rrPH=mysql_fetch_array($ssPH); $CountPH=$rrPH['aaPH']/2; $tPL=$rPL['aPL']+$rrPL['aaPL']+$CountPH;
	 //Check Total SL Availed in month 
	  $sSL=mysql_query("select SUM(Apply_TotalDay) as aSL from ".$AppLeaveTable." where LeaveStatus!=4 AND LeaveStatus!=3 AND LeaveStatus!=2 AND EmployeeID=".$_POST['ei']." AND Apply_FromDate>='".date("Y-m-01",strtotime($_POST['RDate']))."' AND Apply_ToDate<='".date("Y-m-31",strtotime($_POST['RDate']))."' AND (Leave_Type='SL' OR Leave_Type='SH')", $con); $rSL=mysql_fetch_array($sSL);
	  $ssSL=mysql_query("select Count(AttValue) as aaSL from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate>='".date("Y-m-01",strtotime($_POST['RDate']))."' AND AttDate<='".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttValue='SL'", $con); $rrSL=mysql_fetch_array($ssSL);
	  $ssSH=mysql_query("select Count(AttValue) as aaSH from ".$AttTable." where EmployeeID=".$_POST['ei']." AND AttDate>='".date("Y-m-01",strtotime($_POST['RDate']))."' AND AttDate<='".date("Y-m-31",strtotime($_POST['RDate']))."' AND AttValue='SH'", $con); $rrSH=mysql_fetch_array($ssSH);
...
/**** Leave Update Open ****/
   $SqlCL=mysql_query("select count(AttValue)as CL from ".$AttTable." where AttValue='CL' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlCH=mysql_query("select count(AttValue)as CH from ".$AttTable." where AttValue='CH' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlSL=mysql_query("select count(AttValue)as SL from ".$AttTable." where AttValue='SL' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlSH=mysql_query("select count(AttValue)as SH from ".$AttTable." where AttValue='SH' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlPL=mysql_query("select count(AttValue)as PL from ".$AttTable." where AttValue='PL' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlPH=mysql_query("select count(AttValue)as PH from ".$AttTable." where AttValue='PH' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlEL=mysql_query("select count(AttValue)as EL from ".$AttTable." where AttValue='EL' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlFL=mysql_query("select count(AttValue)as FL from ".$AttTable." where AttValue='FL' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlTL=mysql_query("select count(AttValue)as TL from ".$AttTable." where AttValue='TL' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlHf=mysql_query("select count(AttValue)as Hf from ".$AttTable." where AttValue='HF' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlPresent=mysql_query("select count(AttValue)as Present from ".$AttTable." where AttValue='P' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlAbsent=mysql_query("select count(AttValue)as Absent from ".$AttTable." where AttValue='A' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlOnDuties=mysql_query("select count(AttValue)as OnDuties from ".$AttTable." where AttValue='OD' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlHoliday=mysql_query("select count(AttValue)as Holiday from ".$AttTable." where AttValue='HO' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con); $SqlELSun=mysql_query("select count(CheckSunday)as SUN from ".$AttTable." where CheckSunday='Y' AND EmployeeID=".$_POST['ei']." AND AttDate between '".$yy."-".$mm."-01' AND '".$yy."-".$mm."-31'", $con);  
...
/**** Leave update Close ****/
 ...
 {
  //$sUp=mysql_query("UPDATE ".$AttTable." SET AttValue='".$chkAtt."' where AttId=".$_POST['AttId']." AND EmployeeID=".$_POST['ei'], $con);
...
$YearV=date("Y", strtotime($rE['AttDate']));
$BY=date("Y")-1;
if($YearV>=date("Y")){ $AttTable='hrm_employee_attendance'; }
elseif($YearV==$BY AND date("m")=='01'){ $AttTable='hrm_employee_attendance'; }
elseif($YearV==$BY AND date("m")>1){ $AttTable='hrm_employee_attendance_'.$YearV; }
else{$AttTable='hrm_employee_attendance_'.$YearV; }
...
<?php $saE=mysql_query("SELECT * FROM ".$AttTable." WHERE EmployeeID=".$rE['EmployeeID']." AND AttDate='".$rE['AttDate']."' ", $con); $rowaE=mysql_num_rows($saE); $raE=mysql_fetch_assoc($saE); ?>
...
<tr>
<td class="font2" valign="top" bgcolor="#8CC6FF"><b>HR Action</b></td>
<td class="font2" valign="top">:</td>
...
<td class="font2"><input class="font2" type="button" value="Refresh" onClick="javascript:window.location='AttAuthAct.php?id=<?php echo $_REQUEST['id']; ?>'"/><input class="font2" type="submit" name="ReqSubmit" id="ReqSubmit" value="ReqSubmit"/></td>
</tr>
<?php } if($rE['Status']>0){ ?>
<tr>
<td colspan="2" class="font2" valign="top"></td>
