...
if($_POST['ResSubmit'])
{
  $sqlU=mysql_query("update hrm_employee_separation set Emp_ResignationDate='".date("Y-m-d",strtotime($_POST['ResigDate']))."', Emp_RelievingDate='".date("Y-m-d",strtotime($_POST['ERelDate']))."', Emp_Reason='".addslashes($_POST['ResReason'])."', NoticeDay=".$_POST['NoticeDay'].", Rep_RelievingDate='".date("Y-m-d",strtotime($_POST['RRelDate']))."', Rep_Remark='".addslashes($_POST['RRemark'])."', TypeOfExit='".$_POST['TypeOfExit']."', Hod_RelievingDate='".date("Y-m-d",strtotime($_POST['HRelDate']))."', Hod_Remark='".addslashes($_POST['HRemark'])."', HR_UserId=".$_POST['UId'].", HR_RelievingDate='".date("Y-m-d",strtotime($_POST['HrRelDate']))."', HR_Remark='".addslashes($_POST['HrRemark'])."' where EmpSepId=".$_POST['SepId'], $con);  
...
cal.manageFields("f_btn3", "RRelDate", "%d-%m-%Y");</script></td>
   ...
   <tr>
    <td colspan="2" style="font-family:Geneva, Arial, Helvetica, sans-serif;font-size:12px;width:80px;">&nbsp;<b>Type of Exit</b></td>
