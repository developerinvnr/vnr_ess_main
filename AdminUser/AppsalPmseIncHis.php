 ...
 <td>
  ...
  <td colspan="33" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Employee Increment History <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2; ?>&nbsp;)&nbsp;&nbsp;&nbsp;<a href="#" onClick="ExportIncHis(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>')" style="color:#F9F900; font-size:12px;">Export Excel</a>
	 ...
	 <tr bgcolor="#7a6189">
		...
		<?php /*?><td class="th" style="width:60px;">PI</td> <?php */?>
		...
		<td class="th" style="width:60px;">Old CTC</td>
		
		<td class="th" style="width:60px;">PI_CTC</td>
		<td class="th" style="width:60px;">PI_CTC<br>(%)</td>
		<td class="th" style="width:60px;">Corr_CTC</td>
		<td class="th" style="width:60px;">Corr_CTC<br>(%)</td>
		
		<td class="th" style="width:60px;">New CTC</td>
...
{  
  if($_REQUEST['value']==0)
  { $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h INNER JOIN hrm_employee e ON h.EmpCode=e.EmpCode where h.CompanyId=".$CompanyId." AND e.CompanyId=".$CompanyId." order by h.EmpCode ASC, h.SalaryChange_Date ASC", $con); }
    else{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h INNER JOIN hrm_employee e ON h.EmpCode=e.EmpCode INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where h.CompanyId=".$CompanyId." AND g.DepartmentId=".$_REQUEST['value']." AND p.AssessmentYear=".$_REQUEST['YI']." order by h.EmpCode ASC, h.SalaryChange_Date ASC", $con); }
}
elseif($_REQUEST['ee']=='App')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h INNER JOIN hrm_employee e ON h.EmpCode=e.EmpCode INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID where h.CompanyId=".$CompanyId." AND p.Appraiser_EmployeeID=".$_REQUEST['value']." AND p.AssessmentYear=".$_REQUEST['YI']." order by h.EmpCode ASC, h.SalaryChange_Date ASC", $con); }
elseif($_REQUEST['ee']=='Rev')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h INNER JOIN hrm_employee e ON h.EmpCode=e.EmpCode INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID where h.CompanyId=".$CompanyId." AND p.Reviewer_EmployeeID=".$_REQUEST['value']." AND p.AssessmentYear=".$_REQUEST['YI']." order by h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
}
elseif($_REQUEST['ee']=='Rev2')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h INNER JOIN hrm_employee e ON h.EmpCode=e.EmpCode INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID where h.CompanyId=".$CompanyId." AND p.Rev2_EmployeeID=".$_REQUEST['value']." AND p.AssessmentYear=".$_REQUEST['YI']." order by h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
}
elseif($_REQUEST['ee']=='Hod')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h INNER JOIN hrm_employee e ON h.EmpCode=e.EmpCode INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID where h.CompanyId=".$CompanyId." AND p.HOD_EmployeeID=".$_REQUEST['value']." AND p.AssessmentYear=".$_REQUEST['YI']." order by h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
}  
...
<?php $sqlE=mysql_query("select Fname, Sname, Lname from hrm_employee where EmpCode=".$res['EmpCode']." AND CompanyId=".$CompanyId, $con); $resE=mysql_fetch_assoc($sqlE); ?>			
		...
		<td class="tdr"><?php if($cpid==0 && $res['SalaryChange_Date']<'2020-01-01'){ echo floatval($res['Prop_PeInc_GSPM']); } ?></td>
		<td class="tdr"><?php echo floatval($res['PropSalary_Correction']);  ?></td>		
        <td class="tdr"><?php echo floatval($res['TotalProp_GSPM']);  ?></td>
		<td class="tdc">
		<?php if($cpid==0 && $res['SalaryChange_Date']<'2020-01-01'){ ?>
...
<?php //echo $res['TotalProp_PerInc_GSPM']; ?>
		...
		<td class="tdr"><?php if($cpid==0 && $res['SalaryChange_Date']<'2020-01-01'){ echo floatval($res['Incentive']); } ?></td>
		<td class="tdr"><?php if($cpid==0 && $res['SalaryChange_Date']<'2020-01-01'){ echo floatval($res['BonusAnnual_September']); } ?></td>
		
		<td class="tdr"><?php if($cpid==1){ echo floatval($rqryct['EmpCurrCtc']); }elseif($res['SalaryChange_Date']>='2020-01-01' OR $res['SalaryChange_Date']<'2020-01-01'){ echo floatval($rctc['Tot_CTC']); } ?></td>
		
		<td class="tdr"><?php if($cpid==1){ echo floatval($res['ProIncCTC']); } ?></td>
		<td class="tdc"><?php if($cpid==1){ echo $res['Percent_ProIncCTC']; } ?></td>
		<td class="tdr"><?php if($cpid==1){ echo floatval($res['ProCorrCTC']); } ?></td>
		<td class="tdc"><?php if($cpid==1){ echo $res['Percent_ProCorrCTC']; } ?></td>
		
		<td class="tdr"><?php if($cpid==1){ echo floatval($res['Proposed_ActualCTC']); }elseif($res['SalaryChange_Date']>='2020-01-01'){ echo floatval($rctc['Tot_CTC']); } ?></td>
		<td class="tdc"><?php if($cpid==1){ echo $res['Percent_IncNetCTC']; } ?></td>
...
<?php if($resPms['Hod_EmpDesignation']){ $sqlD2=mysql_query("select DesigName from hrm_designation where DesigId=".$resPms['Hod_EmpDesignation'], $con); $resD2=mysql_fetch_assoc($sqlD2); } ?>		
		...
		<td class="tdc"><?php //echo $resPms['Hod_Percent_ProIncSalary']; ?></td>
		<td class="tdr"><?php //echo $resPms['Hod_ProCorrSalary']; ?></td>		
        <td class="tdr"><?php //echo $resPms['Hod_GrossMonthlySalary']; ?></td>
		<td class="tdc"><?php //echo $resPms['Hod_Percent_IncNetMonthalySalary']; ?></td>
		<td class="tdr"><?php //echo $resPms['Incentive']; ?></td>
		<td class="tdr"><?php //echo ''; ?></td>
		<td class="tdr"><?php echo floatval($resPms['EmpCurrCtc']); ?></td>
		
		<td class="tdr"><?php echo floatval($resPms['HR_ProIncCTC']); ?></td>
		<td class="tdc"><?php echo $resPms['HR_Percent_ProIncCTC']; ?></td>
		<td class="tdr"><?php echo floatval($resPms['HR_ProCorrCTC']); ?></td>
		<td class="tdc"><?php echo $resPms['HR_Percent_ProCorrCTC']; ?></td>
		
		<td class="tdr"><?php echo floatval($resPms['HR_Proposed_ActualCTC']); ?></td>
		<td class="tdc"><?php echo $resPms['HR_Percent_IncNetCTC']; ?></td>
		<td class="tdc"><?php echo $resPms['Hod_TotalFinalScore']; ?></td>		
		<td class="tdc"><?php echo $resPms['Hod_TotalFinalRating']; ?></td>	
	</tr>
	<tr> 
	 <td colspan="20">&nbsp;</td>
	</tr>
	 ...
	 <tr><td colspan="35" style=" background-color:#CAC046;">&nbsp;</td></tr>
	<?php } ?>   	
