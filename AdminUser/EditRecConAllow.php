...
{ $PayTable='hrm_employee_monthlypayslip_'.$_POST['Y']; }
 ...
 if($rowMP>0){ 
 $UpSal=mysql_query("update ".$PayTable." set RecConAllow='".$_POST['YRecConAllow']."' where EmployeeID=".$_POST['E']." AND Month=".$_POST['M']." AND Year=".$_POST['Y']."", $con);}
 else{$UpSal=mysql_query("insert into ".$PayTable."(EmployeeID, Month, Year, RecConAllow) values(".$_POST['E'].", ".$_POST['M'].", ".$_POST['Y'].", '".$_POST['YRecConAllow']."')", $con);}
...
{document.getElementById("Save").style.display='block'; document.getElementById("Change").style.display='none'; 
 document.getElementById("YRecConAllow").readOnly=false; document.getElementById("YRecConAllow").style.background='#FFFFFF'; 
...
<tr>
 ...
 <tr>
   ...
   <td align="center" class="Des" style="font-family:Times New Roman;font-size:14px;width:150px;background-color:#7a6189;color:#FFFFFF;"><b>Recovery Convey<sup>n</sup> Allow</b></td>
 </tr>
<?php if($_REQUEST['M']==1){$SelM='January';} elseif($_REQUEST['M']==2){$SelM='February';} elseif($_REQUEST['M']==3){$SelM='March';} 
 ...
 <tr id="TRAmt" style="background-color:#DDDDDD;">
   ...
   <td align="center" class="Amt"><input style="text-align:right;font-family:Times New Roman;font-size:12px;width:150px;border-style:hidden;background-color:#DDDDDD;" id="YRecConAllow" name="YRecConAllow" value="<?php echo intval($res['RecConAllow']); ?>" readonly maxlength="8"/></td>
