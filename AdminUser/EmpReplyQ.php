...
{ $sqlUp=mysql_query("update hrm_employee_queryemp set HRQStatus=1 where QueryId=".$_REQUEST['qI'], $con);
...
{ var agree=confirm("Are you sure you want to close this query?"); 
  if (agree) { var x = "EmpReplyQ.php?qI="+QI+"&page="+P;  window.location=x; } 
}


function ExportQry(ci)
{ 
  window.open("EmpReplyQExp.php?action=ExpQ&ci="+ci,"ExportForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");}
...
<table border="0" style="margin-top:0px; width:100%; height:300px;">
 <tr>
  <td align="left" height="20" valign="top" colspan="3">
   <table border="0">
    <tr>
	  <td width="300" class="heading">&nbsp;Reply/ Answer Query</td>
	  <td><font style="font-family:Times New Roman;color:#004000;font-size:15px; font-weight:bold;"><a href="#" onClick="ExportQry(<?=$CompanyId?>)" style="cursor:pointer; font-size:12px;">Export Excel (With All Query)</a></font></td>
...
<tr>
 ...
 <td colspan="0" class="TableHead" align="center" valign="top" style="background-color:#4F3C6F;"><b>Action</b></td>
</tr>
<tr>
 ...
 <td width="80" class="TableHead" align="center"><b>Status</b></td> 
 <td width="80" class="TableHead" align="center"><b>Status</b></td> 
 <td width="60" class="TableHead" align="center"><b>&nbsp;</b></td>	   
</tr>
<?php $sql_statement = mysql_query("select * from hrm_employee_queryemp INNER JOIN hrm_employee ON hrm_employee.EmployeeID=hrm_employee_queryemp.EmployeeID where hrm_employee.CompanyId=".$CompanyId." AND hrm_employee_queryemp.HRQStatus=0 AND hrm_employee_queryemp.QStatus=2 order by QueryDT DESC", $con); 
$total_records = mysql_num_rows($sql_statement);
if(isset($_GET['page'])) $page = $_GET['page']; else $page = 1;  
$offset = 10;
if ($page){ $from = ($page * $offset) - $offset; }else{ $from = 0;}

$sql_statement = mysql_query("select * from hrm_employee_queryemp INNER JOIN hrm_employee ON hrm_employee.EmployeeID=hrm_employee_queryemp.EmployeeID where hrm_employee.CompanyId=".$CompanyId." AND hrm_employee_queryemp.HRQStatus=0 AND hrm_employee_queryemp.QStatus=2 order by QueryDT DESC LIMIT " . $from . "," . $offset, $con);
...
<?php $sqlD=mysql_query("select DepartmentCode from hrm_department where DepartmentId=".$resQ['QToDepartmentId'], $con); $resD=mysql_fetch_assoc($sqlD);?>	 
 ...
 <td width="80" class="TableHead1" align="center" valign="top" style="background-color:#C4FFC4;"><b>Reply</b></td>
...
<?PHP doPages($offset, 'EmpReplyQ.php', '', $total_records); ?>
</td>
</tr>			   
  </table>    
</td>
