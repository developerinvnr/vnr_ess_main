...
<?php $sql = mysql_query("SELECT * FROM ".$_REQUEST['tbl']." WHERE ApplyLeaveId=".$_REQUEST['id'], $con) or die(mysql_error()); $res = mysql_fetch_array($sql); 
...
$sqlSNextCL=mysql_query("select count(AttValue) as SNextCL from ".$AttTable." where AttValue='CL' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextCL=mysql_fetch_array($sqlSNextCL)){$SumNextCL=$resSNextCL['SNextCL'];}
$sqlSNextCH=mysql_query("select count(AttValue) as SNextCH from ".$AttTable." where AttValue='CH' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextCH=mysql_fetch_array($sqlSNextCH)){$SumNextCH=$resSNextCH['SNextCH']/2;}
$sqlSNextSL=mysql_query("select count(AttValue) as SNextSL from ".$AttTable." where AttValue='SL' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextSL=mysql_fetch_array($sqlSNextSL)){$SumNextSL=$resSNextSL['SNextSL'];}

$sqlSNextSH=mysql_query("select count(AttValue) as SNextSH from ".$AttTable." where AttValue='SH' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextSH=mysql_fetch_array($sqlSNextSH)){$SumNextSH=$resSNextSH['SNextSH']/2;}
$sqlSNextPL=mysql_query("select count(AttValue) as SNextPL from ".$AttTable." where AttValue='PL' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextPL=mysql_fetch_array($sqlSNextPL)){$SumNextPL=$resSNextPL['SNextPL'];}
$sqlSNextEL=mysql_query("select count(AttValue) as SNextEL from ".$AttTable." where AttValue='EL' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextEL=mysql_fetch_array($sqlSNextEL)){$SumNextEL=$resSNextEL['SNextEL'];}
$sqlSNextFL=mysql_query("select count(AttValue) as SNextFL from ".$AttTable." where AttValue='FL' AND EmployeeID=".$res['EmployeeID']." AND AttDate>='".$Sdate."' AND Year=".$Y, $con); while($resSNextFL=mysql_fetch_array($sqlSNextFL)){$SumNextFL=$resSNextFL['SNextFL'];}
