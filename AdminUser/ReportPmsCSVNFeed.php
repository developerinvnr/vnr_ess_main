...
if($_REQUEST['action']='ExportNFeed') 
{
...
if($_REQUEST['value']=='All') { $result = mysql_query("SELECT * from hrm_employee_pms_workenvironment INNER JOIN hrm_employee_pms ON hrm_employee_pms_workenvironment.EmpPmsId=hrm_employee_pms.EmpPmsId INNER JOIN hrm_employee_general ON hrm_employee_pms.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_employee_pms.CompanyId=".$_REQUEST['C']." AND hrm_employee_pms.EmployeeID=".$resE['EmployeeID']." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['Y']." AND hrm_employee_pms_workenvironment.Answer!='' order by WorkEnvironment ASC", $con) or die(mysql_error()); } else {$result = mysql_query("SELECT * from hrm_employee_pms_workenvironment INNER JOIN hrm_employee_pms ON hrm_employee_pms_workenvironment.EmpPmsId=hrm_employee_pms.EmpPmsId INNER JOIN hrm_employee_general ON hrm_employee_pms.EmployeeID=hrm_employee_general.EmployeeID WHERE hrm_employee_pms.CompanyId=".$_REQUEST['C']." AND hrm_employee_pms.EmployeeID=".$resE['EmployeeID']." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['Y']." AND hrm_employee_pms_workenvironment.Answer!='' AND hrm_employee_pms.HR_Curr_DepartmentId=".$_REQUEST['value']." order by WorkEnvironment ASC", $con) or die(mysql_error());} $Sno=1; while($res = mysql_fetch_array($result))
{
