...
<?php  $sqlD=mysql_query("select * from hrm_department where DeptStatus='A' AND CompanyId=".$CompanyId." order by DepartmentName ASC",$con); $Sno=1; while($resD=mysql_fetch_assoc($sqlD)){ 
       $sqlCh=mysql_query("select * from hrm_api_punch_department where DepartmentId=".$resD['DepartmentId']." AND Sts='Y'",$con); $rowCh=mysql_num_rows($sqlCh); ?>
 <div class="tbody">
 <tbody>
 <tr style="background-color:#FFFFFF;"> 
  <td class="tdc"><?php echo $Sno; ?></td>
  <td class="tdl">&nbsp;<?php echo ucwords(strtolower($resD['DepartmentName'])); ?></td>
  <td align="center" id="On3TDL_<?=$resD['DepartmentId']?>" style="background-color:<?php if($rowCh>0){echo '#69D200';  }?>;"><input type="checkbox"  id="UsePunch_<?=$resD['DepartmentId']?>" onClick="FunUsePunch(<?=$resD['DepartmentId']?>)" <?php if($rowCh>0){echo 'checked';} ?> /></td>
  <td align="center" id="On4TDL_<?=$resD['DepartmentId']?>"><input type="text" id="InTime_<?=$resD['DepartmentId']?>" value="<?=$resD['InTime']?>" style="width:100%; border:hidden;text-align:center;" onChange="FunInTime(<?=$resD['DepartmentId']?>)" onKeyPress="return isNumberKey(event)"/></td>
 </tr>
 </tbody>
 </div>
<?php $Sno++; } ?>
...
function FunInTime(did)
{
 var vv=document.getElementById('InTime_'+did).value;
