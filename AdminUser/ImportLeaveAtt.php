...
/*
if(isset($_POST['PCImportFile']))
{
 
  if(($handle = fopen($_FILES['PC_file']['tmp_name'], "r"))!== FALSE) 
  {
    $ctr=1; 
    while(($data = fgetcsv($handle, 1000, ","))!== FALSE) 
    { 
     $c1 = mysql_real_escape_string($data[1]); 
     $seq = mysql_real_escape_string($data[2]); 
     $vr = mysql_real_escape_string($data[3]);
     $pcode = mysql_real_escape_string($data[4]);
	
	 if($ctr>1 AND $c1>0 AND $seq!='' AND $vr!='' AND $pcode!='') 
	 { 
	  $sqlEI=mysql_query("select EmployeeID from hrm_employee where EmpCode=".$c1." AND CompanyId=1", $con); $resEI=mysql_fetch_assoc($sqlEI);
	  if($resEI['EmployeeID']!='')
      { 
	   
		  $sqlUp=mysql_query("update hrm_employee_general set PositionCode='".$pcode."', PosSeq='".$seq."', PosVR='".$vr."' where EmployeeID=".$resEI['EmployeeID'], $con);
      }
	 } 	
     else { $ctr++; }
    }
   ...
   if($sqlUp){ $msg='uploaded successfully.';}
  }   
...
/*
if(isset($_POST['ImportPolicyFile']))
{
 
 if ($_FILES['pcsv_file']['error']>0){ echo "Error:".$_FILES['pcsv_file']['error']."<br />"; }
 else
 {  
  if(($handle = fopen($_FILES['pcsv_file']['tmp_name'], "r"))!== FALSE) 
  {
    $ctr=1; 
    while(($data = fgetcsv($handle, 1000, ","))!== FALSE) 
    { 
     $ec = mysql_real_escape_string($data[0]); $pdate = mysql_real_escape_string($data[5]);
	 $ptype = mysql_real_escape_string($data[6]); $pkm = mysql_real_escape_string($data[10]);
	 if($ctr>1 AND $ec!='' AND $ec>0 AND $pdate!='' AND $pdate!='' AND $ptype!='' AND $pkm!='') 
	 { 
	  $sqlEI=mysql_query("select EmployeeID from hrm_employee where EmpCode=".$ec." AND CompanyId=".$CompanyId, $con); $resEI=mysql_fetch_assoc($sqlEI);
	  if($resEI['EmployeeID']!='')
      { 
		$sqlUp=mysql_query("update hrm_employee_general set VehiclePolicy_EntryDate='".date("Y-m-d",strtotime($pdate))."', VehiclePolicy_Type='".$ptype."', Running_TotalKM='".$pkm."', VehiclePolicy_UpSysDate='".date("Y-m-d")."' where EmployeeID=".$resEI['EmployeeID'], $con);
   ...
   if($sqlUp){ $msg2='Policy Records uploaded successfully...';}
  }
 }
...
<?php /*
<form name="FormImp" method="POST" enctype="multipart/form-data">
 Policy File: <input type="file" name="pcsv_file" id="pcsv_file"/>
 <input type="submit" name="ImportPolicyFile" id="FFile_Up" value="Upload"/>
 <i><?php echo '<br>'.$msg2; ?></i>
</form>
*/ ?>
...
<form name="FormImp" method="POST" enctype="multipart/form-data" onSubmit="return validate(this)">
 ...
 <tr style="height:20px;background-color:#FFFF80;">
  ...
  <td><input type="file" name="PC_file" id="PC_file" style="width:100%;" size="30"/></td>
  <td style="width:10px;"></td>
  <td><input type="submit" name="PCImportFile" value="IMPORT"/></td>
 </tr>   
 */ ?> 
 ...
 <tr style="height:20px;">
  ...
  <td class="td3">
   <select class="Inp2Sel" style="width:300px;" id="Compo" name="CompoN" onChange="FunCompo(this.value)">
	...
	<option value="Arr_Bonus">ARREAR FOR BONUS</option>
	<option value="Arr_LTARemb">ARREAR FOR LTA REIMBU.</option>
	<option value="Arr_RA">ARREAR FOR RELOCATION ALLOWANCE</option>
	<option value="Arr_PP">ARREAR FOR PERFORMANCE PAY</option>
